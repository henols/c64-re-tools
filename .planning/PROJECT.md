...
a store this project owns.** The two paragraphs above are history: The external analyser
...
now stop in the same frame and their 64K captures compare byte-identically, with
the capture sliced out of a VICE `.vsf` snapshot's `C64MEM` body rather than
retyped as hex — behind a gate committed before any measurement, which returned
**`degrade`** and said why. On that substrate a raw image goes through a
**vendored dxa 0.1.5** for a byte-level code/data map, and through **Ghidra
12.1.3** under an NMOS 6502 SLEIGH language this project ships that decodes all
**105 opcode bytes** stock `6502.slaspec` omits, and the recovered structure
annotates itself into `.annostore` — narrowest-range-wins, in-image addresses
skipped, `$01` bank state resolved before address, and a **decline with a
reason** rather than a confident wrong comment wherever bank state is
path-dependent. No agent, no queue walk, no skill in the loop. The `anno_*`
family is **21 tools** as of this milestone (18 at the v0.7.0 close). Every host
binary in the chain — `acme`, `dxa`, `analyzeHeadless` and the rest, **six
tools** — is reached over one typed `host_tool` control op on the broker socket,
with every other route banned by a CI gate observed biting on planted
...
*Still correct after v0.8.0, and the milestone is the strongest evidence yet
rather than a neutral pass.* "Reliably drive" was measured this milestone rather
than asserted: two runs of the same real cracked release stop in the same frame
and capture byte-identically under a reset protocol with five launch
nondeterminism sources pinned. The added engines sit **downstream** of that live
drive — dxa and Ghidra read what the emulator produced — so the ONE thing the
...
`ANNO-01` records that the external analyser is never launched with `--vice`, guarded
...
sealed-question test (`ANNO-10`), the symbol round trip (`ANNO-14`/`ANNO-15`),
and the 17 curated `anno_*` tools — re-weighed one milestone later, not new.
...
the bytes. The `ANNO-01` argument quoted above — that the outlives-the-session
...
- ✓ Five load-bearing assumptions checked against a real analyser build before any plan is written, with the pty/HTTP-MCP one gating the rest — Phase 9 (`ANNO-16`; verdict `degrade` via rule `R4`, see [`docs/phase9-external-analyser-probe-findings.md`](../docs/phase9-external-analyser-probe-findings.md))
- ✓ `c64-program-recon` writes findings as queryable annotation state, not only Markdown prose, so a later session can query instead of re-deriving — Phase 11 (`ANNO-10`; proven by the two-session falsifiability test — session B answered a question sealed before it existed, from store queries alone)
...
- ✓ `acme-build`'s `disasm` verb and its `toacme`-on-PATH prerequisite are removed, replaced by an external analyser route — v0.3.0 Phase 10 (`ANNO-05`; `cmdDisasm`, its dispatch entry, its usage line and ~50 lines of decoder-shaped `SKILL.md` caveats deleted, with a whole-tree grep gate proven to bite on a non-`SKILL.md` file)
- ✓ A `.prg` or flat 64K capture becomes reassemblable ACME source matching this project's `!cpu 6510` expectations, **verified by reassembly** rather than asserted — v0.3.0 Phase 10 (`ANNO-06`; `--verify`'s output parsed by one seam keying strictly on ACME's own result line, proven in both directions on real transcripts — an exit-1 run where ACME still passed, an exit-0 run where ACME never ran)

- ✓ `c1541` and `petcat` are reachable from a container-side skill script over v0.8.0's typed `host_tool` control op, so a disk's real structure and a BASIC stub's handover point are available before any disassembler is spent on the image — v0.9.0 Phase 40 (`PREP-01`, `PREP-02`, `PREP-04`, `PREP-05`; no `spawnSync` of a host binary anywhere in a skill script, held by `check-no-skill-external-spawn.mjs` with its planted-violation controls re-run rather than assumed still valid). **MEASURED, and it is why each tool's own output decides the outcome rather than its exit status:** `c1541` exits **0 on error**, and `petcat` returns non-zero only for a missing file — `0` on garbage input. Each tool carries a planted failure fixture observed producing a refusal, with the non-vacuity control being that an exit-status-only check *passes* on that same input.
  - **Scope honesty — `cartconv` was never built.** The hypothesis as written named three tools; `cartconv` and `PREP-03` were withdrawn by owner direction at the Phase 40 discussion, before any of it existed, so no per-bank image contract and no bank-qualified addressing entered the store. The withdrawal is recorded in `REQUIREMENTS.md`'s Excluded table and `docs/phase40-preprocessing-tools-decisions.md`; it is named here too, because a reader comparing this Validated line against the Active bullet it replaced would otherwise read the missing third tool as delivered.
  - **The `.d64` supersession was executed, not deferred.** The phase opened intending to leave the hand-written `d64-parse.mjs` untouched and defer the question on the record. It was reached and settled instead: `d64-parse.mjs` and its MCP-side duplicate `anno-d64.ts` are both **deleted**, and `c1541` is the ONE `.d64` route, mechanically enforced by `d64-single-route.test.ts`. MEASURED basis for the swap — `c1541 -bam` returns a per-sector allocation map where the deleted parser had only per-track free counts, and `c1541 -entry` returns the first track/sector plus the raw entry bytes plus the next-directory pointer.
  - **`PREP-05` was added mid-phase by owner direction at the UAT, and closing it needed a gap round.** The UAT found `ghidra.analyze`'s per-run project directories landing at `<repoRoot>/tools/ghidra-runs/`, outside D-33's single tool-written root — recorded across five documents as an unavoidable external-tool constraint. Measurement against real Ghidra 12.1.3 falsified that: the dot-segment refusal binds the **absolutized** path argument, and `ProjectLocator` calls `getAbsolutePath()`, never `getCanonicalPath()`, so it does not resolve symlinks. Gap `G-40-1` (plans 40-08..40-11) re-pointed the runs root under the one root behind a broker-minted, verified, relative-target alias handle, and corrected the five documents. See `.planning/notes/ghidra-dot-path-check-semantics.md`.
...
- ✓ Project bootstrap from a raw binary is automated rather than a documented manual step — v0.3.0 Phase 10 (`ANNO-09`; a pure-Node `.regen2000proj` synthesiser a real analyser loads, with `use_illegal_opcodes`/`system` forced explicitly, plus a container-side `.d64` reader that refuses to guess between matching entries)
...
- ✓ an external analyser project stays open across many tool calls in one working session, with crash detection between calls, save-before-return persistence and a single-owner write path — v0.5.0 Phase 18 (`SESS-01`..`SESS-04`)
- ✓ The curated `anno_*` surface covers what the absorbed procedures actually call, `anno_get_address_details`'s D-32 refusal is re-decided (D-36 supersedes D-32; client-side composition), and which packer a binary used is a recon finding — v0.5.0 Phase 18 (`SURF-01`..`SURF-03`)
...
- ✓ A frame-exact emulator stop exists, so two runs of the same release stop in the same frame and their captures compare as equivalent — v0.8.0 Phase 33 (`REPRO-01`..`REPRO-05`, `GATE-01`). **`GATE-01` returned `degrade` by rule `R6`** — derived, not judged: five values transcribed from column-0 outcome lines and walked through rules committed at `2a8ef95` *before* any measurement, with `could-not-run` structurally unemittable and the one available override explicitly declined. The stop turned out to be a **protocol, not a new mechanism**: a monitor-issued hard reset plus five launch nondeterminism sources pinned in `buildViceArgs()`'s stock branch, `-raminitrandomchance 0` the load-bearing one against a factory value of 10. The limit is stated rather than absorbed — the anchor-counted S3 sequence is frame-exact and byte-identical through anchor hit 50 and **lost from hit 75**, because AUTOSTART's power cycle does not reset the absolute emulated clock; both wall-clock controls were observed red, and the whole first measurement pass was voided under `D-11` rather than kept
- ✓ A depacked flat 64K capture is produced without any hex transcription step, by slicing a VICE `.vsf` snapshot's `C64MEM` module body — v0.8.0 Phase 33 (`CAP-01`..`CAP-04`; a strict module-table walk from a derived offset of 58, with **eight named refusals each proven by its own fixture**, reachable from the skill side over a resolution ladder that carries no second copy of the byte layout). The transient allow-list is a named, repeatable derivation rather than a hand list — pairwise union over N≥3 runs, one enumerated entry per address, and a cap of 64 that VOIDS with no artifact left behind — and every capture record carries `(binary sha256, argv digest, seed)` as one key, with the measured 76-byte counterexample quoted inline
- ✓ Every host binary this project reaches — `acme`, `unp64`, `dxa`, Ghidra's `analyzeHeadless` — is invoked over one typed `host_tool` control op on the existing broker socket, never `spawnSync`'d from a skill script — v0.8.0 Phase 34 (`SEAM-01`..`SEAM-07`). The ban is a **tested mechanism, not documentation**: `scripts/check-no-skill-external-spawn.mjs` scans the packed-tarball route and the git-tracked plugin route through one shared predicate, was observed biting on three planted violations while holding on four exemptions, and is wired into CI. No path in any tool's argv is read from the raw wire request — a `HOST_TOOL_PATH_ARG_KEYS` census drives a data-driven refusal loop across all seven declared path-bearing keys, replacing three point fixes. `SEAM-07` binds the JVM **per invocation**, recorded with its measurement and a checkable reversal condition
- ✓ A raw C64 image goes in and a machine-readable code/data map comes out, from a dxa this project vendors and builds at a pinned version — v0.8.0 Phase 35 (`DXA-01`..`DXA-04`; dxa 0.1.5, digest-verified, its reproducibility recorded as **four digests rather than an exit code**, with `THIRD-PARTY-NOTICES.md` quoting the GPL-2.0-or-later headers in the same commit that landed the incorporated source). The listing parser owns all five measured `-a dump` line shapes and **refuses by name — never dxa's exit status**, proven by a genuine unplanted refusal from a real dxa boundary artefact; any overlapping decode is disposed `unclassified` with a stated reason and never awarded to a winner, even when the two claims agree. The source-derived partition tier reproduces Phase 23's independently re-derived 145/131/3 exactly from a committed ACME report, and neither tier ever sees dxa's own output
- ✓ Ghidra headless recovers structure under this project's own committed harness — v0.8.0 Phase 36 (`GHID-01`..`GHID-05`; observed live against real Ghidra 12.1.3 through two manual-only suites). The volatile-I/O carve is proven **by disappearance** on both import routes, via a new additive `## DECOMPILED_TEXT` export section added *after* live measurement showed the pre-existing `## REFERENCES` section never reflects the flag at all — and a genuine, forced `MemoryConflictException` proves a conflict during the carve is loud rather than a silent fall-back. Structural facts export through `DecompInterface`; the `DataTypeManager` control, invoked directly outside the seam, returns near-nothing beside it. The exact-literal thrown-script signal is gated over two REAL captured `analyzeHeadless` logs, including the proof that a naive `error`/`fail` grep false-fires. **Disclosed narrowing, accepted as an override 2026-09-05:** the forced-conflict control is proven on the `flat64k` route only, because `bank.prg` cannot reach the I/O page at the `.prg` route's default load address
...
capability was never built. The two the external analyser surplus entries and the
...
decision: **parity with the external analyser's feature set**, **a per-edit inverse-command
...
*Re-audited at the v0.8.0 close, 2026-09-06. **Every entry's reasoning still
holds and none was removed.** One is worth naming because this milestone tested
it: the fork-backend hedge (`FORK-01`) was never exercised — v0.8.0 ran
end-to-end on genuine stock `x64sc` 3.9, and the three stock losses it hedges
(SID read-back, matrix keyboard, RESTORE/NMI) were not needed by any of the six
phases. That is evidence the hedge is cheap, not evidence it is unnecessary; the
dated reversal criteria in Key Decisions stand unchanged. Added this milestone by
owner decision 2026-09-02, each with its rationale in
[`milestones/v0.8.0-REQUIREMENTS.md`](milestones/v0.8.0-REQUIREMENTS.md) →
`## Out of Scope` rather than restated here: **the text monitor's `stopwatch` in
any form**, **dialling the `-remotemonitor` text channel at all**, **VICE event
record/replay as the reproducibility mechanism** (it **does not exist** —
measured, not assumed), **phase 0's reconstructed cycle clock** (1-cycle-per-frame
drift, measured), **a `da65 .info` generator and a ca65+ld65 rebuild route** (an
**anti-feature**, five of six grounds measured — a `TYPE SKIP` hole collapsed and
the rebuild assembled cleanly to a **wrong binary**), **a persisted
`program.json` as a parallel model**, **a Ghidra post-script writing `.annostore`
directly** (structurally unavailable, not merely discouraged), **vendoring Ghidra
into this repository** (543 MiB), **bank-qualified addressing as a modelled store
feature**, **sprite bitmap locations and the `$DD00` VIC banking axis** (carried
...
- **the external analyser capability surplus, absent a caller** *(added v0.3.0, cut 2026-08-17)* — 4 of the original 16 `ANNO-*` requirements were folded or cut on the same measured test v0.2.0 used. Cut: HTML export with clickable xrefs (`ANNO-07` — a shareable artifact no skill produces or consumes; still available ad-hoc via `--export_html`); the two-project limit as a reported error (`ANNO-04` — folded into install documentation, since building detection for an upstream port collision is work in the wrong place); the static-vs-live tool-selection axis (`ANNO-12` — folded into v0.2.0's `SKILL-01`, which already rewrote the same playbooks); and the `.vsf`/`.raw` bridge as its own requirement (`ANNO-08` — it is which file extension you hand over, not a deliverable). Each stays in `milestones/v0.3.0-REQUIREMENTS.md` with rationale. A separate MCP-server-standup phase was dissolved into a task at the same time.
...
prose. Phase 29 routed it to a Phase 28 verification pass or a milestone audit;
neither the v0.7.0 close nor this one ran either.
...
surface: 17 curated `anno_*` tools plus 7 `vice-mcp anno` CLI verbs, all
...
`.planning/notes/external-analyser-integration.md` grounds v0.3.0 (D-R1..D-R4).
...
- **Compatibility**: The stdio MCP surface is **trimmed per backend** — stock advertises only the tools it implements, so the two backends expose different tool lists. A tool advertised on both keeps the same name and a backward-compatible argument shape — stock may add optional parameters but never removes, retypes, or newly-requires one — and the fork's list is unchanged from v0.1.x. A skill written against the full fork surface therefore *breaks* on stock rather than degrading; the playbooks must name the stock route or the fork requirement. The argument-shape half is pinned by `manifest-arg-compat.test.ts`.
- **Architecture**: The transport swap happens behind `vice.ts`'s `call()` seam for *direct* tools. **Derived tools must be intercepted before `forwardToVice()`, not behind `call()`** — `rewriteArguments()` runs at `vice-proxy.ts:3100` inside `forwardToVice()` (which starts at `:3035`) and before `call()`, so a derived tool sitting behind `call()` receives host-translated paths and acts on them inside the container. Second site with the same cause: `gatherWedgeEvidence()` calls `rewriteArguments()` itself, at `vice-proxy.ts:1529` (the function starts at `:1505`). The `anno_*` family is registered through `buildViceTool()` and never reaches `forwardToVice()`, so neither call site is reachable from it — the constraint is satisfied by construction for that family, not by an interception. (These four line numbers drift whenever anything above them is edited, and they have drifted repeatedly; treat a mismatch as drift to re-verify, never as evidence the constraint itself changed. `docs-linerefs.test.ts` checks all four mechanically — the two call sites by their `vice-proxy.ts:<N>` citation, the two function starts by their `:<N>` shorthand — so a drifted number fails the build instead of waiting to mislead a reader.)
...
- **Dependency**: `CPUHISTORY_GET` (0x86) requires **VICE ≥ 3.10**. Debian trixie/forky/sid and all current Ubuntu ship 3.9, which lacks the opcode entirely. Homebrew and official builds are fine. (`binary monitor only` — the version floor is on the *opcode*, not the capability: `chis` returned CPU history with per-entry cycle counts over the text channel on genuine stock 3.9, MEASURED 2026-08-27. Note the polarity: text-command tracing/profiling support is opt-**out** at build time, the opposite of a version floor. The affected commands do not each carry their own separate guard — two share one guard and one stub string (`memmapshow`/`chis`), two carry none (`bt`/`prof flat`), and one degrades per-chip at runtime instead of refusing (`io`); per-command probing is unaffected — `docs/phase42-text-format-drift-citations.md`. Independently re-confirmed by a separate fixture batch (`FIXTURE_UNSUPPORTED: none`, `docs/phase39-dual-channel-coexistence-gate-findings.md` §7) — `chis` succeeded on genuine stock VICE 3.9 over the text channel via a completely different code path than the binary-monitor opcode's version floor; both citations are collected together in `docs/phase41-text-channel-live-evidence.md`.)
...
- **Architecture**: Every shipped module that spawns the emulator binary must do it in the safe form — an argv **array**, never a shell command string and never a string-interpolated binary path — and the set of modules that spawn it at all is frozen rather than assumed. That set has exactly one member today: `backend-detect.mts`'s `probeBackend()` `--help` probe. `spawn-seam.test.ts` checks both halves as a property over the SHIPPED module set, derived from `package.json`'s `files[]` via `shipped-modules.ts` rather than a raw directory listing — a listing would wrongly catch `acme-gate.ts`, a real but non-shipped spawn site — and its set-equality assertion fails in BOTH directions, so a second spawn site cannot appear unguarded.
...
The external analyser, package installation, or the real broker launch path), an internal mock or
...
| Hand-roll the newline-delimited JSON-RPC client instead of using `@mastra/mcp`'s `MCPClient` | Decided by a five-property live measurement against the real binary, not by preference | ✓ Good — yields six distinct named failure modes instead of one opaque one, plus independent persistence verification for `anno_save_project` |
...
| **REVERSED** (2026-09-12): the forked VICE MCP backend has been removed; the retain decision (FORK-01, originally decided 2026-08-22 at a human blocking checkpoint) no longer holds | The two grounds for retain are both gone. Ground 1 — the fork was the hedge against stock's three hard losses (SID read-back, matrix keyboard, RESTORE/NMI) — collapsed first: the project's own v0.8.0 close audit already recorded in writing that this hedge was never exercised, since none of the six shipped skills needed any of the three hedged routes. Ground 2 — the fork's incremental maintenance cost was near zero because it already existed and was tested — was withdrawn directly by the project owner, who reports the fork never worked in practice (owner scope call, 2026-09-11). With both grounds gone, the upstream `KEYBOARD_MATRIX_SET` reversal trigger this row previously named is now moot: a landed opcode would close a stock gap for everyone, it would not restore a backend that has been removed. The three hard losses this row previously hedged — SID read-back, matrix keyboard, RESTORE/NMI — are now permanent accepted losses rather than hedged ones, recorded with their verbatim hardware reasons in `docs/stock-hard-losses.md`. This reversal re-reverses only if the project owner makes an explicit decision to reinstate the fork; a landed `KEYBOARD_MATRIX_SET` opcode is not, on its own, such a decision. | ✓ Reversed — pinned by the rewritten guard `docs-fork-decision.test.ts`, which now asserts the reversal record rather than the retain decision it originally pinned. The prior adequacy evidence (plan 14-03's live `-mcpserver` exercise, 6/6 passing) is historical: it proved the fork route was followable while it existed, not that it should have been kept. |
...
| Owe **no parity** to the external analyser, and drop the engine coupling with it (v0.7.0 open, 2026-08-26) | v0.6.0's Phase 25 carried the gate *"nothing may delete anno before a replacement demonstrably produces the same facts"*, which makes the removal hostage to a feature set this project did not choose; and Phase 25's *"populated from the engines' output"* dependency on the held Phase 24 would have made the milestone corpus-dependent, and therefore unreachable while Phase 23's `no-go` stands | ✓ Good — both removed, and the pair is what made v0.7.0 *reachable at all*: the store stands on the `disasm-*` decoders this project already owns, so a milestone shipped in 7 days under a live `no-go` gate. The cost is stated rather than hidden — `ANNO-13`/`ANNO-14`/`ANNO-15` went with the removal and no phase owns their return |
...
| One typed control op for every host binary, and the ban on every other route written before the route exists (SEAM-01..07, Phase 34, 2026-09-04) | A skill script that spawns an external binary is spawning it inside the container, where the app does not live. The seam is only worth anything if it is the ONLY route, so the two existing violations had to be migrated before the grep gate banning them could be written at all — a gate written first would have been red on its own repository from birth | ✓ Good — `acme.build`, `ghidra.analyze`, `oracle.probe`/`oracle.run` all cross as path plus digest plus length over the existing broker socket with no emulator lease, no argv passthrough on any of the seven declared path keys, and the whole-tree gate observed biting on a planted violation. Three review rounds; the last one closed CR-05, where the seam's own confinement check was lexical and a planted symlink escaped it — both sides now walk the filesystem, and the equivalence test pins the walk against `anno-types.ts`'s. The check-then-open window is a stated residual, not a handled case |
| Round-trip the recovered const-write facts through the two-call tool surface rather than persisting them in the store (Phase 37, forced by code-review `CR-01`) | The review found `AUTO-04`..`AUTO-07` built, unit-tested and already marked Complete but **unreachable**: `parseConstWrites()` had no production call site and `dispatchJoinMemmap()` never supplied `runMemmapJoin()`’s `constWrites`, so both gated blocks were unconditional no-ops for every real call. Durable persistence in the store’s `bank` column was the obvious home and was rejected as out of scope: `STORE-05` reserves that column with nothing interpreting it, and widening it here would have been a design change smuggled in as a review fix | ✓ Good — `anno_import_ghidra_export` now returns the facts and `anno_join_memmap` accepts them back (`const_writes`, `graphics_map_index`), with a dispatch-layer regression test proving `$d020`’s label genuinely changes when they are supplied. The verifier re-traced the fix from source rather than trusting the fix report. The standing cost is honest and disclosed: the facts survive only across one call pair, so a caller that joins without re-passing them silently gets the unconstrained behaviour — the same shape of gap `CR-01` caught, now visible in the tool schema rather than hidden in a no-op |
| `not-exercised` is a first-class PROOF verdict, distinct from both `pass` and `could-not-run` (Phase 38, 2026-09-05) | PROOF-02 asks whether Ghidra resolves a computed-index dispatch taken from real code. Neither searched depth of `danish.d64` contained a site of that shape — 53 `$6C` sites at loader/depacker depth, 1 at depacked depth, 0 computed-index at either. Reporting `pass` would claim an answer never obtained; reporting `could-not-run` would erase two real live `analyzeHeadless` runs. A third verdict, derived from a rule stated in the record *before* its value, was the only honest option | ✓ Good — the roll-up `PROOF02_COMPUTED_DISPATCH: not-exercised` cites both per-depth files, and each record scopes the verdict to **this corpus at the depths searched** rather than to computed dispatch generally. Cost disclosed rather than absorbed: no entry points were supplied to the depacked-depth run, so its zero decompiled functions is a stated consequence of searching a mid-load capture, not an equivalent measurement to the entry-point-seeded `.prg` route |
| Enumerate dispatch sites Ghidra-independently, before Ghidra runs, with document ordering asserted (Phase 38, 2026-09-05) | A PROOF-02 verdict derived only from Ghidra's own output would be Ghidra grading its own homework. A separate linear byte scan (`proof02-enumerate-sites.mjs`, no Ghidra-export read, no child-process import) gives an independent site count to check the tool against — but only if it is genuinely recorded first, which a claim in prose cannot establish | ✓ Good — the same script ran unchanged at both depths (`--prg` and `--flat64k`), site counts match across both plans' files, and an `awk` line-number check proves the enumeration precedes the first Ghidra-seam mention in each document. dxa cross-check is carried as an annotation, never as a filter |
| Real-release proof figures are stated **beside** the fixture and pivot figures, never in place of them (Phase 38, 2026-09-05) | `PROOF-01`'s real-release number (`100.00 (24/24)` on `BRUCE LEE (DC)`) has a much narrower denominator than the fixture figure — the BASIC loader stub only. Replacing `72.39 (97/134)` with it would read as a large improvement that no measurement supports | ✓ Good — three separate blocks, the fixture figures byte-unchanged, the pivot's non-reproduction labelled a **hypothesis** rather than a finding, and two weaknesses named separately rather than folded together: no independent external check, and a structurally uncomputable false-positive count (`certainCode.size == 0` on the byte-derived tier) that is emitted as that string for every input rather than as a flattering bare `0` |
| Close v0.8.0 without a milestone audit (2026-09-06) | All six phases carry `phase_complete: true` and `verification_status: passed`, and all 43 requirements read `Complete`; the per-phase `VERIFICATION.md` files are the evidence of record. Same posture as the v0.5.0, v0.6.0 and v0.7.0 closes | — Pending — this is now the fourth consecutive close without one, which makes it a **standing practice rather than a per-milestone judgement**. The cost is concrete and named: `STORE-03`'s contradiction, routed to *a verification pass or a milestone audit* by Phase 29, has now survived two closes because neither ran |
| Disclose the same 8 un-acknowledgeable audit items a **second** time rather than falsify Phase 23's evidence tables (v0.8.0 close, 2026-09-06) | Identical mechanism and identical refusal to the v0.7.0 close — the acknowledge writer matches bullets, Phase 23's entries are GFM table rows, and every call returned `no deferred item matched --text`. Both parser escapes were already rejected once on their merits; nothing about them changed | ⚠️ Revisit — a recurrence, not a fresh decision, and that is the finding: 8 items are now **permanently and structurally unclosable** by the close procedure, and every future close will spend the same effort re-deriving that. The fix belongs upstream in the scanner, not in this project's evidence files |
| Skip quick-task archival at the v0.8.0 close (2026-09-06) | `--archive-quick` buckets **every** remaining `.planning/quick/*` into the completing milestone, and there is no on-disk provenance for which milestone a quick task belonged to. 14 directories stand there, spanning several milestones; archiving them would assert a false attribution in the archive index | ✓ Good — `.planning/quick/` is left untouched, exactly as at prior closes. The 9 already-acknowledged quick tasks stay suppressed and stay findable where they were written |
| Supersede the hand-written `.d64` parser with `c1541` inside the same phase, rather than deferring the question (Phase 40, 2026-09-08) | The phase's own success criterion said the supersession would be deferred on the record rather than settled as a side effect of adding a tool. It was settled because the measurement made it one-sided: `c1541 -bam` gives a per-sector allocation map where the parser gave per-track free counts, and `c1541 -entry` gives the first track/sector plus raw entry bytes plus the next-directory pointer — strictly richer inputs than the parser computed | ✓ Good — `d64-parse.mjs` and `anno-d64.ts` both deleted, one route left, enforced by `d64-single-route.test.ts` rather than by convention. The criterion was amended **in place with a dated note** rather than quietly re-read to match what happened |
| Reach the one tool-written root through a non-dotted alias symlink instead of accepting a second root for Ghidra (Phase 40 gap `G-40-1`, 2026-09-08) | The recorded justification for the split — that Ghidra's dot-segment refusal made it unavoidable — was an overstatement. MEASURED from `ProjectLocator`'s own bytecode plus 5 live runs against Ghidra 12.1.3: the refusal binds the **absolutized** path argument and the class calls `getAbsolutePath()`, never `getCanonicalPath()`, so it absolutizes but does not resolve symlinks | ✓ Good — the runs root is physically under `.c64-re-tools/`, reached via a broker-minted handle whose target is the RELATIVE string `.c64-re-tools` (an absolute target would break the container route). The handle **refuses by name and never repairs**. Verified live twice, plus once by the verifier spawning the real broker. The five documents that carried the false constraint were corrected, and the historical records **superseded with dated notes rather than falsified** |
| Close a per-call cache-category defect by making the category error unrepresentable, not by tuning the cache (Phase 42 gap round 2, `CR-02`, 2026-09-09) | `handleIoRegisters` asked `probeTextCapability`'s cache — whose entries are process-lifetime-stable properties of a *binary file* — a question whose answer is per-call, per-address *content*, so every later `io` call was answered from the first call's response. Both reachable wrong answers were live: a real register dump discarded behind an earlier refusal, and a genuine chip degradation reported through the generic parse-failure wrapper. A shorter TTL or a wider key would have made the wrong answer rarer, not impossible, so the remedy is two-part and structural: a pure `textCapabilityVerdictFor()` builder gives one verdict-construction site fed by the response that call actually received, and a frozen `NEVER_CACHED_COMMANDS` set removes `io` from the cache's domain at the write path, the read path **and** the in-flight dial memo, so no future caller can put it back by accident | ✓ Good — both planted controls were observed red against the pre-fix tree and green after (the verifier reproduced this independently by reverting production code to `bbdf4958^` while keeping the current tests), and the fix was proven live over two `io` addresses in one session against genuine stock `/usr/bin/x64sc` (`x64sc (VICE 3.9)`, 2026-09-09): second dial `fromCache=false`, judged on its own reply. `vice_io_registers`'s published `inputSchema` is byte-unchanged. Round-2 code review: `status: clean`. Evidence: `docs/phase42-text-format-drift-citations.md` Blocks 22–24 |
| Take the instrumentation-perturbation A/B **before** the evidence schema existed, with its pass/fail rule fixed first (Phase 43, `EVID-06`, 2026-09-10) | The layer keys its rows by a frame-exact reproducibility that instrumenting the run might itself destroy, and no documentation source answers whether it does — only an A/B at v0.8.0's existing anchor sequence does. Taken after the table shipped, a `perturbation` verdict would have been a retrofit onto a shipped key rather than a schema choice. Both outcomes were planned for, and a control-of-the-control (two un-instrumented runs compared to each other) had to compare `equivalent` before any treatment verdict was trusted. | ✓ Good — verdict `no-perturbation`, MEASURED live against genuine stock VICE 3.9 (`/usr/bin/x64sc`) at anchor hit depths 10 and 50, both byte-identical with an empty allow-list and a passing control at each depth. Run identity therefore stays v0.8.0's bare `(binary sha256, argv digest, seed)` triple with **no** `run_class` column — the decision was recorded for plan 43-02 to read rather than inferred. `docs/phase43-instrumentation-perturbation-ab.md`. |
| Re-affirm the store's strict-equality schema refusal at `SCHEMA_VERSION` 4 rather than add a migration arm (Phase 43, `EVID-02`, 2026-09-10) | The v3 bump justified itself with "no store file exists yet", which a factual check showed is now **stale**: `v0.7.0` (2026-09-01) and `v0.8.0` (2026-09-06) both postdate `anno-store.ts` landing (`4c9cea3c`, 2026-08-27), so a field store became *possible*. But the check observed a **null result** — every `.annostore` on the machine sits in a dated probe cache with fixture names, one of them 0 bytes — and the two directions' costs are asymmetric in reversibility: refusal can gain a migration arm later at version 5 against real evidence, whereas a mutating open path on a module that also has a corruption-refusal path is where silent corruption lives. House style settles the shape: detect, then refuse BY NAME with the remedy in the message. | ✓ Good — the VERSION 4 paragraph corrects the stale v3 framing in place, states that its basis is ONE MACHINE only, and records the reversal condition for a version-5 migration arm. The refusal message names the mismatch, gives two remedies, and states that the refused file is left untouched — hand-authored labels and comments are not reproducible from the binary and must not read as lost. |
| Close `PROOF-01`'s reversal condition by the **text-channel** branch rather than waiting for a binary-monitor execution oracle (Phase 44, `PROOF-04`, 2026-09-10) | The condition named two branches and either discharges it. `CPUHISTORY_GET` needs VICE ≥ 3.10 and this host runs 3.9, so the binary branch was not reachable here; `memmapshow` over the text channel gives a per-address access map with execute as its own bit on genuine 3.9 today. Taking the reachable branch and **saying which one was taken** beats leaving the condition open for a version bump nobody controls | ✓ Good — the count is computed, and the record names the branch taken rather than presenting the closure as branch-agnostic |
...
43/43 requirements, 5 days, 365 commits since the `v0.7.0` tag,
`override_closeout`.
Full record: [`MILESTONES.md`](MILESTONES.md) ·
[`milestones/v0.8.0-ROADMAP.md`](milestones/v0.8.0-ROADMAP.md) ·
[`milestones/v0.8.0-REQUIREMENTS.md`](milestones/v0.8.0-REQUIREMENTS.md)

**This project now measures real cracked code instead of a 279-byte fixture.**
Two runs of the same real release stop in the same frame and their 64K captures
compare byte-identically, and the capture comes out of a `.vsf` snapshot's
`C64MEM` body rather than through hex an agent retyped. On top of that
substrate, two engines this project vendors and drives itself — dxa 0.1.5 and
Ghidra 12.1.3 under an NMOS 6502 SLEIGH language decoding all 105 opcode bytes
stock omits — turn a real release into structural facts that annotate themselves
into the owned store with no agent, no queue walk and no skill in the loop. Every
host binary in that chain is reached over one typed control op, with the
alternative route banned by a CI gate observed biting on planted violations.

**The gate fired and was honoured: `GATE-01` returned `degrade`, by rule `R6`.**
Derived, not judged — five column-0 outcome lines walked through rules committed
at `2a8ef95` before any measurement, with `could-not-run` structurally
unemittable and the one available override explicitly declined. The frame-exact
stop is exact through anchor hit 50 and **lost from hit 75**, because AUTOSTART's
power cycle does not reset the absolute emulated clock. That limit is on record
beside the capability rather than behind it.

**Three forecasts this milestone carried in were falsified by its own
measurements, and each is corrected in place rather than deleted.** VICE event
record/replay — the mechanism the milestone was opened on — **does not exist**;
the stop is a reset protocol plus pinned launch nondeterminism instead.
`docs/undocumented-opcodes-ghidra.md`'s p-code **did not compile** (8 failing
constructors, one root cause), so `OPC-01` was fix → compile → integrate →
verify rather than integrate-and-verify. And eleven of the 21 requirement ids
carried forward from held Phases 24 and 26 were **amended, not byte-identical**,
each with its evidence — against an explicit forecast at the open that they would
carry unchanged.

**`PROOF-01`..`PROOF-03` are measurements, not `could-not-run` — and each is
stated beside the fixture figures rather than replacing them.** `PROOF-01`:
`100.00 (24/24)` data recovery on `danish.d64`'s `BRUCE LEE (DC)`, beside the
unchanged fixture `72.39 (97/134)` and pivot `72.46 (100/138)`, with two
weaknesses named — no independent external check, and a structurally
uncomputable false-positive count. `PROOF-02`: `not-exercised` at **both**
searched depths, a statement about this corpus at the depths searched and not a
clean bill of health, with dispatch sites enumerated Ghidra-independently
*before* each live run and document ordering asserted rather than claimed.
`PROOF-03`: proven in **both** directions on a **synthetic** fixture, with the
`danish.d64` question recorded as open.

**No milestone audit was run, and that is a statement rather than an omission.**
All six phases carry `phase_complete: true` and `verification_status: passed`,
and all 43 requirements read `Complete` in the traceability table — the
per-phase `VERIFICATION.md` files stand as the evidence of record, the same
posture v0.5.0, v0.6.0 and v0.7.0 closed under.

**Closed as `override_closeout`.** The pre-close artifact audit reported 12 open
items against 31 already suppressed. **4 were newly acknowledged** — one pending
todo (`WR-03`, the two holes in `host-tool.mts`'s never-throws contract), one
Phase 36 UAT file already resolved by owner override, and two phase deferred
items (Phase 33's `check-npm-packages.mjs` entry, which its own text records as
RESOLVED by `fc199e7` and keeps as provenance; and Phase 35's named
`audit-root-args.test.ts` scratch-file flake). The other **8 could not be
acknowledged by any CLI path** — the same GFM table rows inside Phase 23's
evidence tables that blocked the v0.7.0 close, refused with `no deferred item
matched --text`. They were disclosed again rather than falsified, which is the
standing disposition rather than a fresh judgement.

**What the milestone proved beyond its requirements.** A control observed **red**
is the only evidence that counts against a silent failure, and this milestone
spent plans producing them on purpose: three scratch-copy mutations of
`memmap-lookup.ts` each yielding a specific named wrong answer, the in-image
guard deleted so `$0800` annotates as `"Unused"`, a one-byte `memmap.json` change
producing a differing digest, the all-105 assertion failing under the stock
language, a forced `MemoryConflictException`, 512 phantom labels before the
graphics feedback and 0 after. Asserting a fix is present proves nothing; making
the failure happen does.

**Prior milestone — v0.7.0 Own the Annotation Store** — 2026-09-01. 6 phases
(27-32), 80 plans, 214 tasks, 28/28 requirements, 7 days, 690 commits,
`override_closeout`. This project stopped renting its analysis state: the
external analyser is deleted — not deprecated, not wrapped — and `.annostore`
replaced it, a `node:sqlite` store behind one seam with a frozen twelve-member
type vocabulary, a narrowest-range-wins paint index proven exact at all 65,536
addresses, and durability and revert proven together across a real `SIGKILL`.
Eighteen `anno_*` tools reach it proxy-locally through `buildViceTool()`. Full
detail below under [`## Shipped: v0.7.0`](#shipped-v070-own-the-annotation-store).
One item carried forward unresolved: Phase 29 logged that `STORE-03`'s
traceability row (`Complete`) contradicts the round-6 verifier's own quoted
sentence, routed to a Phase 28 verification pass or a milestone audit; neither
the v0.7.0 close nor this one ran either, so it stands, disclosed in `STATE.md`.
...
**Prior milestone — v0.3.0 the external analyser static-analysis backend**, 2026-08-21:
...
`anno_*` tools and 7 `vice-mcp anno` CLI verbs over a persistent annotation
...
**Delivered:** an external analyser session that survives many tool calls with
...
The external analyser as the analysis substrate. Measured on a committed 279-byte
...
- **anno unannotated flat-decodes** — every data table rendered as garbage
...
anno is consequently reduced from "the analysis engine" to an annotation store
...
**Goal:** Replace the external analyser as the analysis substrate with dxa + Ghidra
...
- 19,181 lines of the external analyser integration glue deleted
...
| **25** (B) | The annotation store | labels, comments, per-range typing, scopes, enums, undo, persistence; MCP surface; ACME exporter. Deletes 19,181 lines of anno integration glue (9,087 non-test + 9,928 test) |
...
opcodes; `anno-d64.ts` (310 lines) is standalone; `memmap.json` plus
`anno-regbits-gen.ts` plus `anno-enum-gen.ts` already own machine knowledge
and enum generation.

**Dropped deliberately:** anno's unpacker (~5,400 lines including the CPU it
...
The external analyser, and the analysis procedures already absorbed from it run on
that store instead — with the external analyser deleted outright, no parity owed to
...
- Per-range typing at the full vocabulary `DECOMP-01` will need later — **12
  members**, read off `anno_set_data_type`'s own schema. Not the 7 first named
...
- the external analyser integration glue deleted behind a grep gate observed
...
1. **No parity is owed to the external analyser.** v0.6.0's Phase 25 carried a gate —
   *"nothing may delete anno before a replacement demonstrably produces the
   same facts"* — and `STORE-04` was worded as *"the capability `ANNO-11`
   shipped, carried across the substrate swap rather than lost in it"*. Both are
   **removed**. The external analyser is out of scope and is eliminated; the store is
...
|---|---|---|
| `analyze-blocks` | `c64-memory-mapping` | code / data-byte / word / address-table / lo-hi-split-table recognition heuristics |
| `analyze-symbol` | `c64-memory-mapping` | hardware-register / global-block / ROM-routine / pointer / flag / counter / state-variable patterns |
| `analyze-routine` | `c64-program-recon` | the seven-step routine procedure and its pitfalls |
| `analyze-basic` | `c64-program-recon` | BASIC V2 keyword token table and line anatomy |
| `analyze-program` | `routine-queue-walker` | full-program orchestration, adapted away from upstream's parallel fan-out |
...
`anno_*` tool calls. Deleting the tool surface without re-pointing them leaves
...
- **The grep gate's scope is the hard part, and it is measured.** 291 tracked
  files mention the external analyser; **55** outside `.planning/`. So roughly 236
...
- **The exemption set is 10 instances across two trees, not three headers.**
  ...
  each carrying two "the external analyser" lines, and five more sit in the
  gitignored-but-shipped `installer/skills/` tree. Separately,
  `routine-queue-walker/SKILL.md:3`'s YAML `description:` names the external analyser
...
- **Nine guards and two CI scripts are pinned to the deleted subject**, not the
  three first named here, and 13 non-`anno-`named test files reference it. Two
  break on *registration* rather than deletion:
  `generate-tool-support-table.mjs:104` hard-codes a regex matching
  `ANNO_TOOL_DEFINITIONS`, so renaming the collection matches nothing, hits a
  ...
  `check-skill-tool-coverage.mjs:49` statically imports `CURATED_ANNO_TOOLS`
  and throws `ERR_MODULE_NOT_FOUND` on deletion. `hostpath-consumers.test.ts`
  itself goes red on three tests, one via a hard-coded `ANNO_MODULE_FLOOR = 14`.
- **`anno-test-gate.ts` is a keeper with a glue-shaped name.** It owns the ACME
...
- **Prefix-driven deletion would silently un-ship six more survivors than
  `CUT-02` names**: `anno-test-gate`, `-acme-ident`, `-confidence`,
  ...
  - **Updated 2026-08-29 (Phase 29):** the symbols module survives under its
    ...
    `ANNO-14`/`ANNO-15` capability line above carries the matching dated
...
`anno-d64.ts` is standalone; `memmap.json` plus `anno-regbits-gen.ts` plus
`anno-enum-gen.ts` already own the machine knowledge and enum generation. All
of it survives, under names that no longer say `anno`.

**The deletion is smaller than it looks, and the difference matters.** The
`anno-*.ts` surface measures **25,759** lines (10,102 non-test + 15,657 test) —
...
**12.4k**. `anno-coverage.ts` (2,292 lines + 6,484 test) needs only two
...
## Shipped: v0.8.0 Frame-Exact Capture and the Two Engines

**Opened 2026-09-02. Shipped 2026-09-06** (`override_closeout`). 6 phases
(33-38, no inserted decimals), 47 plans, 118 tasks, 43/43 requirements, 5 days,
365 commits since the `v0.7.0` tag. Full record:
[`MILESTONES.md`](MILESTONES.md) ·
[`milestones/v0.8.0-ROADMAP.md`](milestones/v0.8.0-ROADMAP.md) ·
[`milestones/v0.8.0-REQUIREMENTS.md`](milestones/v0.8.0-REQUIREMENTS.md).
No milestone audit was run — all six phases carry `verification_status: passed`
and the per-phase `VERIFICATION.md` files are the evidence of record.
`GATE-01` returned **`degrade`** by rule `R6`.

**The scoping text below is preserved as written at the open, 2026-09-02.** It is
the forecast the milestone was planned against, not a description of what
shipped — several of its claims were falsified by this milestone's own
measurements and are corrected in `## Current State` above rather than edited
away here.
...
**Target features:**
- **Decomposition to closure** (`DECOMP-01`..`04`) — nothing left `Undefined`;
  every code entry point carrying a user-set name and a purpose comment stating
  function, inputs, outputs and side effects, with no `p_XXXX`/`l_XXXX`
  survivors; every referenced non-hardware address named and documented;
  hardware register writes rendering as named enums rather than magic numbers.
- **Rebuildable source** (`BUILD-01`..`07`) — one ACME file per annotation-store
  scope wired by `acme-build`'s `!source`; data tables extracted to their own
  files so graphics, levels and music swap without touching code; every branch,
  `JSR`/`JMP` and data reference through a symbol so code can move; a hazard
  report across the four movement blockers nothing in this stack detects today;
  a provenance-aware rebuild that **reports rather than decides**; a lossless
  export path; and the reassembly-plus-clean-hazard-report gate existing
  **before** the phase it gates.
- **Equivalence and modifiability** (`EQUIV-01`..`04`) — `compare.mjs` driven in
  original-versus-different-binary mode, a mode it has never been run in, with a
  narrowed volatile mask so a real `$D020`/`$D015`/`$D018` regression cannot
  hide; behavioural equivalence demonstrated in VICE with a committed transcript
  as the artifact of record; modifiability demonstrated rather than described;
  fixtures committed and the whole pipeline runnable in CI.
- **A purpose-built synthetic subject** — the enabling deliverable this scoping
  added. Today's committed fixtures (`fixtures/dxa/tracer.prg`,
  `fixtures/ghidra/bank.prg`, `fixtures/export-asm/smc.prg` and their siblings)
  are single-purpose probes. `BUILD-04`'s hazard detector and `EQUIV-03`'s
  modifiability proof are **vacuous** against them, so this milestone writes one
  synthetic C64 program in ACME that deliberately carries all four hazard
  classes, plus sprite/charset/level/music tables and visible on-screen
  behaviour to remove and add.

**The scoping decision that shapes the whole milestone (owner, 2026-09-10):**
**the tool reports, the end-user decides what gets reverse-engineered.** No
requirement in this milestone may instruct the pipeline to remove, strip, drop
or exclude any particular part of a subject binary on its own judgement.
`BUILD-05` was reworded at this open for exactly this reason — its v0.5.0 text
read *"cracker patches are deliberately excluded rather than inherited"*, which
made the tool the decider — and `BUILD-07` was added to make the resulting
invariant checkable rather than implied. `BUILD-04` was already the right model
and is unchanged: it **enumerates** what blocks movement and acts on none of it.

**Two settled bars carried forward unchanged from v0.5.0, restated here because
this milestone is where they finally bind:**
- **Byte-identity is not the acceptance bar.** In the owner's words, *"byte
  ...
  against — only a provenance-graded composite with some ranges `HIGH`
  confidence and others honestly `UNKNOWN`. "Byte-identical to what?" has no
  well-defined answer here.
- **Proving ground is committed synthetic fixtures only.** No copyrighted game
  image enters this repository. Confirmed still true at this open by
  measurement, not assumption: `danish.d64` and `saeger.d64` sit under
  `.planning/phases/23-*/evidence/corpus/` on disk but appear in no
  `git ls-files` output. Applying the pipeline to a real title is downstream
  use, not this milestone's evidence.

**What v0.9.0 changed about this plan, and why it is takeable now.** The single
blocker stated at the v0.8.0 close — the runtime-evidence layer sitting upstream
of the rebuild half — is **discharged rather than restated**. `DECOMP-01`'s
code-vs-data boundary problem in `$8000-$BFFF` is exactly what an execution
oracle answers, and that oracle shipped: `memmapshow`'s execute bit, parsed,
stored per-address in `anno_evid_exec`, joined against the byte-derived block
table by `reconcileObservedExecution()`, which reports **disagreement first**.
`DECOMP-01` therefore starts from where two independent classifiers conflict
rather than from a flat listing — and must respect the soundness asymmetry the
type system already enforces: observed-executing **is** code, while
never-observed proves nothing and `RuntimeExecClass` has no `data` member to
render it as. `BUILD-*` reuses v0.7.0's real-ACME byte-diff oracle rather than
minting a second one — it refuses to read an exit status, refuses to trust the
aggregate line, and carries `skipped` as a third outcome that is never a pass.
`EQUIV-*`'s first question is whether `chis` over the text channel — per-entry
cycle counts on genuine 3.9, now shipped and parsed as `vice_cpu_history` —
lifts the measured ceiling (exact through anchor hit 50, lost from 75, because
AUTOSTART's power cycle does not reset the absolute emulated clock).

**Deliberately NOT taken, and carried rather than dropped.** Each was weighed at
this open and left standing on the owner's decision to scope v1.0.0 to the
rebuild half alone: `PROOF-03` on real cracked code; the `audit-root-args.test.ts`
scratch-fixture race; the 3-failure `test:automated` floor in
`anno-register`/`anno-import`; `STORE-03`'s traceability row contradicting its
own verifier's quoted sentence, unresolved for three closes; `ANNO-13`/`14`/`15`,
still with no route and no owner for a fourth milestone running; and the live
`broker-owned-tool-output-paths` seed, whose trigger condition ("a fourth host
tool, or output found outside `.c64-re-tools/`") is **met** at six host tools
with five of them never checked.
...
1. **The upstream contributions.** Recorded under Out of Scope here and left
   ...
   hardest loss for everyone, not just this plugin), and the external analyser's
...
**v0.3.0 the external analyser static-analysis backend — as it was tracked during execution:**

**Goal:** Adopt the external analyser as a static-analysis-only backend so recon findings
...
- The `ANNO-16` assumption probe answered against a real build, as a standalone
...
- the external analyser adopted static-analysis-only, never launched with `--vice`,
...
- **Phase 9 (8 plans, no product code — the deliverable is evidence).** Verdict
  ...
  none of the external analyser's literal arms and falls through to its own C64
...
- **Phase 10 (9 plans).** The adoption boundary made structural, the bootstrap
  ...
  guess, the `vice-mcp anno` subcommand, one `--verify`-parsing seam keyed on
...
time (`anno-mcp-client.ts`, a hand-rolled JSON-RPC client chosen after measuring
that `@mastra/mcp`'s `MCPClient` cannot retrieve a spawned child's exit code), and
exposes 17 curated `anno_*` tools registered proxy-locally — so they never reach
...
validated on entry via `anno_set_label_name` or `--import_lbl`. Code review
...
Phase 10's requirements (`ANNO-01`, `-02`, `-03`, `-05`, `-06`, `-09`) were
...
close, two with scope-honesty caveats recorded (`ANNO-02`'s devcontainer and
two-projects halves; `ANNO-03`'s licence, which the Phase 9 probe falsified as
written).
...
stands; two owner decisions recorded — no parity is owed to the external analyser
(Phase 25's "same facts" gate and `STORE-04`'s `ANNO-11` carry-across both
...
`--verify` seam invokes the external analyser rather than ACME, so an ACME verify path
...
corrected: the `anno-*` surface is 25,759 lines against `CUT-01`/`CUT-02`'s
