...
- **Architecture**: Every shipped module that spawns the emulator binary must do it in the safe form — an argv **array**, never a shell command string and never a string-interpolated binary path — and the set of modules that spawn it at all is frozen rather than assumed. That set is currently **empty**: its one prior member, a `--help` probe used to tell which VICE backend was installed, was removed when backend detection collapsed to a single stock target. `spawn-seam.test.ts` checks both halves as a property over the SHIPPED module set, derived from `package.json`'s `files[]` via `shipped-modules.ts` rather than a raw directory listing — a listing would wrongly catch `acme-gate.ts`, a real but non-shipped spawn site — and its set-equality assertion fails in BOTH directions, so a second spawn site cannot appear unguarded.
...
- A **text-monitor client** behind the `channel: "binary" | "text"` discriminator
  ...
  inherits the checkpoint-wait code's serialization invariants rather than being a free
