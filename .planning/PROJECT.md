...
- **Architecture**: The backend swap happens behind `vice.ts`'s `call()` seam. That file exists specifically to be the one place transport can change; its header says so.
...
- **Protocol**: Unsolicited `STOPPED` (0x62) / `RESUMED` (0x63) / `JAM` (0x61) events arrive on the same stream at request-id `0xffffffff` and must be demultiplexed from command replies. This is the one genuine transport difference from today's HTTP.
