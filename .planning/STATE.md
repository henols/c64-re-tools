...
Plan: 5 of 7
Status: Plan 4 is COMPLETE. Its live half (Tasks 2/3) executed 2026-09-15 against genuine unpatched stock /usr/bin/x64sc (VICE 3.9): hazard_raster_entry resolved to $108F from a fresh ACME --symbollist run byte-identical to the committed .prg, two 65536-byte captures plus chip-state sidecars committed, checkpoint deletion proved by enumeration, and the volatile mask calibrated and left UNCHANGED (compare-cross-binary-mask-v1 is closed). 50-04-SUMMARY.md now reads status: complete, so 50-05/50-06/50-07 are unblocked. requirements-completed stays [] deliberately -- EQUIV-01 needs a different-binary comparison and EQUIV-02 needs a rebuild; 50-05 and 50-06 earn those. Two broker env overrides are required by every later live plan: VICE_BROKER_NODE (systemd PATH resolves node v20, below the floor) and VICE_BIN=/usr/bin/x64sc (a fork build shadows stock on PATH). Plan 5 (red control, regressed twin) is executing.
