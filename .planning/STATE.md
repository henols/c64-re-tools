...
Last activity: 2026-09-02 — Quick task 260902-tkg unblocked the release build. `scripts/package.sh`'s leak guard matched `/(node_modules|tools)/` anywhere in the zip listing, so the nine committed phase-23 `evidence/tools/` files that `git archive HEAD` packs from `.planning/` failed it on every push since at least 2026-08-29 — 9 matching paths at HEAD and 9 at the last-pushed `c8da604`, identical, so pre-existing. The halves are now anchored separately (`node_modules/` at any depth, `tools/` only at the archive root, which is what the gitignored host-launcher deployment target actually is), listed via `unzip -Z1` so patterns match paths not a rendered table, and a trip names what it found. `bash scripts/package.sh`: exit 0, 1576 files. Planted root `tools/` file: exit 1, paths named. NOT yet confirmed: that CI goes green end to end — the next push proves that.
