...
status: planning
stopped_at: Planned 28-23-PLAN.md (sixth gap-closure round, scoped to CR-10); plan-checker PASSED after one revision
last_updated: "2026-08-28T23:19:46.845Z"
last_activity: 2026-08-29
last_activity_desc: "Phase 28 FIFTH gap-closure round EXECUTED IN FULL (28-19..28-22) and VERIFIED at 11/12 — same score as round 5, but a DIFFERENT truth fails. CR-09 is GENUINELY CLOSED and round 5s WR-08 advisory cleared. ONE NEW BLOCKER, CR-10, raised by the post-execution code review and INDEPENDENTLY REPRODUCED AND BROADENED by the verifier: retype()s new remainder gate checks byte-count PARITY ONLY, but a split table pairs byte i with byte n+i, so trimming either end re-pairs every entry — five accepted geometries each preserve 0 of 8 recorded targets, and anno-overlap.test.ts:592 pins the corrupted set as correct. Four of five ROADMAP success criteria VERIFIED; SC-3 is falsified in its STATED PURPOSE only, its three named clauses all hold. NOT DEFERRABLE — Phase 30s byte-diff oracle cannot catch it, because CR-10 changes no bytes. A SIXTH round scoped to ONE plan is recommended."
state_head: 1a76378e3f1f5eee934d28480c77453de67585f6
progress:
  total_phases: 6
  completed_phases: 1
  total_plans: 28
...
Phase: 28 (The Store Core) — READY TO EXECUTE (sixth gap-closure round, 28-23, scoped to CR-10)
Plan: 23 planned, 22 executed. The FIFTH gap-closure round (28-19..28-22) is EXECUTED IN FULL and VERIFIED: 28-19 (CR-09 + IN-06 + WR-08, the tracer), 28-20 (WR-18 + WR-22 + WR-24), 28-21 (WR-21 + WR-25) and 28-22 (WR-19, the round-5 record, the closing gate). No plan of this round is outstanding. What is outstanding is CR-10, raised after execution — now planned as 28-23 (one plan, three serial tasks, wave 21), plan-checker PASSED at commit 1a76378.
Status: 11/12 must-haves after round 3 (was 10/12). All five ROADMAP success criteria VERIFIED for a third round, and round 2's two gaps are CLOSED (CR-01/CR-02/CR-04 re-verified; CR-03 closed for its reported cause). Two NEW blockers keep the goal's REVERTIBLE clause false and are what this round closes: CR-05 (the ring is keyed on the store path's basename spelling, so a second spelling of the same file makes the next write delete every pointer row) and CR-06/CR-07/WR-12 (a transaction with no structural lifetime, plus two holes in the ViceError family — an ordinary setDataType can report SUCCESS while leaving the handle permanently wedged). Plan 28-13 has now closed CR-05 and CR-07 in the sweep: the reconciler abstains from the pointer-ROW direction entirely and its transaction has a structural lifetime, both pinned behaviourally through production entry points. Plan 28-14 has now closed CR-06 and CR-07's third property: step 8's commit sits inside a handler that rolls back — so a reader in another OS process can no longer make an accepted write escape the ViceError family, hold the store's write lock, or leave currentRevision() reporting a write that never landed — and revertTo always hands back a usable handle for a revert that already succeeded on disk. The CR-06 closure is proven cross-process against a genuinely separate reader, with a negative control that reproduces the review's bare `Error: database is locked` verbatim against the pre-plan code. Plan 28-15 has now closed WR-12, the last of the three: `pathEntryExists` wraps every non-ENOENT stat failure, so a regular-file ancestor, an unreadable ancestor and an ANCESTOR symlink cycle refuse with `AnnoStorePathError` at BOTH entry points instead of aborting outside the ViceError family (measured, six probe rows, before and after) — which also makes `realpathOfNearestExisting`'s doc claim true and closes the 28-07 P3 comment violation round 3 recorded at anno-types.ts:830-833. Truth 12's three clauses are therefore all closed by this round. The round's closing gate is recorded as numbers: 144/144 anno tests at real exit 0 (was 141, baseline 135), `tsc --noEmit` clean, the four named non-vacuity controls re-observed one at a time, and criterion 4's planted red re-observed on the FINAL tree and reverted to an empty `git diff --stat -- src/mcp/vice`. STORE-01, STORE-02, STORE-03 and STORE-07 are marked Complete on that gate's evidence; STORE-04 and STORE-05 stay held, STORE-04 because round 3 contradicted the evidence it was recorded Complete on. THREE RESIDUALS STAY OPEN and are not closed by this round's green suite: 28-13's prohibition P2 under-claim (a second path spelling makes `retainedRevisions()` report `[]` while the first ring's files exist), the `pragma integrity_check` throw path as a human-verification item, and the check-then-open window plus the byte-wise non-normalising path comparison as stated limits. A NEW residual is named rather than fixed: the manual 40-hop symlink bound is structurally unreachable in ancestor position, where the kernel's own MAXSYMLINKS refuses first — the two bounds are both 40 and agree on every input. VERIFICATION OF THIS ROUND: 11/12 must-haves (was 10/12). The verifier re-drove all four handed-over ids through production entry points and confirmed CR-05, CR-06, CR-07 and WR-12 are GENUINELY CLOSED — round 3's gap 2 (error family + open transactions) is closed outright. ONE BLOCKER REMAINS and it is NEW: CR-08 makes the goal's REVERTIBLE clause false for a FOURTH distinct cause. `revertTo` gates on `existsSync` alone (anno-store.ts:1632), then closes the caller's handle and renames an unverified image over the live store (:1647, :1677) — reproduced with a 0-byte snapshot and no hand edit, the live store goes 69632 -> 0 bytes with no handle returned and every later open refused. WR-13 is what makes that input ordinary: stageSnapshot and publishSnapshot never fsync while the pointer row commits durably, though `fsyncPath()` already exists at :322-329 and is used correctly on the revert path. NOT DEFERRABLE — no later milestone phase touches revert integrity, and Phase 29 puts this handle on the MCP surface for a session's lifetime. The verifier also found STORE-05's `Gaps Found` row is WRONG: every clause verifies and no open finding touches it, so it should return to Complete in round 4 (not corrected from the orchestrator seat). STORE-04's `Gaps Found` is correct but for CR-08, not the recorded integrity_check reason. WARNING ON THE GUARDS: all six new review ids (CR-08, WR-13..WR-17) are dispositioned ONLY by 28-VERIFICATION.md, so docs-review-disposition.test.ts is green today and goes red again the moment round 4's verification overwrites that file. See 28-VERIFICATION.md and 28-REVIEW.md. ROUND 4, PLAN 28-16 IS NOW EXECUTED (CR-08 + WR-17, the first of the round's three). CR-08 IS CLOSED AS A CLASS BY ORDERING rather than by a check for one byte pattern: `openStore` gained a JUDGING open (`mustExist` refuses an absent path BY NAME before `new DatabaseSync` and opens `readOnly`, so judging an image can neither create nor modify it), and `revertTo` gained a step 3b that OPENS the STAGED COPY -- the exact bytes step 5 renames -- before step 4 closes the caller's handle. The round-4 verifier's own reproduction re-driven through production entry points: the live store goes 69632 -> 69632 bytes BYTE-IDENTICAL (was 69632 -> 0), the throw is an `AnnoStoreError` naming the store path, the snapshot path, `NOTHING has been replaced` and `Available revisions: 0, 2`, the SAME handle still answers `currentRevision() == 3` and `listRanges()`, and a later `openStore` succeeds (was: no handle returned and every later open refusing permanently). `retained` IS PROMOTED to ONE meaning -- the openable image -- so `retainedRevisions()` reports `[0, 2]` where it reported `[0, 1, 2]`, and the advertisement, `oldestRetainedRevision`, `revertTo`'s step-2 gate and its `Available revisions:` list all read one witness (`snapshotOpenFailure`); the code-scoped counts of `existsSync(snapshotPathFor` and `existsSync(snapPath)` are both 0 (each was 1) and the comment-scoped count of the superseded predicate is 0 (was 1). THE FIX INTRODUCED NO SECOND DESTROYER: the ring's file sweep is re-pointed at the pointer-ROW question (`claimedRevisions`), so a corrupt-but-claimed image survives on disk at 0 bytes with its pointer row intact as EVIDENCE, and the 32-image cost never runs inside the sweep's `begin immediate` where it would have opened up to 32 databases under the store's write lock on every accepted write. WR-17 is closed with it, in the same function: both `openStore` calls after the rename are inside handlers that rethrow a `ViceError` unchanged and otherwise report a revert that LANDED ON DISK (prohibition 28-11 P5). Recorded as numbers: `anno-*.test.ts block-class.test.ts` 165/165 at `# fail 0 / # skipped 0` (was 159, split exactly 2 + 3 + 1 across the three tasks as planned), `anno-store.test.ts` alone 71/71 (was 65), `tsc --noEmit` clean at every commit, and the step 3b planted red observed TWICE -- on the task-1 tree (2 tests `not ok`) and again on the FINAL tree (1 test `not ok`, because task 2's step-2 gate now absorbs the behavioural half and only the folded source-order assertion bites) -- each time reverted to an EMPTY `git diff --stat -- src/mcp/vice/`. ONE PLAN-LETTER DEVIATION IS ON THE RECORD: step 2's arms split on the POINTER ROW rather than on the image's presence, because the plan's item (d) ("no pointer row, or the image is absent") contradicts its own first two acceptance criteria (zero presence tests on a snapshot path); the two file-half sub-cases are distinguished by the QUOTED REASON instead, and the two pre-existing orphan-ROW tests were re-run to confirm the prefix they assert survives. THE STORE-IDENTITY ADMISSION STAYS AN OPEN RESIDUAL and is NOT claimed closed: an image that opens cleanly, passes `integrity_check` and carries the expected `schema_version` but belongs to a DIFFERENT store is still ADMITTED, because nothing in the schema records store identity and adding it is a `SCHEMA_VERSION` bump -- this milestone's one-way decision (threat row T-28-18, `accept`). STORE-04 is NOT marked Complete: `requirements.ready-ids` reports 0/1 ready because 28-17 and 28-18 also declare it, so `.planning/REQUIREMENTS.md` is untouched by 28-16 exactly as that plan's verification required. See 28-16-SUMMARY.md. ROUND 4, PLAN 28-17 IS NOW EXECUTED (WR-13 + WR-16 + WR-14, the second of the round's three), and all three dispositions are `fix`. WR-13: the publish path is made as durable as the row that claims it -- `stageSnapshot` fsyncs the staged image after its `vacuum into` and `publishSnapshot` fsyncs the ring directory after its `renameSync`, both through the module's existing `fsyncPath()`, so the count of `fsyncPath(` in `anno-store.ts` goes 4 -> 6 while `grep -c 'exec("pragma'` stays 0 in both directions (no pragma was reached for to buy durability; header rule 4 forbids the two candidates outright). THE TWO CALLS ARE DELIBERATELY GUARDED DIFFERENTLY and that asymmetry is the plan-letter deviation on the record: the IMAGE fsync is unguarded because it removes the DESTRUCTIVE half-state (a durable pointer row naming a file whose bytes never reached disk -- exactly the input CR-08 was reproduced with, and a failure there refuses before `begin immediate`), while the DIRECTORY fsync is best effort because it removes only a missing directory entry, i.e. an orphan ROW, the direction trap 10's own premise calls harmless, 28-13 made inert and 28-16 refuses BY NAME. The reason is measured rather than argued: a directory fsync needs `openSync(dir, "r")`, the SAME read bit `readdirSync` needs, so at mode 0300 the directory fsync fails EACCES while an fsync of a file INSIDE it succeeds -- an unguarded directory fsync therefore reddened the pre-existing CR-07 control, whose precondition is a writable-but-UNREADABLE ring and whose comment states that the write itself must stay entirely ordinary. The plan's own bound on the reachable outcomes is preserved exactly: after an interruption the outcomes are still 'a missing entry' or 'an entry whose contents ARE durable', and this call's failure moves the outcome from the second to the first rather than out of the pair. THE DURABILITY CLAIM'S EVIDENCE IS STRUCTURAL AND SAYS SO: an `fsync` has no in-process observable, so the new control asserts the SOURCE ORDER of two calls with a needle positive control in `revertTo`'s body, and its own comment states that a behavioural assertion would be measuring something else and calling it durability (28-07 P3). The planted red was observed and reverted: deleting the `fsyncPath` from `publishSnapshot` gives `not ok 50 ... error: 'and publishSnapshot must fsync the ring directory after that rename'` at `# pass 71 / # fail 1`, restoring it gives 72/72. `anno-durability.test.ts` stays 5/5, so the SIGKILL durability proof still holds over the changed publish path. WR-16: three handlers now report the rollback they OBSERVED instead of the one they intended. Each records whether its own `rollback` returned (Node 22's `DatabaseSync` exposes no transaction-state accessor, which is the reason the claim could not be checked and therefore must not be asserted); the publish/pointer-row handler and the CR-06 commit handler branch their message on the recorded fact and carry `rolledBack` in `data`; the rolled-back branch keeps its pinned wording VERBATIM so no refusal a caller already matches on moves, and the not-rolled-back branch states that the rollback ALSO failed, names the open transaction and the store's write lock, tells the caller to close and reopen, and still names the revision the store is at (28-08 P2 holds). Driven behaviourally through `setDataType` by wrapping the handle's own `db.exec` -- no hand edit of `anno-store.ts` -- with both branches asserted in ONE test and the also-failed message asserted NOT to contain the rolled-back wording. The SWEEP site was WIDENED rather than made to throw, because 28-11 P5 forbids converting a committed write into a caller-visible failure and `revertTo`'s step-6 call site is exactly such a path: `reconcileSnapshotRing` returns `{ droppedFiles, deferred, rollbackFailed }` with `rollbackFailed` set explicitly at all three return sites, always present rather than optional. The `deferred` doc paragraph carries its qualifier in the SAME sentence as the 'THIS SWEEP CHANGED NOTHING' claim. `grep -c rolledBack` = 12 and `grep -c rollbackFailed` = 11 in `anno-store.ts`, and `anno-seam.test.ts` is 17/17 with `ok 15 - the seam contains exactly one commit statement`, proving the new string literals introduced no bare word `commit` while that control is still word-based in this wave. WR-14: both root-sensitive controls now skip where the runner can COUNT it -- node:test's real `{ skip: ... }`, in `anno-confinement.test.ts` case 14's form, with both test names byte-identical because this round's records cite them by name. `SKIPPED as root` = 0 (was 2) and the whole-file `assert.ok(\s*true,` count = 0 (was 2). `sudo -n` needs a password on this host, so uid 0 was FORGED via `NODE_OPTIONS=--import` (a module whose whole body sets `process.getuid`), and the pre-task file was run under the identical forgery as a throwaway copy taken from its own commit: BEFORE `# skipped 0` with both as `ok`, AFTER `# skipped 2` with both reason strings visible in the TAP output. That substitution is recorded as a substitution, not claimed as a root run. The verifier's own qualifier is carried too: its round-4 CR-07 evidence did NOT depend on these controls, so this restores the round's STANDING GUARANTEE rather than the verdict's basis. THE ROUND'S NUMBERS AFTER 28-17: `anno-*.test.ts block-class.test.ts` 167/167 at `# fail 0 / # skipped 0` as uid 1000 (derived 165 + 2, actual 167), `anno-store.test.ts` alone 73/73 (was 71, split exactly 1 + 1 + 0 across the three tasks), `tsc --noEmit` clean at every commit. STORE-04 is STILL NOT marked Complete: `requirements.ready-ids` reports 0/1 ready because 28-18 also declares it, so `.planning/REQUIREMENTS.md` is untouched by 28-17 exactly as that plan's verification required. FOUR RESIDUALS ARE NAMED RATHER THAN CLOSED: the host-crash durability claim's only in-process evidence is the source order (filed as the plan's `backstop` truth and as `human_judgment: true`); the best-effort directory fsync is a stated limit; `anno-seam.test.ts`'s single-commit-site control is still the word-based `/\bcommit\b/gi` count until 28-18 replaces it; and the uid-0 skip observation is a forged uid rather than real root. See 28-17-SUMMARY.md. **ROUND 4 IS NOW EXECUTED IN FULL (28-16, 28-17, 28-18) and its verdict is EXECUTED-PENDING-VERIFICATION — nothing below is a verification result.** 28-16 closed CR-08 as a CLASS by ORDERING: `revertTo` opens the STAGED image (`openStore(staging, { mustExist: true })`, step 3b) before anything is closed or renamed, and `retained` was PROMOTED to one meaning read from one witness (`snapshotOpenFailure`), closing WR-17 in the same function. 28-17 closed WR-13 (`stageSnapshot` fsyncs the staged image UNGUARDED, `publishSnapshot` fsyncs the ring directory BEST EFFORT — the asymmetry is measured, not stylistic: a mandatory directory fsync needs the same read bit `readdirSync` needs and would redden CR-07's `chmod 0o300` control), WR-16 (three rollback handlers record the rollback they OBSERVED; the sweep reports it through a widened result because 28-11 P5 forbids throwing) and WR-14 (both root guards moved into `node:test`'s declaration options). 28-18 closed WR-15: the single-commit-site control now matches commit STATEMENTS via an `exec()` call carrying a bare statement literal in any of SQLite's three spellings, instead of counting the WORD `commit` — a defect observed, not argued, because a second FULLY WORKING commit site spelled `db.exec("end")` passed the old control unchanged (`ok 15`, 17/17). Its mirror cost went with it: `anno-store.ts` no longer carries a comment making an error message's wording load-bearing for a control in another file (`grep -c "load-bearing"` 5 -> 4, the `step` value untouched). THE ROUND'S RECORD: all six round-4 ids (CR-08, WR-13, WR-14, WR-15, WR-16, WR-17) carry a recorded `fix` in `28-REVIEW.md`'s new round-4 disposition table, each naming its plan and quoting a number or line reference from that plan's SUMMARY, and the one carried-forward `behavior_unverified` item — `openStore`'s `integrity_check could not be run at all` arm at `anno-store.ts:432` — is recorded as STILL OPEN with its unchanged reason and is claimed closed by nothing. TWO RECORD CORRECTIONS, both transcribed from the round-4 verifier rather than formed as an executor's opinion of its own work: STORE-05 returns to `Complete` (the verifier's own sentence is that its `Gaps Found` row `is NOT correct on the evidence`), and STORE-04 STAYS `Gaps Found` with its recorded reason corrected from the pending `integrity_check` human item to CR-08, which the verifier states is the real blocker. THE CLOSING GATE, as numbers on the final tree: `node --test anno-*.test.ts block-class.test.ts` -> `# tests 169 / # pass 169 / # fail 0 / # skipped 0` at REAL exit 0 (derived as 28-17's recorded 167 + 2; observed 169, so derived and observed agree and the planned 159 + 6 + 2 + 2 sum agrees too); `npx tsc --noEmit` clean; 28-15's four named non-vacuity controls re-run individually and quoted verbatim; this round's THREE plantings plus criterion 4's planted red each re-observed red and reverted with an empty `git diff --stat -- src/mcp/vice` afterwards; and the CR-08 reproduction RE-DRIVEN through production entry points only, recording the byte-length pair **69632 -> 69632** where round 4 recorded **69632 -> 0**, with the thrown `AnnoStoreError` `instanceof ViceError`, the caller's handle still answering `currentRevision() === 3` with 3 rows, a later `openStore` succeeding, and `retainedRevisions()` `[0, 1, 2] -> [0, 2]` where round 4 recorded `[0, 1, 2]` both times. Structural invariants unmoved: one shipped module names `node:sqlite`; exactly one commit statement under the NEW matcher; `anno-store.ts` and `anno-types.ts` both absent from the five-element host-path consumer set; `SCHEMA_VERSION` still 2 and `anno_snapshot` still one column. `docs-review-disposition.test.ts` 7/7 and `audit-integrity.test.ts` 44/44 before AND after task 2 — a recorded NON-REGRESSION: contrary to 28-VERIFICATION.md's closing paragraph, neither guard was ever red for the six round-4 ids, because that guard recognises a phase's own `*-VERIFICATION.md` as disposition source 2 of five, so writing the report is itself what dispositioned them. STILL OPEN going into verification: STORE-04 (`Gaps Found`, blocker CR-08), the `integrity_check` throw arm, and round 3's untouched warnings (WR-03, WR-05..WR-11, IN-01..IN-05, IN-05 half-closed).
Last activity: 2026-08-28 — ROUND 5 IS NOW EXECUTED IN FULL: plan 28-22 closed WR-19, wrote the round's record and ran its closing gate. WR-19: `commitStatements()` in `anno-seam.test.ts` now matches the commit STATEMENT INSIDE an `exec()` string literal, anchored at a statement boundary, instead of requiring the literal to BE one — fixture counts old->new are bare spellings 3->3, semicolon spellings (`commit;`, `end;`, `COMMIT ;`) 0->3, multi-statement (`insert into t values (1); commit`) 0->1, negative fixture 0->0. The planting proves the residual was real: with a second FULLY WORKING commit site spelled `db.exec("commit;")` the OLD matcher counted 1 and PASSED, the new control reports `not ok 15 ... found 2`. The 28-18 P1 coupling was NOT re-introduced — the boundary anchor is the mechanism, named in the function's own comment, and the negative control (identifiers, an `exec()` taking a bare identifier, and two English sentences about a commit) names 28-18 P1 in its assertion message. THE RECORD: all TWELVE round-5 ids now carry a decision in `28-REVIEW.md`'s OWN table (8 `fix`, 4 `accept`), every `fix` cell naming its plan and quoting a number or line reference from that plan's SUMMARY — the verifier's instruction, because a disposition living only in `28-VERIFICATION.md` dies with that file's next overwrite, which is how round 4 lost the disposition it thought it had. TWO explicit STILL-OPEN rows carry the `behavior_unverified` items forward: `openStore`'s `integrity_check could not be run at all` arm (cited BY FUNCTION AND ARM, line re-derived on this tree at `anno-store.ts:530-535` against round 5's `:465-468` and round 4's `:432`, so the drift is visible), and 28-17's `backstop` host-crash durability bound, which the verifier ABSTAINED on as `insufficient_spec` and which was NOT promoted (28-17 P5, 28-18 P3). REQUIREMENTS: STORE-04 moved UP to `Complete` in both the status table and the checkbox list, on the round-5 verifier's OWN quoted authorising sentences with the source file and table named (28-18 P2); STORE-01 and STORE-03 were CONFIRMED already at `Gaps Found` / `[ ]` (commit `737f9e7`) and needed no edit, recorded rather than silently skipped; all six STORE pairs agree. `requirements.ready-ids` reported 5/5 ready and `requirements.mark-complete` was DELIBERATELY NOT RUN: it would have flipped STORE-01 and STORE-03 to `Complete` against the round-5 verifier's explicit `BLOCKED` verdict and against prohibition 28-18 P2. THE CLOSING GATE, as numbers on the final tree: `anno-*.test.ts block-class.test.ts` 210/210 at `# fail 0 / # skipped 0`, REAL exit 0 as uid 1000 (was 209; derived 211, actual 210, the difference of one explained — the seam file went 22 -> 23 tests because the plan's own action text says WIDEN and EXTEND, so only the multi-statement control is genuinely new); `tsc --noEmit` clean; 28-15's four named non-vacuity controls AND this round's four re-run INDIVIDUALLY and quoted verbatim; NINE plantings each a separate hand edit, each re-observed red with its own `not ok` line and restored to an empty `git diff --stat -- src/mcp/vice` (28-19's PLANTING C; 28-21's escape-hatch, `addScope` idempotence and `addScope` overlap-refusal plantings; 28-20's D, E and F; this plan's synonym planting; plus criterion 4's carried planted red, where BOTH the combined test and its planted-violation sibling report `not ok`); the CR-09 reproduction RE-DRIVEN through production entry points only — the second call now THROWS `AnnoSplitRemainderError`, `listRanges()` is the unchanged single row, and the odd-span-split-type filter counts 0; the CR-08 reproduction RE-DRIVEN after three plans rewrote `anno-store.ts` — `69632 -> 69632`, `AnnoStoreError instanceof ViceError`, the same handle at `currentRevision() == 3`, `retainedRevisions()` `[0,1,2] -> [0,2]`, a later `openStore` succeeding at revision 3; six structural invariants recorded individually; `anno-confinement.test.ts` 15/15; both docs guards 7/7 and 44/44. THE ROUND'S ONE CONVENTION CHANGE IS ON THE RECORD: every `<automated>` command in 28-19..28-22 now begins `set -o pipefail`, because a `tail` pipe returns `tail`'s exit status and rounds 1-4's automated verb therefore gated nothing. ONE DEVIATION, inherited: 28-21's `df619ad` left a shipped string literal reading "two of phase 28's blockers", reddening `docs-dangling-refs.test.ts`'s FLOW-02 control and cascading to `audit-integrity.test.ts`'s D-12-02 (43/44). Measured red at THIS plan's base commit `ba278fd`, so it is 28-21's regression; reworded to "this store's recorded blockers" because this plan's own criterion demands 44/44. STILL OPEN going into verification: STORE-01 and STORE-03 (`Gaps Found`, blocker CR-09 — an executor's confidence in its own fix is not a verdict), both human-verification items, and round 3's untouched warnings. See 28-22-SUMMARY.md.
...
Last activity: 2026-08-28 — ROUND 5, PLAN 28-19 IS NOW EXECUTED (CR-09 + IN-06 + WR-08, the round's TRACER, verified end-to-end before any expansion plan). CR-09 IS CLOSED BY DELEGATION, not by a second copy of the rule: `retype()` runs a remainder shape gate over every remainder of every overlapping row BEFORE the first `delete`, and the gate asks `assertRangeShape()` ITSELF — the exact function `setDataType()` calls on a caller's range — so there is one definition of a legal range shape in the repo and a rule added to it later applies to the store's own writer for free. The decision taken is (b) REFUSE THE WHOLE RETYPE, of the three the round-5 verification lists as defensible; (a) demotion to `undefined` was REJECTED because it destroys the recorded split orientation, which the module's own header calls unrecoverable with no field to migrate, and rounding the range outward is forbidden by trap 7. The refusal is a named `AnnoSplitRemainderError` extending `AnnoRangeShapeError` (and so `AnnoStoreError` and `ViceError`), asserted `instanceof` all three, carrying `rowId`/`rowStart`/`rowEndInclusive`/`dataType`/`remainderStart`/`remainderEndInclusive`/`side`, and its message names both conflicting spans plus the two nearest legal caller boundaries. THE DRIVE, re-run through production entry points only: `setDataType($1004..$1004, "byte")` inside a `$1000..$100f lo_hi_address` row now THROWS where it returned `{revision:2, changed:true, contradictedComments:[]}`, and the refusal costs nothing — `listRanges()` is the same single row `id=1 4096..4111 lo_hi_address bank=null` before and after, `currentRevision()` reads 1 before and 1 after, because the gate precedes the first mutation rather than relying on a rollback 28-17 already showed can fail. The legal case is driven too: `setDataType($1004..$1007, "byte")` gives exactly three rows (`4096..4099 lo_hi_address`, `4104..4111 lo_hi_address`, `4100..4103 byte`), each re-accepted at its own boundaries. IN-06 closed on the same two lines: `insertRange()` takes `bank` as a parameter and `retype()` carries the overlapped row's own value onto both remainders while the newly typed range gets `null` — the read-back is a production `setDataType` observed through `listRanges()`, while the NON-NULL input state is filed as a structured `backstop` truth because `insertRange()`'s former hardcoded `null` means no production route can build it and `applyWrite` is the route that was taken; its callers were NOT broadened to manufacture one. WR-08 pinned by value with its id lists: the union retype leaves ONE row `4096..4111` with ids `[1,2] -> [3]`, and the same-type subrange gives three rows with ids `[3] -> [4,5,6]`; both decisions are recorded in `retype()`'s doc comment, the collapse stated as INTENDED and why it does not contradict STORE-02 (the caller asked for exactly one range; the store joined nothing of its own accord). THE SPLIT CLASS IS NO LONGER INVISIBLE: `anno-overlap.test.ts` gained an 8-entry split overlap table over `$1000..$100f lo_hi_address` covering all five geometries — 5 row sets asserted by value plus both invariants, 3 refusals each asserting the error class AND a `listRanges()` deep-equal AND an unchanged revision through ONE shared arm — and its `lo_hi_address` count went 2 -> 29. THE CLASS-LEVEL INVARIANT is in the tree: every row `listRanges()` returns must be re-acceptable at `assertRangeShape()` and decodable by `resolveSplitTargets()`, asserted after EVERY write of a deterministic 11-step sequence (8 accepted, 3 refused, 13 final rows, all four split layouts present) with three non-vacuity counts. PLANTING C observed red TWICE: test-locally, naming `4352..4356 hi_lo_address` by value at step 2 with `4865..4873 hi_lo_word` still standing at the end and its selectivity measured per step (differs on exactly `[2,4,6]`, byte-identical on the other eight); and ON THE REAL TREE with the 13-line gate deleted by hand from `retype()`, giving 6 `not ok` lines (the CR-09 drive, all three refusing table entries, the invariant, and the selectivity control) at `# pass 25 / # fail 6`, then restored to an EMPTY `git diff --stat -- src/mcp/vice` and 31/31 green. NUMBERS: `anno-*.test.ts block-class.test.ts` 186/186 at `# fail 0 / # skipped 0`, real exit 0 (was 169); `anno-overlap.test.ts` alone 31/31 (was 14, +17 named tests); `tsc --noEmit` clean; `SCHEMA_VERSION` still 2; no DDL, column-meaning or directory-layout change. TWO DEVIATIONS ARE ON THE RECORD: `anno-store.test.ts`'s reserved-`bank` control was narrowed from `never READ` to `never INTERPRETED` (IN-06 legitimately adds four pass-through lines; the control gained a non-vacuity floor and an UNCONDITIONAL no-branch/no-compare/no-arithmetic assertion it did not have, so it came out stronger); and `npx tsc` is unusable in this repo (TypeScript is a devDependency of `src/mcp/vice`), so the project's own `node_modules/.bin/tsc` was used rather than fetching anything. Criterion 1's controls are UNTOUCHED and still green, as the round-5 verifier required. See 28-19-SUMMARY.md.
Last activity: 2026-08-29 — round 5 EXECUTED IN FULL (28-19..28-22, gaps-only) and VERIFIED at 11/12. See 28-VERIFICATION.md (round 6) and 28-REVIEW.md. Recommendation on the record: ONE more plan, whose must-have the verifier states as `every target a surviving split row reports was a target the original table reported`. No requirement row was reverted — the verifier demotes none, and STORE-01 is recorded as eligible to move UP to Complete by a plan, not from the orchestrator seat.
...
Last session: 2026-08-28T21:50:09.805Z
Stopped at: Completed 28-22-PLAN.md
