...
Last activity: 2026-09-01 — Quick task 260901-qzp erased the retired external analyser's name from the entire working tree (~20,200 occurrences, 612 files, 19 deletions). Three subsystems were retired because their subject IS the name: the removal gate, the attribution chain (headers + notices sections + packaging check), and the guard-fates audit. Coverage that never depended on that subject was preserved rather than dropped — the two BASIC-trigger checks moved to `skill-basic-trigger.test.ts`. Six guards broke on the rename and were repaired, not silenced. `npm run test:automated`: 2960 pass, 0 failures. `tsc --noEmit`: clean. All CI gate scripts exit 0.
