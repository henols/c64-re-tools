...
Plan: 7 of 7 executed (an eighth, 50-08, is outstanding and unplanned)
Status: All seven planned plans are executed and summarized. Three of four requirements are Complete: EQUIV-01 and EQUIV-02 (50-06 -- the rebuild compared PASS with an empty difference set and BYTE_IDENTICAL yes as a recorded extra; the pre-registered allowlist exercised both ways, 32 allowlisted vs 32 divergence one flag apart), and EQUIV-04 (50-07, on the developer's dated 2026-09-16 decline of a real GitHub Actions run -- the red was observed locally against CI's exact command and environment, residual named in docs/phase50-ci-boundary.md, assumption P5 still unresolved). EQUIV-03 is deliberately WITHHELD and is the phase's one outstanding requirement: its clause reads 'one behaviour removed and one added IN THE REBUILT SOURCE', but the edit was made in the hand-written fixture family (hazard-subject-align-nosprite.a), not in the tree exportAsmTree() emits -- no run has yet edited an exported file and reassembled it. The developer chose the literal reading on 2026-09-16, so a new plan 50-08 must edit an exported .a, reassemble through Phase 49's gate, capture live and compare. Next action: /gsd-plan-phase 50 to author 50-08. Live plans need VICE_BROKER_NODE and VICE_BIN=/usr/bin/x64sc; vice_program_load is now advertised after an MCP server restart.
...
Progress: [████████░░] 84% (8/13 phases complete in v1.0.0, 76/90 plans. Phase 56 completed by a prior plan. Phase 57 was added concurrently during that plan's execution. This plan completed Phase 50 plan 4's offline half. Plan 4 itself is NOT counted as complete -- its live half (Tasks 2/3) remains outstanding. Phase 50 gained plan 50-08 at planning time, which is why the plan total grows by one. Per-phase plan counts are in the ROADMAP Progress table.)
...
| Phase 56 P10 | 35min | 3 tasks | 6 files |
| Phase 56 P11 | 40min | 3 tasks | 3 files |
| Phase 50 P01 | 20min | 3 tasks | 3 files |
...
- [Phase 40]: Phase 40 plan 01: ghidra-project.mts's per-run project directories are exempted from the .c64-re-tools/ path consolidation and stay at tools/ghidra-runs/ -- Ghidra's own dot-segment refusal rejects any ancestor path segment starting with ".", so nesting a project under the dot-prefixed .c64-re-tools/ root would make every ghidra.analyze call fail. Verified directly; documented in ghidra-project.mts, CLAUDE.md, and an addendum on the folded consolidation todo. **SUPERSEDED 2026-09-08 (gap `G-40-1`):** the "verified directly" claim above was produced by running this project's own dot-segment predicate against a synthetic string -- which observes this project's own code and never observed Ghidra. MEASURED against real Ghidra 12.1.3 (`javap` on `ProjectLocator`, five live runs): the refusal binds the absolutized path via `getAbsolutePath()`, never `getCanonicalPath()`, so it does not resolve symlinks -- a broker-minted, verified, relative-target alias handle satisfies Ghidra and the single-root truth at once. See `.planning/notes/ghidra-dot-path-check-semantics.md`. Resolved by plans 40-08/40-09; see the new entry below.
...
- [Phase 56]: Phase 56 Plan 10: repaired thirteen now-false anno-seam.test.ts/capture-predicate.test.ts enforcement credits across D-11's five named production modules -- five more than the plan's action text named, all found by the plan's own mandated blanket-grep completeness gate and fixed under the same keep-the-constraint-drop-the-credit rule. — The plan's action text undercounted (anno-store.ts had 8 occurrences not 4, capture-predicate.ts had 2 not 1), matching the phase's own documented undercount pattern (56-06 precedent). The task's own verify blocks required a whole-file grep to return 0, so completing the task as verified required repairing every occurrence.
- [Phase 56]: Reconciled Phase 56's whole test-count drop against a git-diff-derived ground truth (every test( line removed or gained between the pre-phase baseline commit and plan 56-11's own HEAD) rather than trusting the sum of ten plans' own prose counts. — The two methods agree exactly: 104 removed-name lines minus 4 renames equals 100 net whole-case removals, matching the orchestrator-measured 3753-to-3653 drop across plans 56-01 through 56-10 exactly.
- [Phase 56]: Reported the embedded-case count (100) and the removed test file's own case count (16) as two separate numbers throughout the phase SUMMARY, per success criterion 2's own wording. — Avoids a single combined total of 116 appearing anywhere a reader could mistake it for one measurement.
- [Phase 50]: Phase 50 Plan 01: compare-cross-binary.mjs demotes byte-identity to a recorded extra (BYTE_IDENTICAL: yes/no); classify() always runs, per assumption_delta_decision Signal 2 (promote behavioural equivalence over byte-identity).
- [Phase 50]: Phase 50 Plan 01: the --state chip-state sidecar uses this module's own new schema (route/checkpoint_name/checkpoint_address/registers), not a literal reuse of dump-artifacts.mjs's chip-state.json output, since no committed sidecar carries $Dxxx register values directly today.
- [Phase 50]: 50-02: Built the regressed twin (three single-bit regressions at $D020/$D015/$D018) and the modified subject (sprite construction removed, second SMC construction called), both anchored to named Phase 49 hazard-report findings, via amended deterministic generators
- [Phase 50]: The modified subject's frozen hazard-acknowledgement array was built from a real buildHazardReport() run against hazard-subject-modified.prg at authoring time, not copied from reassembly-gate-run.test.ts's committed array -- it carries 4 findings, not 5, because the page-alignment finding at $088B is absent from the modified subject. — Copying the committed subject's five-entry array would have silently acknowledged a finding the modified subject's report no longer carries, defeating the point of a measured (not asserted) disposition.
...
| 260913-mql | Take the suite from 7 failures to 0 ahead of the first push in 479 commits. Four root causes, each its own commit: two pending todos missing Deferred Items rows; phase 49's `CR-01` undispositioned (substance already fixed in `29d4c467`, only the record missing); a requirement-id **resolver** defect — milestone archival removes the live document and the resolver only fell back when it was absent, so 11 archived-but-real ids across 10 verbs read as undeclared; and `audit-gate.mjs` reporting all 10 docs guards red because its output parser finds no per-file entry under Node 24's spec reporter and falls through to the whole set. No guard weakened, no floor lowered, no audit status changed | 2026-09-13 | 5624a6ec, c363fcaa, b5499297, e342392b |  | [260913-mql-fix-the-red-docs-guards-and-the-audit-ga](./quick/260913-mql-fix-the-red-docs-guards-and-the-audit-ga/) |
...
| 260914-9n4 | Make the suite host-independent where it can be. CI reached its Test step for the first time since Phase 34 and failed on seven tests that pass here — the difference being `vendor/dxa/dxa`, a gitignored build artifact that cannot exist on a clean runner. Six of the seven were RESTRUCTURED to keep running (a shared fake-binary plant for the argv cases, a synthesized cache tarball for the build-gate cases, with `curl` shadowed and asserted unreached); exactly one is gated behind a new `dxa-gate.ts` mirroring `acme-gate.ts`. CI is not taught to build, fetch or install dxa and `ci.yml` is untouched — a skip there is the correct consequence of the never-auto-install constraint, recorded in the gate's own header | 2026-09-14 | b7a11835, bf1f5558, 303c04c3 |  | [260914-9n4-gate-the-dxa-dependent-tests-on-the-vend](./quick/260914-9n4-gate-the-dxa-dependent-tests-on-the-vend/) |
...
| 260915-mmi | c64-program-recon skill: apply the STE100 strict pass, following every rule in .planning/ste100-batch-rules.md, to these eight files — src/skills/c64-program-recon/SKILL.md (baseline 108 violations), src/skills/c64-program-recon/references/control-flow.md, src/skills/c64-program-recon/references/graphics.md, src/skills/c64-program-recon/references/observation-hazards.md, src/skills/c64-program-recon/references/reconstruction.md, src/skills/c64-program-recon/references/sound-and-input.md, src/skills/c64-program-recon/references/tool-selection.md, src/skills/c64-program-recon/templates/memory-map.template.md | 2026-09-15 | c3d5ad24baa9d47d43f2a0b59d6f9a9cc9ea9a15 | — | .planning/quick/260915-mmi-c64-program-recon-skill-apply-the-ste100-strict-pass-followi |
| 260915-mmj | c64-memory-mapping skill: apply the STE100 strict pass, following every rule in .planning/ste100-batch-rules.md, to src/skills/c64-memory-mapping/SKILL.md (baseline 71 violations) | 2026-09-15 | 6d6dba59d09e19c989f23db6e2a5aef6d0b13109 | — | .planning/quick/260915-mmj-c64-memory-mapping-skill-apply-the-ste100-strict-pass-follow |
| 260915-mmk | vice-wedge-triage skill: apply the STE100 strict pass, following every rule in .planning/ste100-batch-rules.md, to src/skills/vice-wedge-triage/SKILL.md (baseline 56 violations) — this file holds the evidence-ledger tables the cell-not-row rule exists for | 2026-09-15 | 91a44accf59a33c53874640069bf4024e63feaa8 | — | .planning/quick/260915-mmk-vice-wedge-triage-skill-apply-the-ste100-strict-pass-followi |
| 260915-mml | routine-queue-walker skill: apply the STE100 strict pass, following every rule in .planning/ste100-batch-rules.md, to src/skills/routine-queue-walker/SKILL.md (baseline 50 violations) | 2026-09-15 | b5ed3c5405706f6d7e6b3c8f3c3c86538261cd3e | — | .planning/quick/260915-mml-routine-queue-walker-skill-apply-the-ste100-strict-pass-foll |
| 260915-mmm | c64-ram-capture skill: apply the STE100 strict pass, following every rule in .planning/ste100-batch-rules.md, to these three files — src/skills/c64-ram-capture/SKILL.md (baseline 46 violations), src/skills/c64-ram-capture/templates/capture-record.template.md, src/skills/c64-ram-capture/transients/README.md | 2026-09-15 | 6bd13eea92672812f8701a72568ef63e39e3579a | — | .planning/quick/260915-mmm-c64-ram-capture-skill-apply-the-ste100-strict-pass-following |
| 260915-mmn | c64-provenance-diff skill: apply the STE100 strict pass, following every rule in .planning/ste100-batch-rules.md, to src/skills/c64-provenance-diff/SKILL.md (baseline 39 violations) | 2026-09-15 | 5d104d75bb96fc97143f5b2f0d96449f9c9bc865 | — | .planning/quick/260915-mmn-c64-provenance-diff-skill-apply-the-ste100-strict-pass-follo |
...
- anno-register.ts (63 planning-vocabulary citations, RATCHET-pinned) needs a scoped follow-up plan: its requirements[] traceability data is real, mechanically-checked ids (anno-register.test.ts Direction 5), but no shipped file may carry a requirement id. Unshipping it (module-classification.ts's precedent) reddens anno-seam.test.ts's completeness guard because the file's anno-* name is in scope for that check. See 51-08-SUMMARY.md's Deviations section for the three options considered.
...
| testing | audit-gate-shared-budget-reds-tail-guards | major | Pending |
