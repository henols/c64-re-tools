...
title: Migrate the two hand-copied ACME gates onto anno-test-gate.ts's shared seam
area: testing
files:
...
`anno-test-gate.ts` exists because six-plus hand-copied `probeAnno()` bodies is how a
...
2. `anno-cli.test.ts:698-726` — hand-copied it for criterion 3, **plus** its own
   hand-rolled `probeAnno()`/`SKIP_REASON` at `:457-472` rather than importing the
   anno seam that already existed.
3. `anno-test-gate.ts` — added by the Phase 11 validation audit (2026-08-21) so
   criterion 1's fixture-reproducibility test would not become a fourth copy.

**Corrected during 11.1-07's dispositioning pass (D-11.1-04, IN-07):** the anno
half of the gate has a *third* hand-copied `probeAnno()`/`SKIP_REASON`/`ANNO_AVAILABLE`
block, not just `anno-cli.test.ts`'s — `anno-project.test.ts:133-175` carries its own,
...
existing copies. `anno-cli.test.ts`'s gate semantics are load-bearing for criterion 3's
...
and both `anno-cli.test.ts`'s AND `anno-project.test.ts`'s local
`probeAnno()`/`SKIP_REASON` (the third copy) — with imports from
`anno-test-gate.ts` (`ACME_BIN`, `ACME_AVAILABLE`, `acmeSkipReasonFor()`,
`assertAcmeRequiredIfEnvSet()`, `skipReasonFor()`, `assertAnnoRequiredIfEnvSet()`).

Then delete the "HONEST SCOPE" paragraph from `anno-test-gate.ts`'s ACME header — it
documents this todo, and it becomes false the moment this is done.

**Verify:** `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-cli.test.ts
disasm-roundtrip.test.ts absorbed-answer-key.test.ts` — same pass counts as before the
...
`anno-project.test.ts` carried a third, independently-divergent
`probeAnno()`/`SKIP_REASON` copy was verified true before editing) now import
from `anno-test-gate.ts`: `anno-cli.test.ts` imports `ANNO_BIN`,
`skipReasonFor`, `assertAnnoRequiredIfEnvSet`, `ACME_AVAILABLE`,
`assertAcmeRequiredIfEnvSet`; `anno-project.test.ts` imports `ANNO_BIN`,
`skipReasonFor`, `assertAnnoRequiredIfEnvSet`; `disasm-roundtrip.test.ts`
...
(`anno-cli.test.ts`'s and `disasm-roundtrip.test.ts`'s own `probeAcme()`)
passed no `spawnSync` timeout; both now share the seam's bounded 10s probe.

**HONEST SCOPE paragraph deleted** from `anno-test-gate.ts`'s ACME header —
...
this todo's own remembered `43`, which had drifted — `anno-cli.test.ts` had
grown to 64 tests by the time this plan ran):**

| File | Default (before → after) | Opt-in `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1` (before → after) |
|---|---|---|
...
Both the external analyser (0.9.20) and ACME (0.97 "Zem") are genuinely installed on
...
`anno-cli.test.ts` ("gated (D-11+D-08): criterion 3 … renders as
