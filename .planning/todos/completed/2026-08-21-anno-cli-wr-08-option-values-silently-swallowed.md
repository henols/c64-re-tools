...
title: anno CLI's parseArgs() accepts a missing or flag-shaped option value and silently loses the caller's intent (10-REVIEW.md WR-08)
area: cli
files:
...
```
$ node vice-proxy.ts anno bootstrap /tmp/wr08test.prg --out
...
already modified extensively (`anno-cli.ts`), but per this project's deviation-scope
...
`<verb>:`-prefixed message, `{ code: 1 }`, never an escaped throw to `runAnnoCli()`'s
last-resort net -- see WR-09's fix in this same file for the established shape).

**Verify:** `node vice-proxy.ts anno bootstrap game.prg --out` refuses with a named
error instead of silently writing `game.regen2000proj`; `node vice-proxy.ts anno
bootstrap game.prg --out --entry FOO` refuses instead of creating a file named
`--entry`; `node --test anno-cli.test.ts` stays green plus new coverage for both cases.
...
`parseArgs()` (`.claude/mcp/vice/anno-cli.ts`) was rewritten exactly along this todo's
...
Pinned by ten new tests in `anno-cli.test.ts` (commit `e0fd305`), including this todo's
