...
| 04-02 T2 | 04-02 | 1 | DERIV-07 | T-04-02-02, T-04-02-03 | `needsSession:false` never reaches `ensureStockSession()`; handler exceptions convert, never escape; derived handler receives the container path (with a proven non-vacuity control) | unit, behavioural | `cd .claude/mcp/vice && node --test stock-dispatch.test.ts stock-derived.test.ts` | ❌ new | ⬜ pending |
...
| 04-05 T3 | 04-05 | 4 | DISASM-01 | T-04-05-03, T-04-05-06, T-04-05-07 | Answer conforms to its own `outputSchema`; bounded at 100 instructions with `limitReached`/`nextAddress`; derived path proven not to host-translate | unit (schema conformance harness) | `cd .claude/mcp/vice && node --test stock-disassemble.test.ts stock-dispatch.test.ts hostpath-consumers.test.ts fork-manifest-surface.test.ts && npm run smoke` | ❌ new + ✅ extend | ⬜ pending |
...
| 04-07 T2 | 04-07 | 6 | DISASM-07 | T-04-07-01, T-04-07-02, T-04-07-03 | Notices file and all five modules present in the packed tarball; runtime dependency set unchanged | packaging gate | `node scripts/check-npm-packages.mjs` | ✅ extend | ⬜ pending |
