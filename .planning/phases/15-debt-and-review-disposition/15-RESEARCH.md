...
| WR-12 | `walkSkills()`/`MCP_PREFIX_RE`/`TOOL_NAME_RE` duplicated verbatim across `check-skill-fork-honesty.mjs` and `check-skill-tool-coverage.mjs` | **STILL OPEN** [VERIFIED: `scripts/check-skill-fork-honesty.mjs:73` comment literally says "Copied from scripts/check-skill-tool-coverage.mjs's walkSkills()"; `scripts/lib/` contains only `r2000-cli-verbs.mjs` and `skill-honesty-checks.mjs` (a *different* shared module, for violation predicates, not the walk/regex duplication) — no `skill-corpus.mjs` or equivalent exists] | `08-REVIEW.md:603-622` names the extraction target `scripts/lib/skill-corpus.mjs` |
...
`.planning/todos/completed/2026-08-21-r2000-cli-wr-08-option-values-silently-swallowed.md`):
...
| `2026-08-21-migrate-hand-copied-acme-gates-to-r2000-test-gate.md` | — | **Fix or explicitly defer** | Real, scoped, ~small fix (import from `r2000-test-gate.ts` instead of 3 hand-copied `probeR2000()` bodies); no phase claims it — either do it here (cheap, self-contained) or promote with a named owner |
...
| A4 | `.vice-snapshots/r2000_probe_vsf.vsf`/`r2000_probe_vsf_v2.vsf` are suitable to reuse for DEBT-03 scenario 1's `.vsf` round-trip test | DEBT-03 § scenario 1 | If their machine-type/content doesn't fit a clean disk-load round-trip test, a fresh `.vsf` needs producing via `vice_snapshot_save`, which is cheap and already a documented tool in this project |
...
2. **Should `2026-08-21-migrate-hand-copied-acme-gates-to-r2000-test-gate.md`
   ...
   - Recommendation: fix it if time allows (it's genuinely cheap), otherwise
     ...
     the next phase that touches r2000 test infrastructure."
