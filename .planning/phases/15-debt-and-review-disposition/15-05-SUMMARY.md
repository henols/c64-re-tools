...
tech-stack:
  ...
  patterns:
    ...
    - "readdirSync-derived file-set scanning for a security-shape gate: reused this repo's established topLevelProductionModules()/stripCommentLines() convention (hostpath-consumers.test.ts, r2000-launch.test.ts) rather than hand-typing a file list, so the gate covers every current and future top-level production .ts/.mjs file in .claude/mcp/vice, not just probe-binmon.mjs."
