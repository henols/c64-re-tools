...
plan: 01
subsystem: infra
tags: [the external analyser, cargo, rustc, acme, illegal-opcodes, supply-chain, probe]

requires: []
provides:
  - "A real the external analyser 0.9.20 build installed, identified, and CLI-surface-recorded (ANNO-16(5) first half)"
...
tech-stack:
  added: ["the external analyser 0.9.20 (cargo, host-installed)", "tmux 3.5a (apt, host-installed)"]
...
key-decisions:
  - "Human authorized cargo install analyser and the tmux install at a blocking checkpoint; both third-party installs performed by the human, never by this agent"
  ...
  - "Real toolchain floor for the external analyser 0.9.20 is rustc >= 1.88 (transitive, undeclared in Cargo.toml), not edition 2024's 1.85 as 09-RESEARCH.md assumed; --locked does not work around it"
...
**A real the external analyser 0.9.20 build is installed and identified (rustc 1.97.1, after a hard >=1.88 toolchain-floor finding), and a byte-verified illegal-opcode `.prg` fixture exists for waves 2-3 -- INSTALLED_VERSION: The external analyser 0.9.20.**
...
- A real `the external analyser 0.9.20` build installed (by the human, per the harness constraint below), version and full `--help` CLI surface recorded verbatim
...
- **This agent's own tool-permission classifier denies `cargo install` outright**, independent of the GSD checkpoint's human authorization. Both installs (`tmux`, `the external analyser`) had to be performed by the human directly. This is a property of the executing harness, not of the external analyser or the GSD workflow, and it means **no agent-driven third-party install was demonstrated in this environment** -- a fact plan 09-02's/Phase 10's automated-bootstrap thinking should account for if it assumes an agent can `cargo install` unattended.
- **`cargo install analyser` fails outright on rustc 1.85.1**, with or without `--locked` -- the crate's real floor (rustc >= 1.88) is undeclared in `Cargo.toml` (no `rust-version` field in either `Cargo.toml` or `Cargo.toml.orig`), so cargo cannot warn early. This cost two full dependency-resolution/download cycles (354 crates / 53.2 MB relayed by the human) before the fix (`rustup update stable`) was applied. Recorded in full as a finding for Phase 10's `ANNO-03` install documentation, not resolved here.

## User Setup Required

None beyond what already happened during this plan's checkpoint: `tmux` and `the external analyser` are now installed on this host by the human. No further manual configuration is required for this plan's own deliverables.
...
- **the external analyser 0.9.20 is installed and on PATH** (`/home/henrik/.cargo/bin/analyser`), qualified by rustc 1.97.1. Every later criterion's evidence should cite this exact pair, not the 1.85.1 reading recorded earlier in this same plan before the toolchain moved.
