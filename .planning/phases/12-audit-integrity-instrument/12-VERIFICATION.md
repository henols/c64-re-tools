...
verified: 2026-08-21T19:11:10Z
status: verified
score: 11/11 truths verified; both former human-verification items closed with live in-session evidence (plan 12-07)
overrides_applied: 0
re_verification:
  previous_status: gaps_found
  previous_score: 6/11
  gaps_closed:
    - "Truth 8 / gap 1 (D-12-04, the single-line Bash append bypass): closed by the unanchored token scan (`declaresGatedStatusUnanchored()`, consulted alongside the still-line-anchored `writtenDeclaresGatedStatus()` at `isHookInScope()`:795). Independently re-reproduced in this pass against the committed exports: `echo \"status: passed\" >> ...MILESTONE-AUDIT.md` -> `isHookInScope` **true** (was `false`), and so do the `printf` and `tee` one-liners. The line-anchored scan alone still returns `false` on the same string, confirming the fix came from the added scan rather than from loosening the frontmatter parser. End-to-end over a genuinely red guard the live `--hook` process exits **2**. D-12-13 preserved: the same one-liner carrying `status: gaps_found` exits **0**, and a non-audit target exits 0. The test-coverage half of the gap is also closed -- `audit-integrity.test.ts` now pins the echo (:737), printf (:748) and `tee -a` (:763) append forms, plus their `gaps_found` counterparts (:796, :804, :815)."
    - "Truth 7 / gap 3 (D-12-03, exit-code contract on the fallback path): closed by the iterative depth-capped leaf walk (`collectStringLeaves()`:510, explicit `{value, depth}` stack, `MAX_LEAF_DEPTH=200`, plus a leaf-count cap) which reports `depthTruncated` instead of throwing. Independently re-reproduced: a genuine 60,000-deep nested array under an unrecognised `tool_input` key (120,050 bytes of real JSON, built as text because `JSON.stringify` is itself recursive) now exits **2** over a red guard with zero `RangeError` lines, and the refusal names the cap (`MAX_LEAF_DEPTH=200`). Depth truncation is treated as IN SCOPE -- `depthTruncated: true`, `shapeKnown: false`, `isHookInScope: true` -- so the most defensive path fails closed rather than exiting 1 with a raw stack trace."
    - "Truth 9 / gap 2 (D-12-14, a gate bug cannot brick unrelated calls): closed. The catastrophic-backtracking regex is replaced by a bounded token locator. Independently re-measured in this pass as total live `--hook` process wall time on `sed -i ` + N filler chars: 51ms / 32ms / 43ms / 44ms at N = 10k / 20k / 30k / 40k -- flat, and dominated by Node startup. The pre-fix curve measured in the original verification was 64 / 304 / 564ms at 10k / 20k / 30k, and the reviewer's independent table was 422ms at 20k and 2196ms at 40k. No super-linear growth remains at any measured size."
  gaps_remaining: []
  regressions: []
  closed_by:
    - "plans 12-05 and 12-06 (the code fixes)"
    - "plan 12-07 (the live in-session hook block and the A2/A3 dispositions)"
  reverified_by: "the Claude Code session that performed plan 12-07's live test, re-running every original reproduction rather than reading the gap-closure plans' own claims"
...
deferred:
  - truth: "RESOLVED FOR THIS PHASE 2026-08-21 (history preserved, not deleted): docs-review-disposition.test.ts is currently red because 12-REVIEW.md's CR-01/CR-02/CR-03/WR-01..WR-04 findings have no recorded disposition, which is why audit-integrity.test.ts's own real-tree D-12-02 assertion currently fails and node scripts/audit-gate.mjs currently refuses on this tree"
    addressed_in: "Phase 15 (the general all-phases requirement); this phase's own contribution to the red state was cleared by plan 12-06"
    resolution: "As of this re-verification the guard is GREEN: `node scripts/audit-gate.mjs` reports `4 docs guards green` and `audit-integrity.test.ts` is 43/43, including the real-tree D-12-02 assertion that used to be the suite's one expected failure. Phase 12's own CR/WR findings now carry recorded dispositions. GATE-02 remains a live Phase 15 requirement because it spans every phase, not only this one -- but it is no longer blocked by anything Phase 12 left behind."
...
human_verification: []
# RESOLVED 2026-08-21 by plan 12-07 (history preserved below, not deleted). Both items were
# discharged live, in-session, by a Claude Code 2.1.238 session that had loaded the committed
# .claude/settings.json PreToolUse entry at startup. Item 1: all three write routes were refused
# live -- Write, Edit (two payload shapes) and a Bash heredoc -- with the gate's stderr visible;
# the identical Write then succeeded after the revert, and the gate counted 7 audits / 5 gated
# with the scratch file present, so the refusals were the gate deciding on a real in-scope
# target. Recorded verbatim in 12-GATE-PROOF.md's `## Live in-session hook block`. Item 2: A3
# CONFIRMED (the heredoc's gated token sits on body line 3 while its first line carries none, so
# the full multi-line body reached tool_input.command); A2 CONFIRMED (the hook fired for a
# general-purpose subagent's Write call), scoped narrowly to this Claude Code version, one
# in-process subagent and the Write route. Recorded in 12-HOOK-STDIN-EVIDENCE.md's `## A2 / A3
# resolution`, body evidence first and frontmatter second per that file's own rule.
#
# PROVENANCE CAVEAT, carried forward deliberately: the observer was the Claude Code session
# itself, not a human reading a screen. The refusal text is the harness's own tool-result payload
# rather than recorder prose, and each blocked write was verified to have left no file -- but no
# human independently reproduced the refusals in a separate session. 12-GATE-PROOF.md's
# "Provenance of these observations" states this in full. A reader who needs
# human-eyeballs-on-screen confirmation should rerun that section's Steps 1-7.
#
# The two original items, verbatim:
...
**Verified:** 2026-08-21T19:11:10Z (re-verification; initial pass 2026-08-21T18:40:00Z)
**Status:** verified
**Re-verification:** Yes — after gap closure (plans 12-05 and 12-06 for the three code defects, plan 12-07 for the live in-session hook block and the A2/A3 dispositions). Every original reproduction was re-run from scratch in this pass rather than taking the gap-closure plans' own claims; the measurements below are this pass's own.
...
|---|-------|--------|----------|
| 1 | (SC1) A guard deliberately turned red is proven to block the audit-`passed` path | ✓ VERIFIED (with caveat, see gap rows) | `12-GATE-PROOF.md` §2-4 transcript; independently reproduced live: `node scripts/audit-gate.mjs` on this real tree right now returns `audit-gate: REFUSED`, exit 1, naming red guards and quoting failing assertion text — the exact D-12-15 three-part refusal shape, on a genuinely red tree. **Re-verified 2026-08-21T19:11Z:** the tree is now green, so this pass re-established the red state by replanting the same one-digit `CLAUDE.md` drift, reconfirmed `REFUSED`/exit 1, and reverted — plus the same refusal is now observed through live PreToolUse dispatch (truth 10), which the original pass could not reach |
...
| 7 | (D-12-03) Hook mode refuses via exit 2 + stderr only for every in-scope call; field-name-agnostic fallback cannot silently no-op | ✓ VERIFIED (was ✗ FAILED) | Re-reproduced from scratch: a real 60,000-deep nested array under an unrecognised `tool_input` key (120,050 bytes, built as text since `JSON.stringify` is itself recursive) now exits **2** over a red guard with **zero** `RangeError` lines, and the refusal names the cap it hit (`MAX_LEAF_DEPTH=200`). `collectStringLeaves()` (`:510`) is an explicit-stack iterative walk reporting `depthTruncated` instead of throwing; truncation is treated as in-scope (`depthTruncated: true`, `shapeKnown: false`, `isHookInScope: true`), so the most defensive path fails closed. Contract holds: only 0 or 2 observed, never 1 |
| 8 | (D-12-04) Hook detects a gated status written through Bash — heredoc, append, and tee shapes | ✓ VERIFIED (was ✗ FAILED) | Re-reproduced from scratch: `echo "status: passed" >> ...MILESTONE-AUDIT.md` → `isHookInScope` **true** (was `false`); same for the `printf` and `tee` one-liners; live `--hook` exits **2** over a red guard. `writtenDeclaresGatedStatus()` alone still returns `false` on that string, so the fix is the added unanchored scan (`declaresGatedStatusUnanchored()`, consulted at `:795`), not a loosened frontmatter parser. D-12-13 intact: the `gaps_found` one-liner exits 0, as does a non-audit target. Committed tests now pin echo (`:737`), printf (`:748`), `tee -a` (`:763`) and all three `gaps_found` counterparts. The heredoc shape is additionally proven by live in-session dispatch — see truth 10 |
| 9 | (D-12-14, scoped) A bug in the gate cannot brick unrelated Write/Edit/Bash calls repo-wide | ✓ VERIFIED (was ✗ FAILED) | Re-measured from scratch as total live `--hook` process wall time on `sed -i ` + N filler chars: **51 / 32 / 43 / 44ms at N = 10k / 20k / 30k / 40k** — flat, dominated by Node startup. Pre-fix this pass's own predecessor measured 64/304/564ms at 10k/20k/30k and the reviewer's table 422ms at 20k / 2196ms at 40k. The backtracking regex is replaced by a bounded token locator. Corroborated incidentally by the live test: the `sed -i` plant and revert commands in `12-GATE-PROOF.md`'s live section are themselves Bash calls dispatched through this hook while the guard was red, and both were allowed promptly |
| 10 | Live in-session hook block (the 12-04-PLAN.md end-of-phase human-check) was performed and recorded | ✓ VERIFIED (was ? UNCERTAIN) | Performed 2026-08-21 on Claude Code 2.1.238 and recorded in `12-GATE-PROOF.md` § `Live in-session hook block`, all 8 slots filled. Four gated writes refused live by the harness's own PreToolUse dispatch — `Write`, `Edit` in two payload shapes (gated token as unchanged context; and as the edit itself, upgrading a real audit to `status: passed`), and a `Bash` heredoc — then the identical `Write` succeeded post-revert, with the gate counting 7 audits / 5 gated while the scratch file existed and 6/4 after deletion. The superseded "not exercised by this document" paragraph is kept verbatim with a dated forward pointer, so promise and discharge sit together. Observer was the session itself, not a human — stated explicitly in that section's "Provenance of these observations" |
| 11 | Hook payload field-shape assumptions A2 (subagent routing) and A3 (heredoc full-body capture) are resolved | ✓ VERIFIED (was ? UNCERTAIN) | Both reach a terminal CONFIRMED state in `12-HOOK-STDIN-EVIDENCE.md` § `A2 / A3 resolution`, body evidence first then frontmatter. **A3**: the live heredoc's gated token sits on body line 3 while its first line carries the target but no gated status, and hook mode needs both halves of its predicate — so the refusal is only reachable if the full multi-line body arrived in `tool_input.command`. Positive evidence, not absence-of-error. **A2**: the hook fired for a `general-purpose` subagent's `Write`, verified to have left no file. A2's scope is recorded narrowly — this version, one in-process subagent, `Write` route only; the untaken ACCEPTED-LIMITATION branches are marked *BRANCH NOT TAKEN* rather than deleted |

**Score:** 11/11 truths verified (was 6/11 — 3 failed as blockers, 2 uncertain). No regressions: the 6 originally-verified truths were each re-checked in this pass and still hold.

### Deferred Items

| # | Item | Addressed In | Evidence |
|---|------|--------------|----------|
| 1 | ~~The current real-tree red state of `docs-review-disposition.test.ts` (caused by 12-REVIEW.md's own CR-01/02/03/WR-01-04 findings having no recorded disposition)~~ **RESOLVED for this phase 2026-08-21** | Phase 15 (still open as the general all-phases requirement) | This phase's contribution was cleared by plan 12-06: the guard is now GREEN in this pass (`4 docs guards green`), and `audit-integrity.test.ts` is 43/43 — including the real-tree D-12-02 assertion that used to be the suite's one expected failure. GATE-02 remains a live Phase 15 requirement because it spans every phase, but is no longer blocked by anything Phase 12 left behind |

Note (original, preserved): the deferral above covers only the *symptom* (the guard being red because findings are undispositioned). It does **not** cover the underlying functional defects (CR-01/02/03) themselves — those are failures of this phase's own plan 12-02 must-have truths (D-12-03, D-12-04, D-12-14) and are listed as gaps above, not deferred, because GATE-02 only requires a *disposition* (fix, accept, or defer with rationale), not that the code actually work as originally claimed.

**Update 2026-08-21:** that distinction turned out to be the load-bearing one, and both halves are now closed by different means — the *symptom* by plan 12-06 recording the dispositions, and the *functional defects* by plans 12-05/12-06 actually fixing CR-01/CR-02/CR-03, each re-reproduced from scratch in this pass (see `re_verification.gaps_closed`). Neither was closed by the other's route, which is why the note above was worth keeping.

### Required Artifacts

| Artifact | Expected | Status | Details |
|----------|----------|--------|---------|
| `scripts/audit-gate.mjs` | Single check point: check mode + `--hook` mode | ✓ EXISTS, SUBSTANTIVE, WIRED — all 3 formerly-confirmed defects (CR-01/02/03) re-tested and closed in this pass | 1185 lines (was 860); exports `checkAuditGate`, `hookMain` (via CLI dispatch), `docsGuardFiles`, `runGuardsLive`, `isGatedStatus`, `frontmatterStatus`, `milestoneAuditFiles`, `extractHookTarget`, `isHookInScope`, `writtenDeclaresGatedStatus` — all confirmed present by direct import |
| `.claude/mcp/vice/audit-integrity.test.ts` | Layer 1 (clean-checkout) + hook-mode + settings-wiring tests | ✓ EXISTS, SUBSTANTIVE, WIRED | 1009 lines (was 711), 43 tests (was 27), 2 suites; **43 pass / 0 fail** in this pass — the formerly-expected real-tree D-12-02 failure is now genuinely green because the tree is green. The 16 added tests include the echo/printf/`tee -a` append forms and their `gaps_found` counterparts, and the depth-cap/bounded-regex cases |
...
| `.planning/phases/12-audit-integrity-instrument/12-GATE-PROOF.md` | Real-tree red/green plant-and-revert transcript **plus** the live in-session hook block | ✓ EXISTS, SUBSTANTIVE, COMPLETE (was ⚠️ INCOMPLETE) | 973 lines (was 380+); the `## Live in-session hook block` section its closing text promised now exists with all 8 observation slots filled, 4 verbatim refusal payloads, a "Provenance of these observations" subsection naming the session (not a human) as observer, and a "Deviations from the kit as written" subsection listing all 4 departures rather than smoothing them over |
| `.planning/phases/12-audit-integrity-instrument/12-HOOK-STDIN-EVIDENCE.md` | Empirical resolution of RESEARCH assumptions A1, A2, A3 | ✓ EXISTS, SUBSTANTIVE — A1 resolved by transcript; **A2 and A3 now CONFIRMED by live dispatch** | A1 still rests on transcript-derived evidence (the four `$HOME/.claude/hooks/` scripts Route A assumed genuinely do not exist on this host). A2/A3 rest on the committed *project* hook — a different hook from the absent user-level ones — observed firing live; the `route:` frontmatter now draws that distinction explicitly instead of implying no hook exists anywhere |
...
| `.claude/settings.json` | `scripts/audit-gate.mjs` | `PreToolUse` `hooks[].command` | ✓ WIRED **and confirmed firing live** (was ? UNCERTAIN) | The declaration was already confirmed by reading; this pass adds live dispatch. A Claude Code 2.1.238 session that loaded the committed entry at startup had four gated write attempts refused through it — `Write`, `Edit` ×2, `Bash` heredoc — plus a subagent-routed `Write`, each returning the gate's own stderr as the tool result. Recorded in `12-GATE-PROOF.md` § `Live in-session hook block` |
...
|----------|---------|--------|--------|
| Check mode refuses on a genuinely red tree | `node scripts/audit-gate.mjs` (guard planted red) | `audit-gate: REFUSED`, exit 1, names all 4 red guards + quotes failing assertion text | ✓ PASS (re-run this pass) |
| Check mode allows on the now-green tree | `node scripts/audit-gate.mjs` | `audit-gate: OK -- 4 docs guards green, 6 milestone audits scanned, 4 declaring a gated status`, exit 0 | ✓ PASS |
| `audit-integrity.test.ts` full suite | `cd .claude/mcp/vice && node --test audit-integrity.test.ts` | 43 pass / 0 fail (was 26/1) — the formerly-expected D-12-02 real-tree failure is now genuinely green | ✓ PASS |
| Hook mode: Bash echo-append detection (CR-03, was a bypass) | `isHookInScope("Bash", {command:'echo "status: passed" >> ...MILESTONE-AUDIT.md'}, extraction)`; then live `--hook` over a red guard | `true` (was `false`); live process exits **2**. `printf` and `tee` forms likewise `true`. `gaps_found` one-liner exits 0; non-audit target exits 0 | ✓ PASS (was ✗ FAIL) |
| Hook mode: deep-nesting fallback (CR-02, was an uncaught crash) | live `--hook` with a real 60,000-deep nested array under an unrecognised key (120,050 bytes) over a red guard | exit **2**, zero `RangeError` lines, refusal names `MAX_LEAF_DEPTH=200`. `depthTruncated: true` / `shapeKnown: false` / `isHookInScope: true` — fails closed | ✓ PASS (was ✗ FAIL) |
| Hook mode: regex backtracking (CR-01, was super-linear) | total live `--hook` process wall time, `sed -i ` + N filler chars, N=10k/20k/30k/40k | **51 / 32 / 43 / 44ms** — flat, Node-startup-dominated (pre-fix: 64/304/564ms at 10k/20k/30k; reviewer's table: 422ms@20k, 2196ms@40k) | ✓ PASS (was ✗ FAIL) |
...
| **Live PreToolUse dispatch** (the whole point of truth 10) | in-session `Write`, `Edit` ×2, `Bash` heredoc, and a subagent `Write`, each targeting a gated-status milestone audit while the guard was red | all five refused by the harness, gate stderr returned as the tool result; identical `Write` succeeded after the revert | ✓ PASS |
| Scope fence, live | the `sed -i` plant/revert on `CLAUDE.md` — real Bash writes dispatched through the hook while the guard was red | both allowed; hook exits 0 before any `spawnSync` for a non-`*MILESTONE-AUDIT*.md` target | ✓ PASS |
