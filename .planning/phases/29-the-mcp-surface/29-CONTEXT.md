...
The external analyser integration in this same phase rather than leaving it standing
...
so nothing is deleted to make them pass."* The user decided otherwise: anno
...
3. **Renaming the 11 `capability` modules** out from under the `anno-` prefix
   (Phase 27 recorded the classification but renamed nothing).
4. **Deleting the binary-driving glue** — the external analyser external dependency
   is gone at this phase's close.
5. **Re-pointing the 9 skill files** naming 18 `anno_*` tools (pulled forward
...
- **D-01 — anno is deleted in Phase 29, not unregistered and not deferred to Phase 32.** The user's directive: *"The anno usage is decided to be removed
...
- **D-09 — `anno_get_disassembly_cursor` is folded into the disassemble verb's address argument, not carried as a verb.** That is what
...
- **D-12 — `docs-absorbed-decisions.test.ts` and `scripts/audit-gate.mjs:136` move together, in one commit.** Their own comment records that they were added in one
...
- **D-14 — D-02's kept-verb set is narrowed to `render-memmap` and `coverage`.**
  **User decision, 2026-08-29.** D-02 verified `anno-cli.ts`'s own import list,
  ...
  `anno-launch.ts` / `anno-tools.ts` *through* the capability modules
  ...
  — **Consequence the plan must carry:** `ANNO-14` / `ANNO-15`'s ✓ Validated
...
- **D-15 — `anno_enum_usage` is added and `SCHEMA_VERSION` bumps 2 → 3, with no migration arm.** **User decision, 2026-08-29.** `anno_apply_enum_usage` is
...
- **D-16 — D-03's rename set is 9, not 11: `anno-test-gate.ts` and `anno-verify.ts` are deleted, not renamed.** *Agent-resolved, taking the
  ...
  as a claim that the module survives a prefix deletion"* (`anno-test-gate.ts`)
  and *"what a later phase inherits is the discipline … NOT the route"*
  (`anno-verify.ts`); the verdict field and the note field are one record, and
  reading only the verdict is the name-based shortcut D-03 exists to prevent, one
  level up. Corroborated by import measurement: `anno-test-gate.ts` has zero
  surviving importers (both live ACME-gated tests already use `acme-gate.ts`) and
  `anno-verify.ts`'s only import is the deleted `anno-launch.ts`.
  — **Consequence the plan must carry:** `anno-verify.ts`'s two pinned
...
- **D-17 — `render-memmap` is rebuilt onto the Phase 28 store in this phase; D-14's "keep the free ones" premise was false, and neither kept verb was free.** **User decision, 2026-08-29.** D-14 kept `render-memmap` on the strength of `29-RESEARCH.md`'s measurement that its module has *"zero local imports"* — asserted at `:267` and `:569` and repeated in this file's `<deferred>` block as *"no store call site"*. All of those measurements were `grep`-based, and `grep` is blind to `src/mcp/vice/anno-memmap-render.ts` because the file carries a literal NUL byte at offset 12862 (line 291, a `"\0"` separator in the sidecar hash canonicalisation). Read with `grep -a`, the module imports `runAnnoTool` from `anno-tools.ts` at `:69`, uses it at `:106` inside `queryAnnoJson()`, and `renderMemoryMap()` calls that three times at `:353-355` (`anno_get_blocks`, `anno_get_symbols`, `anno_get_comments`); `checkRenderedMemoryMap()` reaches the same path via `renderMemoryMap()`. So `render-memmap` drives the external analyser child, and D-14's chosen option had **no** free verb in it.
...
- `.planning/phases/19-absorbed-procedures-and-the-coverage-instrument/upstream-procedure-manifest.json` — the classification the family is derived from: 5 procedures, 20 distinct verbs, dispositions `curated` / `adapt-to-address-input` / `omit`, plus `disposition_rationale` and per-procedure sha256s. Pinned to the external analyser commit `493f840418f1450a342bb220c2fe3d2585dd0525`, version 0.9.20.
- `src/mcp/vice/anno-derivation.test.ts` — what currently enforces manifest-vs-surface agreement; must be re-pointed, not deleted, when `CURATED_ANNO_TOOLS` goes.
...
- `src/mcp/vice/vice-proxy.ts:3263` (`buildViceTool`), `:3401-3402` (the anno loop registration being replaced).
...
- `scripts/generate-tool-support-table.mjs:104` — `discoverSyntheticToolNames`, the hard-coded `ANNO_TOOL_DEFINITIONS` regex and its two deliberate duplicate witnesses.
- `src/mcp/vice/hostpath-consumers.test.ts:188` — `ANNO_MODULE_FLOOR = 14` and the `INT-01` positive control at `:199`.
- `src/mcp/vice/spawn-seam.test.ts` — the other disk-derived module set.
- `scripts/check-skill-tool-coverage.mjs`, `scripts/lib/anno-cli-verbs.mjs`, `src/mcp/vice/anno-verb-coverage.test.ts` — the FLOW-01 guard; the verb parser is `glue` and dies, so the guard is re-expressed over the renamed CLI.
- `scripts/check-npm-packages.mjs` and `src/mcp/vice/package.json` — 16 `anno` entries in `files[]`; deleting a file still listed there makes the packaging check fail.
- `scripts/check-skill-fork-honesty.mjs:504` — the `"anno export-asm"` assertion (D-10).
- `scripts/audit-gate.mjs:136` and `src/mcp/vice/docs-absorbed-decisions.test.ts` (D-12).
...
- `assertCuratedTool()`'s D-33 batch recursion in `anno-tools.ts`: the sanctioned-exception discipline for the batch verb. Read it before deleting the file.
...
- `BACKEND_SEAM_BYPASS_KEYS` is ordered and currently `["RESULT_CONTINUE_TOOL.name", "annoDef.name"]`; the second entry is renamed in place, not added to.
- `docs/tool-support.md` must regenerate **byte-identical** — the generator already excludes the anno loop structurally, so replacing that loop with the `anno_` loop keeps the table unchanged. Verify rather than assume.
...
- The user's words on the anno exit, recorded as data: *"The anno usage is
...
- Research context for D-05, from the discussion's one web search: tool-space
  ...
  one, and a secondary reason D-01's removal of the near-identical `anno_*`
...
- **`render-memmap`'s independence** — ~~it reads `memmap.json` and writes Markdown
  ...
  **CORRECTED 2026-08-29 — the "no store call site" half is FALSE; see D-17
  above.** Re-measured with `grep -a`, `anno-memmap-render.ts` imports
  `runAnnoTool` at `:69`, uses it at `:106`, and `renderMemoryMap()` calls it
