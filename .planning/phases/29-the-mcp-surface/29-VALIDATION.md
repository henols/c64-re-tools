...
| `node scripts/check-skill-tool-coverage.mjs` | `OK` — 37 `vice_*` / 17 `r2000_*` / 8 CLI verbs, 8/8 resolved | < 5 s |
...
|--------|----------|-----------|-------------------|-------------|
| MCP-01 | manifest-derived surface: every `curated`/`adapt` verb routed, every `omit` absent, `delete_project_enum` absent, checked mechanically | unit | `node --test anno-derivation.test.ts` | ❌ W0 (re-points `r2000-upstream-audit.test.ts`) |
...
| MCP-04 | batch pre-validates recursively and refuses WHOLE on an uncurated inner name at any depth | unit | `node --test anno-tools.test.ts` | ❌ W0 (port `r2000-tools.test.ts` batch cases) |
...
| MCP-05 | all three `R2000_TOOL_DEFINITIONS` witnesses moved; generator throws (not silently differs) if one did not | structural | `node --test tool-support-table.test.mjs capability-registry.test.ts` | ✅ re-point |
| MCP-05 | `R2000_MODULE_FLOOR` re-expressed with a real positive control | structural | `node --test hostpath-consumers.test.ts` | ✅ re-point |
...
| D-01 / CUT-02 | the grep gate bites on planted violations and is green over the untouched exemption set | structural + planted | `node scripts/check-no-regenerator2000.mjs` | ❌ W0 |
| D-02 / FLOW-01 | the renamed CLI's verbs are parsed from its own switch, with a floor, and every one is named by a skill file | structural | `node --test anno-verb-coverage.test.ts` | ❌ W0 (ports `r2000-verb-coverage.test.ts`) |
...
- [ ] `scripts/check-no-regenerator2000.mjs` — the grep gate plus its exemption non-vacuity counter
