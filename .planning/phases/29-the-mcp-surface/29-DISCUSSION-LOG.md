...
**Areas discussed:** Naming & the coexistence window, Store handle lifecycle & project addressing, Where the read verbs get their bytes, The derivation rule and the cursor verb, The anno exit (raised by the user)
...
**User's choice:** all four, **plus** a free-text addition: *"The anno usage is decided to be removed and if it can't be removed right now everything about it must be disabled an never be included in any tests."*
...
## The anno exit — what "removed / disabled" means
...
| Unregister only, tests stay green | Stops being advertised; `anno-*.test.ts` keeps running until Phase 32 | |
...
## The anno exit — what rides forward with the deletion
...
| Delete only the binary-driving glue | Delete the modules that spawn or speak to the external analyser; rename the CLI keeping its capability-backed verbs; `export-asm`/`verify` wait for Phase 30 | ✓ (by delegation) |

**User's choice:** "You decide."
**Notes:** Resolved by Claude as the third option, then verified rather than assumed — `anno-cli.ts`'s import list confirms `bootstrap`, `export-asm` and `verify` are the only verbs reaching `anno-launch.ts`, so the external dependency can go without inventing an export route ahead of Phase 30's oracle. Facts presented before the question: `module-classification.ts` marks 11 modules `capability` and 8 `glue`, all 19 still carry the prefix on disk, 26,023 lines under `anno-*.ts`, 16 `files[]` entries, 9 skill files naming 18 distinct `anno_*` tools, and ordering constraints 2, 3 and 5 all point the other way. → CONTEXT D-02, D-03.
...
- Three guard fates were presented as Claude's calls and left unchallenged when the user chose "Ready for context": `check-skill-fork-honesty.mjs:504` (strip the route from the skill, re-point the assertion — D-10), `absorbed-answer-key.test.ts` (rename and keep — D-11), and `docs-absorbed-decisions.test.ts` ↔ `audit-gate.mjs:136` (move together in one commit — D-12).
- Left to the researcher and planner rather than put to the user: the grep gate's scope predicate, the `{available:false, reason}` envelope shape, the batch verb's recursive pre-validation, how `coverage`/`COV-01` re-points off the anno project-JSON shapes, and whether Phases 30–32 are renumbered or merely narrowed.

## Deferred Ideas

- ACME export and the real-ACME oracle — Phase 30, unchanged.
- Automatic annotation — Phase 26, held for v0.8.0.
- `packer-finding.mjs`'s `entropySource = "anno_get_binary_info"` — a stored fact about a past run; fate decided explicitly during the skill re-pointing.
