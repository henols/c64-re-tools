...
tags: [the external analyser, settings, package-gate, validation]
requires:
  - phase: 18-06
...
- Added the mismatched-system refusal check: it raises `AnnoProjectSettingsError`, spawns no child, and leaves the project byte-identical.
...
- `node --test anno-session.test.ts` under Node 22.22 passed with the live D18-35 scenarios run.
