...
This phase delivers **the enabler**: one `regenerator2000 --mcp-server-stdio`
child process stays alive across many `r2000_*` tool calls inside the
...
alignments (`r2000_read_region`, and `r2000_get_address_details`'s D-32
...
**In scope:**
- A new `src/mcp/vice/r2000-session.ts` owning the long-lived child.
- `src/mcp/vice/r2000-mcp-client.ts` **extended** (not replaced) with a
  long-lived session primitive beside the existing one-shot
  `withR2000Session()`.
- `runR2000Tool()` (`r2000-tools.ts`) rewired through the session, with
...
- `r2000_read_region` curated; `r2000_get_address_details` re-decided.
- `r2000-spawn-seam.test.ts` fixtures extended for the long-lived path.
...
- Routing the session through `vice-broker.mts` — the broker exists to cross
  the container/host boundary for `x64sc`; regenerator2000 is container-side
...
- **D18-01:** The session lives **in-proxy** — a new
  `src/mcp/vice/r2000-session.ts` holding module-level mutable state, built on
  an extended `r2000-mcp-client.ts`. A host-side broker route and a new
  ...
  later would change every `r2000_*` call path and require a new IPC surface
  and its own wedge-triage story.
- **D18-02:** **No third spawn site.** `r2000-mcp-client.ts` remains the sole
  place in this repo that spawns `--mcp-server-stdio` and parses a JSON-RPC
  frame. `r2000-session.ts` builds on a new export from it and never spawns
  itself. `r2000-spawn-seam.test.ts`'s enumerated site set is therefore
...
- **D18-03:** The session opens **lazily, on the first `r2000_*` call that
  needs a child.** No `r2000_session_open` tool. The session is an invisible
...
- **D18-06:** **The session is invisible to callers.** No
  `r2000_session_open`/`_close`/`_status` tools. Session health surfaces only
  through named errors on ordinary calls. Curated tool count moves 17 → **19**
  (`r2000_read_region` + `r2000_get_address_details`), not 20+. —
  **Reversibility:** reversible — adding a read-only status tool later is
  additive.
- **D18-07:** The one-shot `withR2000Session()` contract **stays** for CLI-verb
  callers (`r2000-cli.ts`, the enum generator, the memory-map renderer). The
  long-lived primitive is a new export beside it, not a replacement. Its
  header prose ("once per `withR2000Session()` call", "Never keep a child
...
- **D18-08:** **Persistence changes process lifetime, not the durability
  contract.** Every mutating `r2000_*` call still calls
  `r2000_save_project` inside the same session before that tool call resolves
  to its caller, identically to today. `runR2000Tool()`'s existing sequencing
  ...
  reserved for `r2000_save_project` invoked as the outer tool by name. —
...
- **D18-09:** SESS-03 is proven by a **planted-violation** test, and it is
  ...
  1. *Green path:* mutate via `runR2000Tool()`; the instant the call resolves,
...
- **D18-12:** **Dead mid-call → fail loud, never auto-retry.** Surface a
  named, distinguishable error (the existing `R2000ChildExitError` /
  `R2000SessionFailedError` classes). A mutating call killed in that window
...
- **D18-13:** **Wedge detection is passive plus the existing call timeout.** A
  ...
  `DEFAULT_R2000_CALL_TIMEOUT_MS` (30s) and turned into a named error that
...
- **D18-16:** A reader-writer lock (concurrent reads, exclusive writes) is
  **deferred, not rejected**. It would only pay off if regenerator2000's stdio
...
- **D18-18:** **The critical section spans the mutating call *and* its own
  internal `r2000_save_project`.** No second mutation can interleave between a
  change and its flush. Per-tool-call locking would leave exactly the window
  Phase 9's incident lived in. `r2000_batch_execute`'s entire batch — every
...
- **D18-19:** Criterion 4 is proven by a **planted-violation lost-update
  ...
  `r2000-spawn-seam.test.ts`'s `--vice` guard. A lock nobody has watched catch
...
- **D18-23:** For a child orphaned by a `SIGKILL`ed proxy (no exit hook runs):
  **measure the stdin-EOF exit behaviour against real regenerator2000 0.9.20
...
- **D18-24:** `r2000_read_region` is curated, with **both** views
  ...
  `r2000-tools.ts`'s own discipline, and it goes into
  `R2000_TOOL_DEFINITIONS` / `CURATED_R2000_TOOLS` / `READ_ONLY_R2000_TOOLS`.
  — **Reversibility:** reversible.
- **D18-25:** `r2000_read_region` carries a **documented range cap** with a
...
- **D18-26:** The **cursor trio is held** —
  `r2000_get_disassembly_cursor`, `r2000_jump_to_address`,
  `r2000_read_selected` are not curated in this phase. `r2000_read_region`
  ...
  discipline used at the v0.2.0 manifest cut and the v0.3.0 R2000 cut. —
  **Reversibility:** reversible.
- **D18-27:** **`r2000_get_address_details` is composed entirely
  client-side** from the four already-curated reads (`r2000_get_symbols`,
  `r2000_get_comments`, `r2000_get_blocks`,
  `r2000_get_cross_references`) and **never calls upstream's tool at all**.
  ...
  **Constraint already satisfied:** CLAUDE.md requires derived tools to be
  intercepted before `forwardToVice()`; the `r2000_*` family registers through
...
- **D18-29:** **It keeps the name `r2000_get_address_details`** — the name
...
- **D18-30:** **D-32 is superseded by a dated decision with a named reversal
  trigger**: an upstream fix to regenerator2000 issue #42 would let the
...
- **D18-31:** SURF-01's no-drift clause is met **inside the same plan** that
  curates the tools: update `r2000-tools.test.ts`'s count pin (17 → 19) and
  regenerate `docs/tool-support.md`, leaning on the byte-identity drift guard
  built at v0.2.0 plus `r2000-verb-coverage.test.ts`. No new drift test — the
...
- **D18-32:** Settings are forced **before spawn**, in the session-open path:
  ...
  **Reversibility:** reversible.
- **D18-33:** The forcing logic is **extracted from `r2000-project.ts`** into
  one shared "ensure settings" function that both the existing synthesiser and
  the session-open path call. Not a second copy in `r2000-session.ts` with
...
- **D18-36:** This phase is an **explicit reversal of D-17/D-18** and must run
  ...
  now-false header prose in `r2000-mcp-client.ts` is corrected in the **same**
...
- **Live-versus-stub verification:** default to **live** testing against the
  real installed `regenerator2000` 0.9.20 (`~/.cargo/bin/regenerator2000`) for
  ...
  harness in `r2000-mcp-client.test.ts` stays appropriate for protocol-shape
...
- **`withR2000Session()` retirement:** not retired (D18-07). CLI verbs have no
...
- The `r2000_read_region` cap value (D18-25).
- Whether `r2000-session.ts` or the extended `r2000-mcp-client.ts` owns the
...
- `CLAUDE.md` — the derived-tool interception constraint and its note that the
  `r2000_*` family satisfies it by construction (registered through
...
- `.planning/research/ARCHITECTURE.md` §0 ("What was verified live this
  session" — the 28-tool `tools/list`, `r2000_read_region`'s live schema, the
...
- `.planning/phases/11-annotation-store-enums-and-the-symbol-round-trip/11-CONTEXT.md`
  ...
  reversed here), D-32 (the `r2000_get_address_details` exclusion and upstream
  issue #42), D-33 (batch-whole refusal).
- `docs/phase9-regenerator2000-probe-findings.md` — the `degrade` verdict, the
...
- `src/mcp/vice/r2000-mcp-client.ts` — the sole spawn/parse seam; its header
  is the D-17 statement being reversed and must be corrected in the same
  change. `withR2000Session()`, `saveAndVerify()`, and the named error classes
  (`R2000ChildExitError`, `R2000SessionFailedError`,
  `R2000SaveNotPersistedError`) all live here.
- `src/mcp/vice/r2000-tools.ts` — `R2000_TOOL_DEFINITIONS` (17 entries),
  `CURATED_R2000_TOOLS`, `assertCuratedTool()`, `READ_ONLY_R2000_TOOLS` (6
  entries), `runR2000Tool()` and its save-per-mutation sequencing, and the
  D-32 refusal message.
- `src/mcp/vice/r2000-spawn-seam.test.ts` — the enumerated spawn-site guard.
- `src/mcp/vice/r2000-launch.ts` — `buildMcpServerStdioArgs()`,
  `assertNoViceFlag()`, the fixed per-verb argv builders.
- `src/mcp/vice/r2000-project.ts` — the `.regen2000proj` synthesiser whose
...
- `docs/tool-support.md` and its byte-identity drift guard;
  `r2000-tools.test.ts`'s count pin; `r2000-verb-coverage.test.ts`.
...
### Reusable Assets
- **`r2000-mcp-client.ts`'s named error classes** —
  `R2000ChildExitError`, `R2000SessionFailedError`,
  `R2000SaveNotPersistedError` already exist and are exactly what D18-12's
...
- **`resolveStorePath()`** (`r2000-tools.ts`) — already resolves the project
  path per call, so D18-04's "different project path" comparison has an
  existing single source for the key.
- **`assertNoViceFlag()` + the fixed per-verb argv builders**
  (`r2000-launch.ts`) — the `--vice` invariant's dual enforcement. The
...
- **The existing stub-server harness in `r2000-mcp-client.test.ts`** — reusable
...
- **`docs/tool-support.md`'s byte-identity drift guard** (built v0.2.0) and
  `r2000-verb-coverage.test.ts` — D18-31 leans on both rather than adding a
  new test.

### Established Patterns
- **One authoritative place per concern.** `r2000-mcp-client.ts` owns
  spawn/protocol; `r2000-tools.ts` owns tool names and save timing;
  `r2000-session.ts` will own lifecycle only. Research's named anti-pattern is
...
- **Planted-violation non-vacuity proof.** `r2000-spawn-seam.test.ts` and the
...
- **Dynamic import of the client from the tools module** — importing
  `R2000_TOOL_DEFINITIONS` at `vice-proxy.ts` module scope costs no child
...
- **Named constants over magic numbers**, with the env-var override
  convention (`DEFAULT_R2000_CALL_TIMEOUT_MS` is the local example).

### Integration Points
- `runR2000Tool()` is the single call site rewired from the one-shot path to
  the session — its `assertCuratedTool()` → `resolveStorePath()` → dynamic
  import → save-per-mutation ordering is preserved.
- `r2000-session.ts` is a new sibling module, never inline in
  `vice-proxy.ts` (Rule A4, and every `r2000-*.ts` module's existing shape).
...
- No host/container path translation applies anywhere in this phase —
  regenerator2000 is container-side (D-R4, Rule A16), asserted structurally by
...
- **Reader-writer lock for the session** (concurrent reads, exclusive writes)
  ...
  regenerator2000's stdio handler multiplexes requests. Not rejected.
- **A caller-visible `r2000_session_status` tool** — declined in this phase
...
- **The cursor trio** (`r2000_get_disassembly_cursor`,
  `r2000_jump_to_address`, `r2000_read_selected`) — D18-26. Phase 19's
...
- **A startup sweep for stale `regenerator2000 --mcp-server-stdio`
  processes** — becomes a planned task only if D18-23's stdin-EOF measurement
  fails.
- **An upstream patch for regenerator2000 issue #42** — the project's standing
...
- **`r2000_export_source` / `r2000_hazard_report` as MCP tools** — research
