...
|----------|-------------|------------|-------------------|
| New encoders are byte-for-byte accepted by a real VICE binary | DIRECT-01..09 | Body layouts are grounded in the normative docs and the repo's own tested `probe-binmon.mjs` builders. A later live probe (`13-PROBE-RESULTS.md`) has since exercised the four behavioural details against a real (fork 3.10) binary: `-remotemonitoraddress`'s spelling (A1) and `ADVANCE_INSTRUCTIONS` step-over semantics (A2) are now CONFIRMED, no longer `[ASSUMED]`; `JOYPORT_SET`'s bit layout (A3) remains `[ASSUMED]` (INCONCLUSIVE — no CIA1 delta observed on any single-bit write, neither confirming nor refuting the mapping); `AUTOSTART`'s `fileIndex` with the run flag clear (A5) remains `[ASSUMED]` (CONTRADICTED — the real binary still resets and loads a program despite `runAfter=false`, pending correction). A4 (the `stop:false` rate limiter's auto-disable deferral timing) is still open and untouched — deliberately not probed (hot-address CPU-loop stall hazard) | Follow `.planning/todos/pending/2026-08-14-probe-phase3-assumed-wire-details.md`'s remaining A4 acceptance step. **Not a Phase 3 blocker** |
