...
## Validation Architecture (Focus Item 9 — MANDATORY)
...
|---|-------|---------|---------------|
| A1 | `-remotemonitor`'s address-option flag is spelled `-remotemonitoraddress`, symmetric to `-binarymonitor`/`-binarymonitoraddress` | Focus Item 8 | Broker launch fails to bind the text-monitor port at the intended address; caught immediately by a launch smoke-test, low real-world risk since Phase 3 dials nothing on this port. **Post-probe status:** CONFIRMED — `13-PROBE-RESULTS.md` §A1 live-probed a real, accepting text-monitor listener on fork VICE 3.10, corroborated independently by `ss -ltnp`; genuine stock 3.9 was not independently probed this run, but the flag spelling is a symmetrical CLI pair and is not version-sensitive. |
| A2 | `ADVANCE_INSTRUCTIONS`'s `SO` (step-over) byte behaves identically to the fork's `stepOver` semantic (skip a `JSR`'s subroutine as one step) | Family C table (Focus Item 1) | `vice_execution_step`'s `stepOver: true` could silently behave like a plain single-instruction step instead of skipping the subroutine; would surface as a wrong PC after stepping over a `JSR` in manual testing. **Post-probe status:** CONFIRMED — `13-PROBE-RESULTS.md` §A2 live-probed a real `JSR` on fork VICE 3.10; the post-step PC landed at JSR+3, reproduced identically across two independent live sessions. |
| A3 | `JOYPORT_SET`'s `value` bitfield uses VICE's conventional joystick bit layout (bit0=up, bit1=down, bit2=left, bit3=right, bit4=fire) | Family D table (Focus Item 1) | Wrong bits would move the joystick in the wrong direction or fail to register fire; needs a probe-extension task before shipping, not silently trusted. **Post-probe status:** INCONCLUSIVE — `13-PROBE-RESULTS.md` §A3 drove all five single-bit positions individually against fork VICE 3.10 and observed zero CIA1 port-byte delta at either `$DC00` or `$DC01` in two live sessions; this is absence of signal, not evidence the mapping is wrong or right, and the `[ASSUMED]` label stays on. |
...
| A5 | `AUTOSTART`'s `fileIndex` field, when repurposed for D-14's disk-attach approximation, is meaningful for a `.d64`/`.g64` image the same way it is for a multi-program disk's program selection, and defaulting it to 0 is a safe "just attach" behaviour | Family D table (Focus Item 1) | If VICE's `AUTOSTART` handler treats `fileIndex` differently when `runAfter` is false, the disk-attach approximation could behave unexpectedly (e.g. attempting to locate and partially load a specific program even with run suppressed). **Post-probe status:** CONTRADICTED — `13-PROBE-RESULTS.md` §A5 live-probed both indices against fork VICE 3.10; `AUTOSTART` with `runAfter=false` still performed a full machine reset and loaded a program regardless of `fileIndex`. The `[ASSUMED]` label stays on pending correction; a separate advertised-tool-contract finding (`vice_disk_attach`'s D-14 approximation string) is handed to plan 13-05, not redesigned in Phase 13 plan 04. |
