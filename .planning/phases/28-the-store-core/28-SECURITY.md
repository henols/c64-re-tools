...
| T-28-censuszero | 28-03 | Tampering (silent misinformation) | `block-class.ts:130-134`'s comparisons and `r2000-coverage.… | high | mitigate | Dual-vocabulary acceptance in the boundary so no live producer's blocks are reclassified; a derived TOTAL cross-check over all twelve store members w… | closed |
...
| T-28-vacuousdisjoint | 28-03 | Repudiation | `r2000-coverage.test.ts:688-697`'s zero-overlap disjointnes… | medium | mitigate | `PRODUCTION_BLOCK_SPELLINGS` becomes the derived union of both accepted vocabularies, with a no-duplicates assertion and a membership assertion for a… | closed |
...
| T-28-gradeswallow | 28-05 | Tampering (silent exemption) | `parseConfidencePrefix()`'s throw path | medium | mitigate | The store catches `R2000ConfidenceGradeError` and rethrows `AnnoCommentGradeError extends ViceError` with the original message preserved verbatim; it… | closed |
