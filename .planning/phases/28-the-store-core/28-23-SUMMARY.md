...
- **For Phase 29:** `SetDataTypeResult.reinterpretedSplitTables` is a new field on a contract with **zero production callers today** (every reference outside `anno-store.ts` is in a test file). It is revertible in-tree by `git revert` until Phase 29 publishes it on the MCP surface — at which point it becomes a published contract and the reversibility rating hardens. Phase 29 should surface it on `r2000_set_data_type`'s result, because the proxy validates nothing and an agent-driven caller is exactly the caller that most needs to be told what an edit cost.
