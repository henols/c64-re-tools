...
- **`requirements.mark-complete` was deliberately NOT run**, and this is a decision rather than an omission. `requirements.ready-ids` reports `STORE-04` and `STORE-05` ready (`STORE-07` blocked, because 28-15 declares it too). But `STATE.md` and `REQUIREMENTS.md` both record that **all six STORE requirements are held out of `Complete` until this round re-verifies** — they read `Gaps Found` on purpose, WR-12 is still open against the same phase, and 28-15 has not run. Flipping two of them to `Complete` mid-round would erase a recorded phase-level decision and make `REQUIREMENTS.md` claim a completion the round's own verification has not yet granted. The verifier owns that transition.
