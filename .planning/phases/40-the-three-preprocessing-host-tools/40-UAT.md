...
- gap_id: G-40-1
  ...
  missing:
    ...
    - "Docs to correct: ghidra-project.mts:72-94; .gitignore:19-24 / :27-36 (delete the false tracked-tooling claim) / :137-144; CLAUDE.md's D-33 exception bullet; .planning/STATE.md:1111; 40-01-SUMMARY.md (:12,:60,:61,:99,:107,:159,:167, D5 rationale); this UAT's own measured_this_session \"does NOT canonicalise\" wording (it DOES absolutize) -- already-satisfied (2026-09-08, plan 40-10): this list item's own sub-claim was stale even at the time it was written -- the measured_this_session block above ALREADY states the absolutize-but-not-resolve-symlinks framing correctly (\"does NOT resolve symlinks but it DOES absolutize\"), refined during the same debug session this gap's diagnosis came from, before this `missing` list was written; the completed consolidation todo addendum; docs/phase34-host-tool-seam-decisions.md A-07 (\"the ONLY placement\"); 34-03-PLAN.md:89-94 / 34-03-SUMMARY.md / 34-VERIFICATION.md -- already-satisfied for 34-VERIFICATION.md specifically (2026-09-08, plan 40-10): re-checked with `grep -an -i ghidra` against the real file -- its eight Ghidra mentions carry no claim that the location is unavoidable; the one closest candidate (`:40`, \"the no-dot project-path refusal enforced in code\") is still TRUE and needed no change; repo-root.ts:185-207; host-tool.mts:2932. Already correct and usable as the corrective source: .planning/notes/ghidra-dot-path-check-semantics.md and REQUIREMENTS.md PREP-05."
