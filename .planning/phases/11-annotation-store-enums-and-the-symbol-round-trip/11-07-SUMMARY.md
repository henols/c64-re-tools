...
plan: 07
subsystem: infra
tags: [the external analyser, annotation-store, evidence, criterion-1, d-26, two-session-proof]

# Dependency graph
requires:
  - phase: 11-05
    provides: "anno-tools.ts's runAnnoTool()/CURATED_ANNO_TOOLS/resolveStorePath() -- the ONE curated-tool seam this plan's session A drove directly, and the saveAndVerify()-vs-plain-save split that makes cross-call persistence possible under D-17's per-call lifecycle"
provides:
  ...
  - "evidence/criterion1/recon-subject.regen2000proj: the committed annotation store, 7 user labels / 6 D-25-graded line comments / 3 classified blocks / 1 scope, written entirely through the curated anno_* tools"
  ...
  - "absorbed-answer-key.test.ts: the seal-drift and leak guards that keep the sealed answer honest"
affects: [11-09]
...
tech-stack:
  ...
  patterns:
    ...
    - "Cross-process (not just cross-call) persistence proof: the fresh-session re-read ran as a second, independently-started node process, launched only after the first had fully exited, so the proof is against what anno_save_project actually wrote to disk -- not against anything a still-running process might be holding in memory."
...
**A purpose-made ACME fixture is bootstrapped, annotated end to end through the 17 curated `anno_*` tools (one 17-call `anno_batch_execute`), proven to persist across a genuinely separate process, and sealed behind a four-part question whose answer requires three stored human judgements no byte-level analysis can recover.**
...
- **Task 2 — session A.** Bootstrapped the `.prg` into `recon-subject.regen2000proj` via `vice-mcp anno bootstrap`. Annotated it with one `anno_batch_execute` call (17 inner operations): 7 user labels, 3 `anno_set_data_type` classifications (byte_table → `byte`, addr_table → `address`, the ambiguous region → `byte`), one `anno_add_scope` over the IRQ handler, and 6 D-25 confidence-prefixed line comments (`[confirmed-code]` ×2, `[confirmed-data]`, `[probable-data]` ×2, `[unknown]`). Proved persistence with a **second, independently-started `node` process**, launched only after the first had fully exited, re-reading `anno_get_symbols`/`anno_get_comments`/`anno_get_blocks` — all annotations survived intact. Every call was driven directly by this executor agent through `runAnnoTool()`; no nested `claude -p` invocation appears anywhere.
- **Task 3 — the question and its seal.** `QUESTION.md` asks for a label name, a confidence grade, a block type (three stored human judgements) plus a cross-reference count (one part that exercises criterion 2's query layer but is explicitly named as not sufficient alone). Permitted/forbidden inputs are named explicitly, including this plan's own PLAN.md/SUMMARY.md/commits. `ANSWER.md` seals the canonical line `label=border_bump_up confidence=probable-data blocktype=byte xrefcount=2` behind a documented marker fence; `ANSWER.sha256` holds its sha256. `absorbed-answer-key.test.ts` recomputes that hash from `ANSWER.md` itself and asserts it matches, and asserts `QUESTION.md` leaks neither the full line nor the label's distinctive value nor any field's compound `key=value` form.
...
- `.claude/mcp/vice/absorbed-answer-key.test.ts` — the seal-drift and leak guard tests
...
```
$ node --test absorbed-answer-key.test.ts
...
...
None. `the external analyser 0.9.20` (`~/.cargo/bin/analyser`) and ACME 0.97 (`~/.local/bin/acme`) were both already installed on this host from prior phase work.
...
- `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` → 21/21 pass (Task 2's exact verify command; unchanged from plan 11-05, re-run here to confirm nothing regressed).
...
- `node --test absorbed-answer-key.test.ts` → 5/5 pass (Task 3's exact verify command).
...
- FOUND commit: 6d76ee5 (feat(11-07): session A -- annotate recon-subject via the curated anno_* tools)
