...
inputs list. It exists so `.claude/mcp/vice/absorbed-answer-key.test.ts` can recompute
...
`absorbed-answer-key.test.ts` extracts it by finding the text strictly between those two marker lines
...
- **`label=border_bump_up`** — `anno_get_symbols({ project, kind: "user" })` returns an entry
  `{ "address": 2124, "name": "border_bump_up", ... }`. This is the label session A's
  `anno_batch_execute` call gave to address `2124` (`$084C`, the routine that increments
  `$D020`'s border colour). Produced by: `anno_set_label_name` (write), read back via
  `anno_get_symbols` (query).
- **`confidence=probable-data`** — `anno_get_comments({ project })` returns an entry
  ...
  Produced by: `anno_set_comment` (write, carrying the D-25 confidence-prefix convention), read
  back via `anno_get_comments` (query).
- **`blocktype=byte`** — `anno_get_blocks({ project })` returns an entry
  `{ "start_address": 2156, "end_address": 2163, "type": "Byte" }`, lowercased to `byte`. Produced
  by: `anno_set_data_type({ start_address: 2156, end_address: 2163, data_type: "byte" })` (write),
  read back via `anno_get_blocks` (query).
- **`xrefcount=2`** — `anno_get_cross_references({ project, address: 2128 })` returns the array
  ...
  block classification (write), read via `anno_get_cross_references` (query) — the one part of
