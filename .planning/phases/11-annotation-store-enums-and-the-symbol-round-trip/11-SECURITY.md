...
feasible, ran the cited tests live (including gated tests against a real `the external analyser 0.9.20`
...
|----------|-------------|----------------|
| LLM-authored `tools/call` payload → `runAnnoTool()` | Arbitrary tool names, nested batch payloads, and file paths chosen by a model | Tool name, args, project path |
| a caller-supplied `.regen2000proj` path → a spawned child's argv | Path crosses into a spawned `the external analyser` process | Filesystem path |
...
| this repo's code → the `the external analyser` child process | Untrusted responses, exit codes, stderr | JSON-RPC over stdio |
...
|-----------|----------|-----------|-------------|------------|--------|
| T-11-VACUOUS | Repudiation | `anno-launch.test.ts` `stripCommentLines()` | mitigate | Substring-based block-close guard; planted-violation test | closed |
| T-11-FALSESUCCESS-ACME | Tampering | `anno-verify.ts` `acmeVerdict()` | mitigate | Unanimity over ACME lines; refuses on skip/fail/multi-ok | closed |
| T-11-GATE-DRIFT | Repudiation | `anno-test-gate.ts` | mitigate | Single seam, hard-FAIL under `ANNO_BIN=/nonexistent` | closed |
| T-11-D64-TRUNC | Tampering | `anno-d64.ts` `sectorSlice()` | mitigate | Bounds every read against `image.length`, throws naming sector | closed |
| T-11-D64-NAME | Spoofing | `anno-d64.ts` `petsciiName()` | mitigate | Strips `0x00`/`0xa0` padding uniformly | closed |
| T-11-RAW-GUESS | Tampering | `anno-cli.ts` flat-capture dispatch | mitigate | Extension-dispatched before length heuristic | closed |
...
| T-11-FALSESUCCESS | Tampering | `anno-mcp-client.ts` `saveAndVerify()` | mitigate | Independent disk-hash re-read, never trusts child text | closed |
| T-11-VICE | Denial of Service | every spawn in `anno-mcp-client.ts` | mitigate | `assertNoViceFlag()` on all fixed-builder argv | closed |
| T-11-HANG | Denial of Service | unanswered `tools/call` | mitigate | `DEFAULT_ANNO_CALL_TIMEOUT_MS`, bounded per-call | closed |
...
| T-11-BATCH | Elevation of Privilege | `anno_batch_execute` `calls[]` | mitigate | `assertCuratedBatch()` recurses, refuses whole batch, zero-spawn | closed* |
| T-11-UNCURATED | Elevation of Privilege | outer tool name | mitigate | `assertCuratedTool()` is `runAnnoTool()`'s first statement | closed |
| T-11-PATH-ESCAPE | Information Disclosure | `resolveStorePath()` | mitigate | **Closed 2026-08-21 (quick 260821-a86):** `resolveStorePath()`'s ENOENT branch now walks up to the deepest EXISTING ancestor's realpath, rebuilds the candidate from the literal remaining segments, and lstat-guards each remaining segment against being an unresolved (e.g. dangling) symlink — closing the not-yet-existing-leaf bypass while preserving the create path. Pinned by `anno-tools.test.ts`'s two planted-directory-symlink regression tests (shallow + one level deeper) plus a create-path test for an absent intermediate directory; all pre-existing `resolveStorePath` cases still pass. | closed |
| T-11-PATH-XLATE (11-05) | Tampering | host/container path translation | mitigate | `runAnnoTool()` never reaches `forwardToVice()`; `stock-dispatch.test.ts` 127/127 live pass | closed |
| T-11-FALSESUCCESS (tools) | Tampering | `anno_save_project` on tool surface | mitigate | Routed through `saveAndVerify()` | closed |
| T-11-D32 | Spoofing | `anno_get_address_details` | mitigate | Excluded by name + coverage-gate non-vacuity check | closed |
| T-11-PROSE | Repudiation | `anno_*` names in skill prose | mitigate | `check-skill-tool-coverage.mjs` live run: OK | closed |
| T-11-ENUM-NAME | Tampering | `anno-enum-gen.ts` → `anno_create_project_enum` | mitigate | `assertLegalAcmeIdentifier()` on enum+variant names before any spawn; live 23/23 pass incl. real-ACME reassembly | closed |
| T-11-REGBITS-PROSE | Tampering | `anno-regbits-gen.ts` identifier derivation | mitigate | Unmappable `desc` with no OVERRIDES throws naming address | closed |
...
| T-11-GEN-DRIFT | Tampering | `anno-regbits.json` | mitigate | Banner records `memmap.json` sha256; 13/13 live pass | closed |
...
| T-11-LBL-PARSER-DUP | Tampering | third `al C:` parser | mitigate | Zero label-regex occurrences in `anno-symbols.ts` (verified) | closed** |
| T-11-NAME-INJECT (11-08) | Tampering | discovered name → store → generated ACME/Markdown | mitigate | **Closed 2026-08-21 (quick 260821-a86):** the identifier policy moved to a single seam, `anno-acme-ident.ts`'s `assertLegalAcmeIdentifier()` (re-exported from `anno-enum-gen.ts`), now applied pre-spawn on `anno_set_label_name` (outer dispatch AND batch-inner call, `anno-tools.ts`'s `assertLegalLabelArg()`) and in `importLabels()` (`anno-symbols.ts`, before `buildImportLblArgs()`) — REJECT, never sanitize. Pinned by `anno-tools.test.ts` (six illegal-name shapes + batch smuggling) and `anno-symbol-roundtrip.test.ts` (illegal `.lbl` name naming its line). | closed |
...
| T-11-SECOND-STORE | Tampering | address-keyed sidecar JSON | mitigate | Rejected in header; grades live in anno comments (verified) | closed |
...
| T-11-PROSE-FAKE-TOOL | Spoofing | `anno_*` names in skill prose | mitigate | `check-skill-tool-coverage.mjs` live run: OK | closed |
...
not a grep gate). Re-verified independently in this audit (`grep -n "al\\s\|RegExp" anno-symbols.ts`
...
(IN-02) found it has **no production caller** — no `anno_*` tool, CLI verb, or skill invokes it.
...
**File:** `.claude/mcp/vice/anno-tools.ts:633-678`
...
- When the final path component does not exist yet (the documented, deliberately-tolerated case —
  `anno_save_project` creates fresh projects), `realpathSync(resolved)` throws `ENOENT` for the
...
```
A file write through this path (by the spawned `the external analyser` child) would follow the symlink
...
**Resolution (2026-08-21, quick 260821-a86):** `resolveStorePath()` (`anno-tools.ts`) was
...
`anno-tools.test.ts`'s two planted-directory-symlink cases (the PoC shape above, and one level
deeper) now throw naming the resolved outside target; a new create-path case proves
`anno_save_project`'s fresh-file flow still works when an intermediate directory is also absent;
...
**Files:** `.claude/mcp/vice/anno-symbols.ts`, `.claude/mcp/vice/anno-tools.ts`, `.claude/mcp/vice/anno-enum-gen.ts`

Independently re-checked per this audit's instructions, not merely relayed from 11-08-SUMMARY.md:

1. `grep -n "assertLegalAcmeIdentifier" anno-enum-gen.ts` → all call sites are on enum names
   (`createOrUpdateEnum`) or variant names (`sanitizeVariantMap`). **Zero** call sites touch a
   label name.
2. `anno_set_label_name`'s schema (`anno-tools.ts`, `ANNO_TOOL_DEFINITIONS`) type-checks `name`
   as `string` only — no pattern/format constraint.
3. `stock-symbols.ts`'s `VICE_LABEL_LINE_RE` (used by `parseViceLabelFile()`, which
   `anno-symbols.ts` reuses for `importLabels()`) accepts any `\S+` after the dot — far looser
   than a legal ACME identifier.
4. **Export-side control checked (per this audit's explicit instruction 4):** none exists. Neither
   `exportLabels()` (`anno-symbols.ts`) nor `--export_asm`'s own generation path applies
...
5. **Blast radius is wider than 11-08-SUMMARY.md recorded.** Independent code review (WR-04, this
   audit cross-checked `anno-memmap-render.ts:384-431` directly) found `renderMemoryMap()` also
   interpolates `sym.name` and comment `evidence` text into Markdown table cells with **no
   escaping** of `|` or embedded newlines — and `anno_set_comment`'s own schema documents
...
either (a) an LLM-chosen tool argument to `anno_set_label_name`, or (b) a `.lbl` line from a
...
`anno-acme-ident.ts`'s `assertLegalAcmeIdentifier()` (re-exported from `anno-enum-gen.ts` for its
existing consumers), and wired into both entry routes named in finding 4 above: `anno-tools.ts`'s
`assertLegalLabelArg()` runs pre-spawn from both `assertCuratedTool()` (the outer
`anno_set_label_name` dispatch) and `assertCuratedBatch()` (the batch-inner call, refusing the
whole batch and naming `calls[i]`); `anno-symbols.ts`'s `importLabels()` validates every name in
...
`anno-memmap-render.ts`'s new `escapeMarkdownCell()` escapes `|` and collapses `\r\n`/`\n`/`\r` to
`<br>` on every store-derived Markdown interpolation (comment evidence, symbol names), and
`RENDERER_VERSION` was bumped `"1"` → `"2"`. Regression tests: `anno-tools.test.ts` (six illegal
ACME-identifier shapes refused pre-spawn, one legal name accepted, one batch-smuggling case),
`anno-symbol-roundtrip.test.ts` (an illegal `.lbl` name refused naming its line), and
`anno-memmap-render.test.ts` (unit cases for `escapeMarkdownCell()` plus a gated case proving
...
|------|------|-------------|
| WR-02: refusal-shape inconsistency | `.claude/mcp/vice/anno-tools.ts:735-737` (`runAnnoTool`) | `assertCuratedTool()`/`resolveStorePath()` run before the function's own `try` block, so a refused call **rejects** the returned promise instead of resolving with this codebase's usual `{content, isError:true}` shape. If the MCP SDK's wrapper does not catch a rejected `execute()`, the call may hang unresolved from the calling agent's view rather than failing cleanly (confirmed structurally; not exercised against a live MCP client in this audit). |
| WR-03: unbounded batch-recursion depth | `.claude/mcp/vice/anno-tools.ts:544-578` (`assertCuratedBatch`) | Recursion into nested `anno_batch_execute` payloads has no depth ceiling — a payload nesting a few hundred `anno_batch_execute` layers deep can throw `RangeError: Maximum call stack size exceeded` instead of a named refusal, compounding with WR-02's escape from `runAnnoTool()`'s own `try/catch`. |
| WR-05: no `"error"` listener on the external analyser child post-spawn | `.claude/mcp/vice/anno-mcp-client.ts:489-516` (`waitForSpawn`) | The one-shot `"error"` listener is removed on successful spawn and never replaced. A post-spawn `"error"` (e.g. a failed `kill()`) becomes an uncaught exception on an EventEmitter with zero listeners; the global handler keeps the process alive but the specific `withAnnoSession()` call never resolves/rejects cleanly. |
...
`resolveStorePath()` (`anno-tools.ts:698-823`) in full. Ran `anno-tools.test.ts` live (27/27
...
Read `anno-acme-ident.ts` in full (confirmed it imports nothing from this repo, avoiding the
`anno-enum-gen.ts` ⇄ `anno-tools.ts` cycle the commit message describes) and confirmed
`anno-enum-gen.ts` re-exports rather than duplicates it. Confirmed `assertLegalLabelArg()` is
called from both `assertCuratedTool()` (outer dispatch, `anno-tools.ts:667-669`) and
`assertCuratedBatch()` (batch-inner, `:634-636`), and that `assertCuratedBatch()` recurses into a
nested `anno_batch_execute` (`:637-639`). Confirmed `importLabels()` (`anno-symbols.ts:262-280`)
validates every parsed name before `buildImportLblArgs()` is called. Searched for a third entry
route: walked every one of the 17 `CURATED_ANNO_TOOLS` schemas for a free-text identifier field —
only `anno_set_label_name`'s `name` qualifies; `anno_add_scope` takes no name; `anno_create_project_enum`/
`anno_update_project_enum`'s `name`/`new_name`/variant keys are already covered by the pre-existing,
separately-closed T-11-ENUM-NAME check. Confirmed `anno-cli.ts`'s `import-lbl` subcommand calls the
same `importLabels()` (no hand-rolled CLI-side import path). Ran `anno-tools.test.ts` (27/27) and
`anno-symbol-roundtrip.test.ts` (8/8) live. Beyond the committed tests, ran an independent PoC
...
**three-level nested batch smuggle** (`anno_batch_execute` → `anno_batch_execute` →
`anno_batch_execute` → `anno_set_label_name` with an illegal name) — refused, no depth limit
defeated the check; and a legal name (`init_screen`) both direct and inside a batch — accepted, no
false positive. No bypass found.

**WR-04 (Markdown-cell escaping) — CLOSED, confirmed.** Because `anno-memmap-render.ts` contains
...
not the finding). Ran `anno-memmap-render.test.ts` live (18/18 pass, including the gated
...
this repo's build gate) passes clean against the merged state. `anno-acme-ident.ts` is present in
...
|---------|--------|-------|
| T-11-PATH-ESCAPE (WR-01) | Fix | `resolveStorePath()` resolves the **parent** directory's realpath, joins the literal basename, then contain-checks — closing the not-yet-existing-leaf case while preserving `anno_save_project`'s create path. Regression test plants an intermediate directory symlink pointing outside the workspace and asserts a named refusal. |
| T-11-NAME-INJECT (11-08 / 11-11) | Fix | A label-name identifier policy applied on **both** entry routes (`anno_set_label_name`, `importLabels()`), plus WR-04's Markdown-cell escaping in `renderMemoryMap()`. |
...
comment `evidence` text legitimately contains `|` and newlines (`anno_set_comment`'s own schema
...
1. **Fix:** implement WR-01's parent-realpath fix for `resolveStorePath()`, and decide + implement
   a label-name identifier policy (reject/sanitize/quote) for `anno_set_label_name` and
