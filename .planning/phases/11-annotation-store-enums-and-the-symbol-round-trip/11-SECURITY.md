...
| T-11-PATH-ESCAPE | Information Disclosure | `resolveStorePath()` | mitigate | **See finding below — PoC-confirmed bypass** | **open** |
...
| T-11-NAME-INJECT (11-08) | Tampering | discovered name → store → generated ACME/Markdown | mitigate | **`assertLegalAcmeIdentifier()` never called on a label name — confirmed absent on both entry routes** | **open** |
...
| T-11-NAME-INJECT (11-11) | Tampering | live-discovered name entering store | mitigate | Same root cause as 11-08's finding; re-flagged not re-covered | **open (duplicate of above)** |
...
*No other risks are currently accepted. T-11-PATH-ESCAPE and T-11-NAME-INJECT remain OPEN, not
accepted — see Open Findings above for the recommended path to close or formally accept each.*
...
- [ ] `threats_open: 0` confirmed — **currently 3 open** (2 distinct root causes: T-11-PATH-ESCAPE's
      symlink-of-nonexistent-leaf gap, T-11-NAME-INJECT's unvalidated label name on both entry
      routes, widened by WR-04's Markdown-render leg)
- [ ] `status: verified` — **not set; status remains `draft` pending a fix or a formal accept
      decision for the two open findings**

**Approval:** pending — remediation decision recorded below (FIX BOTH)
...
`threats_open` stays at **3** and `status` stays **draft** until the fixes land; re-run
`/gsd-secure-phase 11` afterwards to close them.
