...
This phase adds the `anno_*` static-analysis tool surface (new MCP-client, curated
...
invariants hold up under direct inspection: `anno_*` tools are registered via
...
rules also hold: `anno-mcp-client.ts` is the only spawner of `--mcp-server-stdio`,
`anno-launch.ts` the only spawner of the other verbs, `anno-tools.ts` the only
...
**File:** `.claude/mcp/vice/anno-tools.ts:633-678` (`resolveStorePath`)
...
exist yet — the documented, deliberately-tolerated case, since `anno_save_project`
...
filesystem operation performed by the spawned `the external analyser` child follows the
...
session, or a checked-in symlink). An LLM later calls `anno_save_project` (or any
mutating `anno_*` tool, since every one of them auto-saves) with
...
### WR-02: `runAnnoTool()`'s allow-list and path guards throw instead of returning the codebase's own graceful refusal shape

**File:** `.claude/mcp/vice/anno-tools.ts:735-769` (`runAnnoTool`)
...
`runAnnoTool()` itself) returns a `CallToolResult`-shaped `{content, isError:true}` value
from an `async` function that resolves, not one that rejects. But `runAnnoTool()` calls
`assertCuratedTool(name, args)` and `resolveStorePath(...)` **before** its own `try` block
(lines 736-737), so a smuggled uncurated name inside `anno_batch_execute` (D-33's own
named attack surface) or a path-escape attempt makes the function's returned promise
*reject* with `AnnoUncuratedToolError`/`AnnoStorePathError`, rather than resolve with
`errText(...)`. This is confirmed by the project's own test
(`anno-tools.test.ts:234-274`, `runAnnoTool refuses a smuggled batch WHOLE...`), which
asserts via `assert.rejects(...)`, not by inspecting a resolved `isError:true` result.
**Failure scenario:** An LLM calls `anno_batch_execute` with one legitimate inner call and
one smuggled `anno_get_address_details` (or any other uncurated name). The refusal still
...
catch `AnnoUncuratedToolError | AnnoStorePathError` explicitly and return `errText(...)`
for both, exactly like every other refusal path.

### WR-03: `assertCuratedBatch()` recurses into nested `anno_batch_execute` payloads with no depth limit

**File:** `.claude/mcp/vice/anno-tools.ts:544-578` (`assertCuratedBatch`)
**Issue:** When a batch's inner call name is `anno_batch_execute`, the function recurses
...
unbounded.
**Failure scenario:** A caller submits `anno_batch_execute` with a `calls` array containing
a nested `anno_batch_execute` whose own `calls` array nests another, repeated to a depth in
...
of a clean, named refusal — and per WR-02 above, that exception also escapes `runAnnoTool()`'s
...
**Fix:** Track recursion depth explicitly and throw a named `AnnoUncuratedToolError` (e.g.
...
**File:** `.claude/mcp/vice/anno-memmap-render.ts:384-431` (the block/routines table rows)
...
of `|` (a Markdown table cell delimiter) or embedded newlines. `anno_set_comment`'s own
...
were it wired up — see IN-02) sets a multi-line `anno_set_comment` at an address inside a
...
### WR-05: No `"error"` listener remains on the external analyser child after a successful spawn

**File:** `.claude/mcp/vice/anno-mcp-client.ts:489-516` (`waitForSpawn`), used by
`withAnnoSession` at line 334
...
"stays alive," per that file's own documented policy), but the specific `withAnnoSession()`
...
**File:** `.claude/mcp/vice/anno-enum-gen.ts:565-571`
...
**File:** `.claude/mcp/vice/anno-symbols.ts:301-317`
...
file — no `anno_*` tool, CLI verb, or skill playbook calls it. The actual live-discovery
walkthrough recorded in `11-11-SUMMARY.md` drives the equivalent sequence by hand
(`anno_set_label_name` then a manual `export-lbl`/`vice_symbols_load`), not through this
function.
