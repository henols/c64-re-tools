...
The external analyser's **project state becomes this project's queryable annotation
...
Requirements in scope: `ANNO-10`, `ANNO-11`, `ANNO-13`, `ANNO-14`,
`ANNO-15` (5).

**Not in scope:** `.vsf` as a bootstrap input (D-34), non-ACME export formats,
two-project-limit detection (`ANNO-04`, permanently folded), HTML export with
xrefs (`ANNO-07`, cut).
...
The external analyser **0.9.20** during this discussion, on this host. Nothing in this
section is reasoned-from-source-only unless it says so.

### Store access and lifecycle

- **D-16 (user): the store is reached by proxying `anno_*` tools through our own
  MCP server**, which drives `analyser --mcp-server-stdio` as a child and
  ...
  - **Measured:** `--mcp-server-stdio` speaks MCP `2024-11-05` over stdio,
    answers `initialize` with `serverInfo.name = external-analyser-core-mcp`
  ...
  - Why not a second declared MCP server: MCP server config in `.mcp.json` /
    ...
    and would stop `anno-launch.ts` being the only spawn path, which is what
    makes **D-07's `--vice` guard** load-bearing.
  - Why not CLI-only: criterion 2's xrefs and disassembly search would mean
    re-implementing anno's analysis engine in Node — the one thing the
...
- **D-17 (user): per-call lifecycle — spawn, load, mutate, `anno_save_project`,
  ...
  - **Known cost, accepted:** `anno_undo` / `anno_redo` are useless under this
...
- **D-19 (user): every `anno_*` tool takes an explicit `.regen2000proj` path.**
...
- **D-32 (Claude's call, user said "you decide"): `anno_get_address_details` is
  ...
  - **Also found, do not re-derive:** `analyzer.rs:576`/`:588` and
    ...
    address. `anno_disassemble` on a 64K project was measured working. Do not
    "fix" our side around a bug that is not biting.

- **D-33 (user): `anno_batch_execute` is exposed, and every inner call's tool
  ...
  - anno's own guidance on the tool: "Use only when you have 5+ independent
...
- **The mechanism, measured:** `EnumDefinition.variants` is
  ...
  **instruction address** via `enum_usages` / `anno_apply_enum_usage`, and
...
- **D-21 (user): generated enums live project-level, inside the
  `.regen2000proj`.** Via `anno_create_project_enum` + `anno_save_project` —
  ...
  - Self-contained, diffable, revertible by one file, and writes **nothing
    ...
    with the human's own anno TUI usage, where a name collision silently
...
- **D-22 (user): bit names come from `memmap.json`'s 29 structured `bits`
  ...
  memmap and reviewed once, re-runnable per `ANNO-13`.
  ...
  - **`ANNO-13`'s wording needs correcting:** it credits anno with
...
### The store vs. the Markdown memory map (`ANNO-10`, `ANNO-11`)
...
- **D-25 (user): confidence grades live as a machine-readable prefix inside anno
  line comments** — e.g. a leading `[confirmed-code]` token.
  - **Measured:** line comments persist through save/reload
    (`user_line_comments` in the project file), and both `anno_get_comments`
    and `anno_search_disassembly` (which searches comments by default) can
    filter on them. So "show me everything still `[unknown]`" is a real query
    today with **no new storage**.
  - **Why a convention is needed at all:** anno's block types
...
- **D-29 (user): the store is the merge point — regenerate the full `.lbl` and
  ...
  - Makes `vice_symbols_load`'s **replace** semantics (T-05-02-05, deliberate)
    ...
    framing untouched, and keeps exactly one merge implementation — anno's.
...
- **D-35 (Claude's call, evidence-backed): correct `stock-symbols.ts`'s
  ...
  is now verified.** That file's header says the external analyser's `--export_lbl` is
  *expected* to emit `al C:xxxx .Name` but that `ANNO-16(c)` had never been run,
  and forbids any doc claiming "external-analyser-compatible" as verified. It has
  ...
  - **Note for `ANNO-14`:** `--export_lbl` exports **user-defined labels only**.
...
- **D-32** — excluding `anno_get_address_details`, plus filing upstream issue
  #42. ("you decide and create an issue in anno GitHub project")
...
1. **`.planning/todos/pending/2026-08-20-npm-closure-walk-blind-to-dynamic-imports.md`**
   ...
   **static-import-only** regex, while `vice-proxy.ts:218` reaches the anno seam
   via `await import("./anno-cli.ts")`. None of the five existing anno modules
   ...
   **Why it belongs here:** this phase adds several more dynamically-imported
   anno modules, so the drift the todo warns about becomes materially more
...
2. **`.planning/todos/pending/2026-08-20-anno-review-residual-findings.md`**
   ...
   - **WR-02 is the highest-value item and directly guards this phase's
     ...
     `anno-launch.ts` the only spawn path, so this phase depends on that guard
     being real.
   ...
   - Verified **clean** by the same review and recorded so it is not re-derived:
     ...
     complete; and `anno-verify.ts`'s no-exit-code-trust claim holds in both
     directions.
...
- `.planning/ROADMAP.md` § Phase 11 — the four success criteria and their Notes,
  ...
  **Criterion 3's "named bits" wording is reshaped by D-20** (anno has no
...
- `.planning/REQUIREMENTS.md:82-104` — `ANNO-10`, `ANNO-11`, `ANNO-13`
  (its `--dump-enum-files` claim is corrected by D-22), `ANNO-14`, `ANNO-15`.
...
- `docs/phase9-external-analyser-probe-findings.md` — frontmatter `verdict`
...
- `.claude/mcp/vice/anno-launch.ts` — the ONE authoritative the external analyser
  spawn point, with the fixed per-verb argv builders. **Every new anno process
  in this phase gets a new builder here**, never a bespoke `spawnSync`.
- `.claude/mcp/vice/anno-cli.ts` — the `vice-mcp anno <verb>` entry point and
  the resolution reasoning behind it; reached from `vice-proxy.ts:218` by
  **dynamic** import (folded todo 1).
- `.claude/mcp/vice/anno-project.ts` — `synthesizeProject()`, the ONE place a
...
- `.claude/mcp/vice/anno-verify.ts`, `.claude/mcp/vice/anno-d64.ts` — the other
...
- `.claude/mcp/vice/hostpath-consumers.test.ts` — the closed consumer set
  ...
  anno modules join the "must be absent" side, as Phase 10's D-08 did.
...
the web.
- `external-analyser-core-0.9.20/src/state/types.rs:436-445` — `ImmediateFormat`
...
- `external-analyser-core-0.9.20/src/assets.rs:378-404` (`load_builtin_enums`),
...
- `external-analyser-core-0.9.20/assets/enums/` — the only three built-in enums:
...
- `external-analyser-core-0.9.20/src/state/app_state.rs:70-73` (`enums`,
  `user_global_enums`, `builtin_enums`), `:443` (`validate_new_enum_name`),
  `:464-468` (the `enum_usages` → three-tier resolution order).
- `external-analyser-core-0.9.20/src/state/blocks.rs:555-583` — collects
...
- `external-analyser-core-0.9.20/src/disassembler/formatter_acme.rs:58`,
  `:367-369` (`format_enum_reference` → `EnumName_VARIANT`), `:371-385`
  (`format_enum_definition` → `NAME_VARIANT = $xx` lines).
- `external-analyser-core-0.9.20/src/mcp/handler.rs` — the 28-tool surface.
  `:350-352,1264-1271` — `anno_save_project` takes **no arguments** and errors
...
- `external-analyser-core-0.9.20/src/analyzer.rs:576,:588` and
  `src/exporter/html.rs:264` — the same `as u16` narrowing; `flow_analyze`
  survives by accident (D-32).
- `the external analyser-0.9.20/src/main.rs:33-88` — the flag surface
...
### Upstream
- an upstream repositoryissues/42> — the
...
### Reusable Assets
- **`anno-launch.ts`'s fixed argv builders** — the pattern every new anno
...
- **`stock-derived.ts` / `stock-dispatch.ts` / `stock-handler.ts`'s derived-tool
  ...
  `forwardToVice()`. New `anno_*` tools are derived tools by definition — they
...
- **`disasm-roundtrip.test.ts`'s availability gate** — `SKIP_REASON` at module
  ...
  the external analyser (`VICE_REQUIRE_ANNO`); this phase's live-anno tests inherit
...
- **Never trust a misleading success.** `anno-verify.ts` exists because a
...
- **No build step for the shipped server**; host-bound `.mts` must be compiled to
  ...
  The anno surface is container-side, so it should **not** need to become a
  `resources/` artifact — worth confirming, not assuming.

### Integration Points
- **`vice-proxy.ts:218`** — where the anno seam is reached, by **dynamic**
...
- **`vice-proxy.ts:1746`** — the argument-rewriting site that currently knows
  ...
  path argument to every `anno_*` tool; `CLAUDE.md`'s standing constraint is
...
- **The two-manifest split** — `tools-manifest.json` (fork, 62) and
  ...
  it implements. A new `anno_*` family is backend-independent (it never touches
...
- **"You decide and create an issue in anno GitHub project"** — the user's own
...
- **`.vsf` as a bootstrap input** — explicitly out of scope by **D-34**, which
  ...
  machine-type auto-detection unreliable; the synthesis route never hands anno a
  container format).
- **A fix for `anno_get_address_details` upstream** — issue #42. If it lands,
...
- **Enums as a reusable cross-project asset.** D-20 makes them program-specific
  ...
  shared library of register enums would need either anno's global config dir
...
- **Two-project-limit detection** — permanently out by the `ANNO-04` fold.
  ...
  detected.
- **`anno_undo` / `anno_redo`** — unusable under D-17's per-call lifecycle.
  ...
  (D-21's diffability).
- **`anno_unpack_binary`, `anno_read_region`, `anno_search_memory`,
  `anno_set_immediate_format`, `anno_toggle_splitter`** — real tools, no
