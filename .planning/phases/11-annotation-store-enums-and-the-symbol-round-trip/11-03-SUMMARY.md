...
provides:
  ...
  - "ANNO-13's wording naming the real enum install path, anno_create_project_enum, instead of --dump-enum-files (D-22)"
  - "README's one-project limit narrowed to the external analyser's --mcp-server HTTP route (D-15/D-19)"
...
key-files:
  ...
  modified:
    ...
    - .planning/todos/completed/2026-08-20-anno-review-residual-findings.md

key-decisions:
  - "D-34: .vsf is out of Phase 11; no anno-* requirement covers it as a bootstrap input; filed as backlog with the three real reasons and the reversal trigger (a consumer with only .vsf captures)"
  - "D-35: stock-symbols.ts's --export_lbl compatibility note upgraded from unverified assumption to a verified claim, scoped to the external analyser 0.9.20 and the probe-illegal.prg-derived fixture, not all inputs forever"
  - "D-22: ANNO-13's wording corrected to name anno_create_project_enum as the install path; --dump-enum-files reframed as a TOML-shape discovery tool, not an install path"
  - "D-15/D-19: README's one-project-per-namespace limit narrowed to the external analyser's --mcp-server HTTP route -- this project's --mcp-server-stdio route binds no port, so the limit was sidestepped for this project, not fixed upstream"
...
requirements-completed: []  # ANNO-13/ANNO-14 touched (wording/evidence only) but NOT completed -- see Requirements Traceability Note; both are multi-plan requirements with other Phase 11 plans still pending
...
- D-34: all three sites naming Phase 11 as `.vsf`'s home (`ROADMAP.md`'s standing constraint, Phase 10 criterion 3's parenthetical, the `ANNO-08` cut-table row, and `10-CONTEXT.md`'s Deferred Ideas entry) now point at a real backlog item instead of a dangling forward reference.
- D-35: `stock-symbols.ts`'s header no longer says "STATED ASSUMPTION, NOT A VERIFIED FACT" — it states the measured fact (`al C:0810 .init_screen` matches `VICE_LABEL_LINE_RE` exactly) scoped to the external analyser 0.9.20 and the probe fixture, with the skip-unrecognised-lines behaviour retained and its reason restated for a verified-but-scoped format.
- D-22: `ANNO-13` no longer credits `--dump-enum-files` as an install path; it names `anno_create_project_enum` and explains what `--dump-enum-files` actually does (writes the three built-in enums and exits).
- D-15/D-19: README's one-project-per-namespace limit is now explicitly scoped to the external analyser's `--mcp-server` HTTP route, with this project's `--mcp-server-stdio` route stated as binding no port.
...
- `.planning/todos/pending/2026-08-20-vsf-as-a-bootstrap-input.md` - New backlog item: `.vsf` as an external analyser bootstrap input, with the three real reasons and the reversal trigger
- `.planning/ROADMAP.md` - Three `.vsf` sites corrected (standing constraint line ~187, Phase 10 criterion 3 parenthetical, `ANNO-08` cut-table row)
...
- `.planning/REQUIREMENTS.md` - `ANNO-13`'s wording corrected to name `anno_create_project_enum`; `--dump-enum-files` reframed correctly
- `README.md` - One-project-limit paragraph narrowed to the `--mcp-server` HTTP route, `--mcp-server-stdio` stated as binding no port
- `CLAUDE.md` - `rewriteArguments()` bullet's line citations corrected (`:2943`/`:2878`, `:1422`/`:1398`) plus a new clause on the `anno_*` family's by-construction exemption
...
- `.planning/todos/completed/2026-08-20-anno-review-residual-findings.md` - Moved from `pending/`, resolution section added naming `11-01`/`11-02` per finding

## Decisions Made

- Kept the ROADMAP.md `.vsf`-preference constraint as an emulator-output rule and explicitly stated it does not extend to the external analyser's input set, rather than deleting or hedging it — the constraint is still true for anything actually leaving the emulator.
- Worded CLAUDE.md's new `anno_*`/`buildViceTool()` clause as an architectural fact established by planning for plan `11-05` (confirmed present in `11-CONTEXT.md`, `11-PATTERNS.md`, `11-RESEARCH.md`, and `11-05-PLAN.md`), even though the actual `anno_*` MCP tool registration does not exist in `vice-proxy.ts` yet as of this plan — the family is currently reached only via the `vice-mcp anno <verb>` CLI subcommand at `vice-proxy.ts:217-218`, which also never touches `forwardToVice()`.
...
This plan's frontmatter lists `requirements: [ANNO-13, ANNO-14]`, but both are
multi-plan requirements also claimed by other Phase 11 plans not yet executed
(`ANNO-13`: 11-01 already done, plus 11-06/11-12 pending; `ANNO-14`: 11-04/11-08/11-11
pending). This plan only corrected `ANNO-13`'s wording (D-22) and extended the
verified evidence `ANNO-14` depends on (D-35's `--export_lbl` claim) — it did not
...
- The `anno_*`/`buildViceTool()` clause added to CLAUDE.md anticipates plan `11-05`'s actual registration work; when `11-05` lands, its own verification should confirm the clause remains accurate (it already plans to, per its own `T-11-PATH-XLATE` threat-register entry).
...
- FOUND: `.planning/todos/completed/2026-08-20-anno-review-residual-findings.md`
