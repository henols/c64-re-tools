...
plan: 09
subsystem: testing
tags: [the external analyser, annotation-store, falsifiability, anno-tools, two-session-proof]
...
requires:
  ...
  - phase: 11-05
    provides: "runAnnoTool()/CURATED_ANNO_TOOLS/resolveStorePath() in anno-tools.ts -- the ONE dispatcher this plan's driver script calls to query the store"
provides:
  - "SESSION-B-ANSWER.md: an independent execution context's answer to criterion 1's sealed question, derived entirely from anno_* tool calls, with per-field call/response derivation and a files-read list"
  - "A mechanical seal comparison extending absorbed-answer-key.test.ts: session B's canonical line hashes and is asserted equal to the sealed ANSWER.sha256"
...
tech-stack:
  added: []
  patterns:
    - "Driving runAnnoTool() directly from a standalone Node script (rather than through a live MCP tool-call transport) when the executing session has no anno_* tools wired into its own tool set -- the dispatcher and its guards (assertCuratedTool(), resolveStorePath()) are identical either way, so this is a legitimate query route, not a workaround that bypasses the curated surface"
    - "Session-boundary falsifiability proof: a second execution context, given only the sealed question and the committed store, is checked mechanically (sha256 equality) against a key sealed before that context existed -- extending the SAME test file (absorbed-answer-key.test.ts) rather than adding a parallel checker, so seal-drift and vacuous-check guards stay in one place"

key-files:
  created:
    - .planning/phases/11-annotation-store-enums-and-the-symbol-round-trip/evidence/criterion1/SESSION-B-ANSWER.md
  modified:
    - .claude/mcp/vice/absorbed-answer-key.test.ts

key-decisions:
  - "Queried the store via a Node driver script importing runAnnoTool() directly, rather than through a live MCP session, because this execution context has no anno_* tools registered in its own tool set -- documented explicitly in SESSION-B-ANSWER.md's derivation section as an implementation detail that does not change which function was called or which guards ran"
...
**A second, independent execution context answered all four parts of criterion 1's sealed D-26 question using only `anno_*` queries against the committed annotation store, and its answer's sha256 matched the sealed `ANSWER.sha256` exactly — the two-session falsifiability proof returned MATCH.**
...
- Answered `QUESTION.md`'s four parts entirely from the store, via four `anno_*` calls (`anno_get_symbols`, `anno_get_comments`, `anno_get_blocks`, `anno_get_cross_references`), each recorded with its exact arguments and response fragment in `SESSION-B-ANSWER.md`.
...
- Extended `absorbed-answer-key.test.ts` (from plan 11-07) with a session-B comparison test and a grammar-conformance test, keeping all of 11-07's original seal-drift and leak-detection assertions intact (7/7 pass).
...
- `.claude/mcp/vice/absorbed-answer-key.test.ts` - Two new tests: session-B canonical line hashes to the sealed `ANSWER.sha256`; session-B's canonical line matches `QUESTION.md`'s grammar. Original 5 tests (T-11-SEAL-DRIFT, T-11-LEAK) untouched.

## Decisions Made

- Drove the curated `anno_*` surface via a standalone Node script importing `runAnnoTool()` directly (`/tmp/.../scratchpad/session-b-query.mts`, not committed — a throwaway driver, not a deliverable), since this execution context has no live MCP connection exposing `anno_*` tools as callable tools in its own tool set. This calls the exact same dispatcher function (`runAnnoTool()`) with the exact same guards (`assertCuratedTool()`, `resolveStorePath()`) that `vice-proxy.ts` would use for a real MCP `tools/call` — documented explicitly in `SESSION-B-ANSWER.md` so the derivation's evidentiary weight is stated honestly rather than implied to be a live MCP session.
- Kept the seal-comparison logic inside the existing `absorbed-answer-key.test.ts` rather than adding a second test file, per the plan's explicit instruction ("extend that file rather than adding a second seal test") and this project's one-authoritative-place convention.
...
- Calling `runAnnoTool()` for the first time in this worktree triggered `install-resources.ts`'s one-time host-launcher deployment into `tools/` (an unrelated, pre-existing side effect of `repoRoot()`'s first call). Confirmed harmless: `tools/*.mjs` and `vice-launcher.sh` are gitignored (`.gitignore:30`), and `git status --short` showed no changes from it.

## User Setup Required

None — no external service configuration required. `the external analyser 0.9.20` was already installed on this host (`~/.cargo/bin/analyser`), confirmed via `--version` before querying.

## Files Read This Session (auditability of the session-boundary claim)

Full list is recorded in `SESSION-B-ANSWER.md` itself (the artifact whose purpose is exactly this audit trail). Summary: the plan file, `QUESTION.md`, the five required initial-context files (`PROJECT.md`, `ARCHITECTURE.md`, `ENGINEERING_RULES.md`, `STATE.md`, `CLAUDE.md`), `11-05-SUMMARY.md` (explicitly permitted), `anno-tools.ts`, `anno-cli.ts`, and `absorbed-answer-key.test.ts` (both named in the plan's own `read_first` blocks). None of the eight forbidden files were read, and no `git log -p`/`git show`/`git log` was run over plan 11-07's commits. `recon-subject.regen2000proj` was never opened as text — every value in the answer came from an `anno_*` tool-call response.
...
- No blockers for 11-10/11-11/11-12. The `absorbed-answer-key.test.ts` file now carries 7 tests (5 from 11-07, 2 from this plan) and stays TEST-ONLY (not in `package.json`'s `files[]`).
...
- FOUND: .claude/mcp/vice/absorbed-answer-key.test.ts
