...
**Domain:** MCP client implementation (this repo's first); the external analyser 0.9.20 tool surface; enum-based semantic disassembly; VICE-label round trip
...
curated `anno_*` tool surface through a child `analyser --mcp-server-stdio`
...
`anno_*` family should follow this exact pattern, not the manifest-tool
pattern. Doing so makes CLAUDE.md's "derived tools must be intercepted before
`forwardToVice()`" constraint moot by construction — the `anno_*` runners
...
Also measured, and materially reshaping the plan: `anno_apply_enum_usage`
...
**Primary recommendation:** build the `anno_*` surface as proxy-local synthetic
tools (mirroring `vice_result_continue`/`RECYCLE_TOOL`/`DIAGNOSE_TOOL`), backed
by `@mastra/mcp`'s `MCPClient` driving `anno-launch.ts`'s (extended) argv
builders, registered identically on both backends since the external analyser never
touches VICE.
...
|------------|-------------|----------------|-----------|
| Spawn/lifecycle of the anno child process | Container-side MCP proxy (`vice-proxy.ts` + new `anno-launch.ts` builders) | — | Same tier as every other the external analyser spawn (D-06); never host-side (D-R4) |
...
| Annotation store (labels/comments/blocks/enums) | the external analyser's own `AppState` (external, via the child process) | — | This project owns zero storage; the store lives entirely inside the `.regen2000proj` file the external analyser reads/writes |
...
| Symbol round trip (`--export_lbl`/`--import_lbl`) | Container-side MCP proxy (`anno-launch.ts` argv builders) + `stock-symbols.ts` (existing store) | Stock `x64sc` (live half, `vice_symbols_load`) | `stock-symbols.ts` already owns the symbol-file parser; D-29 makes the store (not `vice_symbols_load`) the merge point |
...
| Confidence-grade convention (`[confirmed-code]` etc.) | anno's own comment storage, read via `anno_get_comments`/`search_disassembly` | — | No new storage; a string-prefix convention enforced by a parser + test on this repo's side |
...
- D-16: the store is reached by proxying `anno_*` tools through our own MCP
  server, which drives `analyser --mcp-server-stdio` as a child and
  forwards calls. Not a second MCP server in the manifest, not CLI-only.
- D-17: per-call lifecycle — spawn, load, mutate, `anno_save_project`, exit.
...
- D-19: every `anno_*` tool takes an explicit `.regen2000proj` path argument.
- D-32 (Claude's call): `anno_get_address_details` excluded; upstream issue
  #42 filed with reproduction and affected/unaffected tool table.
- D-33: `anno_batch_execute` is exposed, with every inner call's tool name
...
- D-21: generated enums live project-level, inside the `.regen2000proj`, via
  `anno_create_project_enum` + `anno_save_project`. Never the global config
  dir.
- D-22: bit names come from `memmap.json`'s 29 structured `bits` entries plus
  ...
  reviewed once, re-runnable per ANNO-13. `$D015`/`$D017`/`$D01A`-`$D01D` are
...
**Store vs. prose in recon (`ANNO-10`, `ANNO-11`)**
- D-24: the store is canonical; the Markdown memory map becomes a rendered
  view.
- D-25: confidence grades live as a machine-readable prefix inside anno line
...
- D-26: criterion 1 is demonstrated by a recorded two-session transcript on
  ...
  prose) answers a question purely from `anno_*` queries.
...
**The symbol round trip (`ANNO-14`, `ANNO-15`)**
- D-28: the inbound leg is `--import_lbl` **plus** `--mcp-server-stdio` **plus**
  `anno_save_project`. `--import_lbl` under plain `--headless` discards
...
- D-32 — excluding `anno_get_address_details`, filing upstream issue #42
...
- Two-project-limit detection (permanently folded, `ANNO-04`).
- `anno_undo`/`anno_redo` — unusable under D-17; git revert of the project
  file is the substitute.
- `anno_set_immediate_format`'s `low_byte`/`high_byte` modes — candidate for
...
|----|-------------|------------------|
| ANNO-10 | `c64-program-recon` writes labels/comments/block types/scopes into queryable state | Priority 6 (store-canonical renderer shape); D-24/D-25 mechanism confirmed against real tool schemas below |
| ANNO-11 | Query cross-references and search labels/comments/instructions | `get_cross_references`/`search_disassembly` argument and return shapes measured below (Priority 2/3 evidence) |
| ANNO-13 | Enums generated from `memmap.json`, re-runnable | Priority 2 — full mechanism measured end-to-end including the exact ACME output and the search-view discrepancy |
| ANNO-14 | Symbols export via `--export_lbl` into the symbol store, resolve through `vice_symbols_load` | Priority 3 — D-28/D-29 sequencing, `stock-symbols.ts` parser already verified compatible |
| ANNO-15 | Names discovered live flow back via `--import_lbl`, closing the loop | Priority 3 — the `--import_lbl` discard trap and its fix are source-confirmed at `main.rs:800-806` |
...
- **Node ≥ 22.18, no build step for the shipped server.** New `.ts` modules
  ...
  compile to `resources/*.mjs`. The anno MCP-client seam is container-side —
  confirm it does **not** need a `resources/` artifact (it does not: it never
  runs outside the container, same as `anno-launch.ts` today).
- **Derived tools must be intercepted before `forwardToVice()`, not behind
  ...
  plan time, as CLAUDE.md itself warns. The `anno_*` surface avoids this
...
- **Any host-facing path goes through the closed `hostpath.ts`/
  ...
  by `hostpath-consumers.test.ts`). New anno modules join the "must be
  absent" side, per D-08's precedent — the external analyser runs container-side
...
- **ENGINEERING_RULES.md §6 (Non-Vacuous Verification):** the deny-by-scan
  guard for `anno-launch.ts` must be shown failing under a planted
...
- **§7 Independent Oracle Rule:** real the external analyser for static-analysis
...
| `the external analyser` (external binary) | 0.9.20 (installed, `~/.cargo/bin/analyser`) | The annotation-store backend this whole milestone integrates | Not an npm package; a required external prerequisite per the milestone's standing constraints |

### Supporting

| Library | Version | Purpose | When to Use |
|---------|---------|---------|-------------|
| `node:child_process` (builtin) | Node 22.18+ | Spawning the anno child, if the hand-rolled fallback is chosen instead of `MCPClient` | Only if `MCPClient`'s abstraction proves a poor fit for D-17's failure-mode requirements (see Priority 1) |
...
```
Claude session
     │  tools/call "anno_search_disassembly" {project, ...}
     ▼
vice-proxy.ts  (existing MCP stdio server, unchanged entry point)
     │
     │  synthetic tool lookup — anno_* names registered directly in the
     │  `tools` record (SAME pattern as vice_result_continue), NEVER through
     │  the manifest loop, NEVER through buildBackendAwareTool()
     ▼
new module: anno-mcp-client.ts (the ONE seam that is "we are an MCP client")
     │
     │  1. runAnnoVerb("mcp-server-stdio", { projectPath })  [anno-launch.ts,
     │     new argv builder — argv = ["--mcp-server-stdio", projectPath]]
     │  2. spawn child, get stdin/stdout pipes
     │  3. write one newline-terminated JSON-RPC request per anno_* call
     ...
     │     mirroring anno-verify.ts's no-exit-code-trust posture
     ▼
The external analyser --mcp-server-stdio <path>.regen2000proj  (child process,
container-side, --vice guarded absent by anno-launch.ts's existing
assertNoViceFlag(), never touches VICE)
     │
     │  reads/mutates AppState in memory; anno_save_project writes it back
     ▼
...
through the SAME `anno-mcp-client.ts` seam plus `stock-symbols.ts`:

```
running x64sc (stock)  --[vice_symbols_lookup / a discovered name]-->
this repo's Claude session  --[writes the name via anno_set_label_name OR
  hands it to the --import_lbl leg below]-->
.regen2000proj (store)
     │  anno_save_project
     ▼
.regen2000proj on disk
     │  vice-mcp anno export-lbl <verb, new> --> spawns
     │  the external analyser --headless --export_lbl <path> --assembler acme <proj>
     ▼
...
append to the SAME .lbl file (or a delta file, merged by regenerating)
     │  the external analyser --headless --import_lbl <path> --mcp-server-stdio <proj>
     ...
     │  early return, THEN the client must still call anno_save_project
...
```
.claude/mcp/vice/
├── anno-launch.ts          # EXTENDED, not replaced: add buildMcpServerStdioArgs(),
│                            #   buildExportLblArgs(), buildImportLblArgs() alongside
│                            #   the existing buildExportAsmArgs()/buildVerifyArgs()
├── anno-mcp-client.ts      # NEW: the ONE "we are an MCP client" seam --
│                            #   spawn, initialize, call, batch, close, with
│                            #   every named failure mode handled explicitly
├── anno-tools.ts           # NEW: the curated anno_* ToolDefinition objects
...
├── anno-enum-gen.ts        # NEW: memmap.json -> curated bit-name table ->
│                            #   program-specific enum variants -> anno_create_project_enum
│                            #   calls. Re-runnable, CI-testable (Priority 7).
├── anno-memmap-render.ts   # NEW: store (via anno_* queries) + a provenance
│                            #   header input -> rendered memory-map.md
├── anno-confidence.ts      # NEW: the [confirmed-code]-style prefix
│                            #   convention: parse/validate/render, pinned by
│                            #   a typo-fails test (D-25)
└── (existing anno-launch.ts, anno-project.ts, anno-verify.ts, anno-d64.ts,
    anno-cli.ts unchanged in shape, extended in content)
```
...
exactly the external analyser's case, since it "never touches VICE" (ROADMAP.md,
...
**What:** D-18's curated set excludes `anno_read_region` and
`anno_get_address_details`, so the enum generator cannot ask "what
instruction sits at address X" directly. It CAN ask `anno_search_disassembly`
...
- **Registering `anno_*` tools through the manifest loop or
  ...
  (`refresh-manifest.ts`) — an `anno_*` entry hand-added there would be
...
- **Calling `rewriteArguments()` or `forwardToVice()` from any `anno_*`
...
- **Trusting `anno_save_project`'s text response as sufficient proof of
  ...
  (`anno-verify.ts`), the client should re-open the saved file (or re-run a
...
loads; the default cap on `anno_search_disassembly` silently returns only
...
### Pitfall 3: Enum variant names are unvalidated by anno — a bad name reaches ACME silently
...
client-side before calling `anno_create_project_enum` — this IS the
...
have the client explicitly call `anno_save_project` over that session —
...
**What goes wrong:** `.claude/mcp/vice/anno-launch.test.ts:39-56`'s
...
depends on `anno-launch.ts` staying the only spawn path, and this guard
...
```javascript
// Source: measured directly against the external analyser 0.9.20 on this host
// (see "Sources" -> Primary, live reproduction).
import { spawn } from "node:child_process";
import { createInterface } from "node:readline";

const child = spawn("the external analyser", ["--mcp-server-stdio", projPath], {
...
await callAnno("anno_create_project_enum", {
  name: "D011",
  variants: { "$1b": "YSCROLL3_ROW25_SCREENON_TEXT" }, // hex string keys accepted (parse_variants, types.rs:481-525)
});
await callAnno("anno_apply_enum_usage", { address: 0x0810, name: "D011" }); // 0x0810 is the LDA address, confirmed
await callAnno("anno_save_project", {});
// Later, out-of-process:
//   the external analyser --headless --export_asm out.a --assembler acme proj.regen2000proj
...
|--------------|------------------|--------------|--------|
| This repo as MCP server only | This repo as MCP server AND MCP client | Phase 11 (this phase) | New failure-mode surface: a child process, not just a socket to a host; needs its own "never trust a misleading success" discipline, same as `anno-verify.ts` already established for CLI verbs |
...
**Deprecated/outdated:**
- `ANNO-13`'s own wording crediting `--dump-enum-files` as "the enum
  ...
  path. The install path is `anno_create_project_enum`.
...
2. **Why does this session's minimal `synthesizeProject()`-bootstrapped project never populate `a_D011`-style auto-labels, while CONTEXT.md's own D-19/D-20 evidence block shows `sta a_D011` in its example?**
   - What we know: `get_symbols({kind: "system"})` and `get_symbols({kind: "auto"})` both returned `[]` on a project built purely via `synthesizeProject()` + `anno_disassemble`; the ACME export rendered `sta $d011 ; VIC Control Register 1` (raw hex, with an auto-attached comment) rather than `sta a_D011`. The `a_` prefix is `LabelType::AbsoluteAddress` (`state/types.rs:387`), populated during analysis (`analyzer.rs:81,116,149,326`).
...
3. **Does `anno_batch_execute`'s inner-call error handling stop on first failure or continue?**
...
|------------|------------|-----------|---------|----------|
| `the external analyser` binary | The entire phase | ✓ (measured, `~/.cargo/bin/analyser`) | 0.9.20 | None — required prerequisite (D-R2), not optional |
...
| Quick run command | `cd .claude/mcp/vice && node --test anno-mcp-client.test.ts` (per-module, once it exists) |
...
|--------|----------|-----------|-------------------|-------------|
| ANNO-10 | Recon writes labels/comments/blocks/scopes to the store | integration (real anno child, gated) | `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` | ❌ Wave 0 — new file |
| ANNO-10 | A LATER session answers a question by querying only | human-witnessed transcript (see below — not sampleable by a single-process unit test) | N/A — recorded artifact | ❌ Wave 0 — new evidence file under the phase's `evidence/` dir |
| ANNO-11 | `get_cross_references`/`search_disassembly` return correct results | integration (real anno child, gated) | `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` | ❌ Wave 0 |
| ANNO-13 | Enum generation from `memmap.json`, re-runnable, produces the literal `lda #imm`/`sta $d011` semantic render | integration (real anno child, `--export_asm`, real `acme`) | `VICE_REQUIRE_ANNO=1 ACME_AVAILABLE=1 node --test anno-enum-gen.test.ts` | ❌ Wave 0 |
| ANNO-14 | `--export_lbl` produces a format `stock-symbols.ts` parses | integration (real anno child) — already partially proven this session | `VICE_REQUIRE_ANNO=1 node --test anno-launch.test.ts` (extend existing) | Partial — `anno-launch.ts`/`.test.ts` exist, need new builders + tests |
| ANNO-15 | `--import_lbl` + `--mcp-server-stdio` + `save_project` persists, and a store-is-merge-point reload closes the loop | integration (real anno child) + live stock `x64sc` walkthrough (D-30, D-31) | `VICE_REQUIRE_ANNO=1 node --test anno-symbol-roundtrip.test.ts`; live half is human-witnessed, not automated in CI | ❌ Wave 0 for the automated fixture half |

### Sampling Rate

- **Per task commit:** the module's own `node --test <file>.test.ts` (no
  `VICE_REQUIRE_ANNO`/real-binary requirement forced locally, following the
  `SKIP_REASON` convention — CI without the real binary SKIPS, never FAILs
  unless `VICE_REQUIRE_ANNO` is explicitly exported).
...
- **Do** structure it as two SEPARATE waves/plans in this phase's own plan
  ...
  writing to the store, closing with `anno_save_project`) and commits the
  ...
  the exact `anno_*` calls it made to derive it) is the transcript D-26
...
**trust boundary between this repo's code and the external analyser child
...
- `analyser --version` → `0.9.20` (this host, `~/.cargo/bin/analyser`).
- Live drive of `analyser --mcp-server-stdio <synthesized .regen2000proj>`
  via a hand-written Node script (newline-delimited JSON-RPC over
  `spawn(..., {stdio:["pipe","pipe","pipe"]})`): confirmed `initialize` returns
  `protocolVersion: "2024-11-05"`, `serverInfo.name: "external-analyser-core-mcp"`,
  `version: "0.9.20"`; confirmed `anno_disassemble`, `anno_search_disassembly`,
  `anno_get_cross_references`, `anno_create_project_enum`,
  `anno_apply_enum_usage`, `anno_save_project` argument/return shapes exactly
  as reproduced in Code Examples above.
- Live `analyser --headless --export_asm out.a --assembler acme
...
- `external-analyser-core-0.9.20/src/mcp/handler.rs` (full tool list at
...
- `external-analyser-core-0.9.20/src/mcp/stdio.rs` — confirmed newline-delimited
...
- `external-analyser-core-0.9.20/src/state/app_state.rs:436-455` —
...
- `external-analyser-core-0.9.20/src/disassembler/formatter_acme.rs:365-369` —
  `format_enum_reference()`'s literal `format!("{enum_name}_{variant_name}")`.
- `the external analyser-0.9.20/src/main.rs:709-711,800-806` — the headless/mcp_server
...
- `.claude/mcp/vice/hostpath-consumers.test.ts` — confirmed the closed
  ...
  `stripCommentLines()`.
- `.claude/mcp/vice/anno-launch.test.ts:39-56` — the buggy block-comment
...
**Valid until:** 30 days, OR immediately upon any `the external analyser` version
