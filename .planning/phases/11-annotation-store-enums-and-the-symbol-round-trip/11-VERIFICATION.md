...
reads, live `node --test` runs against a real `the external analyser 0.9.20`, real ACME 0.97, and — for
...
|---|-------|--------|----------|
| 1 | `c64-program-recon` writes labels/comments/block types/scopes into the store; a **later session** answers by querying the store instead of re-deriving from prose | ✓ VERIFIED | `anno-tools.ts` (17 curated tools) live-tested (`VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` → 70/70 pass incl. cross-session read-back). D-26 two-session proof independently re-verified: `QUESTION.md`'s sealed answer (`ANSWER.sha256`) matches `SESSION-B-ANSWER.md`'s recomputed hash exactly (`e64463d8...`); confirmed the label `border_bump_up` (the leak-sensitive field) appears nowhere in `SESSION-A-TRANSCRIPT.md` (grep, zero hits); `absorbed-answer-key.test.ts` 7/7 pass. `c64-program-recon/SKILL.md` rewritten (plan 11-12) to write via `anno_*` tools and generate the map via `render-memmap` rather than hand-authoring it. |
| 2 | A user can ask which addresses reference a given address, and search labels/comments/instructions | ✓ VERIFIED | `anno_get_cross_references`/`anno_search_disassembly` are 2 of the 17 curated tools, live-tested against a real child (non-empty xref list on `$D020`, bounded `search_disassembly` results). Directly exercised in the criterion-1 evidence (`xrefcount=2`, cross-checked by both sessions). |
| 3 | Enums generated from `memmap.json` render per-bit VIC-II/SID/CIA writes with semantic names (`lda #$1b`/`sta $d011` → named bits), re-runnable from `memmap.json` | ✓ VERIFIED | Independently re-ran `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-cli.test.ts` — test 35 ("criterion 3 … renders as `lda #D011_YSCROLL3_ROW25_SCREENON_TEXT` … reassembles under real ACME") passes, with the ACME export operand printed live in the test's own diagnostic output. `anno-regbits.json` (35 registers) is generated from `memmap.json`'s 29 structured `bits` entries + a documented override table, with a digest-pinned drift guard (`anno-regbits.test.ts`, 13/13 pass). |
| 4 | A symbol annotated in anno resolves live via `vice_symbols_load`; a name discovered against the running machine flows back via `--import_lbl`; demonstrated as ONE closed loop, not two dumps | ✓ VERIFIED | `evidence/criterion4/WALKTHROUGH.md` read in full: 23 ordered steps against genuine unpatched stock `/usr/bin/x64sc` (VICE 3.9, confirmed zero `mcpserver` hits) and real `the external analyser 0.9.20`. Absence proven (Steps 6-7, both store and exported file) strictly before the live discovery (Steps 14-15, `resolvedTarget: 2118` read off a real `beq`), before the name is written to the store (Step 16), before the whole `.lbl` is regenerated (Step 17), before the single `vice_symbols_load` reload (Step 18) — `vice_symbols_load` occurs exactly twice in the transcript (once per file), never incrementally, matching D-29. The `--import_lbl` leg is separately demonstrated on an independent project copy (Steps 21-23). CI-runnable mechanism test `anno-symbol-roundtrip.test.ts` also passes (7/7, gated). |
...
(wave 1) correctly declined to mark `ANNO-13`/`ANNO-14` complete while later plans were still
...
|----|------------------------|---------|----------|
| ANNO-10 | Recon writes queryable annotation state (labels/comments/block types/scopes), not only Markdown prose | **SATISFIED** | 17 curated tools exist and are live-tested; `c64-program-recon/SKILL.md` rewritten to be store-first (plan 11-12); D-26 two-session query proof returned a genuine, mechanically-checked MATCH (plans 11-07/11-09), independently re-confirmed above. |
| ANNO-11 | Cross-references and search across an analysed program | **SATISFIED** | `anno_get_cross_references`/`anno_search_disassembly` implemented, curated, and live-tested; used for real in the criterion-1 evidence. |
| ANNO-13 | Enums generated from `memmap.json`, re-runnable, rendering named bits | **SATISFIED** | `anno-regbits-gen.ts`/`anno-regbits.json` (digest-pinned, drift-guarded, re-runnable) plus `anno-enum-gen.ts`'s `gen-enums` verb; criterion 3's pinned literal example passes against a real ACME reassembly (independently re-run above). One known, honestly-recorded limitation (a literal double-run over already-applied addresses is a safe no-op, not true re-derivation) does not defeat the requirement's own wording — re-running the *table* generator from a changed `memmap.json` is exactly what is proven re-runnable, and the "re-runnable" language in ANNO-13 is about the table, not about idempotent double-application to the same disassembly. |
| ANNO-14 | Symbols annotated in anno export as VICE label files and resolve live via `vice_symbols_load` | **SATISFIED** | `exportLabels()`/`vice-mcp anno export-lbl` produce a `stock-symbols.ts`-parseable `.lbl`; CI-runnable `anno-symbol-roundtrip.test.ts` and the live walkthrough (Steps 3-13) both prove resolution against a real emulator. |
| ANNO-15 | Names discovered live flow back via `--import_lbl` — a round trip, not a one-way dump | **SATISFIED** | D-28's trap (`--import_lbl` under plain `--headless` discards) is pinned live as a regression test; the fixed path (`--import_lbl` + `--mcp-server-stdio` + `anno_save_project`, hash-verified) is proven both in CI (11-08) and live against genuine stock (11-11, Steps 21-23), with the absence-before-discovery ordering that distinguishes a loop from two dumps. |

**Recommendation:** run `requirements.mark-complete` for `ANNO-10`, `ANNO-11`, `ANNO-13`,
`ANNO-14`, `ANNO-15` and check the five boxes in `REQUIREMENTS.md`. This is a bookkeeping gap,
...
|----------|----------|--------|---------|
| `.claude/mcp/vice/anno-tools.ts` | 17 curated `ANNO_TOOL_DEFINITIONS`, `assertCuratedTool()`, `runAnnoTool()` | ✓ VERIFIED | 769 lines; exactly 17 tool names extracted; `assertCuratedTool()` refuses `anno_get_address_details` by name (D-32) and recurses into `batch_execute` (D-33), both live-tested. |
| `.claude/mcp/vice/anno-mcp-client.ts` | The one MCP-client seam, 6 named failure modes | ✓ VERIFIED | 596 lines; `withAnnoSession`/`callAnno`/`saveAndVerify` present; `anno-mcp-client.test.ts` 44+ tests incl. stub-server failure modes and a live-gated real-child test. |
| `.claude/mcp/vice/anno-regbits-gen.ts` + `anno-regbits.json` | Generated, digest-pinned address→bit-name table | ✓ VERIFIED | 35 registers; drift guard re-runs the generator and deep-equals the committed file; 13/13 tests pass. |
| `.claude/mcp/vice/anno-enum-gen.ts` | Value decode, adjacent-pair pass, sanitization, coverage report | ✓ VERIFIED | Criterion 3's pinned literal example independently re-verified against real ACME. |
| `.claude/mcp/vice/anno-symbols.ts` | `exportLabels`/`importLabels`/`regenerateAndReload` (Rule A20) | ✓ VERIFIED | Reuses `stock-symbols.ts`'s parser (no third copy, confirmed by grep); `anno-symbol-roundtrip.test.ts` 7/7 live pass. |
| `.claude/mcp/vice/anno-confidence.ts` + `anno-memmap-render.ts` | D-25 grade convention; D-24/D-27 generated memory map with drift `--check` | ✓ VERIFIED | 15/15 and 12/12 tests pass; live drift transcripts for both a hand-edit and a store-side change are recorded in 11-10-SUMMARY.md and independently plausible given the code's digest composition (store query results + sidecar bytes + renderer version). |
| `vice-proxy.ts` registration | `anno_*` family registered proxy-locally, never reaches `forwardToVice()`/manifests | ✓ VERIFIED | `grep -c "anno_"` on both `tools-manifest.json` and `tools-manifest.stock.json` → 0; registration loop confirmed at `vice-proxy.ts:3287-3288` via `buildViceTool()`, distinct from the manifest loop. |
...
|------|-----|-----|--------|---------|
| `vice-proxy.ts` | `anno-tools.ts` | `buildViceTool()` loop over `ANNO_TOOL_DEFINITIONS` | ✓ WIRED | Confirmed at source; never `buildBackendAwareTool()`. |
| `anno-tools.ts` | `anno-mcp-client.ts` | dynamic `import()` inside the runner | ✓ WIRED | Confirmed; registration itself spawns no child (dynamic import). |
| `anno-enum-gen.ts` | `anno-tools.ts` | `create_project_enum`/`apply_enum_usage`/`save_project` via `runAnnoTool()` | ✓ WIRED | Live-tested end to end via the `gen-enums` CLI verb. |
| `anno-symbols.ts` | `stock-symbols.ts` | reused `parseViceLabelFile()` (exported for this reuse) | ✓ WIRED | Confirmed: zero second-parser regexes in `anno-symbols.ts` (grep `al\s` → 0 as the plan's own acceptance criterion requires). |
...
| `check-skill-tool-coverage.mjs` | `CURATED_ANNO_TOOLS` | anno_* extraction + non-vacuity floor (≥10) | ✓ WIRED | Live run: "anno_*: 10 distinct names extracted, all curated (CURATED_ANNO_TOOLS has 17 entries)." |

### Behavioral Spot-Checks (independently re-run by this verifier, not taken from SUMMARY.md)

| Behavior | Command | Result | Status |
|----------|---------|--------|--------|
| Curated surface round-trips a label across a fresh session | `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts anno-enum-gen.test.ts anno-symbol-roundtrip.test.ts absorbed-answer-key.test.ts anno-memmap-render.test.ts` | 70/70 pass | ✓ PASS |
| Criterion 3's pinned example reassembles under real ACME | `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-cli.test.ts` | 43/43 pass, test 35 named explicitly | ✓ PASS |
| `docs-linerefs.test.ts` citations still match live source | `node --test docs-linerefs.test.ts` | 3/3 pass | ✓ PASS |
| `anno_*` family absent from both manifests | `grep -c "anno_" tools-manifest.json tools-manifest.stock.json` | 0, 0 | ✓ PASS |
...
| No debt markers in phase-touched source | `grep -rnE '\b(TBD\|FIXME\|XXX)\b' anno-*.ts` (excluding tests) | no hits | ✓ PASS |
...
(sha256 comparison re-run, live `node --test` re-run against real the external analyser/ACME/VICE
...
five requirement IDs (`ANNO-10`, `ANNO-11`, `ANNO-13`, `ANNO-14`, `ANNO-15`) remain unchecked
