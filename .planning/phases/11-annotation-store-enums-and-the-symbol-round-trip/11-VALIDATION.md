...
| **Estimated runtime** | ~30 s without a real anno child; longer when `VICE_REQUIRE_ANNO=1` forces live legs |

**Availability gate (inherited, not invented).** Live-anno tests follow
`disasm-roundtrip.test.ts`'s established pattern: a module-scope `SKIP_REASON`,
exactly one never-skipped availability test, and `VICE_REQUIRE_ANNO=1` turning
absence of the real binary into a hard FAIL rather than a silent skip. D-11
already applied this to the external analyser; this phase inherits it.
...
- **After every task commit:** Run the module's own `node --test <module>.test.ts`.
  Do **not** force `VICE_REQUIRE_ANNO` locally — absence of the real binary
  SKIPS by the `SKIP_REASON` convention, and only an explicit
  `VICE_REQUIRE_ANNO=1` export turns absence into a failure.
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|--------|
| 11-01-T1 | 11-01 | 1 | ANNO-10 (folded todo 2, WR-02) | T-11-VACUOUS | D-07's `--vice` deny-by-construction guard is non-vacuous | unit + planted violation | `node --test anno-launch.test.ts` | ✅ |
| 11-01-T2 | 11-01 | 1 | ANNO-13 (folded todo 2, WR-04) | T-11-FALSESUCCESS-ACME | An ACME transcript containing a failure never reports a pass | unit, pinned fixtures | `node --test anno-verify.test.ts` | ✅ |
| 11-01-T3 | 11-01 | 1 | ANNO-10 | T-11-GATE-DRIFT | One D-11 availability gate; its hard-FAIL mode observed failing | unit + env-driven demo | `node --test anno-verify.test.ts` | ✅ |
| 11-02-T1 | 11-02 | 1 | ANNO-10 (WR-05/06) | T-11-D64-TRUNC, T-11-D64-NAME | A truncated `.d64` throws; a printed name is a selectable name | unit, synthetic images | `node --test anno-d64.test.ts` | ✅ |
| 11-02-T2 | 11-02 | 1 | ANNO-10 (WR-07) | T-11-RAW-GUESS | A wrong-size flat capture is refused, not reinterpreted | unit, real files | `node --test anno-cli.test.ts` | ✅ |
| 11-02-T3 | 11-02 | 1 | ANNO-10 (folded todo 1, WR-03) | T-11-HONESTY-BYPASS, T-11-PKG-CLOSURE | Dynamically-imported modules cannot fall out of `files[]`; the honesty exemption is scoped and bounded | CI scripts + planted violations | `node scripts/check-npm-packages.mjs` | ✅ |
| 11-03-T1 | 11-03 | 1 | ANNO-13 (D-34) | T-11-DOC-DANGLE | No document points at Phase 11 as `.vsf`'s home | **was** doc assertion (one-file grep) → **now** unit + planted violation | `node --test docs-dangling-refs.test.ts` | ✅ (**audit-repaired** — see Audit 2026-08-21) |
| 11-03-T2 | 11-03 | 1 | ANNO-14 (D-35), ANNO-13 (D-22) | T-11-DOC-OVERCLAIM | The `--export_lbl` claim is verified and scoped to 0.9.20 + this fixture | doc assertion + existing suite | `cd .claude/mcp/vice && npm run test:automated` | ✅ |
| 11-03-T3 | 11-03 | 1 | ANNO-10, ANNO-14 | T-11-DOC-DRIFT | CLAUDE.md's cited line numbers match the source | unit + planted violation | `node --test docs-linerefs.test.ts` | ✅ |
| 11-04-T1 | 11-04 | 2 | ANNO-14, ANNO-15 (D-28) | T-11-VICE, T-11-PATH-XLATE | `--import_lbl` cannot be built without `--mcp-server-stdio`; no anno module imports `hostpath.ts` | unit, exact-argv | `node --test anno-launch.test.ts hostpath-consumers.test.ts` | ✅ |
| 11-04-T2 | 11-04 | 2 | ANNO-10 | T-11-HANG, T-11-PHANTOM-DEP | Five named client failure modes measured; no direct SDK import | integration vs stub servers | `node --test anno-mcp-client.test.ts` | ✅ |
| 11-04-T3 | 11-04 | 2 | ANNO-10, ANNO-14, ANNO-15 | T-11-FALSESUCCESS, T-11-DEMUX | A save is never reported without an independent re-read; responses correlate on `id` | stubs + live real child, gated | `VICE_REQUIRE_ANNO=1 node --test anno-mcp-client.test.ts` | ✅ |
| 11-05-T1 | 11-05 | 3 | ANNO-10, ANNO-11 | T-11-BATCH, T-11-UNCURATED, T-11-PATH-ESCAPE, T-11-D32 | The curated gate refuses any name outside the D-18 set, including nested batch names, before any spawn | unit + integration, gated | `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` | ✅ |
| 11-05-T2 | 11-05 | 3 | ANNO-10 | T-11-PATH-XLATE | No `anno_*` runner reaches `forwardToVice()`/`rewriteArguments()`; family in neither manifest | structural + proxy handshake | `node --test stock-dispatch.test.ts vice-proxy.test.ts` | ✅ (structural half in CI; handshake half manual — see audit note) |
| 11-05-T3 | 11-05 | 3 | ANNO-10 | T-11-PROSE | An `anno_*` name in skill prose must exist and be curated | CI script + planted violation | `node scripts/check-skill-tool-coverage.mjs` | ✅ |
| 11-06-T1 | 11-06 | 4 | ANNO-13 (D-22) | T-11-REGBITS-PROSE, T-11-GEN-DRIFT | Generated identifiers are legal ACME; the table is digest-pinned to `memmap.json` | unit + drift guard | `node --test anno-regbits.test.ts` | ✅ |
| 11-06-T2 | 11-06 | 4 | ANNO-13 (D-20, D-23) | T-11-ENUM-NAME, T-11-SILENT-CAP, T-11-MISBIND, T-11-GLOBAL-WRITE | No unsanitized identifier reaches `create_project_enum`; coverage reported, never implied | unit + property + zero-spawn refusal | `node --test anno-enum-gen.test.ts` | ✅ |
| 11-06-T3 | 11-06 | 4 | ANNO-13 (criterion 3) | T-11-ENUM-NAME | `lda #$1b`/`sta $d011` renders semantically in the ACME export and reassembles | integration (live anno + real ACME), gated | `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-enum-gen.test.ts anno-cli.test.ts` | ✅ |
| 11-07-T1 | 11-07 | 4 | ANNO-10 | — | The recon subject is reproducible from source | external oracle (real ACME byte-compare), now **mechanized + CI-gated** | `VICE_REQUIRE_ACME=1 node --test absorbed-answer-key.test.ts` | ✅ (**audit-mechanized** — see Audit 2026-08-21) |
| 11-07-T2 | 11-07 | 4 | ANNO-10 (criterion 1, session A) | T-11-LEAK | Findings persist and re-read in a fresh session | integration (live anno) + committed artifact | `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` | ✅ |
| 11-07-T3 | 11-07 | 4 | ANNO-10 (criterion 1, the question) | T-11-GUESSABLE, T-11-SEAL-DRIFT | The answer key cannot drift from the answer it seals | unit (seal recomputation) | `node --test absorbed-answer-key.test.ts` | ✅ |
| 11-08-T1 | 11-08 | 5 | ANNO-14, ANNO-15 | T-11-LBL-PARSER-DUP, T-11-MERGE-DIVERGE | No third `al C:` parser; `vice_symbols_load` stays replace-not-merge | typecheck + source assertions | `npx tsc --noEmit -p tsconfig.json` | ✅ |
| 11-08-T2 | 11-08 | 5 | ANNO-15 (criterion 4, mechanism) | T-11-IMPORT-DISCARD, T-11-LBL-SIZE | `--import_lbl` is never reported as success without a post-save read-back; the discard trap is pinned | integration (live anno), gated | `VICE_REQUIRE_ANNO=1 node --test anno-symbol-roundtrip.test.ts` | ✅ |
| 11-08-T3 | 11-08 | 5 | ANNO-14, ANNO-15 | T-11-LBL-SIZE | The label-file ceilings survive the CLI route | CLI integration, gated | `VICE_REQUIRE_ANNO=1 node --test anno-cli.test.ts` | ✅ |
| 11-09-T1 | 11-09 | 5 | ANNO-10, ANNO-11 (criterion 1, session B) | T-11-CONTEXT-BLEED | A prose-blind session answers from the store alone | human-witnessed transcript (recorded artifact) | `grep -q "canonical answer" SESSION-B-ANSWER.md` | ✅ |
| 11-09-T2 | 11-09 | 5 | ANNO-10 | T-11-RETROFIT, T-11-VACUOUS-CHECK | A mismatch is reported, never repaired; a missing answer FAILS rather than skips | unit (seal comparison) | `node --test absorbed-answer-key.test.ts` | ✅ |
| 11-10-T1 | 11-10 | 6 | ANNO-10 (D-25) | T-11-GRADE-TYPO, T-11-SECOND-STORE | A typo'd grade prefix fails instead of degrading to ungraded | unit, must-fail-on-typo | `node --test anno-confidence.test.ts` | ✅ |
| 11-10-T2 | 11-10 | 6 | ANNO-10 (D-24, D-27) | T-11-GEN-EDIT, T-11-PLACEHOLDER, T-11-SKILLPATH | The memory map is generated; a hand edit or a store change is detected; no placeholder is emitted | golden output + drift guard, gated | `VICE_REQUIRE_ANNO=1 node --test anno-memmap-render.test.ts` | ✅ |
| 11-10-T3 | 11-10 | 6 | ANNO-10 | T-11-GEN-EDIT | `render-memmap --check` exits non-zero on drift | CLI integration, gated | `VICE_REQUIRE_ANNO=1 node --test anno-cli.test.ts` | ✅ |
| 11-11-T1 | 11-11 | 6 | ANNO-14 (criterion 4, outbound) | T-11-FLAG-ORDER, T-11-SINGLE-CLIENT | A store label resolves a live address on genuine stock; `-default` precedes `-binarymonitor` | live stock `x64sc` walkthrough | `grep -q "al C:" outbound.lbl` | ✅ |
| 11-11-T2 | 11-11 | 6 | ANNO-15 (criterion 4, the loop) | T-11-TWO-DUMPS, T-11-NAME-INJECT | The inbound name is proven absent before the live discovery | live walkthrough + human-check | `grep -q "absent before" WALKTHROUGH.md` | ✅ |
| 11-11-T3 | 11-11 | 6 | ANNO-14, ANNO-15 (BACK-02 standing gate) | T-11-OVERCLAIM | The fork is verified unregressed; no gate is claimed without quoted output | regression gate + packaging validation | `npm run test:automated && npm run typecheck && npm run smoke && node scripts/check-npm-packages.mjs` | ✅ |
| 11-12-T1 | 11-12 | 7 | ANNO-10, ANNO-11 | T-11-PROSE-FAKE-TOOL, T-11-TEMPLATE-HANDFILL | Every `anno_*` name in prose exists and is curated; the template offers nothing to hand-fill | CI prose gates + doc assertions | `node scripts/check-skill-tool-coverage.mjs && node scripts/check-skill-fork-honesty.mjs` | ✅ |
| 11-12-T2 | 11-12 | 7 | ANNO-13 | T-11-PROSE-OVERCLAIM | The memmap coverage gap is stated, not glossed | CI prose gate | `node scripts/check-skill-fork-honesty.mjs` | ✅ |
| 11-12-T3 | 11-12 | 7 | ANNO-10 | T-11-PROSE-FAKE-TOOL, T-11-STALE-TARBALL | The reference floor can fail; the installer's generated copy carries the edits | CI script + planted violation + packaging | `node scripts/check-skill-tool-coverage.mjs && node scripts/check-npm-packages.mjs` | ✅ |
...
- [x] The shared D-11 availability gate, `anno-test-gate.ts` — **plan 11-01 (wave 1)**; the four new
      test files import it instead of hand-copying `probeAnno()` a sixth time.
- [x] `.claude/mcp/vice/anno-tools.test.ts` (ANNO-10, ANNO-11) — **plan 11-05, task 1**.
- [x] `.claude/mcp/vice/anno-mcp-client.test.ts` (the MCP-client seam, all five named failure modes:
...
- [x] `.claude/mcp/vice/anno-enum-gen.test.ts` (ANNO-13) — **plan 11-06, tasks 2-3**, plus
      `anno-regbits.test.ts` as the generated-artifact drift guard (task 1).
- [x] `.claude/mcp/vice/anno-symbol-roundtrip.test.ts` (ANNO-15) — **plan 11-08, task 2**.
- [x] Extend `.claude/mcp/vice/anno-launch.test.ts` for the new argv builders (ANNO-14) — **plan
...
**WR-02** — `anno-launch.test.ts`'s `stripCommentLines()` fails to close a block
...
D-16 makes `anno-launch.ts` the only sanctioned spawn path, which is exactly
...
`the external analyser 0.9.20` and real ACME 0.97 "Zem" are both present, so **no gated leg skipped
...
- `:66` — `ANNO-09`'s own requirement text: "`.vsf` is dropped from this requirement's input
  set and deferred to **Phase 11's `c64-ram-capture` extension**".
- `:140` — the `ANNO-08` fold entry: "`.vsf` **moves to Phase 11's `c64-ram-capture`
  extension**".
...
`absorbed-answer-key.test.ts`; the fixture the entire two-session proof was *derived from* was not.
...
**Resolution:** the byte-compare now lives in `absorbed-answer-key.test.ts` (7 tests → 10), the file
...
`disasm-roundtrip.test.ts` and `anno-cli.test.ts` each already carry a hand-copied one — a third
copy is precisely the divergence `anno-test-gate.ts` exists to prevent (the two existing copies
have *already* diverged: neither passes a `spawnSync` timeout, the seam does). The gate was
therefore added to `anno-test-gate.ts`, the sanctioned seam, and the new test imports it. The two
pre-existing copies were **not** migrated: `anno-cli.test.ts`'s gate semantics are load-bearing
...
`runAnnoTool()` reaching no VICE transport, the `annoDef.name` registration bypassing the
backend-aware seam by design, and every curated `anno_*` name being absent from *both*
...
| Command | Result |
|---------|--------|
| `node --test anno-launch.test.ts` | 21 pass |
| `node --test anno-verify.test.ts` | 12 pass |
| `node --test anno-d64.test.ts` | 14 pass |
| `node --test anno-cli.test.ts` | 43 pass |
| `node --test docs-linerefs.test.ts` | 3 pass |
| `node --test docs-dangling-refs.test.ts` | 4 pass *(new)* |
| `node --test anno-launch.test.ts hostpath-consumers.test.ts` | 28 pass |
| `node --test anno-mcp-client.test.ts` | 23 pass |
| `node --test anno-regbits.test.ts` | 13 pass |
| `node --test anno-enum-gen.test.ts` | 23 pass |
| `node --test absorbed-answer-key.test.ts` | 10 pass *(was 7)* |
| `node --test anno-confidence.test.ts` | 15 pass |
| `node --test anno-memmap-render.test.ts` | 18 pass |
| `node --test stock-dispatch.test.ts vice-proxy.test.ts` | 242 pass, 4 skipped |
| `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` | 27 pass |
| `VICE_REQUIRE_ANNO=1 node --test anno-symbol-roundtrip.test.ts` | 8 pass |
| `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-enum-gen.test.ts anno-cli.test.ts` | 66 pass |
| `node scripts/check-npm-packages.mjs` | OK — 73 / 35 files, 6 skills |
| `node scripts/check-skill-tool-coverage.mjs` | OK — 37 `vice_*`, 10 `anno_*` all curated |
...
production modules (`anno-cli.ts`, `anno-launch.ts`, `anno-symbols.ts`, `anno-project.ts`) and
...
**Full suite:** `VICE_REQUIRE_ACME=1 VICE_REQUIRE_ANNO=1 npm run test:automated` → **2021 tests,
...
11 gaps. Real `the external analyser 0.9.20` (`~/.cargo/bin`) and real ACME 0.97 "Zem" (`~/.local/bin`)
...
- `anno-symbols.ts` — a `LIBRARY-ONLY` marker on `regenerateAndReload()` recording that it has no
  production caller, plus the statement that ANNO-15 is satisfied through the
  `export-lbl → vice_symbols_load → anno_set_label_name → import-lbl` sequence, **not** through
  that wrapper. Newly guarded as a **biconditional** in `anno-symbol-roundtrip.test.ts` (8 → 15
  tests): zero callers requires the marker present, one or more requires it absent.
- `anno-project.ts` — the FLOW-02 comment fix; a phase pointer replaced by the backlog path.
- `anno-cli.ts` / `anno-launch.ts` — 11.1's own WR-09/WR-10/IN-06 fixes, carried by 11.1's rows.
...
|----------|----------|----------|
| Byte 10 of the sealed `evidence/criterion1/fixture/recon-subject.prg` flipped | `absorbed-answer-key.test.ts` FAILS | 9 pass, **1 fail** ✅ |
...
| `VICE_REQUIRE_ANNO=1 ANNO_BIN=/nonexistent-anno` | hard-FAIL, never skip | 25 pass, **1 fail**, 1 skipped ✅ |
| `ANNO_BIN=/nonexistent-anno`, env unset | named SKIP, never false pass | 26 pass, 0 fail, **1 skipped** ✅ |
...
|---------|--------|--------------|
| `node --test anno-launch.test.ts` | 26 pass | 21 → 26 |
| `node --test anno-verify.test.ts` | 12 pass | unchanged |
| `node --test anno-d64.test.ts` | 16 pass | 14 → 16 |
| `node --test anno-cli.test.ts` | 54 pass | 43 → 54 |
| `node --test docs-linerefs.test.ts` | 3 pass | unchanged |
| `node --test docs-dangling-refs.test.ts` | 8 pass | 4 → 8 |
| `node --test anno-launch.test.ts hostpath-consumers.test.ts` | 37 pass | 28 → 37 |
| `node --test anno-mcp-client.test.ts` | 23 pass | unchanged |
| `node --test anno-regbits.test.ts` | 13 pass | unchanged |
| `node --test anno-enum-gen.test.ts` | 23 pass | unchanged |
| `node --test absorbed-answer-key.test.ts` | 10 pass | unchanged |
| `node --test anno-confidence.test.ts` | 15 pass | unchanged |
| `node --test anno-memmap-render.test.ts` | 18 pass | unchanged |
| `node --test stock-dispatch.test.ts vice-proxy.test.ts` | 246 pass, 4 skipped | 242 → 246 |
| `VICE_REQUIRE_ANNO=1 node --test anno-tools.test.ts` | 27 pass | unchanged |
| `VICE_REQUIRE_ANNO=1 node --test anno-symbol-roundtrip.test.ts` | 15 pass | 8 → 15 |
| `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-enum-gen.test.ts anno-cli.test.ts` | 77 pass | 66 → 77 |
| `VICE_REQUIRE_ANNO=1 node --test anno-memmap-render.test.ts` | 18 pass | unchanged |
| `VICE_REQUIRE_ANNO=1 node --test anno-cli.test.ts` | 54 pass | 43 → 54 |
| `VICE_REQUIRE_ACME=1 node --test absorbed-answer-key.test.ts` | 10 pass | unchanged |
...
| `node scripts/check-skill-tool-coverage.mjs` | OK — 37 `vice_*`, 10 `anno_*` all curated, **7/7 CLI verbs resolved** | verb check added by 11.1-02 |
...
The 2026-08-21 audit deliberately left `disasm-roundtrip.test.ts` and `anno-cli.test.ts` on their
hand-copied ACME gates rather than re-cutting the ground under evidence it was auditing. Re-checked:
both still hand-roll their gate, `absorbed-answer-key.test.ts` still imports the
`anno-test-gate.ts` seam, and
`.planning/todos/pending/2026-08-21-migrate-hand-copied-acme-gates-to-anno-test-gate.md` still
...
`docs-deferred-ledger.test.ts`, `docs-review-disposition.test.ts`, `spawn-seam.test.ts`,
`anno-verb-coverage.test.ts` and `skill-honesty-checks.test.ts`. Those guards are all green in the
...
- [x] No gated leg skipped silently — real `the external analyser 0.9.20` and real ACME 0.97 both present
