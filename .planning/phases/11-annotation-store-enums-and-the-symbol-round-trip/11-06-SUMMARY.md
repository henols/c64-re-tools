...
plan: 06
subsystem: infra
tags: [the external analyser, memmap, enum-generation, acme, code-generation]

# Dependency graph
requires:
  - phase: 11-05
    provides: "anno-tools.ts: runAnnoTool()/CURATED_ANNO_TOOLS/resolveStorePath() -- the curated allow-list runner every mutating/read call in this plan routes through"
  - phase: 11-02
...
provides:
  - "anno-regbits-gen.ts: buildRegBits()/buildRegBitsDocument(), the generated address->bit-name table with OVERRIDES fixing two OCR-damage cases and six memmap-absent synthetic registers"
  - "anno-regbits.json: the committed, banner-marked (generator + memmap.json sha256, no timestamp) generated artifact 35 registers deep"
  - "anno-enum-gen.ts: variantNameFor()/assertLegalAcmeIdentifier()/pairImmediateLoadsToStores()/createOrUpdateEnum()/generateEnums() -- decode-to-name, sanitize, adjacent-pair, install, and the coverage report"
  - "the gen-enums CLI verb (anno-cli.ts), and criterion 3's acceptance test proving the pinned lda #$1b/sta $d011 example renders correctly in a real ACME export and reassembles"
...
tech-stack:
  ...
  patterns:
    ...
    - "Client-side register/mode narrowing after an exact-mnemonic search, replacing a combined mnemonic+operand regex that the external analyser's own search_disassembly does not support (mnemonic and operand are matched independently, never concatenated) -- measured live, not assumed from RESEARCH.md"

key-files:
  created:
    - .claude/mcp/vice/anno-regbits-gen.ts
    - .claude/mcp/vice/anno-regbits.json
    - .claude/mcp/vice/anno-regbits.test.ts
    - .claude/mcp/vice/anno-enum-gen.ts
    - .claude/mcp/vice/anno-enum-gen.test.ts
  modified:
...
key-decisions:
  ...
  - "createOrUpdateEnum()'s documented precedence: try anno_create_project_enum first; only on the external analyser's own 'already exists' failure (validate_new_enum_name, app_state.rs:443-457 -- there is no upsert) fall back to anno_update_project_enum, which replaces the variant map wholesale"
  - "Sanitization runs entirely client-side (assertLegalAcmeIdentifier on the enum name and every variant name) BEFORE any anno_create_project_enum/_update_ call -- proven zero-spawn via a spy binary, mirroring anno-tools.test.ts's own D-33 smuggling proof"
...
requirements-completed: [ANNO-13]
...
- `anno-regbits-gen.ts` reads `memmap.json`'s 29 structured `bits` entries, normalises single/descending/ascending bit ranges, and mechanically derives a legal ACME identifier per field -- throwing (never placeholdering) when a description is unmappable and uncovered by `OVERRIDES`. `OVERRIDES` fixes two real OCR-damage cases (`$D011` bit 4's "O = Blank" letter-O-for-zero, `$DD0D`'s "Read NMls") and supplies six registers `memmap.json`'s own `io` parser never produced `bits` for at all (`$D015`/`$D017`/`$D01A`/`$D01B`/`$D01C`/`$D01D`), every entry carrying its own WHY comment.
- `anno-regbits.json` is the committed, banner-marked artifact (35 registers, no timestamp) -- the drift guard re-runs `buildRegBits()` in memory and deep-equals it against the committed file; regenerating leaves `git diff` clean.
- `anno-enum-gen.ts`'s `variantNameFor()` decodes `$D011` value `0x1b` to exactly `YSCROLL3_ROW25_SCREENON_TEXT` (the pinned target), and is proven injective across all 256 values for `$D011`/`$D016`/`$D018`/`$D015`.
...
- Corrected RESEARCH.md's own Pattern 2 assumption via a live measurement this session: `anno_search_disassembly` matches `mnemonic` and `operand` INDEPENDENTLY (`state/search.rs:309-313`), never as one concatenated string, so a combined `"^sta \$(...)"`-style query never matches either field. `pairImmediateLoadsToStores()` instead queries exact mnemonics (`"^lda$"`/`"^sta$"`) and narrows to known registers client-side -- still derived from `anno-regbits.json`'s own keys, never a second hardcoded list.
...
1. **Task 1: anno-regbits-gen.ts and the committed curated table (D-22)** - `8b54299` (feat)
2. **Task 2: anno-enum-gen.ts -- value decoding, the adjacent-pair pass, sanitization and the coverage report** - `e83f697` (feat)
3. **Task 3: the gen-enums verb and criterion 3's acceptance test against the ACME export** - `27b6a32` (feat)

## Files Created/Modified

- `.claude/mcp/vice/anno-regbits-gen.ts` - The re-runnable generator: `buildRegBits()`/`buildRegBitsDocument()`, `OVERRIDES`, mechanical identifier derivation, overlap dedup
- `.claude/mcp/vice/anno-regbits.json` - The committed, banner-marked generated table (35 registers)
- `.claude/mcp/vice/anno-regbits.test.ts` - 13 tests: drift guard, digest pin, identifier legality, override-supplied-register presence, WHY-comment coverage, two non-vacuity proofs
- `.claude/mcp/vice/anno-enum-gen.ts` - `variantNameFor`, `assertLegalAcmeIdentifier`, `pairImmediateLoadsToStores`, `createOrUpdateEnum`, `generateEnums`
- `.claude/mcp/vice/anno-enum-gen.test.ts` - 23 tests: pinned target, 256-value injectivity x4, sanitization refusals + zero-spawn proof, pairing arithmetic, truncation wording, live-gated pairing/generation/precedence
- `.claude/mcp/vice/anno-cli.ts` - `gen-enums` verb (`cmdGenEnums`/`parseGenEnumsArgs`), extended `USAGE`
- `.claude/mcp/vice/anno-cli.test.ts` - gen-enums CLI tests (unknown option, missing path, nonexistent file) plus criterion 3's full acceptance test
- `.claude/mcp/vice/package.json` - Added `anno-regbits-gen.ts`/`anno-regbits.json`/`anno-enum-gen.ts` to `files[]`
...
- **`createOrUpdateEnum()`'s create-then-update precedence**, decided and documented per the plan's own instruction: create first, fall back to update only on the external analyser's own "already exists" failure.
- **No standalone final `anno_save_project` call in `generateEnums()`** -- see Deviations #4 below; every mutating call already persists via `anno-tools.ts`'s own per-call auto-save (plan 11-05), so an additional explicit save would only ever observe an unchanged hash and report a spurious `AnnoSaveNotPersistedError`.

## Deviations from Plan

### Auto-fixed Issues

**1. [Rule 1 - Bug] RESEARCH.md's Pattern 2 combined-regex assumption does not match measured the external analyser behavior**
- **Found during:** Task 2, implementing `pairImmediateLoadsToStores()`
- **Issue:** RESEARCH.md's own worked example queried `` `^sta \$(${knownRegisterHexAlternation})` `` expecting it to match a "mnemonic + operand" combined string. Live measurement against a real analyser 0.9.20 child (`state/search.rs:309-313`) showed `mnemonic` and `operand` are matched INDEPENDENTLY (OR semantics) -- neither field alone ever starts with `"sta $d011"`, so the combined-regex query always returned zero rows.
- **Fix:** Query exact mnemonics (`"^lda$"`/`"^sta$"`, case-insensitive via the server's own `(?i)` prefix), then filter client-side: Pass 1 keeps only rows whose `operand` starts with `#` (immediate mode); Pass 2 keeps only rows whose `operand` (normalised to a `$XXXX` key) is a member of the register set derived from `anno-regbits.json`'s own keys -- preserving D-23's "the register list is derived from the table, never a second hardcoded list" requirement, just via a different mechanism.
- **Files modified:** `.claude/mcp/vice/anno-enum-gen.ts` (documented at length in the module's own header, under "MEASURED MECHANISM FACTS")
...
- **Files modified:** `.claude/mcp/vice/anno-enum-gen.ts`
...
- **Issue:** `anno-tools.ts`'s `resolveStorePath()` (T-11-PATH-ESCAPE) requires every `.regen2000proj` path to resolve inside the workspace root. Tests built with `mkdtempSync(join(os.tmpdir(), ...))` (a natural default) were refused with `AnnoStorePathError`, since `/tmp/...` is outside the repo.
- **Fix:** Every gated test now uses a workspace-local temp directory (`mkdtempSync(join(HERE, ".xyz-test-..."))`), mirroring `anno-tools.test.ts`'s own existing convention for its live-gated tests.
- **Files modified:** `.claude/mcp/vice/anno-enum-gen.test.ts`, `.claude/mcp/vice/anno-cli.test.ts`
- **Verification:** All previously-failing gated tests pass under `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1`.
- **Committed in:** `e83f697` and `27b6a32`

**4. [Rule 1 - Bug, plan-text clarification] The literal "then anno_save_project through the curated runner's verified-save route" step is not a separate call**
- **Found during:** Task 2, designing `generateEnums()`'s own persistence step
- **Issue:** Plan 11-05's own `runAnnoTool()` already auto-saves internally after every mutating call (a plain, non-hash-verified save, by design -- see 11-05-SUMMARY's own Deviations #2). A literal standalone `runAnnoTool("anno_save_project", ...)` call issued AFTER a sequence of already-auto-saving `anno_create_project_enum`/`anno_apply_enum_usage` calls would find nothing new pending and throw `AnnoSaveNotPersistedError` -- documented in 11-05-SUMMARY.md as the EXPECTED (not buggy) shape of exactly that call sequence ("rarely required standalone").
...
- **Files modified:** `.claude/mcp/vice/anno-enum-gen.ts` (documented in `generateEnums()`'s own doc comment)
...
- **A genuine limitation, documented rather than silently worked around (not a bug fixed, a boundary named):** once `anno_apply_enum_usage` has been applied to an address, that instruction's own `anno_search_disassembly` operand text switches from the raw immediate (`"#$1b"`) to the applied enum reference (`"#D011.YSCROLL3_..."`, the dot form). A literal `generateEnums()`-then-`generateEnums()`-again re-run over the SAME already-applied instructions therefore finds nothing left to pair on its second pass (a safe no-op, not a crash -- `parseImmediateOperand` correctly refuses the enum-reference text and the miss costs nothing, per D-23's own posture). The create-then-update PRECEDENCE itself is still fully proven (via a direct `createOrUpdateEnum()` call with a hand-built variants map, isolated from the pairing pass), but a literal end-to-end double-run over the identical fixture does not exercise it. Filed as a known follow-up: closing this fully would need either a pre-pass that clears every existing enum usage before re-pairing, or a currently out-of-scope tool (`anno_read_region`, excluded by D-18) to read a raw byte directly regardless of its current enum-usage state.

## User Setup Required

None -- no external service configuration required. `the external analyser 0.9.20` (`~/.cargo/bin/analyser`) and ACME 0.97 (`~/.local/bin/acme`) were already installed on this host from prior phase work; every gated test in this plan ran against them directly.

## Non-Vacuity / Drift Transcripts (recorded per this plan's own `<verification>` requirement)

**Memmap-digest drift transcript** (`anno-regbits.test.ts`, planted violation -- a byte appended to a SCRATCH copy of `memmap.json`, the real file never touched):
```
...
**Zero-spawn injection refusal** (`anno-enum-gen.test.ts`): a variant name `"BAD\nNAME = $00"` passed to `createOrUpdateEnum()` with `ANNO_BIN` pointed at a spy binary that writes a marker file and exits 1 if ever invoked -- the call rejected with `/not a legal ACME identifier/` and the marker file was never created (`existsSync(marker) === false`), confirming sanitization happens entirely client-side before any child process spawn.

**Coverage numbers from the criterion-3 acceptance run** (`anno-cli.test.ts`, `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1`):
```
...
**Separator observed on each surface, recorded live this session** (`anno-cli.test.ts` diagnostics):
```
...
separator comparison: export=underscore, live=dot -- RESEARCH.md's dot-vs-underscore finding still holds on this the external analyser version
```
RESEARCH.md's Assumption A2 (the dot-vs-underscore discrepancy is version-scoped, not permanent) is confirmed unchanged on the installed `the external analyser 0.9.20`.

## Verification Evidence

- `cd .claude/mcp/vice && node --test anno-regbits.test.ts` -> 13/13 pass.
- `cd .claude/mcp/vice && node --test anno-enum-gen.test.ts` -> 19/19 pass locally (no live binary required beyond the always-run gate test); `VICE_REQUIRE_ANNO=1 node --test anno-enum-gen.test.ts` -> 23/23 pass (4 additional live-gated tests run).
- `cd .claude/mcp/vice && VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-enum-gen.test.ts anno-cli.test.ts` -> 47/47 pass.
- `node anno-regbits-gen.ts && git diff --exit-code -- anno-regbits.json` -> clean (byte-identical regeneration).
...
- `node scripts/check-skill-tool-coverage.mjs` -> OK (anno_*: 0 distinct names extracted from skills yet -- expected, no skill mentions an anno_* name until plan 11-09).
- `node scripts/check-skill-fork-honesty.mjs` -> OK.
- Without `VICE_REQUIRE_ANNO`/`VICE_REQUIRE_ACME` set and with `ANNO_BIN`/`ACME_BIN` pointed at nonexistent paths: `node --test anno-cli.test.ts` -> 21 pass, 3 SKIP (never fail), exit 0.

## Next Phase Readiness

- `anno-regbits.json`'s 35-register table and `anno-enum-gen.ts`'s `variantNameFor()`/`registerKeyFor()` are available for any later plan needing address->semantic-name decoding (e.g. plan 11-08's memory-map renderer).
- `gen-enums` is reachable as `vice-mcp anno gen-enums <project> [--max-results N]` through all three distribution routes this repo already resolves the `anno` subcommand from.
...
- No blockers. `the external analyser` and `ACME` both remain available and live-verified on this host for any future plan needing either oracle.
...
- FOUND: .claude/mcp/vice/anno-regbits-gen.ts
- FOUND: .claude/mcp/vice/anno-regbits.json
- FOUND: .claude/mcp/vice/anno-regbits.test.ts
- FOUND: .claude/mcp/vice/anno-enum-gen.ts
- FOUND: .claude/mcp/vice/anno-enum-gen.test.ts
- FOUND: .claude/mcp/vice/package.json
- FOUND: .claude/mcp/vice/anno-cli.ts
- FOUND: .claude/mcp/vice/anno-cli.test.ts
- FOUND commit: 8b54299 (feat(11-06): anno-regbits-gen.ts -- the generated address-to-bit-name table (D-22))
- FOUND commit: e83f697 (feat(11-06): anno-enum-gen.ts -- value decoding, adjacent pairing, sanitization and the coverage report (D-20/D-23))
