...
|----------|-------------|------------|-------------------|
| Core Value verdict is *weighed*, not a bookkeeping edit | CORE-01 | The judgement itself is not mechanizable; only its artifacts are | Confirm the `## Core Value` entry carries (a) an ISO date, and (b) a citation of the specific evidence weighed by name (e.g. Phase 11's sealed-question test, the symbol round-trip, `anno_*`). A paragraph reading only "confirmed, no change needed" fails this check. Mirrors `docs-fork-decision.test.ts`'s requirement that FORK-01 name `KEYBOARD_MATRIX_SET` verbatim. |
