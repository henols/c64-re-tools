...
requires:
  - phase: 11
    provides: "ANNO-10's two-session sealed-question test, ANNO-14/ANNO-15's symbol round trip, and ANNO-01's structural VICE-incapability guard — the evidence this plan's verdict weighs"
  - phase: 14
...
key-decisions:
  ...
  - "The keep-dated verdict was escalated to a human at plan 17-02 task 1's gate=\"blocking-human\" checkpoint. The checkpoint was rendered and the operator answered 298 seconds later in free text 'you decide' rather than selecting an option; the orchestrator then selected keep-dated. Attendance and delegation are on the record; comprehension is not evidenced by any artifact and is not claimed (corrected 2026-08-23 by plan 17-04, gap G-17-1)."
...
**How the verdict was reached, stated precisely and not overstated:** the checkpoint was escalated to the human operator at the `gate="blocking-human"` gate. The checkpoint was rendered — the terminal output carried the current Core Value text and the counted evidence columns (six FOR-restating items, five AGAINST), while the decision prompt itself carried only the two option labels plus one strongest-argument line each. 298 seconds later the operator answered in free text — `you decide` — rather than choosing either label. The orchestrator then selected `keep-dated` on the reasoning recorded in PROJECT.md's new entry (decisive reason: the outlives-the-session property belongs to a component — the external analyser — structurally incapable of driving VICE at all (`ANNO-01`); v0.4.0 produced zero new evidence on the question; the pre-drafted restatement is a conjunction naming two structurally separate subsystems in one sentence). Attendance and delegation are on the record; comprehension is not evidenced by any artifact and is not claimed (corrected 2026-08-23 by plan 17-04, gap `G-17-1`).

**Acceptance criterion honesty note (explicitly required by the checkpoint resolution instructions):** Task 1's acceptance criterion reads "Exactly one of `restate` or `keep-dated` is selected, **by a human**." This was satisfied in the *attended-escalation* sense — a human saw the `gate="blocking-human"` checkpoint and responded to it 298 seconds later in free text (`you decide`) — but **not** in the *literal* sense of the human personally selecting the option word. The human delegated the choice; the orchestrator made the selection. Comprehension is not evidenced by any artifact and is not claimed (corrected 2026-08-23 by plan 17-04, gap `G-17-1`). This is recorded here and inside PROJECT.md's new entry itself precisely so a later verifier reading that criterion literally has the facts in front of them, rather than a record that reads as more human-decided than it was. This phase is fundamentally about audit integrity; overstating human involvement here would be the exact failure mode CORE-01 exists to prevent.

**Correction note (2026-08-23, plan 17-04, gap `G-17-1`).** The narrative this section originally transcribed was supplied to this agent in its own spawn prompt rather than independently observed — this agent has no first-hand knowledge of the checkpoint exchange. That narrative asserted the operator's comprehension of the evidence as observed fact; comprehension is not evidenced by any artifact, so the claim was retracted and the passages above corrected to state only what the primary session transcript evidences (attendance, the free-text `you decide` answer, and the orchestrator's subsequent selection).
