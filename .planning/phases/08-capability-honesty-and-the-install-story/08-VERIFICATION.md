...
| 3 | A user installs the plugin and a working VICE from a package manager by following the documentation, with the backend choice and its consequences stated | ? PARTIAL / HUMAN NEEDED (recorded, approved) | README.md's new "Installing VICE, and choosing a backend" section states the per-ecosystem install commands/versions (live-checked 2026-08-18), the `VICE_BACKEND` switch and its consequences (both processes that must agree, named explicitly per CR-02), the two fork-required tools, and a link to `docs/tool-support.md`. The install-only half of the one human-UAT test was run live in a fresh `debian:trixie` container and caught a real defect (Debian ships `vice` in `contrib`, fixed commit `69e9092`/`5d98504`). The interactive Claude-Code-session half of the walkthrough (driving `c64-ram-capture` end to end against the installed stock VICE) was **not** run — `08-HUMAN-UAT.md` records `status: pending` for it, with explicit human approval to close on that basis. This is the phase's one recorded, approved partial. |
