...
Move the capabilities that a prefix-driven `r2000-*` deletion would silently
take with it out from under that prefix; record a capability-or-glue
classification for every `r2000-*` module **before** anything is deleted; and
prove the whole thing was a move rather than a change.

**Zero `r2000` modules are deleted in this phase.** Deletion is Phase 32
...
(Phase 29), any export work (Phase 30), any repointing of `r2000-*` consumers at
...
- `src/mcp/vice/r2000-test-gate.ts` — the module being split; its header states
...
- `src/mcp/vice/r2000-cli.test.ts:28,679-683` — importer
- `src/mcp/vice/r2000-answer-key.test.ts:219-231,259` — importer
- `src/mcp/vice/r2000-verify.test.ts:175-194` — the existing `files[]`-absence
  assertion, which stays as-is
- `src/mcp/vice/hostpath-consumers.test.ts:204` — enumerates
  `r2000-test-gate.ts` by literal name; check whether `acme-gate.ts` belongs
- `src/mcp/vice/r2000-spawn-seam.test.ts:20-34,53,158-195` — the `codeOnly()` /
  `shippedTsModules()` origin, and two literal `r2000-test-gate.ts` mentions
  (`:23`, `:162`)
- `.planning/todos/completed/2026-08-21-migrate-hand-copied-acme-gates-to-r2000-test-gate.md`
  — the record of why the gate was consolidated in the first place

### The coverage boundary (SEAM-03)
- `src/mcp/vice/r2000-coverage.ts:184-219` — the store input shapes and their
  Rust `Display` doc comments
- `src/mcp/vice/r2000-coverage.ts:1615-1690` — `DerivedClass`,
  `storeBlockTypeAt()`, `classFromBytes()`, `classFromStore()` — the boundary
- `src/mcp/vice/r2000-coverage.ts:1972-1990` — the **third** comparison site
  (divergence sub-report) that the requirement's "two functions" does not name
- `src/mcp/vice/r2000-coverage.ts:59-70,116-135` — the header's independence
...
- `src/mcp/vice/r2000-coverage.test.ts:604-615` — the `COV-01`/`COV-02`
  boundary test that must still pass across the new boundary
- `src/mcp/vice/r2000-confidence.ts` — supplies the grade tokens
...
- **`r2000-test-gate.ts`'s own header** is the model for `acme-gate.ts`'s header:
...
- **`r2000-coverage.test.ts:604`'s mass-rewrite test** is the shape D-13's
...
- **Test-only helpers must not match `*.test.*`** — `package.json`'s `test`
  ...
  collected as a test file. `r2000-test-gate.ts`'s own header records this.
...
- `.github/workflows/ci.yml` — no functional edit expected (D-03); comment
  accuracy only.
- `r2000-coverage.ts`'s `ReproducibilityInput` — the adapter sits between it and
