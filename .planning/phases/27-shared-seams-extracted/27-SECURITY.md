...
Phase 27 is a pure-extraction phase: modules move out from under an `anno`
...
| bytes side ↔ store side (internal, load-bearing) | `anno-coverage.ts` records that neither side may read the other's input. An *integrity* boundary for a measurement, enforced only by structure. | census input / store documentation |
...
| T-27-03-02 | Repudiation | A capability module left depending on a module a prefix deletion removes | high | mitigate | This plan **is** the mitigation (Correction C-1). Verified: `grep anno-project anno-coverage.ts` returns zero | closed |
...
(39 `vice-proxy.test.ts`, 5 `anno-session.test.ts`) are environmental, live in
