...
requires:
  - phase: 19-coverage-instrument
    provides: "anno-coverage.ts — the byte census, its bytes-versus-store independence axis, and the independence test this plan had to keep green across the new boundary"
  - phase: 27-shared-seams-extracted
...
provides:
  ...
  - "A coverage census whose entire contact with the annotation store is one import: no upstream block-type literal, no store-shape type declaration, no local block lookup survives in anno-coverage.ts"
...
affects: [phase-28-store-substitution, anno-coverage, anno-cli, npm-packaging]
...
key-decisions:
  ...
  - "AnnoBlockEntry moved and renamed BlockEntry; AnnoSymbol, AnnoComment and AnnoCrossReference deliberately stayed — only the block shape's doc comment documented the store vocabulary"
  - "anno-cli.ts's type-only import was SPLIT, not rewritten: both coverage imports survive, so the diff reads as an addition"
...
coverage:
  - id: D1
    ...
    verification:
      ...
      - kind: other
        ...
        status: pass
      - kind: other
        ref: "grep -cE 'from \"\\./(anno-|disasm-|stock-|vice|hostpath|containerpath)' src/mcp/vice/block-class.ts => 0"
        status: pass
    human_judgment: false
  - id: D2
    description: "The census's entire store contact is one import: no upstream block-type literal, no moved store-shape type, and no local lookup survive in anno-coverage.ts, and all three comparison regions plus both lookup sites speak the neutral vocabulary with unchanged arithmetic and counter names"
    requirement: SEAM-03
    verification:
      - kind: other
        ref: "grep -cE '\"(Code|Undefined|Byte)\"' src/mcp/vice/anno-coverage.ts => 0; grep -c 'AnnoBlockEntry' => 0; grep -c 'storeBlockTypeAt' => 0; grep -c 'from \"./block-class.ts\"' => 1"
        status: pass
      - kind: unit
        ...
        status: pass
      - kind: unit
        ref: "cd src/mcp/vice && node --test anno-coverage.test.ts anno-coverage-grammar.test.ts anno-cli.test.ts (0 failures)"
        status: pass
      - kind: other
  ...
  - id: D3
    ...
    verification:
      - kind: unit
        ref: "src/mcp/vice/anno-coverage.test.ts#substitutability: feeding the census a second, zero-overlap block vocabulary through the adapter leaves every census byte count exact and moves only the divergence sub-report"
        status: pass
      - kind: unit
        ref: "src/mcp/vice/anno-coverage.test.ts#substitutability: on an UNGRADED comment set every fromStore value moves while every fromBytes value holds -- the sharpest form of the bytes-versus-store claim"
        status: pass
      - kind: unit
        ref: "src/mcp/vice/anno-coverage.test.ts#idempotency: building the coverage report twice over the same fixture through the adapter yields deep-equal reports"
        status: pass
      - kind: other
        ref: "left-behind-site probe: raw-string comparison restored at the divergence loop => the non-vacuity assertion goes RED (observed, then restored; anno-coverage.ts byte-identical to d5b7e3c)"
        status: pass
    human_judgment: false
  - id: D4
  ...
  - id: D5
    ...
    verification:
      - kind: other
        ref: "sed -n '1,140p' anno-coverage.ts | grep -c 'computeDivergence()' => 0; grep -c 'NEVER derive any measure from the store' => 1; grep -c 'Neither side reads the other' => 1; grep -c 'block-class.ts' => 7"
        status: pass
      - kind: unit
...
- **`anno-coverage.ts`'s store contact reduced to one import.** `storeBlockTypeAt()` deleted (not wrapped), `AnnoBlockEntry` deleted and replaced by an import of `BlockEntry`, `classFromStore()` retyped to take `BlockClass | null`, and **all three comparison regions** (`classFromStore`'s two literals, the divergence loop's two) plus **both lookup sites** (the reproducibility comparison and the divergence loop) rewritten in the neutral vocabulary with counter names and arithmetic unchanged. `grep -cE '"(Code|Undefined|Byte)"'` on the census returns **0**.
...
- **Zero `anno` modules deleted or renamed.** `git diff --diff-filter=D --name-only 6c1f569..HEAD -- src/mcp/vice scripts | grep -c anno` is `0`.
...
- `src/mcp/vice/anno-coverage.ts` — store contact reduced to one import; `storeBlockTypeAt` and `AnnoBlockEntry` removed; `classFromStore` retyped; `blockClassifier` threaded (required on `ReproducibilityInput` and `computeDivergence`, optional-with-the-real-default on `CoverageOptions`); two header statements corrected; the divergence section header and two `DivergenceReport` doc comments re-expressed in the neutral vocabulary.
- `src/mcp/vice/anno-coverage.test.ts` — import repointed; `reportFor()` extended to forward an optional classifier with existing defaults untouched; five `computeReproducibility()` call sites supplied the now-required field; **203 lines added** carrying the substitutability section.
- `src/mcp/vice/anno-cli.ts` — the type-only coverage import **split** (both coverage imports survive) with `BlockEntry` taken from the boundary; two uses repointed.
...
- **Verification:** `node --test anno-coverage.test.ts` — 112 tests, 112 pass, 0 fail. Both substitutability tests pass by title.
...
- **Found during:** Task 2.
- **Issue:** Making `blockClassifier` required with no default (the plan's own instruction, and the point of the design) broke five existing `computeReproducibility()` call sites in `anno-coverage.test.ts` at typecheck.
...
- **Files modified:** `src/mcp/vice/anno-coverage.test.ts`
- **Verification:** `npm run typecheck` exits 0; `node --test anno-coverage.test.ts` 0 failures.
...
- **5 in `anno-session.test.ts`** (the plan-18-06 queue tests) — all `AnnoSpawnError: The external analyser was not found on PATH`. A genuine pre-existing gap, not a missing binary: every *other* external-analyser-dependent test in that file is wrapped by `skipReasonFor(...)` and SKIPs cleanly, while these five spawn a real child with no gate. The file was last modified in phase 18 and is untouched by phase 27; the failures reproduce in isolation.

Both are logged in `deferred-items.md` and deliberately **not** fixed — each lies outside this plan's `files_modified`, and the second belongs to the anno session/spawn family plan 27-04 owns.
...
| `node --test anno-coverage.test.ts` | 112 tests, 112 pass, 0 fail |
| plan gate: `node --test block-class.test.ts anno-coverage.test.ts anno-coverage-grammar.test.ts anno-cli.test.ts docs-dangling-refs.test.ts comment-phase-pointers.test.ts hop-chain-comments.test.ts assumption-label-discipline.test.ts spawn-seam.test.ts stock-dispatch.test.ts hostpath-consumers.test.ts` | 402 tests, **395 pass, 7 skip, 0 fail** |
...
| `test "$(git diff --diff-filter=D --name-only 6c1f569..HEAD -- src/mcp/vice scripts \| grep -c anno)" = "0"` | exit 0 — no `anno` module deleted |
| whole-glob `node --test '*.test.*'` | 2337 pass / 44 fail — all 44 in `vice-proxy.test.ts` (MANUAL_ONLY, needs a live host) and `anno-session.test.ts` (pre-existing ungated spawn), see Issues Encountered |
...
Observed with the probe planted (`node --test anno-coverage.test.ts` → **110 pass, 2 fail**):
...
The proof caught it on the non-vacuity assertion, and the structural supplement caught it independently — two detectors, and the primary one is the substitutability proof rather than the string grep. **Restored:** `git diff --stat -- anno-coverage.ts` prints nothing against `d5b7e3c`, i.e. the file is byte-identical to its committed state; `node --test anno-coverage.test.ts` back to 112/112.
...
- No blockers. `deferred-items.md` D-27-02-A (the five ungated queue tests) touches the anno session/spawn family plan 27-04 owns, and is the one item that could reasonably be picked up in-phase.
