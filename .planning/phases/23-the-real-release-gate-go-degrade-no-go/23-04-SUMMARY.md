...
plan: 04
subsystem: testing
tags: [evidence, the external analyser, analyzer, ghidra, dxa, capability-audit, proof-04]
...
coverage:
  - id: D1
    ...
    verification:
      ...
      - kind: other
        ...
        status: pass
    human_judgment: true
    rationale: "The mechanical checks prove the table's shape — every row present, exactly one disposition each. They cannot prove a disposition is the RIGHT one. Each replaced-by: is a claim about what dxa and Ghidra do relative to anno, and the load-bearing E6 row is an explicit both-directions argument. A human must read the arguments and accept or reject them; that is what criterion 4 exists to have judged."
  - id: D2
    ...
    verification:
      - kind: other
        ...
        status: pass
    human_judgment: true
    rationale: "Whether Ghidra's CONDITIONAL_JUMP genuinely records the same fact as anno's Branch, and whether the six store-side rows are honest replacements or deferred work, is a judgment about equivalence that no command can assert."
  - id: D3
  ...
  - id: D5
    description: "The audit was derived offline with no the external analyser process started, and 23-RESEARCH.md was not edited"
    requirement: "PROOF-04"
    verification:
      - kind: other
      ...
      - kind: other
        ...
        status: pass
    human_judgment: true
    rationale: "The git and test checks are mechanical, but 'no the external analyser process was started' is a claim about the whole transcript, not about any one command's output. Only a reader of the transcript can confirm the negative."
...
- **`follow_indirect_jumps` argued in both directions, as the plan required.** Where anno is stronger: it is declaration-driven and purely static, firing on any `JMP ($xxxx)` into a pre-typed `Address` block with no reachability and no constant-foldability requirement — where Ghidra's `COMPUTED_JUMP` simply produces nothing, silently. Where Ghidra is stronger: the one observed `COMPUTED_JUMP` (`082e -> 089a`) resolves a pointer the program *writes at runtime* into zero page, which anno structurally cannot reach (`$00fb` fails `is_internal` for any `.prg` based at `$0801`). The precondition anno requires is named explicitly and shown to be store state the milestone already commits to holding.
...
- **`E6 follow_indirect_jumps` is `replaced-by:` with a shape-mismatch note.** Rationale in full above. The decisive consideration: anno's route *requires* a block already typed `Address`, which is exactly the map dxa produces and the store holds — so the capability is relocated rather than lost, and reading a 16-bit pointer out of a declared address block is arithmetic, not analysis.
...
None — no external service configuration required. The one external input, the cargo registry copy of `external-analyser-core` 0.9.20, was already present on this host and is identified by sha256 in the evidence file so a later reader does not depend on the path.
