...
| Must not rewrite historical narration into false present tense (hop-chain guard) | 16-09 | HELD | The guard's own in-tree negative control (`r2000-answer-key.test.ts:34`) is confirmed seen-but-not-flagged; no historical-narration comment was altered. |
| Must not touch anything but the two scratch segment lists in `r2000-regbits.test.ts` | 16-09 | HELD (not independently re-diffed this round; consistent with the passing `r2000-regbits.test.ts` suite inside the full 2342-pass run) | |
