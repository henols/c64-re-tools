...
plan: 07
subsystem: skills-ci
tags: [acme-build, ci, template.a, anno-05, folded-todo-2]

# Dependency graph
requires:
  - phase: 10-adoption-boundaries-automated-bootstrap-and-the-removal
    provides: "acme-build/SKILL.md as plan 10-06 left it (toacme deletion, anno pointer already in place) -- this plan edits the same file on top of that state"
provides:
...
6. `grep -c 'acme-build/scripts/acme.mjs' .github/workflows/ci.yml` -> `2`; `grep -c 'VICE_REQUIRE_ANNO'` -> `0` (re-checked post-resume per the orchestrator's explicit ask); `grep -c 'the external analyser'` -> `0`. New step (`line 83`) confirmed before `Test` (`line 110`) by file order.
...
- Criterion 4's `anno verify` proof now rests on an ACME installation demonstrably capable of assembling this project's own scaffold, closing the foundational gap the plan's objective named.
