...
parsers — `anno-d64.ts`'s sector walker and `anno-project.ts`'s `parsePrg()`). It does not
...
|----------|-------------|----------------|-----------------|
| A human's shell → `vice-proxy.ts`'s `argv[2] === "anno"` branch → `anno-cli.ts` | CLI verbs (`bootstrap`, `export-asm`, `verify`, `gen-enums`, `export-lbl`, `import-lbl`, `render-memmap`) | Filesystem paths, `.prg`/`.d64`/`.raw`/`.bin` bytes | **No** — gated before any MCP server/JSON-RPC construction (verified below) |
| A `.d64`/`.prg`/flat-64K file of unknown provenance (scene archive, cracked release) → `anno-d64.ts` / `anno-project.ts` | Untrusted disk-image and program bytes | Sector chains, directory entries, load-address bytes | **No** — these two modules are imported only by `anno-cli.ts` (CLI-only) and `anno-test-gate.ts` (test-only, never shipped); not imported by `anno-tools.ts` or `anno-mcp-client.ts`, the Phase-11 MCP surface |
| `anno-cli.ts`/`anno-verify.ts` argv → `spawnSync(the external analyser)` | Caller-controlled file paths reaching a child process argv | Filesystem paths | No — CLI-only, and the child never touches stock VICE's binary monitor by construction |
...
`anno-d64.ts` or `anno-project.ts` — is reachable only by a human typing `vice-mcp anno <verb>`
at a local shell, never by a model through a `tools/call` payload.** Verified by:
`grep -rln "anno-d64\|anno-project" .claude/mcp/vice/*.ts` (excluding `*.test.ts`) returns only
`anno-d64.ts`, `anno-project.ts` (self), `anno-cli.ts`, and `anno-test-gate.ts`
(test-only, asserted absent from `package.json` `files[]`). `anno-tools.ts` (the curated
`anno_*` MCP tool family, `vice-proxy.ts:3287-3289`) imports neither. This caps every
...
|-----------|----------|-------------|-----------------|----------|--------|
| T-10-01 | Denial of Service | mitigate | 10-01, 10-02(transfer), 10-04, 10-06, 10-09 | `FORBIDDEN_ANNO_FLAGS = ["--vice"]` (`anno-launch.ts:77`); `assertNoViceFlag()` iterates it and throws `AnnoViceFlagError` (`:122-130`); called as the **first statement** of `runAnno()` (`:311`) and of `withAnnoSession()` in `anno-mcp-client.ts:330`. Structurally enforced (not just called-by-convention) by `spawn-seam.test.ts`, present on disk, which derives every production spawn call site and fails if any spawns unguarded. `10-06`/`10-09`'s documentation-only leg (a *user's own* separate debugger) verified present in `.claude/skills/vice-wedge-triage/SKILL.md:18,65` (`monitor_held_elsewhere`, "Never recycle"). | **CLOSED** |
| T-10-02 | Tampering | mitigate | 10-01, 10-02, 10-03, 10-04, 10-05, 10-07 | `grep -rn "shell:\s*true" .claude/mcp/vice/anno-*.ts .github/workflows/ci.yml` → 0 matches (only a comment stating the rule). `runAnno()` (`anno-launch.ts:317`) and `withAnnoSession()` (`anno-mcp-client.ts:332`) both call `spawnSync`/`spawn` with an argv array, never a shell string. CI's ACME step (`.github/workflows/ci.yml:97-100`) uses fixed literal args, no interpolation of untrusted input. | **CLOSED** |
| T-10-03a | Tampering | mitigate | 10-02 | `synthesizeProject()` (`anno-project.ts:124-131`): `origin` range-checked to `0..0xffff`, zero-length payload rejected, both throw naming the offending value. | **CLOSED** |
| T-10-03b | Tampering | mitigate | 10-03 | `.d64` sector-chain traversal: `listEntries()` (`anno-d64.ts:148-200`) and `extractEntry()` (`:220-296`) each carry an independent `visited`-`Set` cycle guard that throws on a revisited `{track,sector}` key, plus an `isInImage()` bounds check against the 35-track geometry before every walk step. | **CLOSED** |
| T-10-04 | Repudiation | mitigate | 10-01 | `assertNoViceFlag()` throws `AnnoViceFlagError` (named, carries `argv`) rather than filtering; `grep -n "filter\|strip" anno-launch.ts` around the guard shows no removal path — confirmed by reading `:122-130` directly (no `.filter()` on argv anywhere in the guard). | **CLOSED** |
| T-10-05 | Information Disclosure | mitigate | 10-01, 10-03, 10-04 | `hostpath-consumers.test.ts:178-220` now derives the anno module set from disk (`annoProductionModules()`, filtered on `/^anno-.*\.ts$/`) with a non-vacuity floor, rather than the hand-typed 10-module list the v0.3.0 milestone audit flagged as stale (INT-01) — that gap is closed: a dedicated regression test (`:199-206`) pins the four previously-uncovered modules (`anno-acme-ident.ts`, `anno-regbits-gen.ts`, `anno-symbols.ts`, `anno-test-gate.ts`) as present in the derived set. Live re-check: `grep -rn "hostpath\|containerpath" .claude/mcp/vice/anno-*.ts` (excluding tests) → 0 matches. | **CLOSED** |
| T-10-06 | Denial of Service | mitigate | 10-02, 10-05 | `anno-project.test.ts:134,193` and `anno-verify.test.ts:208,229` both pass explicit `timeout` options to `spawnSync`/`node:test`; `anno-verify.test.ts:201-202` wraps temp-dir cleanup in `finally`. Neither file appears in `MANUAL_ONLY_TESTS` (grep confirms only comment-level "must never be added" markers, no actual listing). | **CLOSED** |
| T-10-07 | Spoofing | mitigate | 10-03, 10-04 | `extractEntry()` (`anno-d64.ts:225-236`) throws on 0 matches and on >1 matches, never returns a guess. `bootstrapProject()` (`anno-cli.ts:354-375`) requires `opts.entry` for `.d64` input and prints the listing + exits 2 otherwise. | **CLOSED** |
| T-10-08 | Tampering | mitigate | 10-04 | `vice-proxy.ts:271` — `if (process.argv[2] === "anno")` dispatches to `runAnnoCli()` and calls `process.exit(code)` (`:303-306`), **above** `HERE_DIR`/backend-probe/server construction (`:309` onward). Confirmed by reading the file directly: no MCP server object exists yet when this branch runs. | **CLOSED** |
| T-10-09 | Spoofing | mitigate | 10-05 | `grep -c "status === 0" anno-verify.ts` → `0`. `acmeVerdict()` (`anno-verify.ts:116-160`) derives `ok` only from parsed ACME `VerifyLine`s: a `"skipped"` or `"failed"` ACME line is `ok:false` regardless of the child's exit code. | **CLOSED** |
...
| T-10-12 | Spoofing | mitigate | 10-06 | Documented `vice-mcp anno <verb>` commands checked against `check-skill-tool-coverage.mjs`'s live output: "anno CLI verbs: 7 parsed from anno-cli.ts, 7/7 resolved (named by at least one skill file)." No dangling/invented verb. | **CLOSED** |
...
| T-10-14 | Denial of Service | accept | 10-07 | `grep -n "the external analyser\|cargo install" .github/workflows/ci.yml` → 0 matches; the accepted risk (no anno install in CI, ~5 min cost) holds. | **CLOSED** (accepted risk, documented) |
...
`"--entry"`, and `writeFileSync("--entry", projectJson)` (`anno-cli.ts:442`) creates that file —
...
running this CLI directly — **it is not reachable via any MCP tool** (confirmed above: `anno-cli.ts`
is gated behind `argv[2] === "anno"`, checked before any JSON-RPC server exists, so no
...
(`.planning/todos/pending/2026-08-21-anno-cli-wr-08-option-values-silently-swallowed.md`) but
...
`parseExportLblArgs()` already uses (`anno-cli.ts:684-699`) to the shared `parseArgs()` used by
...
(`.planning/todos/pending/2026-08-21-anno-cli-wr-08-option-values-silently-swallowed.md`) that
...
| IN-01–IN-07 | Not re-verified individually; none carry a `T-10-*` ID and none describe an unmitigated attacker-reachable path per this audit's own re-reading of `anno-cli.ts`, `anno-verify.ts`. |

---

## Baseline (re-run live, this audit)

- `cd .claude/mcp/vice && VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 npm test` → **2192 tests, 2157 pass, 0 fail, 30 skipped, 5 todo** (matches the required baseline exactly).
...
- `git status --short` → clean after the run (no fixture or scratch file left behind; no malicious `.d64`/`.raw` fixtures were written to the repo tree for this audit — verification relied on hand-traced code reading plus the project's own existing pinned fixtures in `anno-d64.test.ts`/`anno-cli.test.ts`, which already exercise truncated images, NUL-padded names, and the various parser edges).
