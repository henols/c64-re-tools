...
plan: 08
subsystem: docs
tags: [the external analyser, third-party-notices, ci-honesty-guard, anno, d-13, d-14, d-15, toacme, deletion-pin]
...
provides:
  - "README.md's ## Installing the external analyser section: measured D-15 facts (cargo install, rustc >= 1.90 floor, both container-size figures, verified version/date, dual MIT OR Apache-2.0 licence), and the one-project-per-namespace limit stated as documented-not-detected"
  - "The extended one-holder rule near ## Installing VICE...: 'exactly one process may hold the binary monitor' stated positively with named traps (nc, second Claude session, -remotemonitor, any 6502 debugger including the external analyser's own --vice), plus the note that this project's own route refuses --vice by construction and by scan"
  - "THIRD-PARTY-NOTICES.md's the external analyser entry: the correct dual MIT OR Apache-2.0 licence, verified provenance (0.9.20, the upstream author, 2026-07-11), and the checkable claim that nothing of it ships in either published package"
  - "scripts/check-skill-fork-honesty.mjs D-13 inversion: The external analyser moved from FORBIDDEN_README_SUBSTRINGS to REQUIRED_README_SUBSTRINGS, header comment corrected"
  - "A permanent ANNO-05 regression pin walking the whole .claude/skills tree for toacme/cmdDisasm/the standalone disasm verb token, with exactly one line-scoped exemption for diff-images.test.mjs's provenance-ledger evidence string"
affects: ["10-09 (still owns correcting the stale Apache-2.0-only claim in ANNO-03's requirement text and .planning/notes/external-analyser-integration.md)"]
...
key-decisions:
  ...
  - "Placed '## Installing the external analyser' as its own top-level section immediately after '## Installing VICE, and choosing a backend' (before '## How it locates the project'), mirroring that section's own lead-in/table/what-breaks-without-it shape per 10-PATTERNS.md's guidance"
  - "Added a third analyser mention (naming it inside the new required tuple's whatIsLost string) to clear the plan's own acceptance criterion requiring at least 3 occurrences in the guard script (header clause + array entry weren't enough on their own)"
  - "Kept the acme-build/SKILL.md replacement-pointer assertion (anno export-asm) as a single named-file positive check, separate from the tree-walk negative checks -- the plan explicitly asked for this as an addition to the walk, not folded into it"
...
- `README.md` - new `## Installing the external analyser` section (measured D-15 facts table); one-holder statement near `### Verifying a stock install` extended into the positive rule with named traps
- `.claude/mcp/vice/THIRD-PARTY-NOTICES.md` - new the external analyser notice under a "Build/CI tools — not incorporated" style entry, stating the true dual `MIT OR Apache-2.0` licence and verified provenance
- `scripts/check-skill-fork-honesty.mjs` - D-13 array inversion (`the external analyser` moved `FORBIDDEN` → `REQUIRED`), header comment corrected (items 2 and a new item 4), permanent ANNO-05 whole-tree deletion pin added with its one line-scoped exemption and the replacement-pointer positive check
...
- Placed the new `## Installing the external analyser` section immediately after the existing VICE install section, mirroring its lead-in/table/what-breaks shape per `10-PATTERNS.md`.
- Added a third `the external analyser` mention inside the guard's new `whatIsLost` string to satisfy the plan's own acceptance criterion (>= 3 occurrences in the script) — the array entry and header clause alone totalled only 2.
- Kept the `anno export-asm` replacement-pointer assertion as a separate named-file positive check rather than folding it into the tree-walk negative checks, matching the plan's explicit "additionally assert" instruction.
...
- **Mutation check (a) (README required-string removal) was run and recorded before the interruption:** temporarily replacing every `the external analyser` occurrence in README.md and re-running the guard produced:
  ```
  check-skill-fork-honesty: FAIL
    - README.md is missing the required string "the external analyser" -- without it, a reader is not
      told the external analyser is a required prerequisite, so they hit the static-disassembly route
...
None. `the external analyser` was already installed on this host (`0.9.20` at `~/.cargo/bin/analyser`); no new dependency was introduced by this plan.

## Next Phase Readiness

- Criterion 5 (`ANNO-03`)'s install-story documentation is complete: README names the external analyser as required with every measured D-15 fact, the notices file carries the true dual licence, and the CI honesty gate now requires the name instead of forbidding it.
- The ANNO-05 deletion (plan 10-06) is now permanently guarded by CI: a reintroduced `toacme`/`cmdDisasm`/`disasm` anywhere under `.claude/skills` fails the build, observed live to bite on the exact file (`acme-build/SKILL.md`) it is meant to protect.
- **Downstream dependency for plan 10-09:** `.planning/notes/external-analyser-integration.md` and `REQUIREMENTS.md`'s `ANNO-03` requirement text still wrongly state the licence as Apache-2.0 only. This plan corrected the notice itself (`THIRD-PARTY-NOTICES.md`) per D-14/D-15, but did **not** touch either of those two documents — that correction is explicitly plan 10-09's job, not this plan's `files_modified` scope.
