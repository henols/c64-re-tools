...
plan: 03
subsystem: infra
tags: [d64, anno, the external analyser, bootstrap, node-test, c64]
...
tech-stack:
  added: []
  patterns:
    - "Second, independent copy of d64-parse.mjs's sector-chain-walk algorithm, container-side, scoped to only what the anno bootstrap needs (package-boundary constraint: .claude/skills/** is not in @henols/vice-mcp's files[])"
...
key-decisions:
  - "anno-d64.ts is a fresh, independent module (not an extension of d64-parse.mjs) per the plan's own package-boundary decision -- d64-parse.mjs is left untouched, confirmed by git diff against cf22caf"
  ...
  - "The composition test (extractEntry -> parsePrg) probes for anno-project.ts on disk and soft-skips with a named reason when absent, since 10-02 (which creates that file) runs in a concurrent, isolated wave-1 worktree and that module does not exist in this checkout"
...
**Container-side `.d64` reader (`anno-d64.ts`) that lists directory entries and extracts a named entry's raw bytes with a cycle-guarded sector-chain walk, refusing to guess when zero or multiple entries match a requested name.**
...
1. **Task 1: anno-d64.ts — directory listing plus named-entry extraction, with a cycle guard** - `467a41a` (feat)
...
2. **Task 2: anno-d64.test.ts — round-trip a synthesised image, and prove every refusal path** - `89cad0d` (test)
...
- `.claude/mcp/vice/anno-d64.ts` - Pure `.d64` directory listing and named-entry extraction; no filesystem access inside the functions, no `hostpath.ts`/`containerpath.ts` import, no `process.exit`/console output.
- `.claude/mcp/vice/anno-d64.test.ts` - 11 `node:test` cases: directory listing, byte-identical round-trip (non-254-multiple payload), case-insensitive match, unknown-name refusal, ambiguous-name refusal, corrupt-chain refusal, out-of-image-pointer refusal, `assertPlainImage` both ways, a composition test against `anno-project.ts`'s `parsePrg()`, and a `sectorsPerTrack` sanity check.

## Decisions Made
- Kept `anno-d64.ts` and `d64-parse.mjs` as two independent implementations of the same sector-chain-walk algorithm, per the plan's own package-boundary reasoning (verified: `git diff --name-only cf22caf HEAD -- .claude/skills/c64-ram-capture/scripts/d64-parse.mjs` is empty).
...
- **Files modified:** `.claude/mcp/vice/anno-d64.ts`
- **Verification:** `node --test anno-d64.test.ts`'s byte-equality assertion on a deliberately non-254-multiple payload (302 bytes) passes.
...
- **Found during:** Task 2, writing the composition test the plan explicitly requires (test 9: "extracted bytes feed the synthesiser... import `parsePrg` from `anno-project.ts`")
- **Issue:** `anno-project.ts` is created by plan 10-02, which is wave 1 (parallel, not `depends_on`) and runs in its own isolated git worktree. It does not exist in this worktree and will not until the orchestrator merges both wave-1 branches. A literal `import("./anno-project.ts")` would fail `tsc --noEmit` in this checkout even though the module is guaranteed to exist post-merge; and a plain runtime `import()` of a missing file would throw at module load and crash the entire test file, not just this one test.
...
- **Files modified:** `.claude/mcp/vice/anno-d64.test.ts` (no change needed to the plan's other file, `anno-project.ts`, is out of this plan's scope and was correctly left uncreated)
- **Verification:** `node --test anno-d64.test.ts` exits 0 (10 pass, 1 skip) in this isolated worktree; `npm run typecheck` exits 0.
- **Not committed as a workaround needing follow-up in isolation** — this is expected to resolve itself: **once the orchestrator merges plans 10-02 and 10-03's branches together, `anno-project.ts` will exist on disk and this exact same test will run for real (not skip)** with no further code change required. The orchestrator/verifier should re-run `node --test anno-d64.test.ts` after the wave-1 merge and confirm the composition test reports `ok`, not `# SKIP`, at that point — if it still skips post-merge, that is a real problem worth investigating (e.g. a name mismatch with what 10-02 actually exported).

---

**Total deviations:** 2 (1 auto-fixed bug, 1 documented cross-plan sequencing gap)
**Impact on plan:** The bug fix was necessary for correctness and was caught before ever being observed passing incorrectly. The cross-plan gap is a consequence of plans 10-02 and 10-03 both being wave 1 (parallel, independent worktrees) while plan 10-03's own test spec requires importing a module 10-02 creates — this should have been reflected as a `depends_on` relationship, or the composition proof deferred entirely to plan 10-04 (which already owns wiring the two modules together per this plan's own `key_links`). No scope creep: `anno-project.ts` was correctly left uncreated in this worktree.

## Known Stubs

None. `anno-d64.ts` has no stubbed exports; every function documented in the plan's `must_haves.artifacts` is implemented and exercised by a real test, except the one composition test noted above, which soft-skips only because of the cross-worktree isolation described above, not because of an unfinished implementation.

## Threat Flags

None beyond what the plan's own `<threat_model>` already names. `anno-d64.ts` introduces no new network endpoint, no new auth path, and no new file access pattern — it operates purely on an in-memory `Uint8Array` the caller supplies.

## Issues Encountered
- `npm run test:automated` (the automated regression gate) shows one pre-existing failure unrelated to this plan's files: `repo-root.test.ts`'s "path agreement... the agreed path is not under `.claude`" assertion fails in this worktree, almost certainly because this worktree's own filesystem path is `.../.claude/worktrees/agent-a4dabc9876024cdbd/...` (i.e. genuinely under a `.claude` directory, by construction of the GSD worktree execution model itself) -- not a regression caused by `anno-d64.ts`/`anno-d64.test.ts`, which neither import nor touch `repo-root.ts`. Confirmed out of scope per this plan's file boundary (`.claude/mcp/vice/anno-d64.ts`, `.claude/mcp/vice/anno-d64.test.ts`) and left unfixed, per the Scope Boundary rule. Flagging here rather than in a shared `deferred-items.md` (per this plan's parallel-execution instructions, to avoid an add/add conflict with sibling worktree agents).
...
- `.claude/mcp/vice/anno-d64.ts` is ready for plan 10-04's CLI seam to call `extractEntry()` and hand its output to `anno-project.ts`'s `parsePrg()`/`synthesizeProject()`.
- Once plan 10-02's branch merges alongside this one, re-run `node --test anno-d64.test.ts` and confirm the composition test (`composition: extracted bytes feed parsePrg()...`) reports `ok` rather than `# SKIP` — this is the one remaining verification this plan could not complete in isolation.
...
- FOUND: `.claude/mcp/vice/anno-d64.ts`
- FOUND: `.claude/mcp/vice/anno-d64.test.ts`
- FOUND: `.planning/phases/10-adoption-boundaries-automated-bootstrap-and-the-removal/10-03-SUMMARY.md`
- FOUND commit `467a41a` (feat: anno-d64.ts)
- FOUND commit `149ad0c` (fix: off-by-one)
- FOUND commit `89cad0d` (test: anno-d64.test.ts)
