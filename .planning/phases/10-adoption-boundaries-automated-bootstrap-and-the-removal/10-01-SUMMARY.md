...
plan: 01
subsystem: infra
tags: [the external analyser, spawnSync, deny-list, guard-seam, hostpath, closed-consumer-set, node-test]

# Dependency graph
requires:
  - phase: 09-external-analyser-assumption-probe
    provides: "verified the external analyser 0.9.20 --help surface, including --vice <HOST:PORT> confirmed live"
provides:
  - "anno-launch.ts: the one authoritative seam that spawns the external analyser -- fixed per-verb argv builders, --vice deny-by-construction and deny-by-scan, AnnoViceFlagError, runAnno() spawnSync wrapper"
  - "anno-launch.test.ts: 9 tests pinning both halves of D-07, including a source-text reintroduction regression proven red-then-green under a live mutation"
  - "hostpath-consumers.test.ts extended with a 6th test naming all five future anno modules absent from the hostpath.ts consumer set (D-08), proven red-then-green under a live mutation"
affects: [10-02, 10-03, 10-04, 10-05, phase-11-anno-mcp-surface]
...
key-decisions:
  - "AnnoViceFlagError mirrors ViceError's options-object constructor shape (vice.ts:245-260), carrying a plain public argv field"
  ...
  - "anno-launch.test.ts's stripCommentLines() strips full JSDoc block comments as well as //-line comments (extending hostpath-consumers.test.ts's //-only idiom) because anno-launch.ts's own header prose discusses \"a rest parameter\" and \"the rest of the function\" in plain English, which would otherwise false-positive the source-text reintroduction check"

requirements-completed: [ANNO-01, ANNO-02]
...
**`anno-launch.ts` is now the sole seam that spawns the external analyser: `--vice` is unreachable by fixed per-verb argv builders and additionally denied by a scan that throws `AnnoViceFlagError`, with both guarantees pinned by tests proven to fail under live reintroduction mutations, and the seam's absence from host-path translation asserted structurally alongside four modules that don't exist yet.**
...
- Built `anno-launch.ts`: `ANNO_BIN`, `FORBIDDEN_ANNO_FLAGS`, `AnnoViceFlagError`, `viceFlagRefusalMessage()`, `assertNoViceFlag()`, `buildExportAsmArgs()`, `buildVerifyArgs()`, and `runAnno()` (argv-array `spawnSync`, ENOENT-specific install-hint error, `assertNoViceFlag()` as the function's first statement).
- Pinned both halves of D-07 in `anno-launch.test.ts` (9 tests): deny-by-scan for both the plain `--vice` token and the `--vice=<value>` single-token form; the never-strip/void-contract behavior plus a source-text assertion that no `.filter(` call exists over the deny list; no false positive on a filename containing the `--vice` substring; `runAnno()` enforcing the scan before any subprocess spawn (proven with `ANNO_BIN` pointed at a name guaranteed not to exist); the deny-by-construction source-text assertion (no rest parameter typed as a string array, no identifier named `extraArgs`/`passthrough`/`rest`); and builder output shape.
- Extended `hostpath-consumers.test.ts` with one new sibling test naming all five future anno modules (`anno-launch.ts`, `anno-project.ts`, `anno-d64.ts`, `anno-cli.ts`, `anno-verify.ts`) as absent from the hostpath.ts consumer set, leaving `EXPECTED_IMPORTERS`'s five-element positive array untouched.
- Both mutation checks required by the plan's acceptance criteria were performed live: reintroducing a `...extra: string[]` rest parameter on `buildExportAsmArgs` made `anno-launch.test.ts`'s construction-half test fail (`true !== false`, `not ok 8`), then reverting restored 9/9 green; adding `import { hostPath } from "./hostpath.ts";` to `anno-launch.ts` made the new `hostpath-consumers.test.ts` test fail (`not ok 4`, `anno-launch.ts must not import hostpath.ts`), then reverting restored 7/7 green.

## Task Commits

1. **Task 1: anno-launch.ts -- fixed argv builders, the named-error deny scan, and the spawn wrapper** - `24db0eb` (feat)
2. **Task 2: anno-launch.test.ts -- pin both halves of D-07, including the reintroduction regression** - `742ba7d` (test)
3. **Task 3: extend hostpath-consumers.test.ts with the anno absence assertion (D-08)** - `0d14f4c` (test)
...
- `.claude/mcp/vice/anno-launch.ts` - the one authoritative seam that spawns the external analyser: fixed argv builders, `--vice` deny-by-construction and deny-by-scan, `spawnSync` wrapper
- `.claude/mcp/vice/anno-launch.test.ts` - 9 tests pinning both halves of D-07, safe for the automated gate (no network, no emulator, no the external analyser binary required)
- `.claude/mcp/vice/hostpath-consumers.test.ts` - one new sibling test asserting the anno module family's absence from the hostpath.ts consumer set (D-08)
...
- **Found during:** Task 1, immediately after writing `anno-launch.ts` and running its acceptance-criteria greps
...
- **Files modified:** `.claude/mcp/vice/anno-launch.ts` (part of Task 1, before its commit)
...
- **Files modified:** `.claude/mcp/vice/anno-launch.test.ts` (part of Task 2, before its commit)
- **Verification:** `node --test anno-launch.test.ts` passes 9/9; confirmed the construction-half test still goes red under the required mutation (see Accomplishments) and green again on revert.
...
- `anno-launch.ts`'s `runAnno()`, `buildExportAsmArgs()`, and `buildVerifyArgs()` are ready for plan 10-02 (bootstrap synthesis, `anno-project.ts`) to compose against once that plan's `.regen2000proj` synthesis function exists.
- `hostpath-consumers.test.ts`'s new test already names `anno-project.ts`, `anno-d64.ts`, `anno-cli.ts`, and `anno-verify.ts` -- any of 10-02 through 10-05 introducing those files inherits the no-translation guarantee automatically; no further edit to that test file is needed unless a *sixth* anno module is introduced under a different name.
