...
|------|---------|---------------------------|
| Skill/fork honesty | `node scripts/check-skill-fork-honesty.mjs` | ANNO-03 / D-13 array-move edit |
| npm package contents | `node scripts/check-npm-packages.mjs` | New seam modules must appear in `files[]` (transitive-closure walk) |
| Resources sync | `cd .claude/mcp/vice && node --test resources-sync.test.ts` | Confirms the anno modules stay container-side and need no `resources/*.mjs` artifact |
...
- **After every task commit:** Run the specific new/modified test file's quick command
  (e.g. `cd .claude/mcp/vice && node --test anno-launch.test.ts`)
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 10-01 T1+T2 | 10-01 | 1 | ANNO-01 | T-10-01 | `--vice` unreachable by construction; a scan throws a named, loud error — never silently strips | unit | `cd .claude/mcp/vice && node --test anno-launch.test.ts` | ✅ | ✅ green |
| 10-01 T3 | 10-01 | 1 | ANNO-02 | — | No argument reaching the external analyser is host-translated; absence is structurally asserted | unit (extends existing) | `cd .claude/mcp/vice && node --test hostpath-consumers.test.ts` | ✅ | ✅ green |
| 10-02 T2+T3 | 10-02 | 1 | ANNO-09 | T-10-02 | `.prg`/`.d64`(named entry)/`.raw` → `.regen2000proj` with no human; `use_illegal_opcodes` and machine `system` explicitly forced, never inherited from auto-detection | unit (pure synthesis) + integration (real anno load) | `cd .claude/mcp/vice && VICE_REQUIRE_ANNO=1 node --test anno-project.test.ts` | ✅ | ✅ green |
| 10-05 T1+T2 | 10-05 | 3 | ANNO-06 | — | `--verify`'s ACME result line parsed as `✓`; fails on `skipped`; never trusts exit code alone | integration (subprocess, gated) | `cd .claude/mcp/vice && VICE_REQUIRE_ANNO=1 node --test anno-verify.test.ts` (CI: named SKIP per D-11, and CI never sets the variable) | ✅ | ✅ green |
| 10-06 T1-T3 | 10-06 | 4 | ANNO-05 | — | `disasm` verb, `## Disassembly` caveat section, and `toacme` prerequisite are gone from both `acme.mjs` and `SKILL.md` | negative assertion (grep) | `! grep -qn "disasm\|toacme" .claude/skills/acme-build/scripts/acme.mjs .claude/skills/acme-build/SKILL.md` | ✅ → permanent CI regression assertion, `node scripts/check-skill-fork-honesty.mjs` (whole-`.claude/skills`-tree scan, per plan 10-08 task 3) | ✅ green (both the original two-file grep and its whole-tree successor pass) |
| 10-08 T1+T3 | 10-08 | 5 | ANNO-03 | — | README names the external analyser as required prerequisite, states `cargo install` cost + one-project-per-namespace limit; `THIRD-PARTY-NOTICES.md` carries the dual `MIT OR Apache-2.0` notice (D-14 corrects the Apache-2.0-only wording) | documentation-honesty CI gate | `node scripts/check-skill-fork-honesty.mjs` | ✅ | ✅ green |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky*

**Audit note (2026-08-21):** all six declared commands still resolve under their original names —
none were renamed or moved by Phase 11.1's edits to `anno-cli.ts`, `anno-launch.ts`,
`anno-verify.ts`, `hostpath-consumers.test.ts`, `check-skill-fork-honesty.mjs`, or `acme.mjs`.
Every command was re-run live on this host (real `the external analyser 0.9.20`, real ACME 0.97, so the
`VICE_REQUIRE_ANNO=1`-gated legs of ANNO-09/ANNO-06 genuinely executed rather than skipping) and
observed green. Two commands are corrected from the pre-execution snapshot's text to what they
actually are on this host: ANNO-09's row is run with `VICE_REQUIRE_ANNO=1` set (its own file
declares a gated integration leg that silently skips without it — running it unset would not have
proven the "real anno load" half of the row's own Test Type column); ANNO-05's declared
...
**Independent mutation-kill re-confirmation (2026-08-21).** `anno-launch.test.ts`'s D-07 guard
pin was re-verified non-vacuous on the *current* tree, not merely re-run: `anno-launch.ts` and
`anno-launch.test.ts` were copied to a scratch directory outside the repo, the
`assertNoViceFlag(argv)` call was removed from the copy's `runAnno()`, and the copy's own test
suite was re-run against the mutated copy — 25/26 pass, 1 fail, and the failing test is exactly
`"runAnno throws AnnoViceFlagError before spawning..."`. The live tracked file was never touched
...
Phase 11.1 widened the guard's own scope (`spawn-seam.test.ts` now also polices the second,
async spawn site in `anno-mcp-client.ts` — a strengthening, not a regression, and outside this
row's own declared command).

**Annotation — 10-02 T2+T3 / ANNO-09: the declared command under-covers its own row text, but the
missing half is genuinely tested elsewhere.** The row's Secure Behavior column names three input
shapes — `.prg`, `.d64` (named entry), and `.raw` — but `anno-project.test.ts` (the row's only
declared command) contains zero `.d64` references; `.d64` named-entry extraction is
`anno-d64.ts`'s job, and the end-to-end "bootstrap on a `.d64` writes a `.regen2000proj` with no
human" path is exercised in `anno-cli.test.ts` (e.g. the D-02 fail-loud-without-`--entry` test at
line 201). This is not a coverage gap — both files exist, both pass, and 10-VERIFICATION.md's own
Requirements Coverage table already attributes ANNO-09 to plans 10-02/10-03/10-04 jointly, not to
10-02 alone. It is a ledger-precision issue: the row's *declared automated command* names only one
of the three files a full re-run of ANNO-09 actually needs. Re-run for completeness during this
audit: `VICE_REQUIRE_ANNO=1 node --test anno-d64.test.ts` → 16/16 pass;
`VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-cli.test.ts` → 54/54 pass (both gated
...
| Command | Result |
|---------|--------|
| `node --test anno-launch.test.ts` | 26 pass |
| `node --test hostpath-consumers.test.ts` | 11 pass |
| `VICE_REQUIRE_ANNO=1 node --test anno-project.test.ts` | 13 pass (gated leg ran live) |
| `VICE_REQUIRE_ANNO=1 node --test anno-verify.test.ts` | 12 pass (both gated legs ran live) |
...
| `VICE_REQUIRE_ANNO=1 node --test anno-d64.test.ts` (completeness re-run, ANNO-09) | 16 pass |
| `VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 node --test anno-cli.test.ts` (completeness re-run, ANNO-09) | 54 pass |
| `cd .claude/mcp/vice && VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 npm test` (full suite) | 2192 tests, 2157 pass, 0 fail, 30 skipped, 5 todo |
...
- [x] `.claude/mcp/vice/anno-launch.ts` + `anno-launch.test.ts` — ANNO-01 (D-07 construction + scan guard) → **plan 10-01, tasks 1-2**
- [x] `.claude/mcp/vice/anno-project.ts` + `anno-project.test.ts` — ANNO-09 (D-01/D-04/D-05 synthesis, forced settings) → **plan 10-02, tasks 2-3**
- [x] `.claude/mcp/vice/anno-verify.test.ts` — ANNO-06 (D-09/D-10/D-11 gated `--verify` proof) → **plan 10-05, tasks 1-2**.
...
- [x] `hostpath-consumers.test.ts` extension — ANNO-02 (D-08) → **plan 10-01, task 3**. No new
      file; `EXPECTED_IMPORTERS` stays a five-element array, and all five future anno module
      names are placed on the negative/absence side in one test.
- [x] `.d64` named-entry extraction — D-02 → **plan 10-03**, landing as
      `.claude/mcp/vice/anno-d64.ts` rather than inside `d64-parse.mjs`. Open Question #2 is
...
run against the real installed `the external analyser 0.9.20` and real ACME 0.97. This is a **stale
...
`the external analyser 0.9.20`, real ACME 0.97 "Zem", genuine unpatched stock `/usr/bin/x64sc` (VICE 3.9)
...
| Annotations (not gaps — see ANNO-09 above) | 1 |

**No declared command had moved or been renamed.** Phase 11.1 modified `anno-cli.ts`,
`anno-launch.ts`, `anno-verify.ts`, `hostpath-consumers.test.ts`, `check-skill-fork-honesty.mjs`
...
`cd .claude/mcp/vice && VICE_REQUIRE_ANNO=1 VICE_REQUIRE_ACME=1 npm test` → **2192 tests, 2157
...
**Mutation-kill re-confirmation.** `anno-launch.test.ts`'s D-07 `--vice` guard pin was re-verified
...
`runAnno()` and re-running the copy's own test suite produced 25/26 pass, 1 fail, and the one
failure named exactly the guard behavior ANNO-01 claims
(`"runAnno throws AnnoViceFlagError before spawning..."`). `git status`/`git diff` confirmed the
...
exist and say what the ledger claims — re-confirmed against a live `analyser --version`
re-run for the first, and against the still-passing gated `anno-verify.test.ts` legs for the
second.

**No implementation code was modified by this audit.** The one gap-shaped finding surfaced
(ANNO-09's declared command under-citing its own `.d64` half) resolved to an **annotation**, not
a BLOCKER or a new test: the behavior it names is already pinned by existing, passing,
non-vacuous tests (`anno-d64.test.ts`, `anno-cli.test.ts`), just not the one file the row's
