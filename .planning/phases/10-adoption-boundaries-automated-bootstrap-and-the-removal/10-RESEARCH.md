...
**Domain:** Rust CLI shell-out integration (the external analyser, Tier 1), Node.js/TypeScript guard seams, skill-script deletion
...
- **D-06 (Claude's call): the guarded launch seam is a module under `.claude/mcp/vice/`, with a thin skill-side entry point for CLI ergonomics.** Decided because `hostpath-consumers.test.ts` enumerates only top-level modules of `.claude/mcp/vice/` and CI runs `npm test` only inside that directory — no skill-side `*.test.mjs` runs in CI today. A guard test in a skill script would be green-by-absence. Phase 11 puts the `anno_*` MCP surface in that same directory anyway. **Open for research (see Open Questions below):** exactly how the skill-side entry reaches the seam across the package boundary once `@henols/vice-mcp` is an installed dependency rather than a sibling directory.
...
- **D-08: criterion 2's no-translation absence is asserted by extending `hostpath-consumers.test.ts`'s closed consumer set**, adding the anno module to the "must be absent" side — the mirror of `DERIV-07`. Do not write a new bespoke test file for this.

**The reassembly proof (criterion 4 — ANNO-06):**
- **D-09 (user): lean on the external analyser's own `--verify`** rather than an independent export-assemble-diff harness. On the synthesised fixture it reported `✓ ACME — byte-identical (44 bytes)` (and ca65 likewise). The caveat (this is anno checking its own export) was put to the user and they chose it anyway.
...
- **D-11 (Claude's call): CI does not install the external analyser.** Named SKIP when anno is absent, hard FAIL under `VICE_REQUIRE_ANNO` — mirroring `disasm-roundtrip.test.ts`'s `VICE_REQUIRE_ACME` pattern including its "exactly one test always runs, never skipped" availability gate. `cargo install analyser` measured 4m48s-5m39s and needs rustc ≥ 1.90; a 5-minute Rust build on every merge contradicts "cheapest by far".
...
- **D-13: the CI honesty guard must be inverted, not worked around.** `scripts/check-skill-fork-honesty.mjs:253` currently lists `["the external analyser", "D-B: ... must stay external-analyser-free"]` in `FORBIDDEN_README_SUBSTRINGS`. Criterion 5 requires the name **in** README.md. Move it to `REQUIRED_README_SUBSTRINGS` with a `whatIsLost` string, and update the file's header comment (line 14, "the external analyser name Phase 8 removed").
- **D-14: the license is `MIT OR Apache-2.0` (dual), and the notices must say the true thing.** Both `LICENSE-MIT` and `LICENSE-APACHE` ship in the crate. `.planning/notes/external-analyser-integration.md:253` and `REQUIREMENTS.md`'s `ANNO-03` wording still say Apache-2.0 only — correct the requirement text and the note as well as writing the notice.
...
Delegated by the user, recorded as decisions rather than left open (do not re-litigate): **D-01** (bootstrap mechanism), **D-05** (forced settings), **D-06** (seam location), **D-12** (replacement surface). **D-11** (CI does not install anno) follows from the `--verify` choice.
...
- `.vsf` as a bootstrap input → Phase 11's `c64-ram-capture` extension (`ANNO-14`/`ANNO-15`).
...
- The `anno_get_address_details` u16 overflow (`handler.rs:1894`) — upstream report / Phase 11 workaround.
- Non-ACME export formats (`64tass`, `ca65`, `kick`) — not scope.
- Two-project-limit detection — permanently out (`ANNO-04` fold).
...
|----|-------------|------------------|
| ANNO-01 | the external analyser never launched with `--vice`, enforced in code and tested | `vice.ts` `DENY_LIST`/`denyListRefusalMessage()` pattern verified at lines 201-243, 693-699 (Architecture Patterns, Code Examples). Confirmed live: `analyser --help` really does expose `--vice <HOST:PORT>` — the exact flag D-07 must make unreachable. |
| ANNO-02 | No path translation applies to any anno argument, asserted as an absence | `hostpath-consumers.test.ts` structure verified in full (Architecture Patterns) — its existing "absent from consumer set" test pattern (for `disasm-*.ts` etc.) is the literal template for D-08's new assertion. |
| ANNO-03 | the external analyser declared prerequisite alongside VICE; toolchain cost + one-project limit stated; Apache-2.0 notice in THIRD-PARTY-NOTICES.md | README.md structure, `check-skill-fork-honesty.mjs` REQUIRED/FORBIDDEN substring gate, and `.claude/mcp/vice/THIRD-PARTY-NOTICES.md`'s existing notice format all verified (Architecture Patterns, Code Examples). Corrected to dual MIT/Apache-2.0 per D-14. |
| ANNO-09 | `.prg`/`.d64`/flat-64K bootstrap to `.regen2000proj` without a human | the external analyser 0.9.20 `--help` output verified live (Standard Stack) confirms `--headless`, `--verify`, `--export_asm`/`--export_lbl`/`--import_lbl`, `--assembler`, `--mcp-server[-stdio]` all exist as documented in CONTEXT.md's D-01 evidence. `d64-parse.mjs`'s exports verified (no file-extraction function yet — a real gap, see Open Questions). |
| ANNO-05 | `acme-build`'s `disasm` verb + `toacme` prerequisite removed | `cmdDisasm()` and all SKILL.md caveat text verified byte-for-byte (Code Examples, Don't Hand-Roll). Full blast-radius grep performed (Common Pitfalls / Deletion Blast Radius). |
| ANNO-06 | `.prg`/flat-64K reassembles under `!cpu 6510`, verified by running a real assembler | `--verify`'s exact output shape (including the ACME-skipped-but-exit-0 trap) already captured in D-09/D-10; `disasm-roundtrip.test.ts`'s **unrelated** Phase-4 pattern (SKIP_REASON/VICE_REQUIRE_ACME) verified as the structural template for the new gate — **not the same file, must not be edited for this purpose** (see Common Pitfalls). |
</phase_requirements>

## Summary

This phase is unusual: `/gsd-discuss-phase` already produced a CONTEXT.md with live-evidence-backed implementation decisions (D-01 through D-15) rather than open alternatives, because the user delegated every mechanism question ("not sure it's your job to figure that out... you decide") and the deciding agent ran real commands against a real, already-installed `the external analyser 0.9.20` to settle them. This research's job is therefore narrower than usual: verify the current repo state each decision will land against, resolve the small number of items CONTEXT.md itself left open, and surface anything a planner needs that a discussion transcript would not capture (exact line ranges, exact CLI `--help` output, exact test-file shapes, and one real architectural gap in the "thin skill-side entry point" idea).

Everything CONTEXT.md's D-01 assumed about the external analyser's CLI surface was independently re-verified live on this host: `the external analyser 0.9.20` is installed at `~/.cargo/bin/analyser`, and `--help` confirms every flag the decisions depend on — `--headless`, `--verify` (not `--verify-roundtrip`, already corrected in Phase 9), `--export_asm`/`--export_lbl`/`--import_lbl`/`--export_html`, `--assembler <NAME>`, `--mcp-server`/`--mcp-server-stdio`, `--dump-enum-files`, and, critically, `--vice <HOST:PORT>` — the exact flag ANNO-01 forbids, confirmed to genuinely exist as a live CLI option, not a hypothetical. `tmux`, `acme` (0.97 "Zem"), `rustc`/`cargo` 1.97.1, and Node 22.22.0 are all present, so nothing in this phase is blocked by environment.
...
|------------|-------------|----------------|-----------|
| `--vice` guard / argv construction (ANNO-01) | Container-side CLI shell-out module (`.claude/mcp/vice/`) | Skill-side thin wrapper (CLI ergonomics only) | This is Tier 1 per ROADMAP: no MCP tool, no lifecycle. The guard must live where the closed-consumer-set test machinery already runs in CI (D-06's own deciding fact), not in the skill layer where nothing is tested in CI today. |
| No host-path translation (ANNO-02) | Container-side CLI shell-out module | — | `hostpath.ts`/`containerpath.ts` never touch this module by construction; the absence is what D-08 asserts. Running container-side (D-R4) is what makes devcontainer + two-simultaneous-projects work with no upstream patch. |
| `.regen2000proj` synthesis (ANNO-09) | Container-side CLI shell-out module (pure Node transform: gzip + base64 + JSON) | — | No filesystem/network/emulator dependency — pure data transform, belongs beside the guard module, not in a skill script (same D-06 test-reachability argument). |
| `.d64` directory-entry extraction (ANNO-09, D-02) | Skill layer (`c64-ram-capture/scripts/d64-parse.mjs` or a sibling) | — | `d64-parse.mjs` already owns BAM/directory parsing and is skill-scoped, offline, emulator-independent — the natural home for "follow this entry's sector chain and return its bytes," a pure-data operation with no MCP/container concern. |
| ACME reassembly gate (ANNO-06) | Container-side CLI shell-out module's test suite (`.claude/mcp/vice/*.test.ts`) | — | Only `.claude/mcp/vice/` tests run in CI (`npm test` scope); a skill-side test would be green-by-absence exactly as D-06 found for the guard tests. |
| Verb deletion (ANNO-05) | Skill layer (`acme-build/scripts/acme.mjs`, `acme-build/SKILL.md`) | — | Pure removal of skill-local code and docs; no MCP/container surface involved. |
| Install documentation (ANNO-03) | Documentation (README.md, THIRD-PARTY-NOTICES.md, check-skill-fork-honesty.mjs) | — | No runtime tier — a CI-enforced documentation-honesty concern. |
...
|------|---------|---------|--------------|
| `the external analyser` | `0.9.20` [VERIFIED: installed at `~/.cargo/bin/analyser`, confirmed via `analyser --version` and `cargo install --list` on this host] | Static-analysis backend: disassemble, auto-analyse, export ACME source/labels, self-verify roundtrip | Adopted per `.planning/notes/external-analyser-integration.md` (D-R1..D-R4) and Phase 9's `pass`/`degrade` verdict; this is the one Tier-1 (CLI shell-out) slice of that adoption |
...
| Rust toolchain (`rustc`/`cargo`) | `1.97.1` installed [VERIFIED live]; **floor is `>= 1.90`** per Phase 9's corrected finding (`docs/phase9-external-analyser-probe-findings.md` Corrections #1) | Only needed to `cargo install analyser` — a host/CI prerequisite, not a project dependency | D-15's install-story facts must state `>= 1.90`, not the superseded `>= 1.85`/`>= 1.88` readings |
...
**the external analyser `--help`, verified live on this host (`the external analyser 0.9.20`):**
```
Usage: The external analyser [OPTIONS] [FILE]
...
Options:
  ...
  -V, --version
```
[VERIFIED: live `analyser --help` output on this host, 2026-08-20] — this confirms every flag CONTEXT.md's D-01/D-09/D-10/D-11 depend on actually exists, spelled exactly as those decisions assume, **and** confirms `--vice <HOST:PORT>` is a real, live option — the flag ANNO-01/D-07 must make structurally unreachable.
...
| D-09's reliance on anno's own `--verify` | A hand-rolled export→assemble→byte-diff harness | Already decided by the user (D-09) with the internal-check caveat explicitly raised and accepted. Not revisited here. |
...
The external analyser itself is a Rust crate consumed as an **external CLI binary**, not an npm/PyPI/crates.io dependency of this project's own build. It was already put through registry/provenance verification in Phase 9, source-read (not merely `npm view`-style trust): crates.io API confirmed publisher `the upstream author` (matching the linked GitHub repo owner), version `0.9.20` published 2026-07-11, license `MIT OR Apache-2.0` (`docs/phase9-external-analyser-probe-findings.md`, "Build tested" table). That verification stands; this phase does not re-run it. `slopcheck`/registry-verification of an installed system CLI binary (as opposed to a package this project's manifest declares) is out of scope for the Package Legitimacy Gate as written — flagging this explicitly rather than silently skipping it.

**Disposition:** No packages to audit. The planner does not need a `checkpoint:human-verify` gate for a *new* package install — the external analyser is already installed on this development host, and D-11 explicitly keeps it out of CI. Any planner task that assumes a **different** host (e.g. a fresh devcontainer, per criterion 2's "a devcontainer run works with no upstream patch" requirement) must still gate the actual `cargo install analyser` step behind human awareness of the ~5-minute / ~250MB-1.26GB cost D-15 names — that is a cost-disclosure gate, not a legitimacy gate.
...
├── anno-project.ts         # NEW (D-01/D-04/D-05): pure synthesise(bytes, origin,
│                             # system) -> Buffer (gzip+base64+JSON), no I/O.
├── anno-launch.test.ts     # NEW: --vice unreachable-by-construction AND
│                             # denied-by-scan, both asserted (criterion 1).
├── anno-verify.test.ts     # NEW (D-11): mirrors disasm-roundtrip.test.ts's
│                             # SKIP/VICE_REQUIRE_ANNO *shape* only — separate
│                             # file, does not touch or import that file.
├── hostpath-consumers.test.ts   # EXTENDED (D-08): add anno-launch.ts /
│                             # anno-project.ts to the "must be absent from
...
### Pattern 1: Deny-by-construction plus deny-by-scan (D-07, ANNO-01)
...
```
The D-07 analog scans a `string[]` argv (not a tool name) for `--vice` (case-sensitive, exact token match — not a substring match against the whole command line, to avoid a false positive on a filename that happens to contain the substring) and throws a named error (e.g. `AnnoViceFlagError`) before `spawnSync`/`spawn` is called.

### Pattern 2: Closed consumer set as an absence proof (D-08, ANNO-02)
...
```
D-08's new test is a sibling assertion in this same file, naming `anno-launch.ts`/`anno-project.ts` (or whatever the seam module is actually called) in an identical `assert.equal(importers.includes(name), false, ...)` list. **`EXPECTED_IMPORTERS` (the positive/must-import set) is not touched** — the anno module must never import `hostpath.ts`, so it belongs only on the negative side, never added to the 5-element positive array.

### Pattern 3: Availability-gated, never-silently-skipped CI proof (D-11, ANNO-06)
...
```
D-11's new test file (e.g. `anno-verify.test.ts`) reuses this *shape* with `ANNO_AVAILABLE`/`VICE_REQUIRE_ANNO`, and — because D-11 says CI never sets `VICE_REQUIRE_ANNO` — the availability-gate test will simply pass trivially in CI forever (the `if` body never executes there), while still running unconditionally locally and failing hard for any future maintainer who does set the env var without the external analyser present.

### Anti-Patterns to Avoid

- **Editing `disasm-roundtrip.test.ts` to add the anno `--verify` check.** That file is Phase 4's stock-disassembler byte-exact round-trip test (`stock-disassemble.ts`/`vice_disassemble`), a **standing-constraint-protected** asset ("Phase 4's disassembler stays... All ~61KB of source and ~55KB of tests are load-bearing" — ROADMAP.md Standing Constraints). It shares only a naming coincidence ("disasm") with the verb this phase deletes. Create a new file.
- **Adding the anno seam module to `EXPECTED_IMPORTERS`.** That array is the *positive* closed set of modules that DO import `hostpath.ts`. The anno module belongs only in a *negative* absence assertion.
...
**How to avoid:** create a new test file for the anno `--verify` gate; only borrow the SKIP/FAIL-gate *shape*, never the file itself.
**Warning signs:** a diff touching `disasm-roundtrip.test.ts`, `disasm-opcodes.ts`, `disasm-decoder.ts`, or `disasm-renderer.ts` in a Phase 10 plan is almost certainly a scope error — none of them are named by any of ANNO-01/02/03/09/05/06.
...
**What goes wrong:** the test does `assert.deepEqual(importers, EXPECTED_IMPORTERS); assert.equal(importers.length, 5)`. If the new anno module accidentally imports `hostpath.ts` (even transitively re-exported), this exact-match test fails loudly — which is the intended behavior, but a planner reading only "extend the closed consumer set" prose might expect to add the new module to `EXPECTED_IMPORTERS` rather than to a negative-absence list. D-08 explicitly requires the negative form.
...
**What goes wrong:** the file currently has `["the external analyser", "D-B: this phase's install docs must stay external-analyser-free"]` in `FORBIDDEN_README_SUBSTRINGS` (line 253) **and** a header comment (line 14) still narrating "the external analyser name Phase 8 removed." Moving only the array entry and leaving the header comment stale would leave a self-contradicting file.
...
## Deletion Blast Radius (criterion 4, ANNO-05)
...
**c64-program-recon/SKILL.md** currently has no mention of `disasm`, `toacme`, or `the external analyser` — D-12 requires adding a pointer to the new route here (this is new documentation work, not a deletion).

## Code Examples

### the external analyser's `--verify` output, both the honest pass and the D-10 trap (from Phase 9's live evidence, reproduced here for planner reference)
...
| `--verify-roundtrip` (documented in early planning/notes) | `--verify` (the real flag name, implies `--headless`) | Corrected in Phase 9 (`docs/phase9-external-analyser-probe-findings.md` Corrections #9) | Already fixed everywhere current; confirmed again live in this research's own `--help` capture |
| Apache-2.0-only license claim | `MIT OR Apache-2.0` dual license | Corrected in Phase 9, applied in this phase (D-14) | `THIRD-PARTY-NOTICES.md` and `REQUIREMENTS.md`'s ANNO-03 wording both still carry the stale claim as of this research pass — this phase is where both get fixed |

**Deprecated/outdated:** `acme-build`'s `toacme`-on-PATH prerequisite is fully retired by this phase (ANNO-05) — `acme.mjs` keeps only its `acme` dependency.
...
1. **How does the skill-side thin entry point reach the D-06 seam across the package boundary?** (CONTEXT.md's own open item)
   - **RESOLVED: plan 10-04 — argv-subcommand dispatch on the existing published `vice-mcp` bin (`anno-cli.ts` dispatched from `vice-proxy.ts`, before any MCP server side effect), exactly as this question's own recommendation below proposed. Verified end to end against the real published bin.**
   ...
   - Recommendation: give the existing `vice-mcp` bin (`vice-proxy.ts`, or a thin wrapper it delegates to at the top of its `main()`) an argv-subcommand branch — e.g. `npx @henols/vice-mcp anno-bootstrap <file>` — that short-circuits before the Mastra MCP-stdio server starts. This is reachable identically across all three install routes (it's the one thing already proven to resolve everywhere: `npx <pkg>` / `node vice-proxy.ts` under the plugin), requires no new `bin` entry, and mirrors how many npm CLI tools dispatch subcommands from a single published bin. **Not yet verified end-to-end against a real npx-resolved install** — flag for the plan to include a smoke test analogous to `smoke.mjs`'s existing pattern before relying on it.

2. **Should `.d64` extraction extend `d64-parse.mjs` in place, or land beside the D-06 seam?**
   - **RESOLVED: plan 10-03 — a fresh, independent container-side module (`anno-d64.ts`), landed *beside* the seam, going AGAINST this question's own recommendation below. `d64-parse.mjs` is left untouched (verified: `git diff --name-only` against the pre-plan commit is empty for that file). The reason the research recommendation did not hold: `.claude/skills/**` is not in `@henols/vice-mcp`'s published `files[]`, so a container-side module cannot import `d64-parse.mjs` across that package boundary — the coupling the research page argued was fine turns out to be structurally unreachable. Phase 11 inherits this package-boundary reasoning directly: any future container-side consumer of `.d64` parsing needs its own copy or a shared library extracted below both packages, not an import of the skill-side file.**
   ...
   - What's unclear: whether extending this module (adding an `extractFile(buffer, entry)` export) creates an unwanted coupling between `c64-ram-capture` and the new anno route, versus a small duplicate helper beside the D-06 seam.
...
3. **Exact CLI verb names for the new route.**
   - **RESOLVED: plan 10-04 gave `bootstrap` and `export-asm`; plan 10-05 added `verify`. All three dispatch as `vice-mcp anno <verb>` on the published `vice-mcp` bin rather than inside `acme.mjs`, consistent with D-12's "no second binary dependency" constraint on that file.**
   ...
   - What's unclear: whether the new route exposes verbs through `acme.mjs` itself (adding a dependency on the external analyser to a file whose scope statement says "assembling only" — contradicts D-12's intent that `acme.mjs` end up wrapping `acme` alone) or through a wholly separate skill script/CLI invoked directly.
   - Recommendation: a **separate** thin skill-side script (not inside `acme.mjs`), consistent with D-12's explicit statement that `acme.mjs` "gains no second binary dependency" after the deletion. Exact naming (e.g. `anno.mjs` under `acme-build/scripts/` or `c64-program-recon/scripts/`) is a planner decision informed by which skill's `SKILL.md` most naturally documents it — CONTEXT.md says both `acme-build` and `c64-program-recon` should point at the single route, suggesting the script itself should not live exclusively "inside" either skill's conceptual ownership. A neutral location (e.g. directly under `.claude/mcp/vice/` as a thin CLI, invoked identically by both skills' documentation) avoids the ownership question entirely and is consistent with D-06's "thin skill-side entry point" wording being about ergonomics, not about which skill owns it.

## Environment Availability

| Dependency | Required by | Available | Version | Fallback |
|------------|------------|-----------|---------|----------|
| `the external analyser` | ANNO-09, ANNO-06 (whole phase) | ✓ [VERIFIED live] | `0.9.20` | — (already installed on this dev host; D-11 keeps CI from needing it) |
| `tmux` | Not required by D-01's chosen mechanism | ✓ [VERIFIED live] | `3.5a` | N/A — not a new prerequisite this phase introduces |
| `acme` | ANNO-06 (`--verify`'s ACME line) | ✓ [VERIFIED live] | `0.97 "Zem"` | Already a project-wide dependency (Phase 4, `acme-build`) |
| `rustc`/`cargo` | Only to (re-)install the external analyser, not to run this phase's code | ✓ [VERIFIED live] | `1.97.1` (floor `>= 1.90` per Phase 9) | Not needed if the external analyser is already installed |
...
**Missing dependencies with fallback:** none applicable; the external analyser install cost (D-15's ~5-minute/~250MB-1.26GB figures) is a documentation concern for ANNO-03, not a blocker for writing this phase's code, since the tool is already installed here.
...
| Quick run command | `cd .claude/mcp/vice && node --test anno-launch.test.ts` (or the specific new test file) |
...
|--------|----------|-----------|-------------------|-------------|
| ANNO-01 | `--vice` unreachable by construction AND denied by a scan that throws | unit | `cd .claude/mcp/vice && node --test anno-launch.test.ts` | ❌ Wave 0 (new file) |
| ANNO-02 | No argument passed to the external analyser is host-translated (structural absence) | unit (extends existing file) | `cd .claude/mcp/vice && node --test hostpath-consumers.test.ts` | ✅ exists, extended not created |
| ANNO-09 | `.prg`/`.d64`(named entry)/`.raw` → `.regen2000proj` with no human, `use_illegal_opcodes`+`system` forced | unit (pure synthesis fn) + integration (real anno load) | `cd .claude/mcp/vice && node --test anno-project.test.ts` | ❌ Wave 0 (new file) |
| ANNO-05 | `disasm` verb, `## Disassembly` section, `toacme` prerequisite all gone | negative assertion (grep-based, mirrors `check-skill-fork-honesty.mjs`'s own style) or a manual PR-diff check | `grep -n "disasm\|toacme" .claude/skills/acme-build/scripts/acme.mjs .claude/skills/acme-build/SKILL.md` (expect zero matches) | N/A — deletion proof, not a new automated gate unless the planner wants a permanent regression test |
| ANNO-06 | `--verify`'s ACME line parsed as `✓`, fails on `skipped`, never trusts exit code alone | integration (subprocess, gated) | `VICE_REQUIRE_ANNO=1 node --test anno-verify.test.ts` (locally, with anno installed) / plain `node --test anno-verify.test.ts` in CI (named SKIP, per D-11) | ❌ Wave 0 (new file) |
| ANNO-03 | README names the external analyser as required, states cost + limit; THIRD-PARTY-NOTICES.md has dual-license notice | documentation-honesty CI gate | `node scripts/check-skill-fork-honesty.mjs` | ✅ exists, requires the D-13 array-move edit |

### Sampling Rate
- **Per task commit:** the specific new/modified test file's quick command (e.g. `node --test anno-launch.test.ts`).
...
### Wave 0 Gaps
- [ ] `.claude/mcp/vice/anno-launch.ts` + `anno-launch.test.ts` — covers ANNO-01 (D-07's construction + scan)
- [ ] `.claude/mcp/vice/anno-project.ts` + `anno-project.test.ts` — covers ANNO-09 (D-01/D-04/D-05 synthesis, forced settings)
- [ ] `.claude/mcp/vice/anno-verify.test.ts` — covers ANNO-06 (D-09/D-10/D-11's gated `--verify` proof) — **new file, must not edit `disasm-roundtrip.test.ts`**
- [ ] `hostpath-consumers.test.ts` extension — covers ANNO-02 (D-08), no new file needed
...
- `analyser --version` / `analyser --help` — installed version and full CLI surface
- `command -v the external analyser`, `cargo install --list` — install location and registry record
...
- `.claude/mcp/vice/resources-sync.test.ts` — read in relevant part (confirms anno module needs no `resources/*.mjs` build artifact, since it's container-side, not host-bound)
...
- `docs/phase9-external-analyser-probe-findings.md` — Phase 9's full go/no-go verdict, evidence tables, accepted limits, corrections
...
- `.planning/notes/external-analyser-integration.md` — grounding notes (D-R1..D-R4)

### Tertiary (not verified this pass, flagged in Assumptions Log)
- `external-analyser-core-0.9.20/src/state/project.rs`'s exact field-level structure (A1) — relied on CONTEXT.md's citation rather than re-reading the crate source directly

## Metadata

**Confidence breakdown:**
- Standard stack (the external analyser CLI surface, environment): HIGH — every flag and tool version independently re-verified live on this host
...
**Valid until:** 30 days, EXCEPT the environment-availability table (the external analyser/rustc/acme versions), which should be re-checked at execution time if more than a few days pass — the external analyser is an eight-month-old, actively-developed project per Phase 9's own maturity note.
