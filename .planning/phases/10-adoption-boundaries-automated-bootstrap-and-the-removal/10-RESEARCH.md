...
1. **How does the skill-side thin entry point reach the D-06 seam across the package boundary?** (CONTEXT.md's own open item)
   - **RESOLVED: plan 10-04 — argv-subcommand dispatch on the existing published `vice-mcp` bin (`r2000-cli.ts` dispatched from `vice-proxy.ts`, before any MCP server side effect), exactly as this question's own recommendation below proposed. Verified end to end against the real published bin.**
...
3. **Exact CLI verb names for the new route.**
   - **RESOLVED: plan 10-04 gave `bootstrap` and `export-asm`; plan 10-05 added `verify`. All three dispatch as `vice-mcp r2000 <verb>` on the published `vice-mcp` bin rather than inside `acme.mjs`, consistent with D-12's "no second binary dependency" constraint on that file.**
