...
plan: 02
subsystem: infra
tags: [the external analyser, gzip, base64, node-zlib, node-test, acme, illegal-opcodes]

# Dependency graph
requires:
  - phase: 09-external-analyser-assumption-probe
    provides: "Verified the external analyser 0.9.20 install, D-01/D-04/D-05 decisions, .vsf machine-type limit finding"
provides:
...
key-decisions:
  - "Confirmed DRIFT: none -- installed the external analyser 0.9.20 and all seven load-bearing flags match RESEARCH.md's 2026-08-20 capture exactly"
...
patterns-established:
  - "A future .regen2000proj writer imports synthesizeProject from anno-project.ts rather than hand-building the shape -- this module is now the one authoritative place"
...
# Phase 10 Plan 02: The external analyser project bootstrap synthesiser Summary

**Pure Node `.regen2000proj` synthesiser (gzip+base64+minimal JSON) that a real analyser 0.9.20 loads and exports ACME source from, with a forced `use_illegal_opcodes`/`system` pair pinned by planted-violation-verified tests.**
...
## Accomplishments
- Confirmed the installed analyser (0.9.20) exactly matches the CLI surface every D-01/D-04/D-05/D-09/D-10/D-11 decision in this phase was measured against, with a first-hand (not paraphrased) crate-source re-verification of `ProjectState`'s exact JSON shape
...
2. **Task 2: anno-project.ts -- the minimal, forced-settings `.regen2000proj` synthesiser** - `a2beb78` (feat)
3. **Task 3: anno-project.test.ts -- shape unit tests plus a gated real-the external analyser load** - `6734c2d` (test)

**Plan metadata:** committed alongside this summary (docs)

## Files Created/Modified
- `.planning/phases/10-adoption-boundaries-automated-bootstrap-and-the-removal/evidence/10-environment-recheck.txt` - Execution-time re-check of the external analyser's version/flags against RESEARCH.md's capture (`DRIFT: none`), plus first-hand crate-source citations for `ProjectState`'s exact shape
- `.claude/mcp/vice/anno-project.ts` - Pure `.regen2000proj` synthesiser: `synthesizeProject`, `parsePrg`, `flatImageOrigin`, `decodeRawData`, `ANNO_SYSTEM_C64`
- `.claude/mcp/vice/anno-project.test.ts` - 11 always-run unit tests pinning the exact JSON shape, plus a gated integration test (`ANNO_BIN`/`VICE_REQUIRE_ANNO`) that runs a real analyser against a synthesised project

## Decisions Made
- Re-verified Assumption A1 directly against `external-analyser-core-0.9.20`'s source rather than trusting CONTEXT.md's citation, per Task 1's explicit instruction -- confirmed the three no-default `ProjectState` fields are exactly `origin`/`raw_data_base64`/`blocks`, `origin` is a plain JSON number (`Addr` is `#[serde(transparent)]` over `u16`), and `System::C64` is the exact literal `Commodore 64` (`#[serde(transparent)]` over `String`). One item noted for completeness but not requiring a synthesis change: `ProjectState.settings` itself carries its own `#[serde(default)]`, so an entirely-absent `settings` object would still parse -- the two forced fields are forced by this module's own choice to always write `settings`, not because the schema requires the object's presence. This sharpens, but does not contradict, D-05.
...
1. Temporarily omitted `settings.use_illegal_opcodes` from the synthesised object.
   - RED: `not ok 2 - synthesizeProject: exact settings key set, forced values (D-05, pinned)` (diff showed the missing key) and `not ok 13` (the gated integration test's exported-`.a`-contains-`lax` assertion also failed, since the external analyser no longer decoded the illegal opcode without the forced setting).
...
- FOUND: `.claude/mcp/vice/anno-project.ts`
- FOUND: `.claude/mcp/vice/anno-project.test.ts`
