...
2. **Neither `tools-manifest.json` nor `tools-manifest.stock.json` is modified** — verified: `grep -c r2000` returns `0` on both, and `refresh-manifest.ts` would wipe a hand-added entry on the next refresh. `docs/tool-support.md` likewise contains zero r2000 mentions.
3. **The deletion is ~12k lines, not ~25.7k.** Of the 25,759 lines across `src/mcp/vice/r2000-*.ts`, roughly **13,700 survive under new names** — `r2000-coverage.ts` alone (2,292 + 6,484 test) has no r2000-process coupling at all beyond two string literals. Framing the milestone as a 25.7k-line deletion will produce a phase plan that deletes reusable assets (§2.4).
...
|-----------|----------------|-------------------------------------|
| `anno-tools.ts` | The tool surface: `ANNO_TOOL_DEFINITIONS`, `CURATED_ANNO_TOOLS`, `assertCuratedTool()`, `resolveStorePath()`, `runAnnoTool()` | Direct structural analogue of `r2000-tools.ts` (1,214 lines), minus the child-process runner |
...
| `acme-verify.ts` | Spawn a real ACME, parse **ACME's own** diagnostics, byte-compare the reassembled `.prg` | Rewrite of `r2000-verify.ts` (184 lines) — see the correction in §3.3 |
| `coverage.ts` | Derived-from-bytes coverage census the store's block table cannot move | Rename of `r2000-coverage.ts`, two functions repointed (§2.3) |
...
Two edits, both 1:1 substitutions of the existing r2000 route:
...
| `BACKEND_SEAM_BYPASS_KEYS` (:1504) | `["RESULT_CONTINUE_TOOL.name", "r2000Def.name"]` — the exact allow-list of registrations permitted to bypass `buildBackendAwareTool()` | `"r2000Def.name"` → `"annoDef.name"` |
...
| the runner-purity test (:1549) | Slices `runR2000Tool()`'s body out of `r2000-tools.ts` and asserts it contains none of `["forwardToVice", "ensureViceSession", "rewriteArguments"]` | repoint at `runAnnoTool(` in `anno-tools.ts`; **this is the mechanical expression of the CLAUDE.md constraint and must not be dropped** |
| the manifest-absence test (:1563) | Every `CURATED_R2000_TOOLS` name is absent from both manifests | repoint at `CURATED_ANNO_TOOLS` |
| `import { CURATED_R2000_TOOLS } from "./r2000-tools.ts";` (:44) | static import — breaks on deletion | repoint |

Also `registrations.length >= 5` (:1508) — a non-vacuity floor that stays satisfied by a 1:1 swap.

**Do not add anything to `capability-registry.ts`.** Read at HEAD: 26 entries, all `vice_*`, zero `r2000_*`. Its header states the exclusion rule explicitly — the registry is the *set difference between the two backend manifests*, minus registration artifacts. A proxy-local family is in neither manifest, so it contributes no delta. Its own doc comment names `vice_diagnose`/`vice_recycle` as the precedent: "A naive set-difference over the two manifests misclassifies them as a divergence; they are not one, and including them here would be a factual error, not merely an omission." The same reasoning covers the annotation family exactly. The registry's four consumers (`vice-proxy.ts`, `stock-dispatch.ts`, `scripts/generate-tool-support-table.mjs`, `scripts/check-skill-fork-honesty.mjs`) need no registry-driven change.
...
The pattern to carry over verbatim is **`resolveStorePath()`** in `r2000-tools.ts` (: search `export function resolveStorePath`). It is already the right shape and needs only its extension literal changed:
...
`r2000-tools.ts`'s own header states the rule the new module inherits: *"Never import the VICE host-path/container-path translation modules here … a project path is resolved against `repoRoot()` only. Asserted structurally by the closed host-path consumer-set test."*

**(b) The r2000-family absence tests must become anno-family absence tests.** The file contains a `readdirSync`-derived family scan that is *itself* pinned to the deleted subject:
...
- the INT-01 positive control naming `r2000-acme-ident.ts`, `r2000-regbits-gen.ts`, `r2000-symbols.ts`, `r2000-test-gate.ts` by literal filename — goes red on rename
- the absence test, whose own `assert.ok(r2000Modules.length > 0)` non-vacuity guard goes red
...
`src/mcp/vice/r2000-test-gate.ts` owns **two** gates, not one:

| half | symbols | consumers that must survive the deletion |
|---|---|---|
| regenerator2000 | `R2000_BIN`, `probeR2000()`, `R2000_AVAILABLE`, `skipReasonFor()`, `assertR2000RequiredIfEnvSet()` | all r2000 tests (deleted) + `r2000-launch.ts`, `r2000-mcp-client.ts` (deleted) |
...
| `docs/tool-support.md` | **NONE** (regenerated byte-identical) | contains zero r2000 mentions today |
...
| `scripts/generate-tool-support-table.mjs` | `:104` `R2000_LOOP_VAR_RE` | third witness; **throws** if missed (§1.2) |
| `src/mcp/vice/hostpath-consumers.test.ts` | family regex, floor, positive control (3 tests) | §1.3(b) |
| `src/mcp/vice/package.json` `files[]` | remove 15 `r2000-*.ts` entries + `r2000-regbits.json`; add the new/renamed set | `check-npm-packages.mjs`'s transitive-closure walk (static **and** dynamic imports) fails otherwise |
| `scripts/check-npm-packages.mjs` | `REQUIRED_DERIVED_MODULES` `:201` (`["r2000-cli.ts", "R2000-09"]`); the r2000 family names in the closure-walk comment `:218` | a listed-but-absent file fails the `need()` |
| `scripts/check-skill-tool-coverage.mjs` | `:49` static import of `CURATED_R2000_TOOLS`; `R2000_TOOL_NAME_RE` `:88`; the ≥10-name non-vacuity floor `:447`; the CLI-verb section `:457-490` | **CI gate — throws `ERR_MODULE_NOT_FOUND` on deletion** |
| `scripts/lib/r2000-cli-verbs.mjs` + `.d.mts` | rename; repoint the dispatch-switch parse at `anno-cli.ts`; `R2000_CLI_VERB_FLOOR` | parses the CLI's own switch, never a hand-typed array |
| `scripts/check-skill-fork-honesty.mjs` | the R2000-05 deletion pin `:412-500`; the positive pointer `need(... includes("r2000 export-asm"))` `:504` | **a direct contradiction: the pointer string must change or the new gate fires on it** (§5.2) |
| `scripts/audit-gate.mjs` | `:136` `"docs-r2000-decisions.test.ts"` in the gated-guard list | guard fate (§5.3) |
| `src/skills/*/SKILL.md` (5 files) + 4 reference/template/script files | ~148 `r2000` and ~27 `regenerator2000` mentions re-pointed; `routine-queue-walker`'s `description:` frontmatter rewritten | the five absorbed procedures' routes (§4, step 5) |
| `CLAUDE.md` | the `r2000_*` clause in the Architecture constraint bullet | the family it names ceases to exist |
| `.planning/ARCHITECTURE.md` | Rule A21 (`resolveStorePath`, `ChildProcess`, `r2000-mcp-client.ts`) | pinned by `docs-r2000-decisions.test.ts` (§5.3) |

### 2.3 RENAMED / REPOINTED — survives, do not rebuild

| Today | Becomes | Non-test lines | Repoint needed |
|---|---|---|---|
| `r2000-acme-ident.ts` | `acme-ident.ts` | 97 | none (zero imports) |
| `r2000-d64.ts` | `d64.ts` | 310 | none (zero imports) |
| `r2000-confidence.ts` | `confidence.ts` | 233 | none (zero imports) |
| `r2000-regbits-gen.ts` + `r2000-regbits.json` | `regbits-gen.ts` + `regbits.json` | 421 | none (zero imports) |
| `r2000-memmap-render.ts` | `memmap-render.ts` | 531 | none (zero imports) |
| `r2000-enum-gen.ts` | `enum-gen.ts` | 574 | drop `r2000-tools.ts` → `anno-store.ts` |
| `r2000-symbols.ts` | `anno-symbols.ts` | 388 | drop `r2000-launch.ts` / `r2000-mcp-client.ts` / `r2000-tools.ts` → `anno-store.ts`; keeps `stock-symbols.ts` (the live round trip) |
| `r2000-coverage.ts` | `coverage.ts` | 2,292 | **two functions only**: `storeBlockTypeAt()` (:1662) and `classFromStore()` (:1683) compare against upstream's Rust `Display` strings `"Code"` / `"Undefined"`; retarget at the store's own type representation. Its `decodeRawData` import becomes unnecessary (base64 is an upstream wire artifact). Everything else — including `AUTO_NAME_PREFIX_RE` (:1384) — is untouched |
| `r2000-project.ts` (part) | `prg-image.ts` | ~60 of 344 | `parsePrg()`, `flatImageOrigin()` are pure and reusable; `synthesizeProject()` / `ensureProjectSettings()` / `decodeRawData()` are r2000-shaped and go |
| `r2000-verify.ts` | `acme-verify.ts` | 184 | **rewrite, not rename** — see §3.3 |

Surviving test files by the same rule: `r2000-d64.test.ts` (396), `r2000-confidence.test.ts` (154), `r2000-regbits.test.ts` (250), `r2000-memmap-render.test.ts` (464), `r2000-enum-gen.test.ts` (353), `r2000-coverage.test.ts` (4,315), `r2000-coverage-grammar.test.ts` (2,169) — **8,101 test lines survive**, the great majority of it the coverage instrument's own six anti-vacuity controls.

### 2.4 DELETED outright

| File | Lines | Why it cannot survive |
|---|---|---|
| `r2000-launch.ts` | 357 | the `regenerator2000` spawn seam |
| `r2000-mcp-client.ts` | 795 | the NDJSON JSON-RPC client to the child |
| `r2000-session.ts` | 693 | the long-lived child session, FIFO queue, crash recovery |
| `r2000-tools.ts` | 1,214 | replaced by `anno-tools.ts` (surface shape carried, body replaced) |
| `r2000-cli.ts` | 1,503 | replaced by `anno-cli.ts` (verbs carried, bodies replaced) |
| `r2000-project.ts` (rest) | ~284 | `.regen2000proj` synthesis |
| `r2000-test-gate.ts` (r2000 half) | ~55 | the availability probe for a binary no longer used |
| their `*.test.ts` | 527 + 775 + 966 + 1,029 + 1,650 + 437 + 627 + 617 + 192 + 247 | see §5.3 for the guard-by-guard fate |

**Measured deletion arithmetic** (`wc -l src/mcp/vice/r2000-*.ts`): 25,759 total = 10,102 non-test + 15,657 test. Genuinely deleted: **~5,300 non-test + ~7,100 test ≈ 12,400 lines.** Surviving under new names: **~4,800 non-test + ~8,100 test ≈ 12,900 lines.** *A roadmap phase written to "delete 25,700 lines" will delete the coverage instrument and the enum generator. Size the deletion phase at ~12k and the rename phase separately.*
...
- **Names that no longer say `r2000`, per the seed's own instruction** — "All of it survives, under names that no longer say `r2000`."
...
The seed names five (`zpp_`/`zpa_`/`f_`/`a_`/`e_`). `r2000-coverage.ts:1384` — a **surviving** module — already owns the authoritative set:

```ts
export const AUTO_NAME_PREFIX_RE = /^(zpf_|f_|zpa_|a_|p_|zpp_|e_|j_|s_|b_|r_)/;
```

Eleven prefixes, exercised by a real fixture list in `r2000-coverage.test.ts:946`. The exporter and the store's naming rules should both read this one constant rather than re-declaring a subset. A committed real-world witness of the output format exists at `.planning/notes/dxa-ghidra-pivot-evidence/r2000.asm` — including four live `=*+$01` mid-instruction labels at lines 51, 81, 135, 145 — which makes an excellent golden fixture for the exporter.
...
**Persistence relative to the tool-call boundary:** *inside* it, before the result is constructed. Nothing is buffered across calls, so there is no session, no crash-recovery state machine, and no `saveAndVerify()` — the three most expensive pieces of the architecture being deleted (`r2000-session.ts` 693 + `r2000-mcp-client.ts` 795 + their 1,741 test lines) exist only because persistence lived in another process.
...
**§3.3 — a correction to the milestone's own premise, and the most important architectural finding in this section.** PROJECT.md's Active list says the store *"exports ACME source verified by a real ACME through the existing `--verify` seam."* Read at HEAD, `r2000-verify.ts` does **not** invoke ACME. It invokes `regenerator2000 --verify` (`import { buildVerifyArgs, runR2000 } from "./r2000-launch.ts"`) and parses **regenerator2000's** per-assembler summary transcript:
...
**Plan-level consequence:** a phase criterion worded "reuse the existing `--verify` seam" is unachievable as written. Word it as *"reuse `acmeVerdict()`'s five verdict rules, replacing the r2000 invocation with a direct ACME spawn matching `acme.mjs`'s argv."* `r2000-verify.test.ts` (247 lines) pins two real transcripts verbatim — the honest pass and the false-pass trap — which are r2000-format and must be re-recorded from real ACME output. This is a genuine cost the deletion incurs; do not plan it as a rename.
...
**Note the manifest disposition, deliberately departed from.** `upstream-procedure-manifest.json` classifies `r2000_undo` as **`omit`**, with the recorded rationale that a curated tool must serve a named criterion and undo did not. v0.7.0's Active list *supplies* that criterion ("undo … proven by planted violation"). The manifest's own third re-sync trigger anticipates precisely this: *"A future phase needs an upstream call this manifest currently disposes of as omitted … Adding one requires updating that entry's disposition in the same commit, so the manifest cannot silently disagree."* **Update the `r2000_undo` disposition in the same commit that adds `anno_undo`,** or `r2000-upstream-audit.test.ts`'s "every non-curated upstream call carries a justification and a citation" assertion becomes a record of a decision that has been reversed.
...
The precedent is `scripts/check-skill-fork-honesty.mjs:412-500` (the R2000-05 deletion pin). Its properties, each of which the new gate should copy:
...
| **a positive check that the replacement pointer still exists** | `need(acmeBuildSkillSource.includes("r2000 export-asm"), "the deletion must not be 'fixed' by deleting the pointer to the proven route too")` |
...
**Correction 2: each block carries TWO lines containing "regenerator2000", not one** — the `Adapted from regenerator2000.` line and the `Source repository: https://github.com/ricardoquesada/regenerator2000` line. So the exemption set is **10 lines across 5 blocks in 3 files**, and the non-vacuity counter should require exactly 10, not 3. Verified line numbers:
...
|---|---|---|
`r2000-spawn-seam.test.ts` (627) | `EXPECTED_R2000_SPAWN_SITES` + `assertNoViceFlag` before every r2000 spawn; derives its module set from `package.json` `files[]`; reads `.planning/phases/18-*/evidence/` | **Delete with the subject.** The `--vice` invariant it enforces is moot when nothing spawns r2000. Its `codeOnly()` string-literal stripper and `shippedTsModules()` idiom are reused by `stock-dispatch.test.ts:2889,2919` — **extract those two helpers first** or that file breaks. |
`docs-r2000-decisions.test.ts` (in `scripts/audit-gate.mjs:136`) | `.planning/ARCHITECTURE.md`'s Rule A21 (names `resolveStorePath`, `ChildProcess`, `r2000-mcp-client.ts`) and PROJECT.md's D-36 row | **Split.** The D-36/D-32 supersession assertions are pure historical-record checks and stay green if the Key Decisions rows stay (they should — Key Decisions is a ledger, not live documentation). Rule A21 describes a module that ceases to exist: mark it superseded with a date in `.planning/ARCHITECTURE.md` and narrow the guard to assert the supersession note, or retire rule+guard together and remove the name from `audit-gate.mjs:136`. **Removing the name from `audit-gate.mjs` without removing the test, or vice versa, breaks the audit gate** — its own comment records that the two were added in the same commit for this reason. |
`r2000-answer-key.test.ts` (282) | `.planning/phases/11-*/evidence/criterion1/` with **no existence guard**; the sealed-question hash chain; a committed ACME fixture byte-compare | **Rename and keep** (`sealed-question.test.ts`). It is an evidence-integrity guard over a historical artifact, not a test of r2000 code. Keeping it also keeps PROJECT.md's "do not archive phase directories" decision intact — deleting it would silently relax one of that decision's two stated reasons and should be an explicit choice, not a side effect. |
**`hostpath-consumers.test.ts`** (326) | `r2000ProductionModules()`, `R2000_MODULE_FLOOR = 14`, an INT-01 positive control naming four r2000 filenames | **Repoint** (§1.3b). Three tests go red on deletion. |
`r2000-verb-coverage.test.ts` (192) | `parseR2000CliVerbs()` over `r2000-cli.ts`'s dispatch switch; `REAL_VERBS` (8) | **Rewrite** against `anno-cli.ts`. |
`r2000-symbol-roundtrip.test.ts` (617) | the live r2000↔VICE symbol round trip; reads `.planning/phases/` | **Rewrite** as `anno-symbol-roundtrip.test.ts`. The capability (`R2000-14`/`R2000-15`) survives in `anno-symbols.ts`; only the static half's provider changes. |
`r2000-verify.test.ts` (247) | two verbatim r2000 `--verify` transcripts; asserts `r2000-test-gate.ts` is absent from `files[]` | **Rewrite** (§3.3). Fixtures must be re-recorded from real ACME. Preserve the `files[]`-absence assertion, repointed at `acme-test-gate.ts`. |
`r2000-upstream-audit.test.ts` (207) | `.planning/phases/19-*/upstream-procedure-manifest.json` — **not** r2000 code | **Rename and keep** (`upstream-procedure-audit.test.ts`). This guards the manifest the new tool surface is *derived from*; deleting it removes the derivation's own integrity check at the exact moment it matters most. |
...
`scripts/check-skill-tool-coverage.mjs` | static `import { CURATED_R2000_TOOLS } from "../src/mcp/vice/r2000-tools.ts"`; `R2000_TOOL_NAME_RE`; ≥10-name floor; CLI-verb floor 8 | **Repoint.** Throws `ERR_MODULE_NOT_FOUND` the moment `r2000-tools.ts` is deleted. |
`scripts/generate-tool-support-table.mjs` + its two independent witnesses | `R2000_LOOP_VAR_RE` hard-codes `R2000_TOOL_DEFINITIONS` | **Repoint all three** (§1.2). **Throws — and therefore reddens `docs/tool-support.md`'s byte-identity drift guard — the moment the collection is renamed, i.e. before any deletion.** |
...
`r2000-spawn-seam.test.ts`, `skill-attribution.test.ts`, `r2000-answer-key.test.ts`, `r2000-coverage.test.ts`, `r2000-verify.test.ts` (plus `r2000-session.test.ts`, `r2000-tools.test.ts`, `r2000-symbol-roundtrip.test.ts`, `r2000-upstream-audit.test.ts` — nine in total by measurement). Two are worst-case: `r2000-answer-key.test.ts` reads `.planning/phases/11-*/evidence/` with **no existence guard**, and `skill-attribution.test.ts` / `r2000-upstream-audit.test.ts` read the Phase 19 manifest by an absolute relative path.

**Roadmap constraint, restated for the planner:** `.planning/phases/` accumulates by design (PROJECT.md Key Decisions, 2026-08-23), and the milestone close must pass `--no-archive-phases`. The set of guards forcing that just changed shape — two of them (`docs-review-disposition.test.ts`, `skill-attribution.test.ts`) survive v0.7.0 regardless, so the constraint stands even if `r2000-answer-key.test.ts` is deleted. Do not treat the deletion as a route to relaxing it.
...
**Deliver:** `acme-test-gate.ts` (the ACME half of `r2000-test-gate.ts`, §1.4); `prg-image.ts` (`parsePrg`, `flatImageOrigin`); the `codeOnly()` / `shippedTsModules()` helpers `stock-dispatch.test.ts:2889,2919` borrows from `r2000-spawn-seam.test.ts`. Repoint `disasm-roundtrip.test.ts` and `skill-acme-build-cli.test.ts` at the new ACME gate.

**Why first:** these are the modules with surviving consumers *outside* the r2000 family. Every one of them is a silent breakage if the deletion runs first, and each is a pure move with no behaviour change — the cheapest possible phase, and it makes every later phase's diff readable.

**Green bar:** full `npm test` unchanged (not `test:automated` — that skips `MANUAL_ONLY_TESTS`). No r2000 module deleted yet.

### Phase 28 — The store: model, persistence, undo

**Deliver:** `anno-model.ts`, `anno-store.ts`. The 12-member type vocabulary. Ranges, scopes, project enums, labels, comments. Atomic write + in-document undo journal. `resolveStorePath()` carried over from `r2000-tools.ts` with the extension literal changed.
...
**Green bar:** `npm test` green **including** `tool-support-table.test.mjs` and `docs/tool-support.md`'s byte-identity drift guard; `node scripts/check-skill-tool-coverage.mjs` exits 0; `node scripts/check-npm-packages.mjs` green. **Both r2000 and anno families are registered simultaneously at this phase's close.** That is the point: the replacement is demonstrably ready before anything is removed.
...
**Verify externally, per the project's own six-times-learned lesson:** the acceptance oracle is a real ACME 0.97 reassembling the export to bytes identical to the input, using `.planning/notes/dxa-ghidra-pivot-evidence/r2000.asm` as the committed golden witness of the target format. Re-record `r2000-verify.test.ts`'s two transcripts from real ACME output — the honest pass and a deliberately-constructed false-pass trap.
...
**Green bar:** `skill-attribution.test.ts` green; `check-skill-tool-coverage.mjs` green with **zero** `r2000_*` names extracted and its floor re-expressed over `anno_*`; `check-skill-fork-honesty.mjs` green with the positive pointer repointed to `"anno export-asm"` (§5.2).

### Phase 32 — The deletion and the gate

**Deliver:** delete the ~12.4k lines in §2.4; rename the ~12.9k in §2.3; repoint `coverage.ts`'s two functions; discharge every guard fate in §5.3; build the whole-tree grep gate with the 10-line block-scoped attribution exemption and its exactly-10 non-vacuity counter; **observe the gate biting on a planted reintroduction** before accepting it; update `CLAUDE.md`'s constraint bullet, `.planning/ARCHITECTURE.md`'s Rule A21, and `scripts/audit-gate.mjs:136` in step with `docs-r2000-decisions.test.ts`'s chosen fate.
...
27 (extract shared seams)
 └─► 28 (store: model, persistence, undo)
      └─► 29 (MCP surface + every registration-time guard)   ← r2000 and anno coexist here
           ├─► 30 (ACME exporter + real-ACME round trip)
           └─► 31 (re-point the five absorbed procedures)
                └─► 32 (deletion + grep gate + guard fates)   ← r2000 gone
                     ▲
...
**What people do:** carry `r2000_save_project` across as `anno_save`, with mutations buffered in memory until it is called.
...
**Instead:** all twelve, read verbatim off `r2000_set_data_type`'s schema.
...
| **regenerator2000** | **removed** | `cargo install` only, rustc ≥ 1.90, ~5 min build — which is why `VICE_REQUIRE_R2000` was never set in CI. Its removal deletes a documented install prerequisite from the README. |
...
1. **Tool prefix.** `anno_*` is assumed throughout. `store_*` and `vice_anno_*` are alternatives; `vice_*` is unavailable (it would collide with the manifest namespace and confuse `check-skill-tool-coverage.mjs`'s extraction regexes). Whichever is chosen, the grep gate's rule becomes "no shipped tool name may contain `r2000_`", which is prefix-independent.
2. **`docs-r2000-decisions.test.ts` / Rule A21.** Two defensible fates (§5.3). This is a decision, not a mechanical fix, and it moves `scripts/audit-gate.mjs`.
3. **`r2000-answer-key.test.ts`.** Keeping it preserves the "do not archive phase directories" decision's second leg. Deleting it should be an explicit choice recorded as such.
...
- `src/mcp/vice/hostpath-consumers.test.ts` — full file; `EXPECTED_IMPORTERS`, `r2000ProductionModules()`, `R2000_MODULE_FLOOR`, `DERIVED_TOOL_MODULES`
- `src/mcp/vice/r2000-tools.ts` — header, `R2000_TOOL_DEFINITIONS` (19 names), `CURATED_R2000_TOOLS:634`, `resolveStorePath()`, `r2000_set_data_type` schema (12-member enum)
- `src/mcp/vice/stock-dispatch.test.ts` — `:44`, `:1485-1575`
- `src/mcp/vice/r2000-verify.ts` — header, `:47` (`buildVerifyArgs`/`runR2000` import), exports
- `src/mcp/vice/r2000-test-gate.ts` — both gate halves; consumer scan
- `src/mcp/vice/r2000-coverage.ts` — `:131-134`, `:1384`, `:1662`, `:1683`
- `src/mcp/vice/r2000-project.ts`, `r2000-symbols.ts` — export lists
- `src/mcp/vice/docs-linerefs.test.ts`, `capability-registry.test.ts:154-174`, `tool-support-table.test.mjs:62-78`
- `src/mcp/vice/package.json` — `files[]` (15 r2000 entries + `r2000-regbits.json`)
...
- `.planning/notes/dxa-ghidra-pivot-evidence/r2000.asm` — committed real ACME export with 4 `=*+$01` labels
...
Measurements: `wc -l src/mcp/vice/r2000-*.ts` (25,759 = 10,102 non-test + 15,657 test); `grep -c r2000` on both manifests and `docs/tool-support.md` (0, 0, 0); `grep -rc regenerator2000 src/skills` (27 across 8 files); `grep -rc r2000 src/skills` (~148 across 10 files).
