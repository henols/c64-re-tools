...
**`STORE-01` and `STORE-03` stay `Gaps Found`, blocked by `CR-09`, and no edit was needed to keep them there.** Both were already at `Gaps Found` / `[ ]` before this round's record task ran, applied by commit `737f9e7`; the two moves the round-5 report asks for in that direction were therefore already satisfied, and this sentence records that they were CONFIRMED rather than forgotten. `CR-09`, in one sentence: `retype()`'s split-and-preserve wrote a split-table row with an ODD byte count — a shape the store refuses at its own entry point and its own resolver cannot decode. The same Requirements Coverage table states for `STORE-01` that *"the store persists a per-range typing it refuses at its own entry point and its own resolver cannot decode (CR-09)"*, and for `STORE-03` that the requirement's third clause, *"the behaviour when a typed range is partially overwritten"* pinned, *"is NOT met for the four split members"*. `CR-09` is dispositioned `fix` by 28-19 in `28-REVIEW.md`'s round-5 disposition table and its reproduction is re-driven on the final tree by 28-22's closing gate.

**This round is EXECUTED, not VERIFIED, and `STORE-01` and `STORE-03` move only when a verification pass says so.** 28-19, 28-20, 28-21 and 28-22 have run and committed; no verification pass has yet scored the tree they produced. The phase verifier remains the authority on every row above, and an executor's confidence in its own fix is not a verdict. The `pragma integrity_check` throw arm inside `openStore` (the `catch` around `db.prepare("pragma integrity_check").all()`, `anno-store.ts:530-535` on this round's tree) and 28-17's `backstop`-tagged host-crash durability bound both remain OPEN human-verification items, carried forward as explicit STILL-OPEN rows in `28-REVIEW.md`'s round-5 disposition section and claimed closed by nothing in round 5.
