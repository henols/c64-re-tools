...
- [x] **CUT-01**: The regenerator2000 integration is deleted — a **net 6,309 lines** removed from the **26,023-line, 35-file** `r2000-*.ts` surface (10,035 non-test + 15,988 test), the remaining **19,714 lines surviving under new names as measured at commit `f16d0b1`**. The surviving figure names its commit deliberately rather than floating with HEAD: the surviving `anno-*` files keep growing with ordinary post-deletion work — the same predicate yields 20,960 at `6715a75` — so a HEAD-relative figure is not re-derivable by a later reader **by construction**, which is the exact defect this entry exists to prevent. These figures supersede v0.6.0's `CUT-01`/`CUT-02`, which asserted 19,181: the surface was 26,023 rather than 19,181, and a phase sized at 26.0k deletes the coverage instrument and the enum generator.

  **Provenance, carried here rather than in a footnote, so a later reader re-derives every figure instead of taking it on trust.** Pre-phase end measured at commit `8f21d77` (the Phase 29 open); surviving end at `f16d0b1` (the phase HEAD this correction was written against). File-selection predicate at `8f21d77`: `git ls-tree -r --name-only 8f21d77 -- src/mcp/vice` filtered by `^src/mcp/vice/r2000-.*\.ts$` — 35 files, with `r2000-regbits.json` excluded because the requirement is a `.ts` claim. Predicate at `f16d0b1`: the `anno-*` name-descendants of that same set, plus the three renamed test files `absorbed-answer-key.test.ts`, `spawn-seam.test.ts` and `docs-absorbed-decisions.test.ts` that `29-VERIFICATION.md` enumerates individually. Counting command at **both** ends, so "lines" has one definition across the subtraction rather than two that merely get compared: `git show <commit>:<path> | wc -l`, summed over the selected set. `wc -l` counts newline characters, so a file whose last line lacks a trailing newline contributes one fewer than its visible line count — accepted deliberately, because the identical rule applies at both ends of the subtraction. What this measurement adds to the predicate, which is unchanged: of the 35 pre-phase files, **15 have no name-descendant at `f16d0b1`** and were removed outright (enumerated by name in `29-21-SUMMARY.md`), while **20** survive as `anno-*` descendants and are joined by the three renamed test files — **23 surviving files**. The descendant map is **injective**: no two pre-phase paths map onto one survivor and no survivor is counted twice.

  **Re-derivation status, stated rather than implied, because a figure whose provenance does not reproduce is the condition this entry exists to fix. BOTH ENDS NOW RE-DERIVE.** The pre-phase end re-derives **exactly**: 26,023 = 10,035 non-test + 15,988 test across 35 files at `8f21d77`. The surviving end re-derives **exactly** at the commit the sentence above names: **19,714** at `f16d0b1` across 23 files (20 `anno-*` name-descendants totalling 18,728, plus the three named files totalling 986), so the net removal is the subtraction 26,023 − 19,714 = **6,309**. The same predicate yields **19,714** at `d30b63e` as well — this entry previously recorded 18,728 for that commit, which was the 20-descendant subtotal quoted as though it were the total, and that clause is corrected here — and 20,960 at `6715a75`, which is why the surviving figure is anchored rather than left floating. The measurement was re-run with the working tree dirty and returned identical figures (`git show` reads the object store, not the tree), and re-derived a second time with the path list sorted in a different order, equal both times. **The surviving-line and net-removal figures previously recorded in this entry were superseded on 2026-08-30 by this measurement**, and are deliberately not restated here as live figures — a record whose whole purpose is that its numbers are the measured ones does not carry falsified numerals in its live text. The superseded numerals remain legible in the dated paragraphs below the traceability table, in `29-VERIFICATION.md`, and in `29-21-SUMMARY.md`, which quotes every command and its raw output
...
- [x] **CUT-04**: Every guard and CI script pinned to the deleted subject has a recorded fate and none passes **vacuously** — measured mechanically at the v0.7.0 open, resolving the research documents'
  ...
  - **Gap 1 was closed by ADJUDICATION rather than by demoting this row — recorded here, beside the row, on 2026-09-01 in gap-closure round 1.** **What the verifier found:** `32-VERIFICATION.md` scored `CUT-04` ⚠ **SATISFIED WITH ONE NAMED EXCEPTION** — 61/61 fates recorded, 35/35 machine-captured reds with green controls, and the guard non-vacuous under ten independent registry mutations — **but** `docs-linerefs`, named in this requirement's own "Named explicitly:" list, had no fate and no exclusion. The substance held; only the record was short. **The decision and whose it is:** the verifier's own recommendation, quoted rather than paraphrased — *"My recommendation is to close Gap 1 (a paragraph, not code) rather than to demote the row, and to record that decision beside it."* That recommendation was followed. **Where the closure lives:** `evidence/32-audited-set-reconciliation.md` § **6.1 `CUT-04`'s NAMED list, adjudicated name by name**, which resolves all eleven names against the registry's `historicalPath` basenames under a stated, re-runnable equality rule and gives each one a verdict — **10 CITED, 1 EXCLUDED, 0 AMBIGUOUS**, quoted from the section as written. Each CITED name quotes its own registry row's `historicalPath` and `verdict`; the one EXCLUDED name, `docs-linerefs`, carries its measured occurrence count at **both** pinned commits (0 at `0394cbc`, 0 at `345d5c4`, with `grep -a`), the measurement that it was not added between them, and the place its non-vacuity was discharged instead — plan 32-04's widening of the guard onto `.planning/PROJECT.md` with a per-document floor at `docs-linerefs.test.ts:192` and its planted violations. The audit's figures are **cited to `32-VERIFICATION.md`**, which measured them, and are not restated here as fresh measurements. **What was deliberately NOT done, so a later reader does not re-litigate it:** the row is **not demoted** — per this file's standing rule the checkbox, the sizing sentence and the traceability row move in one edit, and this note moves **none of the three**; **no registry row was added** for the excluded guard, because `scripts/check-guard-fates.mjs:672`'s stranger-row rule rejects a row naming a path the derivation does not admit and would turn the blocking CI step red for exactly the reason that gate exists; and **no `overrides:` entry was written**, because the substantive claim was never in dispute and recording a divergence as acceptable would defeat this requirement's own purpose.
...
| REPOINT-03 | Phase 31 | Complete |
| REPOINT-04 | Phase 31 | Complete |
...
**`CUT-01` MOVES BACK UP to `Complete` on 2026-08-30, on a measurement this plan TOOK rather than transcribed — superseding the move-back-down paragraph below, which is kept as the record rather than deleted.** Plan 29-21 executed the predicate this entry itself documents, at the two commits it names, with the single counting command it names, and wrote **whatever the commands returned**: pre-phase 26,023 across 35 files at `8f21d77` (10,035 non-test + 15,988 test); surviving **19,714** across 23 files at `f16d0b1`; net removal **6,309**. Those figures agree with `29-VERIFICATION.md`'s independent measurement of 2026-08-30T15:20Z exactly, and the sizing sentence now **names `f16d0b1`** so the surviving figure re-derives for the next reader instead of drifting with HEAD — the same predicate returns 20,960 at `6715a75`. The **substantive** claim was never in dispute and is unchanged: zero `r2000-*.ts` files remain under `src/mcp/vice/`, and the removal gate is green tree-wide with an **empty** temporary allow-list, re-observed by this plan rather than assumed. **No `overrides:` entry was written**, and that was a decision rather than an omission: `29-VERIFICATION.md` offered one accepting the non-reproducing figures as decided, but the owner's 2026-08-30 decision was *"correct the text to the measured figures"*, and recording a divergence as acceptable would defeat this requirement's own stated purpose — that its numbers are the measured ones. Per this file's standing rule the checkbox, the sizing sentence, both provenance paragraphs and the traceability row all moved in **one** edit. The superseded surviving-line and net-removal numerals are not restated in the live entry; they are preserved verbatim in the two dated paragraphs below.
...
| `MCP-04` | Complete | `29-VERIFICATION.md` (Requirements Coverage, `MCP-04` row) is the authority, and its re-verification pass of 2026-08-30T15:20Z scores this requirement ✓ **SATISFIED (was BLOCKED)**: *"Both defects re-executed and closed; named tests exist for each (`anno-tools.test.ts:823`, `:846`, `:1352`, `:1409`)."* Both findings that held the row down are closed. **CR-01** — the refusal-by-name clause: *"`anno_disassemble` now refuses an out-of-image address with `{available:false, reason}`, the SAME verdict `anno_read_region` gives … Re-executed through `runAnnoTool()`, not read from a summary."* **CR-06** — the documented nesting: *"a nested `anno_batch_execute` relying on the documented store inheritance now VALIDATES and EXECUTES at depth 2, and depth 5 is refused by name … Re-executed."* That the verifier **re-executed** both rather than reading the fix is the property that makes this promotion trustworthy, and it is why the row moves. Evidencing summaries, per this table's convention: `29-03-SUMMARY.md` and `29-06-SUMMARY.md` (explicit addressing, idempotent edits, per-item batch status — the three clauses that already held), `29-13-SUMMARY.md` (the two repairs the verdict re-executed) |
...
**`REPOINT-03` and `REPOINT-04` MOVE UP to `Complete` on 2026-08-31, on Phase 31's re-verification round 2, and on nothing else.** Both rows read `Pending` throughout gap-closure round 1 **by design**: plan `31-04` closed both of round 1's gaps and deliberately declined to promote either requirement, on the standing rule that a status row must not move ahead of the re-verification verdict that scores it — `31-04-PLAN.md` carries the refusal as an explicit prohibition, and `31-04-SUMMARY.md` records `requirements-completed` as `[]`. That verdict now exists. `31-VERIFICATION.md` round 2 scores the phase **`passed`, 13/13**, and closes both of round 1's gaps at the root cause — re-executed rather than read. Gap 1: round 1's own reproduction now inverts, a relocated `installer/skills/` under `CI=1` yielding `# pass 0 # fail 1` where round 1 got `# pass 1 # fail 0 # skipped 0`, because `.github/workflows/ci.yml:126-128` materialises the tree through its single existing producer before the `Test` step at `:130`, and because `emptyRootVerdict()` replaced `skippableEmptyRoot()` — keyed on ABSENCE rather than emptiness, never skipping under CI, with an empty-but-present tree live-proven to fail. Gap 2: `namingLineCountsIn()` now takes `readonly string[]` fed only by a line-anchored `attributionBlockLines()`, so a capture-group boundary fragment is not expressible; the verifier audited all nine call sites by hand and confirmed zero `attributionBlocks(...)` splits in executable code, then proved the two new boundary plants non-vacuous by re-evaluating the pre-fix path over the same texts. That the verifier **re-executed** both gaps rather than reading the fix is the property that makes this promotion trustworthy, and it is why the rows move. Per this file's standing rule the checkboxes (`:120`, `:121`), the traceability rows and this paragraph moved in **one** edit. As with `REPOINT-01`/`-02` this is a **two**-site edit rather than four — neither requirement appears in the `MCP-*`/`STORE-06` status table above. Recorded from the orchestrator seat during `/gsd-execute-phase 31 --gaps-only`, completing the checkbox-and-row flip that `phase.complete` had already made: unlike `MCP-04`, no later gap-closure plan is scheduled under this phase to carry it, and leaving `- [x]` standing with no dated evidence would be the one-file-two-answers condition this section exists against.

**`REPOINT-01` and `REPOINT-02` MOVE UP to `Complete` on 2026-08-30, on re-verification round 3 (`2026-08-30T17:46Z`), and on nothing else.** Both rows read `Gaps Found` throughout gap-closure round 2 **by design**: all four of that round's plans (`29-18`, `29-19`, `29-20`, `29-21`) closed items under these two requirements and each one deliberately declined to promote them, on the standing rule that a status row must not move ahead of the re-verification verdict that scores it. That verdict now exists. `29-VERIFICATION.md` round 3 scores the phase **`passed`, 6/6**, and closes **all four** of the `missing` items round 2 had left open under gap 1 — re-executed rather than read: a memory map rendered under one workspace root and byte-copied to a second root at a different absolute path now reports `in sync`, exit 0, with three negative controls (a hand edit, a store label rename, and a store moved relative to the root) each still reporting `drifted` at a named line, so the gate was not merely blunted. The mechanism is one new seam, `workspaceRelativePath()` (`src/mcp/vice/anno-types.ts:1245`), consumed at `src/mcp/vice/anno-memmap-render.ts:467-468`. `WR-01` closed with it: round 2's exact planted invocation (`anno coverage game.prg`, required `--store` omitted) was re-planted and the gate now exits 1 naming the missing flag. Per this file's standing rule the checkboxes (`:118`, `:119`), the traceability rows and this paragraph moved in **one** edit. Unlike `MCP-04` this is a **two**-site edit rather than four — neither requirement appears in the `MCP-*`/`STORE-06` status table above.

**`MCP-04`'s row MOVES UP to `Complete` on 2026-08-30, on the re-verification verdict of 2026-08-30T15:20Z — which is exactly the condition the paragraph below names, and nothing else.** That paragraph set the rule: *"The row moves back UP only on a **re-verification verdict** — never on the completion of the gap-closure plans that address `CR-01` and `CR-06`, and never on an executor's reading of its own work."* That verdict now exists. `29-VERIFICATION.md` § Requirements Coverage scores `MCP-04` ✓ **SATISFIED (was BLOCKED)**, and the two findings that held it down are closed: **CR-01**, the out-of-image refusal, and **CR-06**, the nested batch's documented store inheritance. Each was **re-executed** by the verifier rather than read — *"Re-executed through `runAnnoTool()`, not read from a summary"* — and each has named tests (`anno-tools.test.ts:823`, `:846`, `:1352`, `:1409`). All four sites moved in one edit: the checkbox, the traceability row, this table's status cell **and its evidence cell**, because leaving the superseded BLOCKED quote standing beside a `Complete` status would recreate the exact one-file disagreement the paragraph below was written to end. The promotion was taken **by a gap-closure plan (29-21) rather than from the orchestrator seat**, which is how `STATE.md` recorded it had been left: *"MCP-04 is scored SATISFIED by the verifier but its row still reads Gaps Found — a promotion left for the next gap plan rather than taken from the orchestrator seat."* No other requirement's status moved with it: `REPOINT-01` and `REPOINT-02` stay `Gaps Found` until a re-verification says otherwise.
