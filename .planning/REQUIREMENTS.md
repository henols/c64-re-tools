...
**Five STORE rows read `Complete` on 2026-08-28, at the close of the FOURTH gap-closure round (28-16..28-18), and one deliberately does not.** All six were reverted out of `Complete` after round-3 verification scored the phase 10/12. `STORE-01`, `STORE-02`, `STORE-03` and `STORE-07` were re-established by 28-15's closing gate, on re-observed evidence rather than on the aggregate: the split-orientation control and its collapse planting, the exhaustive 65,536-address index cross-validation with its `comparisons` non-vacuity assertion, the fully-contained overlap case with its filter-and-insert planting, and the no-splitter structural scan with its `STORE-02` non-vacuity companion were each run and quoted individually; `node:sqlite` resolves to exactly one shipped module and both store modules remain absent from the five-element host-path consumer set. 28-18's closing gate re-observes every one of those controls individually on the final tree of round 4.

**`STORE-05` returns to `Complete`, and the authorising sentence is the phase verifier's own, not this round's opinion of its own work.** Round-4 verification (`.planning/phases/28-the-store-core/28-VERIFICATION.md`, Requirements Coverage, STORE-05 row) scores it `✓ SATISFIED` and states: *"Every clause verified in the DDL and by green controls: `schema_version`, `bank` nullable on all four annotated tables and never read, `access_kind not null` with a fifth value refused. No open finding touches STORE-05. **The `Gaps Found` row is NOT correct on the evidence** — it was held pending this verification pass, and this pass clears it."* The row was held for that pass; the pass has run.

**`STORE-04` stays `Gaps Found`, and its recorded REASON is corrected.** The same round-4 Requirements Coverage table states that the `Gaps Found` row is correct *"for a stronger reason than the one recorded"*: this file previously cited the pending `pragma integrity_check` human-verification item, and **the real blocker is `CR-08`** — `revertTo` replacing the live store with a snapshot image it never opened, which falsifies "an edit can be reverted" in the destructive direction. The proof obligation as literally worded is met (the combined planted-violation test and its live planting are green, and criterion 4's red is re-observed on round 4's final tree), but that is the durability CONTROL, not the requirement's full text. `CR-08` is dispositioned `fix` by 28-16 and its reproduction is re-driven on the final tree by 28-18's closing gate; the row moves only when a verification pass says so. The `pragma integrity_check` throw arm (`anno-store.ts:432`) remains an OPEN human-verification item, carried forward in `28-REVIEW.md`'s round-4 disposition table and claimed closed by nothing in round 4. The phase verifier remains the authority on every row above.
