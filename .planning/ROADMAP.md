...
| 11.1 Close v0.3.0 Audit Items (INSERTED) | v0.3.0 | 3/7 | In Progress|  |
...
- **Prefer `.vsf` over `.raw` for anything leaving the emulator.** VICE snapshots
  ...
  flag to override it. **This is an emulator-output rule, not a regenerator2000
  input-set rule** (amended post-hoc, D-03/D-34): it does not extend to
  regenerator2000's input set. The D-01 synthesis route never hands
  regenerator2000 a container format at all — it synthesises the
  `.regen2000proj` directly in Node from `.prg`/`.d64`/flat-64K, so that is the
  whole supported input set. `.vsf` as a bootstrap input is deferred as backlog
  (`.planning/todos/pending/2026-08-20-vsf-as-a-bootstrap-input.md`, D-34) —
  regenerator2000's auto-detected machine-type field is correct only by
  coincidence for C64 snapshots
  (`docs/phase9-regenerator2000-probe-findings.md` § Accepted limits, entry 2),
  and no `R2000-*` requirement covers this capability.
...
- [x] 11-04-PLAN.md — The MCP client seam (D-16/D-17) and the three fixed argv builders, including D-28's unbuildable-discard pairing

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 11-05-PLAN.md — The 17 curated `r2000_*` tools, the D-33 batch gate, proxy-local registration, and the manifest answer asserted twice

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 11-06-PLAN.md — Enums from `memmap.json`: the re-runnable curated bit-name table, the adjacent-pair pass, and criterion 3 against the ACME export
- [x] 11-07-PLAN.md — Criterion 1 session A: annotate a real program into the store and seal a store-only question
...
- [x] 11-10-PLAN.md — The confidence convention (D-25) and the generated memory map (D-24/D-27) with a drift-detecting `--check`
- [x] 11-11-PLAN.md — Criterion 4 live on genuine stock `x64sc`: one closed loop, plus this phase's BACK-02 gate
...
Plans:
- [x] 11.1-01-PLAN.md — FLOW-02: reword both `.vsf` surfaces to name the backlog file, retarget the test that pinned "Phase 11", and guard every shipped string literal against phase pointers (wave 1)
- [x] 11.1-02-PLAN.md — FLOW-01: document `gen-enums` in `c64-memory-mapping` and the `export-lbl`/`import-lbl` round trip in `c64-program-recon`, behind a verb-coverage gate derived from `r2000-cli.ts` (wave 1)
- [x] 11.1-03-PLAN.md — INT-01 + Phase 10 IN-02: make the `R2000-02` absence assertion self-deriving with a floor, and teach it multi-line and dynamic imports (wave 1)
...
| `.vsf`/`.raw` bridge as its own requirement | `R2000-08` | Reduced to a note on Phase 10 criterion 3 — it is which file extension you hand over, not a deliverable. Prefer `.vsf` for anything leaving the emulator, but that preference does not extend to regenerator2000's input set (D-34): the project's own synthesis route only ever hands over `.prg`/`.d64`/flat-64K. |
