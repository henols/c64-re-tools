...
**Plans**: 4/4 — 3/3 original plans executed in 3 waves, then **verified at 5/7 — `gaps_found`**, so **one gap-closure plan (31-04, in a fourth wave) was added and executed**, after which **re-verification round 2 scored the phase `passed`, 13/13**, closing both gaps at the root cause and promoting `REPOINT-03` and `REPOINT-04` to `Complete`. The verification report is explicit that **both ROADMAP success criteria are VERIFIED by independent direct measurement rather than read from the SUMMARYs** — the 10 ABS-02 instances across two trees with both naming lines byte-identical in every one, all three shared files byte-identical between trees, the description rewrite substantive with `ABS-03` green at a 0.250-versus-0.35 margin, and the manifest landing in exactly one commit (`7adcbaf`, 1 file, 9+/8−) with `r2000_undo`'s `omit` intact and `STORE-04` attached. The two gaps are narrowly about the **durability of the guard plan 31-02 committed**, and the report fences them to two files: (1) `installer/skills/` is gitignored and materialised only by `prepack`, and CI's `Test` step precedes every step that generates it, so the two-tree assertion silently scores ONE tree and reports a clean pass with zero skips (reproduced by relocating the tree); (2) the naming-line predicate is exact but is handed a mid-line-anchored regex capture, so its documented `grep -rx` whole-line equality is false at both block boundaries — latent today, because all 10 real blocks have empty boundary fragments, but a false-ACCEPT hole plus a code comment the tree falsifies. `REPOINT-03` and `REPOINT-04` both stay **`Pending`**: `REPOINT-04` is scored ✓ SATISFIED on the merits and moves with `REPOINT-03` on the re-verification verdict, per the standing four-sites-one-edit rule, not ahead of it.
...
**Wave 4** *(gap-closure round 1 — `31-VERIFICATION.md` scored 5/7 must-haves with BOTH success criteria VERIFIED; the two gaps are the guard's reach in CI and its whole-line claim at block boundaries. Scoped by the report itself to `src/mcp/vice/skill-attribution.test.ts` and `.github/workflows/ci.yml`, plus the durable disposition record. The plan's own frontmatter carries `wave: 1` — the round holds a single plan, so its round-local wave is 1; this heading numbers waves cumulatively across the phase)*

- [x] 31-04-PLAN.md — Close both durability gaps at the root cause, tracer-first: a CI step ordered before `Test` materialises the gitignored shipped tree through the one existing producer (`installer/scripts/sync-skills.mjs`, the command `prepack` already runs), and the empty-root classifier becomes a three-valued verdict keyed on **absence** rather than emptiness, refusing to skip under CI, recording a named reason and announcing every unscored root with a `t.diagnostic()` plus an unconditional scored-of-declared summary — so a one-tree run can no longer read as a two-tree run (gap 1, `CR-01`, `WR-01`). Then the naming-line predicate's parameter becomes an array of whole physical lines fed by a line-anchored block extractor, making the `grep -rx` claim true by construction, with both boundary positions the interior plants cannot reach planted in memory and paired with one-newline controls (gap 2, `CR-02`). `WR-03` (a magic floor derived from `manifest.procedures.length`) and `WR-06` (a plant no longer depending on the derived upstream name's casing) are fixed on their merits; all eleven review findings get a durable disposition in a new `31-REVIEW-FIX.md`, which is what clears the already-red `docs-review-disposition.test.ts` and its `audit-integrity.test.ts` cascade. Both removal-gate per-path pins are held (12 for the test file, 1 for the workflow) and no manifest, gate, `STATE.md`, `ROADMAP.md` or `REQUIREMENTS.md` content is touched (`REPOINT-03`, `REPOINT-04`)
...
- [x] 32-01-PLAN.md — TRACER: the fate machinery proven end-to-end on one audited-set member (registry, derive-from-disk guard, mutation harness, observed red, D-02's set reconciliation)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 32-02-PLAN.md — D-07's ordering boundary: `--root` added to the five audited scripts that lack it, plus the measured root-override inventory
- [x] 32-03-PLAN.md — CUT-06 part A: PROJECT.md's citations and `r2000_*` pointer, ARCHITECTURE.md's Rule A21 dated, and the removal gate's stale NUL-offset header

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 32-04-PLAN.md — D-10: widen `docs-linerefs.test.ts` onto PROJECT.md with a per-document non-vacuity floor and a citation-aware bullet predicate
- [x] 32-05-PLAN.md — CUT-06 part B: the document sweep ledger, one verdict row per swept file, with the discussion-time numbers reconciled

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 32-06-PLAN.md — The sweep, first half: plant descriptors and control-gated observed reds for the 16 renamed members

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 32-07-PLAN.md — The sweep, second half: the 21 same-path members classified mechanically, with the `vice-proxy.test.ts` hang recorded rather than laundered

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 32-08-PLAN.md — The 6 deleted rows, the 15 set-B rows, both deferred fates against their new triggers, and the fate guard taken green
...
- [x] 32-10-PLAN.md — TRACER for Gap 2: `parseRootArg()` extracted into `scripts/lib/audit-root.mjs`, wired into the one WRITING script, proven to refuse a mistyped root without touching the real `docs/tool-support.md`. Leads with a `checkpoint:decision` on the verifier's fix-versus-revert fork
- [x] 32-13-PLAN.md — Gap 1: `evidence/32-audited-set-reconciliation.md` §6.1 adjudicates every guard `CUT-04` names by NAME, with `docs-linerefs`'s measured exclusion and its in-band discharge; and the non-demotion decision recorded beside `CUT-04`'s row
- [x] 32-14-PLAN.md — Gap 3: the harness writes what it records (`CR-02`, replacer function plus post-condition) and stops turning a signal-killed child into a red (`CR-04`); one row re-measured, one authorised dated line correction, and the restore-on-signal invariant attempted mechanically

**Gap-closure wave 2** *(blocked on gap-closure wave 1)*

- [x] 32-11-PLAN.md — Gap 2 expansion: the five remaining argv readers deleted, all six scripts on the shared strict parser, and the full per-script spawned matrix `gaps[1].missing[2]` asks for

**Gap-closure wave 3** *(blocked on gap-closure wave 2)*

- [x] 32-12-PLAN.md — Gap 2, second half (`CR-03`): the split read settled — four gates refuse a root their statically-imported comparison data cannot follow, the false docblock claim deleted, and a contract assertion that reds if it ever returns

**Gap closure, round 2** *(planned 2026-09-01 from `32-VERIFICATION.md`, `status: gaps_found`, 11/15 must-haves, THREE gaps — two of them regressions introduced by round 1's own remedies. Run with `/gsd-execute-phase 32 --gaps-only`; these five carry `gap_closure: true` and the fourteen above are untouched. Their wave numbers are their own, independent of Waves 1–7 and of round 1's gap-closure waves. Fixed in the order the verifier's Gaps Summary gives — Gap 4, then Gap 2, then Gap 5 — with one dependency-forced adjustment recorded in each plan.)*

**Gap-closure wave 1**

- [x] 32-15-PLAN.md — TRACER, Gap 4 (`CR-06`): the plant post-condition counts what the mutation INTRODUCED rather than what the file contains, its message stops telling the operator to edit the evidence, and the whole-set `--all` sweep completes. Also closes a SECOND, previously unrecorded blocker measured at plan time — the sweep throws on the 26 rows whose verdict owes no observed red, at registry index 22, BEFORE it ever reaches the post-condition at index 23
...
- [x] 32-20-PLAN.md — TRACER, Gap 2 (`CR-10`): the restore machinery stops being disarmable. The module-level latch on the now-exported restore path was set once and never reset, so any in-process consumer that completed one restore cycle permanently no-opped all four handlers; it is deleted, the map clear it already ends with being the idempotence it was added for. Lands with the discriminating case — plant, restore, plant AGAIN, signal — captured RED against a re-introduced latch before it is trusted, plus `WR-29` (the porcelain attribution admits the tree a mis-contained plant lands in) and `WR-28`; and the `WINDOWS.md` entry-33 close paired atomically with its successor entry, appended first
