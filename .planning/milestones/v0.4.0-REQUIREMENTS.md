# Requirements Archive: v0.4.0 Debt discharged, decisions settled
...
- [x] **CORE-01**: PROJECT.md's Core Value either states what v0.3.0 proved — that what a session learns outlives it — or records a dated confirmation that it should not, with the evidence weighed either way
  ...
  > → `## Core Value`, dated 2026-08-23. Evidence weighed: `ANNO-01`'s structural
  > argument that the external analyser is never launched with `--vice` and is guarded in code,
  > so the outlives-the-session property belongs to a component structurally incapable of
  > driving VICE at all; Phase 11's two-session sealed-question test (`ANNO-10`) and the
  > symbol round trip (`ANNO-14`/`ANNO-15`) — this project's strongest persistence
