# Roadmap Archive: v0.3.0 regenerator2000 static-analysis backend
...
| Decision | Rationale |
|---|---|
| **D-R1 / D-07 — regenerator2000 never touches VICE.** `--vice` is unreachable by construction (fixed per-verb argv builders) *and* denied by a scan that throws `R2000ViceFlagError` | Two independent guarantees, because a single guard is one refactor away from silently lapsing. Makes the whole family backend-agnostic — identical on the fork and stock backends |
| **The `r2000_*` family is registered proxy-locally via `buildViceTool()`**, never reaching `forwardToVice()` | Satisfies CLAUDE.md's derived-tool constraint *by construction* rather than by an interception: neither `rewriteArguments()` call site is reachable from it, so a container path can never be host-translated underneath it |
| **`R2000-16` promoted from a criterion inside Phase 9 to a standalone go/no-go phase** (2026-08-19) | Its failure mode is reconsider-the-milestone, not replan-the-phase. A phase boundary makes that gate structural rather than skippable |
...
| **D-34 — the synthesis route hands over `.prg` / `.d64` / flat-64K only**, never a container format | Phase 9 found `.vsf` machine-type auto-detection to be a coincidental default fallback, not a real read. `R2000-08` was already reduced to a note; this settles it in code |
...
- **A cross-session broker finding** caught and fixed mid-task during criterion
  3(4), plus a real regenerator2000 defect recorded separately upstream-facing.
- **A self-inflicted EPIPE crash** in the r2000 CLI dispatch, found while fixing
...
- **`.vsf` as a bootstrap input** —
  ...
  `R2000-*` requirement covers it; reverses if a consumer has only `.vsf`
  captures and cannot re-capture as `.raw`.
- **`IN-07` — two hand-copied `probeR2000()` gates** not yet migrated to
  `r2000-test-gate.ts`, because `r2000-cli.test.ts`'s gate semantics are
...
- **A test-only env hatch**, `VICE_TEST_R2000_CLI_STDOUT_FILL_BYTES` in
  ...
  bytes of stdout on this host's regenerator2000.
- **`r2000-project.ts:57`'s comment-only phase pointer** is permanently outside
  ...
  todo.
- **`r2000-cli.test.ts` duplicates `writeChain()`'s used-byte formula** — used
