...
**Plans**: 16/16 plans executed — 9/9 original plans executed in 4 waves, plus 5 gap-closure plans (42-10..42-14) executed in 3 further waves after verification returned `gaps_found`, plus 2 round-2 gap-closure plans (42-15..42-16) in 2 further waves after the re-verification scored 4/5 on one new finding
...
- [x] 42-07-PLAN.md — The four remaining tools, classify-before-parse in all five handlers, the shared backtrace name and its registry consequence, and the published file set
- [x] 42-08-PLAN.md — The structural single-owning-module guard: family floor, injective owner map, closed per-format literal and importer sets, byte-safe census, planted-violation control
...
- [x] 42-10-PLAN.md — CR-01: the `io` decoded-prose completeness gate, its `incomplete-decoded-state` refusal code, the drift controls a rename and an empty block each trip, and the interface-vs-constant census that keeps the guard honest

**Wave 6** *(blocked on Wave 5)*

- [x] 42-11-PLAN.md — WR-02 (+ IN-01): the chip gate — a CIA or SID address inside the tool's own advertised range refuses by the chip's name with its dump reported as clean, the handler stops framing a chip fact as a parse failure, and the published `inputSchema` is proven unchanged
- [x] 42-12-PLAN.md — G5: the live harness asserts teardown of the broker child it actually spawns, sweeps for strays scoped to its own scratch path, proves the scratch directory is gone, and carries an unskipped planted control proving the assertion can fail
...
- [x] 42-14-PLAN.md — The round's evidence measured on the final tree — including a live re-run of all five formats — with every deliberately-excluded item restated as open, and `PARSE-03`/`PARSE-04` returned to Complete only on that measurement

**Gap closure (round 2)** — added after the re-verification (`42-VERIFICATION.md`, 2026-09-09T21:15:00Z) scored the phase 4/5 with all five round-1 gaps genuinely closed and **exactly one new gap remaining**: `CR-02`, a Blocker against Criterion 5 found by the post-round code review, which demoted `PARSE-04` back to `Pending`. Two plans, each carrying `gap_closure: true`. Scope is bounded to `CR-02` alone; the five declared accepted-scope open items in the verification's `deferred:` block are restated as open, never chased, and `PARSE-01`/`PARSE-02`/`PARSE-03` are left exactly as they are.

**Wave 9** *(tracer — the one Blocker, closed end to end before any evidence work)*

- [x] 42-15-PLAN.md — CR-02: `io`'s per-address chip-degradation classification leaves the binary-wide capability cache — a pure `textCapabilityVerdictFor()` builder with one verdict-construction site, `NEVER_CACHED_COMMANDS` closing the write, read and in-flight-memo paths, and one planted control per reachable wrong answer, each red against the pre-change tree

**Wave 10** *(blocked on Wave 9)*

- [x] 42-16-PLAN.md — The fix proven live over two `io` addresses in one genuine-stock session, the round's evidence recorded with every excluded item restated as open, `PARSE-04` returned to Complete only on that measurement, and the CR-02 disposition closed together with its ledger row *(runs with `USE_WORKTREES_FOR_PLAN=false` — it delivers STATE.md content)*
...
- **The re-check this note asked for has happened.** Plan `42-06` independently re-verified every drift citation against the raw `NEWS`/source trees rather than trusting the research document's own summarization, and it produced corrections rather than confirmation for two of the three: the `chis` cycle-column change is now attributed to VICE 3.5–3.6 (not 3.0), and the `memmapshow` access-class addition to VICE 3.9–3.10 (not 3.5); the `mc`/`ms` glyph inversion at 3.4 was confirmed exactly, independently, in both trees. The same re-check found the build-guard claim in success criterion 5 factually wrong and corrected it. Every citation, with its raw quoted line, its file, its line number and its tree, now lives in `docs/phase42-text-format-drift-citations.md`.
...
- [x] 43-04-PLAN.md — `evid-reconcile.ts`, the pure disagreement-first join: four separately-named buckets, an explicit denominator, no rate anywhere, and the classifier independence proven through a substituted vocabulary

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 43-05-PLAN.md — `evid-ingest.ts` and `anno_evid_ingest`: one `memmapshow` reply becomes durable rows, a row only for an observed execute bit, and run identity derived by the shipped digest at one site
...
- [x] 43-07-PLAN.md — the two properties no single verb can prove: one derived key walk refusing a rate and demanding a denominator across every answer, plus the concurrent two-identity SIGKILL planting
...
- [x] 44-02-PLAN.md — the measurement itself: one licensed run at anchor hit target 50 inside `EVID-06`'s proven region and one deeper run labelled `narrowed` with the hit-50 / hit-75 boundary quoted beside it, each joined and committed as its own transcript with its depth, its depth label, its broker state and its verdict
