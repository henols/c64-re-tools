...
- [x] **SESS-01**: an external analyser project stays open across many tool calls in one working session instead of being respawned per call, without adding an unguarded spawn site — `spawn-seam.test.ts`'s enumerated set stays intact and the `--vice` invariant stays guarded in code
...
- [x] **SURF-01**: The curated `anno_*` surface covers what the absorbed procedures actually call — starting with `anno_read_region`, so a routine can be read at a range instead of exporting the whole program — and the tool-count pin and generated `docs/tool-support.md` move with it without drift
- [x] **SURF-02**: `anno_get_address_details`'s D-32 refusal is re-decided against the still-live upstream `u16` overflow at `handler.rs:1894` — fixed, worked around, or refused with a documented route, but not carried unexamined a second milestone
...
- **FUT-01**: BASIC token decoding (upstream's `analyze-basic`) — commercial C64 games captured post-loader almost universally reduce to a one-line `SYS` stub
...
| A GUI, and HTML export | HTML export was already cut once at v0.3.0 (`ANNO-07`) as an artifact no skill produces or consumes; still available ad-hoc via `--export_html` |
...
| Routing the session through `vice-broker.mts` | The broker exists to cross the container/host boundary for `x64sc`; the external analyser is container-side and has no such boundary. Reuse its single-owner pattern, not its code |
