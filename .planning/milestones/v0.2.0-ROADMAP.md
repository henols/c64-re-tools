...
- 📋 **v0.3.0 the external analyser static-analysis backend** — Phases 9-10 (proposed, not opened)
...
- `DERIV-04` gains a second reason to exist beyond parity: it is the consumer half of the external analyser symbol round trip (v0.3.0). Build the loader so a VICE `.lbl` file from any producer works.
...
- Re-check for prebuilt the external analyser binaries when writing criterion 3, in case v0.3.0's prerequisite can be stated without `cargo install`.
...
# Milestone v0.3.0: The external analyser static-analysis backend (PROPOSED)
...
**Phase 9's assumption probe (`ANNO-16`) may be pulled forward now**, ahead of
...
write those docs **once**, already naming the external analyser.

**What v0.2.0 still has to finish regardless of this milestone**, because
The external analyser replaces none of it: stock advertises 26 tools against the fork's
...
**Requirements:** `ANNO-01`..`ANNO-16` in `REQUIREMENTS.md` (proposed block).
...
- **the external analyser runs on the MCP proxy's side of the container boundary.** No
  `hostpath.ts` / `containerpath.ts` translation is applied to any argument
  passed to it (`ANNO-02`). This is what makes devcontainer use and two
...
Whether the TUI tolerates a pty with no real TTY is `ANNO-16`(a) and gates
Phase 9.
...
user (`ANNO-04`) rather than worked around. Synthesizing a `.regen2000proj`
...
**Goal**: The bet is de-risked, project creation needs no human, and the one thing the external analyser makes obsolete is gone
**Depends on**: nothing — **may run now, ahead of v0.2.0 Phases 5-8** (see the dependency note above)
**Requirements**: ANNO-16, ANNO-01, ANNO-02, ANNO-03, ANNO-05, ANNO-06, ANNO-09
**Success Criteria** (what must be TRUE):

  1. All five assumptions in `ANNO-16` are answered against a real build and recorded in the repo, with any failure recorded as an accepted limit stating what it breaks.
  2. A raw `.prg` or a `.vsf` snapshot becomes a `.regen2000proj` **without a human** — HTTP MCP mode under a pty, auto-analysis on load, then `anno_save_project`. If `ANNO-16`(a) fails, this degrades to a documented one-time interactive step and every affected playbook says so.
  3. The launch path **refuses** to pass `--vice`, enforced in code and tested, and no argument passed to the external analyser is host-translated.
  ...
  5. The install documentation names the external analyser as a prerequisite, states the toolchain cost plainly, and its Apache-2.0 notice is in `THIRD-PARTY-NOTICES.md`.

**Plans**: TBD

Notes:

- **Criterion 1 gates everything, including whether Phase 10 is worth starting.** Run it first, alone, and read the result before planning further. If the external analyser cannot be driven without a human, the annotation store is not reachable from a skill and the milestone should be reconsidered rather than replanned.
- **Run this phase before v0.2.0 Phase 8.** It has no v0.2.0 dependency, and knowing its answers lets Phase 8 write the install story once — already naming the external analyser — instead of writing it and then rewriting it here. That is the only genuine rework between the two milestones.
...
- Criterion 4 is the entire deletion this milestone earns: a 14-line `spawnSync` wrapper around `toacme` (`scripts/acme.mjs:208-223`) plus ~50 lines of `SKILL.md` caveats that exist only because `toacme` does a flat linear decode. Prefer the external analyser's own `--verify-roundtrip` over building a reassembly gate — note it implies `--headless`, so criterion 2 comes first.
...
**Success Criteria** (what must be TRUE):
  ...
  4. Symbols annotated in the external analyser resolve live addresses through `vice_symbols_load`, and names discovered against the running machine flow back into the store — a round trip, not a one-way dump.
...
- Criterion 3 is the most distinctive thing available here — **neither project can do it alone.** `memmap.json` holds the bit tables; the external analyser holds the enum mechanism and `--dump-enum-files`.
...
| Two-project limit as a reported error | `ANNO-04` | Folded into Phase 9's install documentation as a stated limitation. Building detection-and-reporting for an upstream port collision is work in the wrong place. |
| Static-vs-live tool-selection axis | `ANNO-12` | Folded into v0.2.0's `SKILL-01`, which is already rewriting the same playbooks for backend routing. One pass over `c64-program-recon`, not two. |
| `.vsf`/`.raw` bridge as its own requirement | `ANNO-08` | Reduced to a note on Phase 9 criterion 2 — it is which file extension you hand over, not a deliverable. |
