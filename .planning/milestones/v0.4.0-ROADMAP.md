# Roadmap Archive: v0.4.0 Debt discharged, decisions settled

**Archived:** 2026-08-23
**Status:** ✅ SHIPPED 2026-08-23
**Phases:** 12-17 (six phases, no inserted decimals)
**Total plans:** 44 (119 tasks)
**Requirements:** 16/16 satisfied, zero cut, zero deferred
**Final audit:** round 1, `tech_debt` — zero blockers, zero open gaps
  ([`v0.4.0-MILESTONE-AUDIT.md`](v0.4.0-MILESTONE-AUDIT.md))
**Requirements as shipped:** [`v0.4.0-REQUIREMENTS.md`](v0.4.0-REQUIREMENTS.md)

This file is the full, unabridged roadmap as it stood at the v0.4.0 close —
every phase's Goal, Depends-on, Requirements and Success Criteria, plus the
sequencing rationale. `.planning/ROADMAP.md` keeps only the collapsed one-line
summary. The v0.2.0 and v0.3.0 sections below were already collapsed at their
own closes and point at their own archives.

**Milestone summary — decisions, issues and debt:** the decision register lives
in PROJECT.md → Key Decisions (`FORK-01` retain, `CORE-01` keep-dated, `PKG-04`
accepted-risk, and the audit-instrument-first sequencing). Issues resolved: the
19 inherited ledger items drained to 0; 150 review findings all dispositioned;
Phase 03's last partial UAT scenario closed by live experiment. Issues deferred:
9 follow-on items promoted with named owners (see `v0.4.0-REQUIREMENTS.md` →
`### Promoted by DEBT-01`, `### Fork Backend Follow-on`,
`### Control-Plane Bind Follow-on`) plus `UP-01`/`UP-02`, which stay Out of
Scope as pull requests against projects this repo does not own. Technical debt
incurred: five of six phases left a `VALIDATION.md` at `status: draft`, and only
Phase 17 produced a `SECURITY.md` — both recorded as coverage TODOs in the
audit, neither blocking.
...
**Notes (Phase 17 complete, 2026-08-23)**: Criterion-by-criterion outcome —
  ...
  1. **Satisfied.** PROJECT.md → Core Value carries a dated (2026-08-23), evidence-citing
     ...
     (`ANNO-10`), the symbol round trip (`ANNO-14`/`ANNO-15`), and `ANNO-01`'s
     structural argument that the external analyser is never launched with `--vice` — was weighed
...
requirements satisfied. 4 of the original 16 `ANNO-*` requirements were cut or
folded on 2026-08-17 and remain in `milestones/v0.3.0-REQUIREMENTS.md` with
rationale. `ANNO-16` was satisfied with criterion 3(4) scoring `partial`,
...
**v0.4.0 as shipped:** 6 phases (12-17), 44/44 plans, 119 tasks, 16/16 in-scope requirements satisfied,
