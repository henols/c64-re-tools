...
modifiable — by absorbing the external analyser's own analyze procedures into this
...
- [x] **Phase 18: Persistent Session and Tool Surface** - an external analyser session survives many tool calls in one working session, and the curated surface covers what absorbed procedures need
...
**Goal**: an external analyser project stays open across a whole working session
instead of being respawned per tool call, and the curated `anno_*` surface is
...
**Success Criteria** (what must be TRUE):
  ...
  1. A caller can issue many `anno_*` tool calls in one working session without the underlying analyser process being respawned per call, and the `--vice` invariant stays guarded in code — `spawn-seam.test.ts`'s enumerated spawn-site set is unchanged or explicitly extended, never silently widened.
  ...
  5. The curated tool surface includes `anno_read_region` (so a routine can be read at a range instead of exporting the whole program), and `anno_get_address_details`'s D-32 refusal is re-decided — fixed, worked around, or refused with a documented route — against the still-live upstream `u16` overflow, not carried unexamined into this milestone.
...
- [x] 18-03-PLAN.md — TRACER: one held the external analyser child answers many `anno_*` calls end to end, plus the three-scenario save-discipline gate and the spawn-seam proof
...
- [x] 18-05-PLAN.md — Curate `anno_read_region` with a documented cap; compose `anno_get_address_details` client-side under D-36; move every count pin and prose mention with no drift
...
**Success Criteria** (what must be TRUE):
  ...
  1. The five upstream analyze procedures are absorbed at a pinned upstream commit into `c64-program-recon`/`c64-memory-mapping` and new skills where nothing currently owns the job (a routine-queue-walker, at minimum), with every tool call each absorbed procedure makes diffed explicitly against the curated `anno_*` surface — no absorbed step calls a tool this project does not expose — and with zero runtime dependency on `.agent/skills/`, which the published crate excludes.
...
> the external analyser's annotation store, data-typing tools and scope-based export,
...
- **Open this phase by implementing the two tools Phase 19 proposed** (`19-DECISIONS.md` decision 2): `anno_toggle_splitter` serves DECOMP-01 — without it two adjacent tables merge into one block and there is no boundary to cut on — and `anno_set_immediate_format` serves BUILD-03. Both are mutating and must go through `anno-session.ts`'s existing seam; neither may add a second child-launch site. If DECOMP-01 is reached on the fixtures without ever needing a table boundary, the splitter proposal is withdrawn rather than implemented.
...
**Success Criteria** (what must be TRUE):
  ...
  1. Export emits one ACME source file per the external analyser scope, wired by `acme-build`'s `!source`, with data tables extracted to their own files, and the whole set assembles to a single working output.
