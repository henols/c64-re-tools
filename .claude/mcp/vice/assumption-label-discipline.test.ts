...
// `.claude/mcp/vice/` carrying an `[ASSUMED]`-bearing comment that names
...
//   - Any file outside `.claude/mcp/vice/` (e.g. `.planning/phases/03-direct-
...
//
// SCOPE: authored `.ts`/`.mts` files directly under `.claude/mcp/vice/`,
...
test("scanned source file set meets the minimum-count floor (a broken filter must fail loudly, not scan nothing)", () => {
  ...
  assert.ok(
    ...
    `expected at least ${MIN_SCANNED_FILES} authored .ts/.mts files under .claude/mcp/vice/, found ` +
