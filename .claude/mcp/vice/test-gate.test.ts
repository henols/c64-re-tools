...
test("gate: MANUAL_ONLY_TESTS contains exactly the four dispositioned files", () => {
  assert.deepEqual(
    [...MANUAL_ONLY_TESTS].sort(),
    ["broker-e2e.test.ts", "stock-live.test.ts", "vice-broker-launch.test.ts", "vice-proxy.test.ts"].sort(),
  );
});
