...
- **Dependency**: `CPUHISTORY_GET` (0x86) requires **VICE ≥ 3.10**. Debian trixie/forky/sid and all current Ubuntu ship 3.9, which lacks the opcode entirely. Homebrew and official builds are fine. (`binary monitor only` — the version floor is on the *opcode*, not the capability: `chis` returned CPU history with per-entry cycle counts over the text channel on genuine stock 3.9, MEASURED 2026-08-27. Polarity distinction `PARSE-04` depends on: text-command tracing/profiling support is opt-**out** at build time, the opposite of a version floor. The affected commands do not each carry their own separate guard — two share one guard and one stub string (`memmapshow`/`chis`), two carry none (`bt`/`prof flat`), and one degrades per-chip at runtime instead of refusing (`io`); per-command probing is unaffected — `docs/phase42-text-format-drift-citations.md`. Independently re-confirmed by Phase 39's own fixture batch (`FIXTURE_UNSUPPORTED: none`, `docs/phase39-dual-channel-coexistence-gate-findings.md` §7) — `chis` succeeded on genuine stock VICE 3.9 over the text channel via a completely different code path than the binary-monitor opcode's version floor; Phase 41 plan 41-06 collects both citations together — `docs/phase41-text-channel-live-evidence.md`.)
