...
//   authors under src/mcp/vice/ ever imports "@mastra/core/telemetry" or
...
test("structural guard: no .ts/.mts source file in src/mcp/vice/ imports @mastra/core/telemetry or ./telemetry, in any form", () => {
...
test("live installed tree: every @mastra/core subpath @mastra/mcp's installed entry files reference resolves to a file with zero posthog/usage-telemetry/telemetry markers", () => {
  ...
  assert.ok(
    ...
    "no @mastra/mcp dist entry found under node_modules/@mastra/mcp/dist/{index.js,index.cjs} -- " +
      "Task 2's `npm install --prefix src/mcp/vice` must run before this test; an empty entry list " +
