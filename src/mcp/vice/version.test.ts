...
test("resolveVersion(): the repo's real VERSION resolves to 0.2.0 against an injected published 0.1.12 (D-3)", () => {
  // HERE is src/mcp/vice/ -- three levels up is the repo root (plan 16-04:
  // the same three-segment depth as the pre-move .claude/mcp/vice/, so this
  // assumption is still true unchanged).
...
// ============================================================================
// Placeholder-consistency invariant (R-2) -- guards the "six derived
// strings, one self-evident placeholder" requirement so the four-wrong-
// strings problem (D-1) can never recur silently.
// ============================================================================
...
test("PROXY_VERSION guard: vice-proxy.ts assigns no version literal and references runtimeVersion", () => {
  // Resolved HERE-relative rather than root-plus-hardcoded-subdirectory,
  // since this test lives beside the file it reads (plan 16-04).
