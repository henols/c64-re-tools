...
export function ghidraRunsRoot(repoRoot: string): string {
...
export function ghidraRunsRealRoot(repoRoot: string): string {
 ...
 */
export function ensureGhidraRunsHandle(repoRoot: unknown): EnsureGhidraRunsHandleResult {
  ...
  if (!stat.isSymbolicLink()) {
    ...
    return {
      ok: false,
      message:
        `ensureGhidraRunsHandle refuses: ${handlePath} already exists and is ${kind}, not a symbolic link. ` +
  ...
  try {
    ...
    return {
      ok: false,
      message:
        `ensureGhidraRunsHandle: ${handlePath} verified as a symbolic link but its target could not be read ` +
  ...
  if (target !== GHIDRA_RUNS_HANDLE_TARGET) {
    return {
      ok: false,
      message:
        `ensureGhidraRunsHandle refuses: ${handlePath} is a symbolic link but points to "${target}", not the expected ` +
...
export function buildAnalyzeHeadlessArgv(input: unknown): BuildAnalyzeHeadlessArgvResult {
  ...
  if (typeof dataRangesPath === "string") argv.push("-preScript", DATA_RANGE_SEED_SCRIPT_NAME, dataRangesPath);
