...
/*
      addr = (ADDR_T)((int)(char)Memory[(ADDR_T)(address + 1)] +
                      address + size);
*/
      reloffs = (int)Memory[(ADDR_T)(address + 1)];
      reloffs -= (reloffs > 127) ? 256 : 0; 
      addr = (address + size + reloffs);

      /* Did this overflow? */
      if (address == 65535) { /* Can't be valid */
        SetMemType(address, MEM_DATA);
        return ((Options & M_DATA_BLOCKS) == O_DBL_STRICT);
      }

      if (address >= 65408 && addr < 256) { /* Overflows */
        SetMemType(address, MEM_DATA);
        SetMemType(address + 1, MEM_DATA);
        return ((Options & M_DATA_BLOCKS) == O_DBL_STRICT);
      }
...
#endif
{
  ...
  for (address = scanstart;; address += size) {
    ...
    if (instr->admode == rel) {
/*
      addr = (ADDR_T)((int)(signed char)Memory[(ADDR_T)(address + 1)] +
                      address + size);
*/
      reloffs = (int)Memory[(ADDR_T)(address + 1)];
      reloffs -= (reloffs > 127) ? 256 : 0; 
      ...
      /* Did this overflow? */
      if (address == 65535) goto Failure; /* Can't be valid */
      if (address >= 65408 && addr < 256) goto Failure; /* Overflows */
