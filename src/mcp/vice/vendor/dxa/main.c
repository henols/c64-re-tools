...
/* new or altered dxa options */
  ...
  if (argc - optind > 1) {
  Usage:
    fprintf (stderr, "%s v0.1.5 -- symbolic 65xx disassembler\n", prog);
    fprintf (stderr, "Based on d65 copyright (C) 1993-4 Marko M\"akel\"a\n");
    fprintf (stderr, "Changes for dxa copyright (c) 2006-22 Cameron Kaiser\n\n");
...
/* AddEntry (StartAddress, StartAddress, RTN_SURE); */
  ...
  if (EndAddress <= StartAddress) { /* oops */
    /* Distinguish a file ending right at $FFFF from a true overflow. */
    if(fgetc(file) != EOF) {
  ...
  if ((Options & B_DETECT_BASIC) && ((EndAddress - StartAddress) > 11) && (0 ||
    ...
    */
	...
	int ok = 0;
    	...
    	/* See if there's an encoded SYS address as the first BASIC command. */
    	if (Memory[StartAddress+4] == 0x9e /* SYS */) {
		for(; offs < StartAddress + 11; offs++) { /* stop overrun */
    			if (Memory[offs] == 0x00 || Memory[offs] == ':') {
				ok = 1;
				break;
			}
