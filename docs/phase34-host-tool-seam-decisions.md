...
| A-07 | The Ghidra per-run project root is `<repoRoot>/tools/ghidra-runs/<runId>`. **SUPERSEDED 2026-09-08 (gap `G-40-1`):** the location moved to `<repoRoot>/.c64-re-tools/runs/ghidra/<runId>`, reached through a non-dotted ALIAS symlink (`<repoRoot>/c64-re-tools` -> `.c64-re-tools`, minted host-side by `vice-broker.mts`) rather than a real directory at `tools/ghidra-runs/`. See `.planning/notes/ghidra-dot-path-check-semantics.md`. | `install-resources.ts`'s `installTargetDir(root) = join(root, "tools")` is already non-dot-prefixed and already inside the bind-mounted workspace — the only placement satisfying both Finding 2's every-segment dot rule and the requirement that a result path be translatable by `containerPath()`. `.vice-supervisor/` and `.planning/` are disqualified as ancestors at any depth. **AMENDED 2026-09-08:** "only placement satisfying" was true of the candidate SET CONSIDERED at the time — existing real directories already in the tree — and false of the full option space, because a symlinked HANDLE was never itself a candidate. MEASURED (`.planning/notes/ghidra-dot-path-check-semantics.md`, real Ghidra 12.1.3): the refusal binds the ABSOLUTIZED path argument (`ProjectLocator` calls `java.io.File.getAbsolutePath()`, never `getCanonicalPath()`), so it absolutizes but does not resolve a symlink — a broker-minted alias handle satisfies this row's own two original conditions (non-dot-prefixed, inside the bind-mounted workspace) while landing the bytes physically under `.c64-re-tools/`. | Costly — the path is now baked into `resolveGhidraProject()`, its test suite, `.gitignore`'s `tools/ghidra-runs/` entry and the live evidence transcript; moving it touches all four in the same commit. **NOTE 2026-09-08:** this estimate held — gap-closure plans `40-08`/`40-09` touched exactly those four places (plus the broker's own startup mint) to land the amendment above. |
